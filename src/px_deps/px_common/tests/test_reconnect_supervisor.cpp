...
std::shared_ptr<PxReconnectSupervisor> MakeSupervisor(const std::shared_ptr<PxAsyncRuntime>& runtime) {
    return PxReconnectSupervisor::Create(runtime, PxReconnectSupervisorOptions{
                                                      ...
                                                      .backoff = PxReconnectBackoffOptions{.retry_delay = 1ms},
                                                  });
}

void StopRuntime(const std::shared_ptr<PxReconnectSupervisor>& supervisor, const std::shared_ptr<PxAsyncScope>& scope,
...
TEST(ReconnectSupervisor, RetriesRecoverableStartupFailuresUntilReady) {
    ...
    PxReconnectSupervisorHooks hooks{
        .start_attempt =
            [supervisor, state](std::uint64_t) {
                const auto attempt = state->starts.fetch_add(1, std::memory_order_acq_rel) + 1;
                if (attempt < 4) {
                    return PxResult<void>::Failure(MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected, "test.start", "offline", true));
                }
    ...
    ASSERT_TRUE(scope->Spawn("retry-until-ready",
...
TEST(ReconnectSupervisor, DisconnectAfterReadyStartsANewGeneration) {
    ...
    PxReconnectSupervisorHooks hooks{
        ...
        .on_ready =
            [supervisor, state, reconnected](std::uint64_t) {
                if (state->starts.load(std::memory_order_acquire) == 1) {
                    static_cast<void>(
                        supervisor->MarkDisconnected(MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected, "test.disconnect", "offline", true)));
                } else {
                    reconnected->set_value();
                }
            },
        .on_lost =
            [state](std::uint64_t, const PxAsyncError&, const bool was_ready) {
                state->losses.fetch_add(1, std::memory_order_acq_rel);
                state->loss_was_ready.store(was_ready, std::memory_order_release);
            },
    };
    ASSERT_TRUE(scope->Spawn("disconnect-reconnect",
                             [supervisor, hooks = std::move(hooks)]() mutable { return PxReconnectSupervisor::Run(supervisor, std::move(hooks)); }));
...
TEST(ReconnectSupervisor, AdapterMustStopBeforeAnotherStartAttempt) {
    ...
    PxReconnectSupervisorHooks hooks{
        ...
        .stop_attempt = [state](std::chrono::steady_clock::time_point) -> PxAwaitable<PxResult<void>> {
            ...
            if (stop == 1) {
                co_return PxResult<void>::Failure(MakePxAsyncError(PxAsyncErrorCode::kTimeout, "test.stop", "still stopping", true));
            }
            co_return PxResult<void>::Success();
        },
        .on_ready = [ready](std::uint64_t) { ready->set_value(); },
    };
    ASSERT_TRUE(scope->Spawn("reset-before-retry",
                             [supervisor, hooks = std::move(hooks)]() mutable { return PxReconnectSupervisor::Run(supervisor, std::move(hooks)); }));
...
TEST(ReconnectSupervisor, RejectedAttemptRetriesUntilExplicitStop) {
    const auto runtime = PxAsyncRuntime::Create({.worker_threads = 1});
    ASSERT_TRUE(runtime);
    ASSERT_TRUE(runtime->Start());
    const auto scope = PxAsyncScope::Create(runtime, PxAsyncLane::kState);
    const auto supervisor = MakeSupervisor(runtime);
    const auto state = std::make_shared<SupervisorTestState>();
    const auto retried = std::make_shared<std::promise<void>>();
    auto retried_future = retried->get_future();

    PxReconnectSupervisorHooks hooks{
        .start_attempt =
            [state, retried](std::uint64_t) {
                const auto starts = state->starts.fetch_add(1, std::memory_order_acq_rel) + 1;
                if (starts == 3) {
                    retried->set_value();
                }
                return PxResult<void>::Failure(MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "test.auth", "rejected", false));
            },
        .stop_attempt = [state](std::chrono::steady_clock::time_point) -> PxAwaitable<PxResult<void>> {
            state->stops.fetch_add(1, std::memory_order_acq_rel);
            co_return PxResult<void>::Success();
    ...
    ASSERT_TRUE(scope->Spawn("rejected-attempt-retry",
                             [supervisor, hooks = std::move(hooks)]() mutable { return PxReconnectSupervisor::Run(supervisor, std::move(hooks)); }));

    ASSERT_EQ(retried_future.wait_for(1s), std::future_status::ready);
    EXPECT_GE(state->starts.load(std::memory_order_acquire), 3U);
    EXPECT_GE(state->stops.load(std::memory_order_acquire), 2U);
    EXPECT_GE(supervisor->Statistics().reconnect_waits, 2U);
    StopRuntime(supervisor, scope, runtime);
}
...
TEST(ReconnectSupervisor, ScopeStopCancelsLongBackoffPromptly) {
    ...
    const auto supervisor = PxReconnectSupervisor::Create(runtime, PxReconnectSupervisorOptions{
                                                                       ...
                                                                       .connection_timeout = 100ms,
                                                                       .adapter_stop_timeout = 20ms,
                                                                       .backoff = PxReconnectBackoffOptions{.retry_delay = 5s},
                                                                   });
    ...
    ASSERT_TRUE(scope->Spawn("cancel-backoff",
                             [supervisor, hooks = std::move(hooks)]() mutable { return PxReconnectSupervisor::Run(supervisor, std::move(hooks)); }));
...
TEST(ReconnectSupervisor, StaleGenerationSignalsCannotCompleteTheActiveAttempt) {
    ...
    PxReconnectSupervisorHooks hooks{
        .start_attempt =
            [supervisor](const std::uint64_t generation) {
                EXPECT_FALSE(supervisor->MarkReady(generation + 1));
                EXPECT_FALSE(supervisor->MarkDisconnected(
                    generation + 1, MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected, "test.stale", "stale disconnect", true)));
                EXPECT_FALSE(
                    supervisor->FailActive(generation + 1, MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "test.stale", "stale failure", true)));
    ...
    ASSERT_TRUE(scope->Spawn("generation-bound-signals",
                             [supervisor, hooks = std::move(hooks)]() mutable { return PxReconnectSupervisor::Run(supervisor, std::move(hooks)); }));
