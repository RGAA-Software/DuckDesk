  ...
  public:
    TemporaryUploadFile() {
        path_ = std::filesystem::temp_directory_path() /
                std::format("px_http_cancel_{}.bin", std::chrono::steady_clock::now().time_since_epoch().count());
        {
  ...
  public:
    SlowUploadServer()
        : io_(std::make_shared<asio::io_context>()),
          acceptor_(std::make_shared<asio::ip::tcp::acceptor>(*io_, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), 0))),
          socket_(std::make_shared<asio::ip::tcp::socket>(*io_)), accepted_(std::make_shared<std::atomic_bool>(false)) {}
...
class DownloadServer final {
  public:
    DownloadServer(std::string body, std::chrono::milliseconds chunk_delay)
        : body_(std::move(body)), chunk_delay_(chunk_delay), io_(std::make_shared<asio::io_context>()),
          acceptor_(std::make_shared<asio::ip::tcp::acceptor>(*io_, asio::ip::tcp::endpoint(asio::ip::tcp::v4(), 0))),
    ...
    void Start() {
        ...
        worker_ = std::thread([acceptor, socket, body, chunk_delay]() {
            ...
            const auto header = std::format("HTTP/1.1 200 OK\r\nContent-Length: {}\r\nConnection: close\r\n\r\n", body.size());
            asio::write(*socket, asio::buffer(header), error);
            constexpr std::size_t kChunkSize = 4096;
            for (std::size_t offset = 0; !error && offset < body.size(); offset += kChunkSize) {
                const auto length = std::min(kChunkSize, body.size() - offset);
                asio::write(*socket, asio::buffer(body.data() + offset, length), error);
...
TEST(HttpClientCancellation, CancelsMultipartUploadAlreadyInProgress) {
    ...
    std::thread request([client, cancellation, response, path]() { *response = client->PostMultiPart({{"token", "test"}}, {}, {{"file", path}}); });
...
TEST(HttpClientCancellation, PreCancelledMultipartDoesNotWaitForTimeout) {
    ...
    client->SetCancellationSignal(cancellation);
    const auto started_at = std::chrono::steady_clock::now();
    const auto response = client->PostMultiPart({{"token", "test"}}, {}, {{"file", file.Path().string()}});
    const auto elapsed = std::chrono::steady_clock::now() - started_at;
    server.Stop();
...
TEST(HttpClientCancellation, ManagedBlockingWorkerJoinsAfterCancellation) {
    ...
    client->SetCancellationSignal(cancellation);
    const auto path = file.Path().string();
    const auto executor = PxBlockingExecutor::Create({.thread_count = 1, .max_pending_tasks = 8});
    const auto submit_result = executor->TryPost([client, cancellation, completed, path]() {
        static_cast<void>(client->PostMultiPart({{"token", "test"}}, {}, {{"file", path}}));
        completed->store(true, std::memory_order_release);
    });
    ASSERT_EQ(submit_result, PxBlockingSubmitResult::kAccepted);
...
TEST(HttpClientCancellation, CancelsGetAlreadyInProgress) {
    ...
    const auto response = std::make_shared<HttpResponse>();
    const auto started_at = std::chrono::steady_clock::now();
    std::thread request([client, response]() { *response = client->Request(); });
...
TEST(HttpClientCancellation, DownloadStreamsBodyAndReportsProgress) {
    ...
    // Windows CI can briefly delay loopback delivery while many freshly linked executables are scanned.
    // Keep this success-path timeout comfortably above that delay; cancellation has a separate strict test.
    ...
    const auto response = HttpClient::Download(std::format("http://127.0.0.1:{}/download", server.Port()), std::move(options));
    server.Stop();
...
TEST(HttpClientCancellation, CancelsStreamingDownload) {
    ...
    options.write_callback = [bytes, cancellation](std::string_view chunk) {
        const auto total = bytes->fetch_add(chunk.size(), std::memory_order_acq_rel) + chunk.size();
        if (total >= 32 * 1024) {
            cancellation->store(true, std::memory_order_release);
        }
        return true;
