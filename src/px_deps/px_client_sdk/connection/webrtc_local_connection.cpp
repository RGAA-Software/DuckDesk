...
WebRtcLocalConnection::WebRtcLocalConnection(const std::shared_ptr<ThunderSdkParams>& params, const std::shared_ptr<MessageNotifier>& notifier)
...
void WebRtcLocalConnection::CreateRtcClientInstance() {
    rtc_client_ = CreateRtcClient();
    if (!rtc_client_) {
        LOGE("RTC local client creation failed: stage=create_client outcome=unavailable recoverable=false");
        return;
    }
    LOGI("RTC local client created: boundary=direct_cpp_dll outcome=success");
}
...
void WebRtcLocalConnection::InitRtcClient() {
    ...
    rtc_client_->SetVideoTrackCount(sdk_params_->file_transfer_only_ ? 0 : kReservedRtcMonitorTrackCount);
        ...
        if (state == kIceStateConnected || state == kIceStateCompleted) {
            self->RunInRtcThread([weak_self]() {
                ...
                for (int i = 0; i < 100 && !locked->stopped_; ++i) {
                    const bool ready = locked->sdk_params_->file_transfer_only_ ? locked->IsFtChannelReady() : locked->IsMediaChannelReady();
                    if (ready) {
                        break;
                    }
...
void WebRtcLocalConnection::RequestAnswerSdp(const std::string& offer_sdp) {
    ...
    if (resp.status != 200) {
        LOGE("Request rtc local answer failed, http status: {}, code: {}, message: {}, error: {}", resp.status, code, message, resp.error_message);
        if (resp.status == 403 && msg_notifier_) {
            if (code == kRtcLocalRespPasswordRejected) {
                ...
                const auto rejection = code == kRtcLocalRespOccupied
                                           ...
                                           : ((code == kRtcLocalRespTicketRejected || code == kRtcLocalRespDirectGrantRejected ||
    ...
    LOGI("Got rtc local answer sdp, size: {}, will set remote desc.", answer_sdp.size());
    {
        ...
        if (!track_monitors_.empty()) {
            ...
            for (size_t i = 0; i < track_monitors_.size(); ++i) {
                ...
                LOGI("  track #{} -> {} {}x{} @({},{})-({},{})", i, m.name_, m.width_, m.height_, m.left_, m.top_, m.right_, m.bottom_);
            }
        } else {
...
void WebRtcLocalConnection::OnEncodedVideoFrame(int track_index, bool key, int w, int h, std::shared_ptr<Data> encoded) {
    ...
    uint64_t frame_index = 0;
    {
        ...
        if (!track_got_keyframe_[track_index]) {
            ...
            LOGI("Rtc local track #{}({}): first key frame, start feeding the decoder chain.", track_index, mon.name_);
        }
...
void WebRtcLocalConnection::UpdateTrackMonitors(const ServerConfiguration& config) {
    ...
    for (size_t index = 0; index < track_monitors_.size(); ++index) {
        ...
        const auto active_monitor = std::find_if(active_monitors.begin(), active_monitors.end(),
                                                 [&track_slots, index](const auto& monitor) { return monitor.name_ == track_slots[index]; });
        track_monitors_[index] = active_monitor != active_monitors.end() ? *active_monitor : RtcLocalTrackMonitor{};
        ...
        LOGI("Rtc local track slot #{} rebound: '{}' -> '{}'", index, old_name, track_monitors_[index].name_);
    }
}
