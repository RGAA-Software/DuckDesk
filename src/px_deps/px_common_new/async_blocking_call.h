...
// result.
template<typename T>
PxAwaitable<PxResult<T>> AwaitBlockingCall(
    const PxBlockingTaskPoster& post_blocking,
    ...
    if (cancellation->load(std::memory_order_acquire)) {
        co_return PxResult<T>::Failure(MakePxAsyncError(
            ...
            std::move(stage),
            "blocking operation was cancelled before dispatch"));
    }
    if (std::chrono::steady_clock::now() >= deadline) {
        co_return PxResult<T>::Failure(MakePxAsyncError(
            PxAsyncErrorCode::kTimeout,
            std::move(stage),
            "blocking operation deadline expired before dispatch",
            true));
    }

    const auto operation = PxAsyncOneShot<T>::Create(
        std::move(completion_executor));
    try {
        post_blocking(
            [operation, cancellation, call = std::move(call), stage]() mutable {
                if (cancellation->load(std::memory_order_acquire)) {
                    static_cast<void>(operation->TryFail(MakePxAsyncError(
                        PxAsyncErrorCode::kCancelled,
                        stage,
                        "blocking operation was cancelled before execution")));
                    return;
                }
                try {
                    ...
                    if (cancellation->load(std::memory_order_acquire)) {
                        static_cast<void>(operation->TryFail(MakePxAsyncError(
                            PxAsyncErrorCode::kCancelled,
                            stage,
                            "blocking operation was cancelled during execution")));
                        return;
                    }
                    static_cast<void>(operation->TryComplete(
                        PxResult<T>::Success(std::move(value))));
                }
                    ...
                    static_cast<void>(operation->TryFail(MakePxAsyncError(
                        PxAsyncErrorCode::kProtocolError,
                        stage,
                        "blocking operation threw a non-standard exception",
                        true)));
                }
            });
    }
        ...
        co_return PxResult<T>::Failure(MakePxAsyncError(
            PxAsyncErrorCode::kServiceStopped,
            std::move(stage),
            "blocking executor rejected the operation",
            true));
    }
