...
PxCallbackQuiescence::Lease::Lease(std::shared_ptr<State> state) : state_(std::move(state)) {}

PxCallbackQuiescence::Lease::~Lease() {
    Release();
}

PxCallbackQuiescence::Lease::Lease(Lease&& other) noexcept : state_(std::move(other.state_)) {}
...
PxCallbackQuiescence::PxCallbackQuiescence() : state_(std::make_shared<State>()) {}
...
PxAwaitable<PxResult<void>> PxCallbackQuiescence::WaitUntilQuiescent(std::shared_ptr<PxCallbackQuiescence> gate,
                                                                     const std::chrono::steady_clock::time_point deadline, std::string stage) {
    if (!gate) {
        co_return PxResult<void>::Failure(
            MakePxAsyncError(PxAsyncErrorCode::kInvalidArgument, std::move(stage), "callback quiescence gate is missing"));
    }
    ...
    while (gate->Outstanding() != 0) {
        ...
        if (now >= deadline) {
            co_return PxResult<void>::Failure(
                MakePxAsyncError(PxAsyncErrorCode::kTimeout, std::move(stage), "callback quiescence deadline expired", false));
        }
