...
namespace px_cms
{
    ...
    std::shared_ptr<CmsDevice> CmsDevice::FromJson(const std::string& body) {
        try {
    ...
    std::shared_ptr<CmsDevice> CmsDevice::FromObj(const json& obj) {
        try {
            auto device = std::make_shared<CmsDevice>();
