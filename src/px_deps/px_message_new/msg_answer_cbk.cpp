...
void MsgAnswerCallbackStructure::Add(const std::shared_ptr<px::Message>& msg, OnMsgParseRespCallbackFuncType callback) {
...
void MsgAnswerCallbackStructure::On100MsTimer() {
	...
	for (auto it = send_msg_resp_callback_map_.begin(); it != send_msg_resp_callback_map_.end();) {
		if (now - it->second.send_time >= std::chrono::seconds(4)) {
			auto timeout_message = std::make_shared<px::Message>();
			timeout_message->set_file_operate_resp_code(px::kRespCodeTimeout);
...
void MsgAnswerCallbackStructure::HandleRespAnswerMessage(const std::shared_ptr<px::Message>& message) {
	std::lock_guard<std::mutex> lg(mutex_);
