...
namespace px
{
	...
	public:
		...
		std::vector<std::vector<unsigned char>> Encode(std::span<const opus_int16> pcm, int frame_size);
		std::vector<std::vector<unsigned char>> Encode(std::span<const std::byte> pcm, int frame_size);

		bool valid() const noexcept { return valid_; }
		int SampleRate() const noexcept { return sample_rate_; }
		int Channels() const noexcept { return num_channels_; }
		int Bits() const noexcept { return bits_; }

	private:
		std::vector<unsigned char> EncodeFrame(std::span<const opus_int16> frame, int frame_size);
