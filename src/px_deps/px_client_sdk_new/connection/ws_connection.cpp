...
void WsConnection::Start() {
    ...
    reconnect_supervisor_ = PxReconnectSupervisor::Create(
        runtime, MakeWebSocketReconnectOptions("sdk_ws"));
    ...
    client_->bind_init([weak_self]() {
        ...
        if (asio2::get_last_error()) {
            LOGW("event=transport.connection_attempt component=sdk_ws code={} operation=connect "
        ...
        if (asio2::get_last_error()) {
            LOGW("event=transport.connection_attempt component=sdk_ws code={} operation=upgrade "
                 "outcome=failed recoverable=true reason={}",
                 asio2::last_error_val(), asio2::last_error_msg());
            static_cast<void>(self->reconnect_supervisor_->FailActive(MakePxAsyncError(
    ...
    PxReconnectSupervisorHooks hooks{
        ...
        .stop_attempt = [client = client_](const std::chrono::steady_clock::time_point deadline) {
            return StopWebSocketAdapter(client, deadline, "sdk-ws.retry-reset");
        },
...
std::shared_ptr<PxAsyncScope> WsConnection::BeginStop() {
    ...
    static_cast<void>(RequestAsioClientStop(client_, "sdk-ws.adapter-stop"));
    if (async_scope_) {
...
void WsConnection::Stop() {
    ...
    static_cast<void>(RequestAsioClientStop(client_, "sdk-ws.adapter-stop-confirm"));
