...
RelayClientSdk::RelayClientSdk(const RelayClientSdkParam& param) : RelayClientSdk(param, std::shared_ptr<PxAsyncRuntime>{}) {}

RelayClientSdk::RelayClientSdk(const RelayClientSdkParam& param, std::shared_ptr<PxAsyncRuntime> async_runtime)
    : RelayClientSdk(param, std::make_shared<RelayWsClient>(param.host_, param.port_, param.device_id_, param.device_name_, param.stream_id_,
                                                            param.appkey_, param.force_gdi_, param.remote_device_id_, std::move(async_runtime))) {}

RelayClientSdk::RelayClientSdk(const RelayClientSdkParam& param, std::shared_ptr<RelayNetClient> net_client)
...
std::shared_ptr<FileTransferWritableSignal> RelayClientSdk::AcquireFileTransferWritableSignal() {
...
void RelayClientSdk::RelayProtoMessage(std::shared_ptr<Data> msg) {
    ...
    const auto weak_self = weak_from_this();
    net_client->PostNetTask([weak_self, msg = std::move(msg), room_id = std::move(room_id)]() {
        const auto self = weak_self.lock();
        if (!self) {
            return;
        }
...
void RelayClientSdk::OnCreatedRoomResp(const std::shared_ptr<RelayMessage>& msg) {
    ...
    LOGI("on create room response, device id: {}, remote device id: {}, room id: {}", sdk_param_.device_id_, sdk_param_.remote_device_id_,
         room_->room_id_);
}
...
void RelayClientSdk::RequestControl() {
    ...
    sub->set_safety_pwd_md5(sdk_param_.remote_password_hash_);
    this->PostBinMessage(rl_msg.SerializeAsString());
