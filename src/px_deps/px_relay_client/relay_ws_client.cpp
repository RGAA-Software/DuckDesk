...
RelayWsClient::RelayWsClient(const std::string& host, int port, const std::string& device_id, const std::string& device_name,
    ...
    : RelayNetClient(), adapter_slot_(std::make_shared<PxReconnectAdapterSlot<asio2::ws_client>>()), async_runtime_(std::move(runtime)) {
    this->host_ = host;
    this->port_ = port;
...
void RelayWsClient::Start() {
    ...
    auto ws_path = std::format("/relay?device_id={}&remote_device_id={}&device_name={}&stream_id={}&appkey={}", device_id_, remote_device_id_,
                               device_name_, stream_id_, appkey_);
    LOGI("Will connect relay websocket: {}:{}, device: {}, remote: {}", host_, port_, device_id_, remote_device_id_);
    const auto adapter_slot = adapter_slot_;
    const auto supervisor = reconnect_supervisor_;
    PxReconnectSupervisorHooks hooks{
        .start_attempt =
            [weak_self, adapter_slot, supervisor, host = host_, port = port_, path = std::move(ws_path)](const std::uint64_t generation) {
                const auto self = weak_self.lock();
                if (!self || self->exiting_) {
                    return PxResult<void>::Failure(
                        MakePxAsyncError(PxAsyncErrorCode::kServiceStopped, "relay-ws.start", "Relay websocket owner is stopping"));
                }
...
void RelayWsClient::PostReliableBinaryMessage(std::string msg, std::function<void(bool)> completion) {
    ...
    PostReliableWebSocketWrite(client, Data::From(std::move(msg)), std::move(completion),
                               [weak_self = weak_from_this(), weak_client = std::weak_ptr<asio2::ws_client>(client), generation] {
                                   ...
                                   return self->adapter_slot_ && self->adapter_slot_->Snapshot() == weak_client.lock() &&
                                          ...
                                          self->reconnect_supervisor_->IsReady();
                               });
}
...
void RelayWsClient::PostNetTask(std::function<void()>&& task) {
    ...
    if (!exiting_ && client && client->is_started() && supervisor && supervisor->IsReady()) {
        client->post_queued_event([t = std::move(task)]() { t(); });
    }
}

std::shared_ptr<FileTransferWritableSignal> RelayWsClient::AcquireFileTransferWritableSignal() {
