...
namespace px
{
    ...
    void PeerCallback::OnIceConnectionChange(webrtc::PeerConnectionInterface::IceConnectionState new_state) {
        ...
        if (conn_) {
            conn_->OnIceStateChanged((int)new_state);
        }
    }
    ...
    void PeerCallback::OnAddTrack(rtc::scoped_refptr<webrtc::RtpReceiverInterface> receiver,
        ...
        if (track->kind() == webrtc::MediaStreamTrackInterface::kVideoKind && conn_) {
            conn_->OnVideoTrack(static_cast<webrtc::VideoTrackInterface*>(track));
        }
        else if (track->kind() == webrtc::MediaStreamTrackInterface::kAudioKind && conn_) {
            conn_->OnAudioTrack(static_cast<webrtc::AudioTrackInterface*>(track));
        }
    }
