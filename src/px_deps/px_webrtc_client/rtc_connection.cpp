...
void InitializeRtcProcessResources() {
    static const auto log_forwarder = std::make_unique<WebrtcLogForwarder>();
    static std::once_flag initialize_once;
    std::call_once(initialize_once, []() {
        rtc::LogMessage::AddLogToStream(log_forwarder.get(), rtc::LS_WARNING);
        rtc::InitializeSSL();
    });
}
...
bool RtcConnection::Init(const std::string& remote_device_id) {
    ...
    create_sess_callback_->SetOnCreateSdpFailedCallback([=, this](const std::string& msg) { LOGE("Create sdp failed: {}", msg); });
    ...
    set_local_sdp_callback_->SetSdpFailedCallback([=, this](const std::string& msg) { LOGI("Set local desc failed: {}", msg); });
    ...
    set_remote_sdp_callback_->SetSdpFailedCallback([=, this](const std::string& msg) { LOGI("Set remote desc failed: {}", msg); });
...
void RtcConnection::CreatePeerConnection() {
    ...
    if (!file_transfer_only_) {
        ...
        RTCErrorOr<rtc::scoped_refptr<DataChannelInterface>> r_dc = peer_conn->CreateDataChannelOrError(ch_name, &config);
        if (!r_dc.ok()) {
...
void RtcConnection::RequestStats() {
    ...
    rtc::scoped_refptr<webrtc::RTCStatsCollectorCallback> callback(new rtc::RefCountedObject<RtcStatsJsonCallback>(stats_json_cbk_));
