...
async function decode(log: (v: string) => void) {
    ...
    const iformat = await getAVFormat(
        ioReader,
        file || formatUrl('video/test.mp4')
    );
    ...
    const stream = iformatContext.getStreamByMediaType(
        AVMediaType.AVMEDIA_TYPE_VIDEO
    );

    const decoder = new WasmVideoDecoder({
        resource: await compileResource(
            getWasm('decoder', stream.codecpar.codecId),
            true
        ),
    ...
    console.log("stream.codecpar: ", stream.codecpar);
