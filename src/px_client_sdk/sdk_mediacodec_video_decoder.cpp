...
constexpr int kMaximumOutputDequeueAttempts = 25;
constexpr std::int64_t kOutputDequeueTimeoutMicroseconds = 2'000;

std::int64_t MonotonicTimeMicroseconds() {
    timespec current_time{};
    clock_gettime(CLOCK_MONOTONIC, &current_time);
    return static_cast<std::int64_t>(current_time.tv_sec) * 1'000'000 + static_cast<std::int64_t>(current_time.tv_nsec) / 1'000;
}
...
int MediacodecVideoDecoder::Init(const std::string& mon_name, VideoType codec_type, int width, int height, const std::string& frame,
    ...
    if (VideoDecoder::Init(mon_name, codec_type, width, height, frame, img_format, ignore_hw) != 0) return -1;
    ...
    media_status_t status = AMediaCodec_configure(media_codec_.get(), media_format_.get(), window_.get(), nullptr,
...
Result<std::shared_ptr<RawImage>, int> MediacodecVideoDecoder::Decode(std::span<const std::uint8_t> encoded) {
    ...
    if (input_index >= 0) {
        ...
        if (AMediaCodec_queueInputBuffer(media_codec_.get(), input_index, 0, encoded.size(), MonotonicTimeMicroseconds(), 0) != AMEDIA_OK) {
            return TRError(-1);
        }
    }
    for (int attempt{}; attempt < kMaximumOutputDequeueAttempts; ++attempt) {
        AMediaCodecBufferInfo info{};
        const auto output_index = AMediaCodec_dequeueOutputBuffer(media_codec_.get(), &info, kOutputDequeueTimeoutMicroseconds);
        if (output_index >= 0) {
            ...
            const std::unique_ptr<AMediaFormat, decltype(&AMediaFormat_delete)> format{AMediaCodec_getOutputFormat(media_codec_.get()),
                                                                                       &AMediaFormat_delete};
            std::int32_t width{};
            std::int32_t height{};
        ...
        if (output_index != AMEDIACODEC_INFO_TRY_AGAIN_LATER && output_index != AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED &&
...
bool MediacodecVideoDecoder::Ready() { return inited_; }
