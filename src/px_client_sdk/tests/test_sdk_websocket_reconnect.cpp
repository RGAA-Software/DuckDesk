...
std::shared_ptr<WsConnection> MakeConnection(const std::shared_ptr<MessageNotifier>& notifier, const int port) {
    return std::make_shared<WsConnection>(notifier, "127.0.0.1", port, "/sdk-reconnect-test");
}

std::shared_ptr<WssConnection> MakeSecureConnection(const std::shared_ptr<MessageNotifier>& notifier, const int port) {
...
TEST(SdkWebSocketReconnect, InitiallyUnavailableServerIsRetriedUntilItStarts) {
    ...
    connection->RegisterOnConnectedCallback([connected] { connected->fetch_add(1, std::memory_order_acq_rel); });
    connection->Start();
    ...
    ASSERT_TRUE(WaitUntil(
        [connection, generation_before_server_start] {
...
TEST(SdkWebSocketReconnect, RealServerRestartsAdvanceEveryGeneration) {
    ...
    connection->RegisterOnConnectedCallback([connected] { connected->fetch_add(1, std::memory_order_acq_rel); });
    connection->RegisterOnDisConnectedCallback([disconnected] { disconnected->fetch_add(1, std::memory_order_acq_rel); });
    connection->Start();
    ...
    for (int cycle = 0; cycle < 3; ++cycle) {
        ...
        ASSERT_TRUE(WaitUntil(
            [connection, previous_generation] { return connection->IsAlive() && connection->ConnectionGeneration() > previous_generation; }, 10s))
            << "cycle=" << cycle;
...
TEST(SdkWebSocketReconnect, IngressRejectsStaleAndStoppedAttemptGenerations) {
    ...
    const auto supervisor = PxReconnectSupervisor::Create(runtime, {.component = "sdk-ingress-test", .backoff = {.retry_delay = 1ms}});
...
TEST(SdkWebSocketReconnect, RealAdmissionRejectionRetriesAndAcceptsTheRecoveredSession) {
    const auto notifier = std::make_shared<MessageNotifier>();
    const auto server = std::make_shared<asio2::ws_server>();
