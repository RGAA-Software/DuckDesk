...
WsConnection::WsConnection(const std::shared_ptr<MessageNotifier>& notifier, const std::string& host, const int port, const std::string& path)
            ...
            [weak_self, adapter_slot, supervisor, host = host_, port = port_, path = path_](const std::uint64_t generation) {
                ...
                if (!self || self->exiting_) {
                    return PxResult<void>::Failure(
                        MakePxAsyncError(PxAsyncErrorCode::kServiceStopped, "sdk-ws.start", "SDK websocket owner is stopping"));
                }
                ...
                client
                    ...
                    .bind_connect([weak_self, weak_client, supervisor, generation]() {
                        ...
                        if (asio2::get_last_error()) {
                            static_cast<void>(supervisor->FailActive(generation, MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected,
                                                                                                  "sdk-ws.connect", asio2::last_error_msg(), true)));
                            return;
                        }
                        LOGI("event=transport.tcp_connected component=sdk_ws local_address={} local_port={}", current->local_address(),
                             current->local_port());
                    })
                    .bind_disconnect([weak_self, supervisor, generation]() {
                        if (const auto self = weak_self.lock(); self && !self->exiting_.load(std::memory_order_acquire)) {
                            ...
                            static_cast<void>(
                                supervisor->MarkDisconnected(generation, MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected, "sdk-ws.disconnect",
                                                                                          "SDK websocket disconnected", true)));
                        }
                    })
                        ...
                        if (const auto self = weak_self.lock(); self && !self->exiting_.load(std::memory_order_acquire)) {
                            if (asio2::get_last_error()) {
                                static_cast<void>(supervisor->FailActive(
                                    generation, MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "sdk-ws.upgrade", asio2::last_error_msg(), true)));
                                return;
                            }
...
void WsConnection::Stop() {
    const auto adapter = [self = weak_from_this().lock()] {
        if (!self) {
            return std::shared_ptr<asio2::ws_client>{};
        }
        std::lock_guard lock(self->stop_mutex_);
        return self->adapter_slot_ ? self->adapter_slot_->Snapshot() : std::shared_ptr<asio2::ws_client>{};
    }();
