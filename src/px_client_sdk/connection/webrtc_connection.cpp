...
#include "px_client_sdk/connection/rtc_ice_restart_workflow.h"
#include "px_client_sdk/sdk_messages.h"
#include "px_client_sdk/thunder_sdk.h"
#include "px_client_sdk/connection/relay_connection.h"
...
std::shared_ptr<WebRtcConnection> WebRtcConnection::Make(const std::shared_ptr<RelayConnection>& relay_conn,
...
WebRtcConnection::WebRtcConnection(const std::shared_ptr<RelayConnection>& relay_conn, const std::shared_ptr<ThunderSdkParams>& params,
                                   const std::shared_ptr<MessageNotifier>& notifier)
    ...
    if (msg_notifier_) {
        async_scope_ = PxAsyncScope::Create(msg_notifier_->GetAsyncRuntime(), PxAsyncLane::kState);
        if (async_scope_) {
            ice_restart_workflow_ = RtcIceRestartWorkflow::Create(async_scope_->Executor());
        }
    }
...
void WebRtcConnection::Prepare() {
    ...
    msg_listener_->Listen<SdkMsgNetworkConnected>([](const SdkMsgNetworkConnected&) { LOGI("Sdk msg, network connected."); });
...
void WebRtcConnection::Stop() {
    ...
    if (ice_restart_workflow_) {
        static_cast<void>(
            ice_restart_workflow_->Cancel(MakePxAsyncError(PxAsyncErrorCode::kServiceStopped, "rtc_shutdown", "RTC connection is stopping")));
    }
    if (async_scope_) {
...
void WebRtcConnection::Init() {
    ...
    RunInRtcThread([weak_self]() {
        ...
        self->rtc_client_->SetOnLocalIceCallback([weak_self](const std::string& ice, const std::string& mid, int sdp_mline_index) {
            if (auto locked = weak_self.lock()) {
        ...
        self->rtc_client_->SetOnVideoFrameCallback([weak_self](int w, int h, std::shared_ptr<Data> i420) {
        ...
        self->rtc_client_->SetOnAudioDataCallback([weak_self](std::shared_ptr<Data> pcm, int sample_rate, int channels) {
...
void WebRtcConnection::CreateRtcClientInstance() {
    ...
    RunInRtcThread([weak_self]() {
        ...
        self->rtc_client_ = CreateRtcClient();
        if (!self->rtc_client_) {
            LOGE("RTC client creation failed: stage=create_client outcome=unavailable recoverable=false");
            self->NotifyDisconnectedOnce();
            return;
        }
        LOGI("RTC client created: boundary=direct_cpp_dll outcome=success");
    });
}
...
void WebRtcConnection::SetOnVideoFrameCallback(const std::function<void(int w, int h, std::shared_ptr<Data> i420)>& cbk) {
    video_frame_cbk_ = cbk;
}

void WebRtcConnection::SetOnAudioDataCallback(const std::function<void(std::shared_ptr<Data> pcm, int sample_rate, int channels)>& cbk) {
    audio_data_cbk_ = cbk;
}
...
void WebRtcConnection::RunInRtcThread(std::function<void()>&& task) {
    thread_->Post([task = std::move(task)]() { task(); });
}

void WebRtcConnection::SendSdpToRemote(const std::string& sdp) {
    if (sdk_params_->connection_ticket_.empty() && sdk_params_->connection_nonce_.empty()) {
...
bool WebRtcConnection::RestartIce(const std::string& ice_config_json, const std::string& connection_ticket, const std::string& client_nonce,
                                  const std::string& instance_id, std::uint64_t revision) {
    if (stopped_ || !rtc_client_ || ice_config_json.empty() || connection_ticket.empty() || client_nonce.empty() || !ice_restart_workflow_ ||
        !async_scope_) {
        return false;
    }
    const auto begin = ice_restart_workflow_->ApplyConfiguration(revision);
    if (!begin.ShouldApplyConfiguration()) {
        LOGI("Ignore duplicate/stale RTC ICE configuration, revision={}, disposition={}", revision, static_cast<int>(begin.disposition));
        return true;
    }
    if (begin.StartedWorkflow() && !SpawnManagedIceRestartWait(begin)) {
        static_cast<void>(ice_restart_workflow_->Cancel(
            MakePxAsyncError(PxAsyncErrorCode::kServiceStopped, "rtc_restart_scope", "RTC async scope rejected the restart workflow")));
        return false;
    }
    ...
    RunInRtcThread([weak_self, weak_workflow, ice_config_json, generation, apply_sequence]() {
        ...
        if (!self || !self->rtc_client_ || self->stopped_ || !self->rtc_client_->RestartIce(ice_config_json)) {
            if (workflow->MarkApplyFailed(generation, apply_sequence,
                                          MakePxAsyncError(PxAsyncErrorCode::kServiceRejected, "set_configuration",
                                                           "active RTC SetConfiguration/RestartIce failed", true,
...
void WebRtcConnection::BeginManagedIceRestart() {
    ...
    if (!msg_notifier_->PublishAppMessage(SdkMsgRtcIceRestartNeeded{})) {
        static_cast<void>(ice_restart_workflow_->Cancel(
            MakePxAsyncError(PxAsyncErrorCode::kQueueFull, "request_rtc_configuration", "could not queue the managed RTC restart request", true)));
        return;
    }
...
bool WebRtcConnection::SpawnManagedIceRestartWait(const RtcIceRestartBegin& begin) {
    ...
    return async_scope_->Spawn("managed-rtc-ice-restart",
                               [weak_connection, workflow, begin]() { return AwaitManagedIceRestart(weak_connection, workflow, begin); });
}

PxAwaitable<void> WebRtcConnection::AwaitManagedIceRestart(std::weak_ptr<WebRtcConnection> weak_connection,
                                                           std::shared_ptr<RtcIceRestartWorkflow> workflow, RtcIceRestartBegin begin) {
    ...
    auto result = co_await RtcIceRestartWorkflow::Operation::WaitUntil(begin.operation, std::chrono::steady_clock::now() + kRestartDeadline);
    ...
    if (!result.HasValue()) {
        ...
        if (!connection->stopped_) {
            ...
            LOGE("Managed RTC ICE restart failed: code={}, stage={}, reason={}", result.Error().StableCode(), result.Error().stage,
                 result.Error().message);
            connection->NotifyDisconnectedOnce();
        }
        co_return;
    }
    LOGI("Managed RTC ICE restart completed, generation={}, revision={}", result.Value().generation, result.Value().revision);
    co_return;
}
...
void WebRtcConnection::UpdateTransportStats(const std::string& json) {
    try {
        ...
        for (const auto& entry : entries) {
            ...
            if ((type == "local-candidate" || type == "remote-candidate") && entry.contains("id")) {
                candidates[entry.value("id", "")] = entry;
            } else if (type == "candidate-pair" && entry.value("nominated", false) && entry.value("state", "") == "succeeded") {
                selected = entry;
            }
        }
        if (selected.empty()) {
        ...
        const auto rtt = selected.value("currentRoundTripTime", selected.value("current_round_trip_time", 0.0));
        const auto bitrate = selected.value("availableOutgoingBitrate", selected.value("available_outgoing_bitrate", 0.0));
...
void WebRtcConnection::NotifyConnectedWhenReady() {
    ...
    const bool channel_ready = sdk_params_->file_transfer_only_ ? IsFtChannelReady() : IsMediaChannelReady();
    if (!channel_ready) {
        return;
    }
    bool expected = false;
