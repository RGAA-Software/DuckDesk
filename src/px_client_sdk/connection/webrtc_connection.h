...
#include "px_client_sdk/connection/connection.h"
...
class WebRtcConnection : public Connection, public std::enable_shared_from_this<WebRtcConnection> {
  public:
    static std::shared_ptr<WebRtcConnection> Make(const std::shared_ptr<RelayConnection>& relay_conn, const std::shared_ptr<ThunderSdkParams>& params,
                                                  const std::shared_ptr<MessageNotifier>& notifier);

    explicit WebRtcConnection(const std::shared_ptr<RelayConnection>& relay_conn, const std::shared_ptr<ThunderSdkParams>& params,
                              const std::shared_ptr<MessageNotifier>& notifier);
    ~WebRtcConnection();
    ...
    void SetOnVideoFrameCallback(const std::function<void(int w, int h, std::shared_ptr<Data> i420)>&);
    void SetOnAudioDataCallback(const std::function<void(std::shared_ptr<Data> pcm, int sample_rate, int channels)>&);
    ...
    bool RestartIce(const std::string& ice_config_json, const std::string& connection_ticket, const std::string& client_nonce,
                    const std::string& instance_id, std::uint64_t revision);

  private:
    void Init();
    void Prepare();
    void CreateRtcClientInstance();
    ...
    static PxAwaitable<void> AwaitManagedIceRestart(std::weak_ptr<WebRtcConnection> weak_connection, std::shared_ptr<RtcIceRestartWorkflow> workflow,
  ...
  private:
    ...
    std::shared_ptr<RtcClient> rtc_client_ = nullptr;
