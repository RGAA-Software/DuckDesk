...
// - watchdog:30s 收不到任何 UDP 包视为媒体面断开。Render 会回送认证心跳，因此静态 DDA 桌面不依赖视频帧保活。
class UdpDirectConnection : public Connection, public std::enable_shared_from_this<UdpDirectConnection> {
  public:
    ...
    void Start(const std::string& host, int udp_port, const std::string& stream_id, const std::string& association_code);
    void Start() override {}
    void Stop() override;
  ...
  private:
    ...
    std::uint16_t selected_datagram_size_{1200};
    ...
    static constexpr int64_t kNoFrameTimeoutMs = 15000;
