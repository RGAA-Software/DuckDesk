...
class DataStoreRemoteSessionPreferencesRepository private constructor(
    ...
    override suspend fun load(deviceKey: String): RemoteSessionPreferences {
        ...
        return decodeRemoteSessionPreferences(
            ...
            decoderMode = stored[stringPreferencesKey("${keyPrefix}_decoder")],
            connectionRoute = stored[stringPreferencesKey("${keyPrefix}_route")],
        )
    }
    ...
    override suspend fun save(deviceKey: String, preferences: RemoteSessionPreferences) {
        ...
        dataStore.edit { stored ->
            ...
            stored[stringPreferencesKey("${keyPrefix}_decoder")] = preferences.decoderMode.name
            stored[stringPreferencesKey("${keyPrefix}_route")] = preferences.connectionRoute.name
        }
    }
...
internal fun decodeRemoteSessionPreferences(
    ...
    decoderMode = decoderMode?.let { stored -> runCatching { RemoteDecoderMode.valueOf(stored) }.getOrNull() } ?: RemoteDecoderMode.Automatic,
    connectionRoute = connectionRoute?.let { stored -> runCatching { RemoteConnectionRoute.valueOf(stored) }.getOrNull() }
        ?: RemoteConnectionRoute.Automatic,
)
