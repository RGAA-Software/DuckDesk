package com.pixels.yun.client.widgets;
...
public class RockerView extends View {
    ...
    private int DEFAULT_ROCKER_RADIUS = DEFAULT_SIZE / 8;
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        int negativeBorder = -32768;
        int positiveBorder = 32767;
        ...
        if (lenXY + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            ...
            targetPoint = new Point(showPointX, showPointY);
        }
        ...
        if (targetPoint.x < centerPoint.x) {
            // to left of center
            percentX = 1.0f - (targetPoint.x - mRockerRadius) * 1.0f / (centerPoint.x - mRockerRadius);
            //percentX *= -1.0f;
            thumbX = (int) (percentX * negativeBorder);
        } else {
            // to right of center
            percentX = (targetPoint.x - centerPoint.x) * 1.0f / (centerPoint.x - mRockerRadius);
            thumbX = (int) (percentX * positiveBorder);
        }

        float percentY = 0;
        if (targetPoint.y < centerPoint.y) {
            // to top of center
            percentY = 1.0f - (targetPoint.y - mRockerRadius) * 1.0f / (centerPoint.y - mRockerRadius);
            thumbY = (int) (percentY * positiveBorder);
        } else {
            // to bottom of center
            percentY = (targetPoint.y - centerPoint.x) * 1.0f / (centerPoint.y - mRockerRadius);
            //percentY *= -1;
            thumbY = (int) (percentY * negativeBorder);
        }
        //Log.i(TAG, "percent X : " + percentX + ", percent Y : " + percentY);

        // 回调 返回参数
        callBack(angle, thumbX, thumbY);
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle, int thumbX, int thumbY) {
        mCurrentAngle = (int) angle;
        mCurrentThumbX = thumbX;
        mCurrentThumbY = thumbY;

        if (null != mOnAngleChangeListener) {
            mOnAngleChangeListener.angle(angle, thumbX, thumbY);
        }
        if (null != mOnShakeListener) {
    ...
    public int getCurrentThumbX() {
        return mCurrentThumbX;
    }

    public int getCurrentThumbY() {
        return mCurrentThumbY;
    }
    ...
    public interface OnAngleChangeListener {
         ...
         */
        void angle(double angle, int thumbX, int thumbY);
