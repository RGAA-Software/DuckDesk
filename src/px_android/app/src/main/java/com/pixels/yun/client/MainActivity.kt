...
import com.pixels.yun.client.databinding.ActivityMainBinding
import com.pixels.yun.client.events.OnAddScanInfo
import com.pixels.yun.client.events.OnServerScanned
import com.pixels.yun.client.ui.BaseFragment
import com.pixels.yun.client.ui.MainTopRightMenu
import com.pixels.yun.client.ui.server.ServerFragment
import com.pixels.yun.client.ui.steam.SteamAppFragment
import com.pixels.yun.client.ui.me.SettingsFragment
import com.pixels.yun.client.ui.effects.EffectDisplayFragment
import com.pixels.yun.client.util.ScreenUtil
...
//            add(MeowBottomNavigation.Model(ID_MUSIC_EFFECTS, R.drawable.ic_eq))
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 100) {
            ...
            if (message != null) {
                val info = Base64.decode(message, Base64.DEFAULT).toString(Charsets.UTF_8)
    ...
    fun onAddScanInfoEvent(event: OnAddScanInfo) {
        ...
        NetworkChecker(appContext).checkScanInfoAvailable(event.scanInfo, object: NetworkChecker.OnScanInfoCheckAvailableCallback{
            override fun onCheck(scanInfo: ScanInfo) {
                Log.i(TAG, "target ip: ${scanInfo.workingIp}, can connect: ${scanInfo.canConnect()}")
    ...
    fun changeToTab(tab: Int) {
        when (tab) {
            ID_MACHINE -> {
                ...
                setActionBarTitle(getString(R.string.devices));
            }
            //ID_GAMES -> {
