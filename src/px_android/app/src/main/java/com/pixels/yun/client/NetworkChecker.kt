...
class NetworkChecker(val appContext: AppContext) {
    ...
    fun checkScanInfoAvailable(scanInfo: ScanInfo, cbk: OnScanInfoCheckAvailableCallback) {
        appContext.spawnInNewThread {
            scanInfo.deviceIpInfo.forEach {
                val url = "http://" + it.ip + ":" + scanInfo.panelServerPort + ServerApi.API_PING;
                ...
                try {
                    ...
                    Log.e(TAG, "check exception: " + e.message)
                    scanInfo.workingIp = ""
                ...
                if (data == "Pong") {
                    ...
                    scanInfo.workingIp = it.ip;
                    scanInfo.workingIpType = it.type;
                    cbk.onCheck(scanInfo);
