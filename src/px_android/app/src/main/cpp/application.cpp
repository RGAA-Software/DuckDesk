...
namespace px
{
    ...
    void Application::Init(const std::shared_ptr<ThunderSdkParams>& params, JNIEnv* env, jobject surface, bool hw_codec, bool use_oes, int oes_tex_id) {
        ...
        thunder_sdk_->SetOnServerConfigurationCallback([=, this](std::shared_ptr<px::Message> msg) {
            ...
            for (int i = 0; i < config.monitors_info_size(); i++) {
                const auto& monitor = config.monitors_info(i);
                LOGI("monitor index: {}, name: {}", i+1, monitor.name());
            }
    ...
    void Application::ChangeMonitor(int index, const std::string& name) {
        if (thunder_sdk_) {
            auto msg = ProtoMessageMaker::MakeChangeMonitor(index, name, sdk_params_->device_id_, sdk_params_->stream_id_);
            thunder_sdk_->PostMediaMessage(msg);
        }
    }
