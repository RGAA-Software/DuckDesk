...
M0–M2 已完成。当前应用已包含 Pixels 品牌与最终包名、设备发现和扫码、Quick Connect、Console 账号与设备、远程应用、短期连接票据、前台会话服务、MediaCodec Surface 视频、AAudio、完整桌面输入、虚拟/实体手柄、多显示器、虚拟显示协议、双向文本剪贴板、基于 SAF 的双向文件传输和任务中心、直接复用编码码流并发布到 MediaStore 的本地录制，以及经 Windows 用户同意的双向 Opus 语音通话。

语音通话使用 AAudio 通信流，支持麦克风/远端声音静音、听筒/耳机与扬声器切换，并在 Android 路由改变时重建音频流而不中断会话。M3 的手柄震动与特定环境验收仍待收口；M4 后续继续实现图片/文件 URI 剪贴板；M5–M6 的完整网络矩阵和发布门禁尚未完成。未完成能力不会以占位实现伪装为可用。
