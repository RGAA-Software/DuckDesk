...
//
// Deprecated:
// Historical C++ websocket entrypoint for the old service implementation.
// The active implementation is Rust.
    ...
    void ServiceMsgServer::Start() {
       ...
       .bind_recv([=, this](auto& sess_ptr, std::string_view data) {
           ...
           auto sw = [&]() -> std::shared_ptr<SessionWrapper> {
               if (auto session = sessions_.TryGet(socket_fd); session.has_value()) {
