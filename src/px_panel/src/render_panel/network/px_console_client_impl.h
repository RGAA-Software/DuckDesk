...
class PxConsoleClientImpl : public PxConsoleClient, public std::enable_shared_from_this<PxConsoleClientImpl<ClientType>> {
  public:
    explicit PxConsoleClientImpl(const std::shared_ptr<PxContext>& ctx, const std::string& host, int port, const std::string& device_id) {
        context_ = ctx;
        host_ = host;
        port_ = port;
    ...
    void Start() override {
            ...
            if (const auto runtime = notifier->GetAsyncRuntime()) {
                {
                    ...
                    reconnect_supervisor_ = PxReconnectSupervisor::Create(runtime, MakeWebSocketReconnectOptions("panel_console"));
                }
        ...
        try {
            record_fetch_blocking_executor_ = PxBlockingExecutor::Create({.thread_count = 1, .max_pending_tasks = 32});
            record_list_blocking_executor_ = PxBlockingExecutor::Create({.thread_count = 1, .max_pending_tasks = 32});
        } catch (const std::exception& error) {
            LOGE("event=module.start component=panel_console code=BLOCKING_EXECUTOR_CREATE_FAILED operation=start_client "
        ...
        PxReconnectSupervisorHooks hooks{
            .start_attempt =
                [weak_self, supervisor, host = host_, port = port_](const std::uint64_t generation) {
                    const auto self = weak_self.lock();
                    if (!self || self->stopping_) {
                        return PxResult<void>::Failure(
                            MakePxAsyncError(PxAsyncErrorCode::kServiceStopped, "panel-console.start", "Panel Console owner is stopping"));
                    }
                    ...
                    client
                        ->bind_init([weak_self, weak_client]() {
                            ...
                            current->set_upgrade_target(std::format("{}?appkey={}&token={}&ts={}&nonce={}&device_id={}&user_id={}", endpoint,
                                                                    grApp->GetAppkey(), token.token, token.ts, token.nonce, owner->device_id_,
                                                                    user_id));
                        })
                        .bind_connect([weak_self, weak_client, supervisor, generation]() {
                            ...
                            if (asio2::get_last_error()) {
                                static_cast<void>(
                                    supervisor->FailActive(generation, MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected,
                                                                                        "panel-console.connect", asio2::last_error_msg(), true)));
                                return;
                            }
                        ...
                        .bind_upgrade([weak_self, supervisor, generation]() {
                            if (const auto owner = weak_self.lock(); owner && !owner->stopping_) {
                                if (asio2::get_last_error()) {
                                    ...
                                    static_cast<void>(
                                        supervisor->FailActive(generation, MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "panel-console.upgrade",
                        ...
                        .bind_disconnect([weak_self, supervisor, generation]() {
                            if (const auto owner = weak_self.lock(); owner && !owner->stopping_) {
                                static_cast<void>(supervisor->MarkDisconnected(
                                    generation, MakePxAsyncError(PxAsyncErrorCode::kServiceNotConnected, "panel-console.disconnect",
                                                                 "Panel disconnected from Console", true)));
                            }
                        })
    ...
    void Stop() override {
        ...
        if (first_stop && record_list_blocking_executor_) {
            record_list_blocking_executor_->RequestStop(PxBlockingShutdownMode::kCancelPending);
            record_list_blocking_executor_->Join();
            record_list_blocking_executor_.reset();
        }
        if (first_stop && fetch_queue_) {
            fetch_queue_->Stop();
        }
        if (first_stop) {
            if (const auto cancellation = active_record_upload_cancellation_.exchange({})) {
        ...
        if (first_stop && record_fetch_blocking_executor_) {
            record_fetch_blocking_executor_->RequestStop(PxBlockingShutdownMode::kCancelPending);
            record_fetch_blocking_executor_->Join();
            record_fetch_blocking_executor_.reset();
        }
        if (first_stop) {
        ...
        const auto result = StopWebSocketConnectionBlocking(client, connection_scope, std::chrono::seconds(5), "panel-console.stop");
        if (!result.Succeeded()) {
            LOGE("event=async.scope_drain component=panel_console code=ASYNC_SCOPE_DRAIN_TIMEOUT operation=stop "
                 ...
                 result.scope_drained, result.adapter_stopped, connection_scope ? connection_scope->GetStatistics().outstanding : 0);
            return;
        }
        std::lock_guard lock(network_mutex_);
        client_.reset();
  ...
  private:
    ...
    void RefreshRtcConfigAsync(uint64_t expected_revision) {
        ...
        auto weak_self = this->weak_from_this();
        if (!scope->Spawn("panel-rtc-config-refresh", [weak_self]() { return RunRtcConfigRefresh(weak_self); })) {
            rtc_config_refresh_gate_->AbortStart();
            LOGW("Panel RTC config refresh was rejected by the async scope");
        }
    }

    static PxAwaitable<PxResult<PanelRtcConfigHttpResponse>> FetchRtcConfig(std::shared_ptr<PxContext> context, asio::any_io_executor executor,
                                                                            std::string host, int port, std::string appkey) {
        const auto operation = PxAsyncOneShot<PanelRtcConfigHttpResponse>::Create(executor);
        context->PostNetworkTask([operation, host = std::move(host), port, appkey = std::move(appkey)]() {
            try {
                const auto client = HttpClient::MakeSSL(host, port, "/api/v1/rtc/ice-config", 5000);
                client->SetHeader("x-px-appkey", appkey);
                const auto response = client->Request();
                static_cast<void>(operation->TryComplete(PxResult<PanelRtcConfigHttpResponse>::Success({
                    .status = response.status,
                    .body = response.body,
                })));
            } catch (const std::exception& error) {
                static_cast<void>(
                    operation->TryFail(MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "panel_rtc_config_http", error.what(), true)));
            }
        });
        co_return co_await PxAsyncOneShot<PanelRtcConfigHttpResponse>::WaitUntil(operation,
                                                                                 std::chrono::steady_clock::now() + std::chrono::seconds(6));
    }

    static PxAwaitable<void> RunRtcConfigRefresh(std::weak_ptr<PxConsoleClientImpl<ClientType>> weak_self) {
        while (true) {
            ...
            auto result = co_await FetchRtcConfig(std::move(context), std::move(executor), std::move(host), port, grApp->GetAppkey());
            const auto current = weak_self.lock();
            ...
            if (!result.HasValue()) {
                LOGW("Pull Panel RTC ICE configuration failed: code={}, stage={}, reason={}", result.Error().StableCode(), result.Error().stage,
                ...
                try {
                    ...
                    if (envelope.value("code", -1) == 200 && revision >= attempt.expected_revision &&
    ...
    void HandleRecordListReq(const console_panel::RecordListReq& req) {
        ...
        if (!scope->Spawn("panel-record-list",
                          [weak_self, gate, attempt, req_id]() { return RunRecordListRequest(weak_self, gate, attempt, req_id); })) {
            record_list_request_gate_->Finish(attempt.sequence);
            SendRecordListResponse(req_id, {}, "record list request was rejected");
        }
    }

    static PxAwaitable<PxResult<std::vector<RecordFileInfo>>> ScanRecordFilesAsync(std::shared_ptr<PxBlockingExecutor> blocking_executor,
                                                                                   fs::path records_dir,
                                                                                   std::shared_ptr<std::atomic_bool> cancellation_signal) {
        const auto completion_executor = co_await asio::this_coro::executor;
        const PxBlockingTaskPoster poster = [blocking_executor](std::function<void()> task) {
            if (!blocking_executor || blocking_executor->TryPost(std::move(task)) != PxBlockingSubmitResult::kAccepted) {
                throw std::runtime_error("record list blocking executor rejected task");
            }
        };
        co_return co_await AwaitBlockingCall<std::vector<RecordFileInfo>>(
            poster, completion_executor, std::chrono::steady_clock::now() + std::chrono::seconds(30), cancellation_signal, "panel_record_list_scan",
            [records_dir = std::move(records_dir)](const auto& cancellation) { return ScanRecordFiles(records_dir, cancellation); });
    }

    static PxAwaitable<void> RunRecordListRequest(std::weak_ptr<PxConsoleClientImpl<ClientType>> weak_self,
                                                  std::shared_ptr<RecordListRequestGate> gate, RecordListRequestAttempt attempt, std::string req_id) {
        std::shared_ptr<PxBlockingExecutor> blocking_executor;
        {
            const auto current = weak_self.lock();
            if (!current || current->stopping_ || !current->record_list_scope_ || !current->record_list_blocking_executor_) {
                gate->Finish(attempt.sequence);
                co_return;
            }
            blocking_executor = current->record_list_blocking_executor_;
        }
        auto result = co_await ScanRecordFilesAsync(std::move(blocking_executor), fs::path(GetRenderRecordsDir()), attempt.cancellation_signal);
        ...
        if (!result.HasValue()) {
            current->SendRecordListResponse(req_id, {}, std::format("{}: {}", result.Error().StableCode(), result.Error().message));
            co_return;
        }
    ...
    void SendRecordListResponse(const std::string& req_id, const std::vector<RecordFileInfo>& files, const std::string& error) {
        console_panel::ConsolePanelMessage msg;
    ...
    void StartRecordFetchPump() {
        ...
        const auto weak_self = this->weak_from_this();
        if (!scope->Spawn("panel-record-fetch", [weak_self]() { return RunRecordFetchPump(weak_self); })) {
            queue->AbortPump();
            LOGW("Panel record fetch pump was rejected by the async scope");
        }
    }

    static PxAwaitable<PxResult<std::string>> UploadRecordFileAsync(std::shared_ptr<PxBlockingExecutor> blocking_executor, RecordFetchTask task,
                                                                    std::string appkey, std::shared_ptr<std::atomic_bool> cancellation_signal) {
        const auto completion_executor = co_await asio::this_coro::executor;
        const PxBlockingTaskPoster poster = [blocking_executor](std::function<void()> blocking_task) {
            if (!blocking_executor || blocking_executor->TryPost(std::move(blocking_task)) != PxBlockingSubmitResult::kAccepted) {
                throw std::runtime_error("record upload blocking executor rejected task");
            }
        };
        co_return co_await AwaitBlockingCall<std::string>(
            poster, completion_executor, std::chrono::steady_clock::now() + std::chrono::seconds(3605), cancellation_signal, "panel_record_upload",
            [task = std::move(task), appkey = std::move(appkey)](const auto& cancellation) { return UploadRecordFile(task, appkey, cancellation); });
    }

    static PxAwaitable<bool> WaitForRecordFetchRetry(asio::any_io_executor executor, std::chrono::milliseconds delay) {
        ...
        const auto cancellation = co_await asio::this_coro::cancellation_state;
        co_return !wait_error && cancellation.cancelled() == asio::cancellation_type::none;
    }

    static PxAwaitable<void> RunRecordFetchPump(std::weak_ptr<PxConsoleClientImpl<ClientType>> weak_self) {
        ...
        const auto coroutine_executor = co_await asio::this_coro::executor;
        while (!queue->IsStopped()) {
            ...
            {
                const auto current = weak_self.lock();
                if (!current || current->stopping_ || !current->record_fetch_scope_ || !current->record_fetch_blocking_executor_) {
                    queue->Finish(task.filename);
                    queue->AbortPump();
                    co_return;
                }
                blocking_executor = current->record_fetch_blocking_executor_;
                current->active_record_upload_cancellation_.store(cancellation_signal);
            }
            auto result = co_await UploadRecordFileAsync(std::move(blocking_executor), task, grApp->GetAppkey(), cancellation_signal);
            ...
            if (result.HasValue()) {
                ...
                err = std::format("{}: {}", result.Error().StableCode(), result.Error().message);
            }
            if (err.empty()) {
                queue->Finish(task.filename);
    ...
    static std::string UploadRecordFile(const RecordFetchTask& task, const std::string& appkey,
        ...
        auto client = ssl ? HttpClient::MakeSSL(host, port, upath, 3600 * 1000) : HttpClient::Make(host, port, upath, 3600 * 1000);
  ...
  private:
    ...
    std::shared_ptr<PanelRtcConfigRefreshGate> rtc_config_refresh_gate_ = PanelRtcConfigRefreshGate::Create();
    ...
    std::shared_ptr<PxBlockingExecutor> record_fetch_blocking_executor_ = nullptr;
    ...
    std::shared_ptr<PxBlockingExecutor> record_list_blocking_executor_ = nullptr;
    std::shared_ptr<RecordListRequestGate> record_list_request_gate_ = RecordListRequestGate::Create();
};
