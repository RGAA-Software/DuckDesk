...
namespace px
{
    namespace {
        bool IsDbReady(const std::shared_ptr<PxDatabase>& db) {
            if (!db || !db->IsReady()) {
                LOGE("DBGameOperator ignored because database is not ready");
                return false;
            }
            return true;
        }
    ...
    DBGameOperator::DBGameOperator(const std::shared_ptr<PxContext>& ctx, const std::shared_ptr<PxDatabase>& db) {
        context_ = ctx;
        db_ = db;
    }
