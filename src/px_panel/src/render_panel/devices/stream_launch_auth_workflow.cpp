...
template <typename T>
PxAwaitable<PxResult<StreamLaunchConsoleCall<T>>>
RunBlockingCall(StreamLaunchAuthHooks hooks, asio::any_io_executor executor, std::chrono::steady_clock::time_point deadline,
                std::shared_ptr<std::atomic_bool> cancelled, std::string stage, std::function<StreamLaunchConsoleCall<T>()> call) {
    co_return co_await AwaitBlockingCall<StreamLaunchConsoleCall<T>>(
        hooks.post_blocking, std::move(executor), deadline, std::move(cancelled), std::move(stage),
        [call = std::move(call)](const std::shared_ptr<std::atomic_bool>&) mutable { return call(); });
}

PxAsyncError ConsoleFailure(std::string stage, px_console::ConsoleApiError error, std::string server_message) {
    const auto message = server_message.empty() ? std::string(px_console::ConsoleApiErrorAsString(error)) : std::move(server_message);
    const bool retryable = error == px_console::ConsoleApiError::kNetworkUnavailable || error == px_console::ConsoleApiError::kRateLimited ||
                           error == px_console::ConsoleApiError::kServiceUnavailable;
    return MakePxAsyncError(PxAsyncErrorCode::kServiceRejected, std::move(stage), message, retryable, std::to_string(static_cast<int>(error)));
}

template <typename T> PxResult<T> TakeConsoleValue(PxResult<StreamLaunchConsoleCall<T>> call_result, std::string stage) {
    if (!call_result) {
        return PxResult<T>::Failure(call_result.Error());
    }
    auto call = call_result.TakeValue();
    if (!call.value) {
        return PxResult<T>::Failure(ConsoleFailure(std::move(stage), call.error, std::move(call.server_message)));
    }
    return PxResult<T>::Success(std::move(*call.value));
}

PxAwaitable<bool> WaitForPoll(asio::any_io_executor executor, std::chrono::steady_clock::time_point deadline,
    ...
    timer->expires_at(std::min(deadline, std::chrono::steady_clock::now() + std::chrono::milliseconds(800)));
    asio::error_code error;
    co_await timer->async_wait(asio::redirect_error(asio::use_awaitable, error));
    co_return !error && !cancelled->load(std::memory_order_acquire) && std::chrono::steady_clock::now() < deadline;
}

PxResult<void> ValidateRequest(const StreamLaunchAuthRequest& request, const StreamLaunchAuthHooks& hooks) {
    const bool common_valid = !request.client_nonce.empty() && !request.permissions.empty() && request.deadline > std::chrono::steady_clock::now() &&
...
std::shared_ptr<StreamLaunchAuthWorkflow> StreamLaunchAuthWorkflow::Create(const std::shared_ptr<PxAsyncRuntime>& runtime) {
...
StreamLaunchAuthWorkflow::StreamLaunchAuthWorkflow(std::shared_ptr<PxAsyncScope> scope) : scope_(std::move(scope)) {}

StreamLaunchAuthWorkflow::~StreamLaunchAuthWorkflow() {
    Stop();
}

std::optional<std::uint64_t> StreamLaunchAuthWorkflow::Start(StreamLaunchAuthRequest request, StreamLaunchAuthHooks hooks,
    ...
    if (!scope->Spawn("stream-launch-auth", [request = std::move(request), hooks = std::move(hooks), completion = std::move(completion), cancellation,
                                             generation]() mutable {
            return Run(std::move(request), std::move(hooks), std::move(completion), cancellation, generation);
        })) {
...
PxAwaitable<void> StreamLaunchAuthWorkflow::Run(StreamLaunchAuthRequest request, StreamLaunchAuthHooks hooks, StreamLaunchAuthCompletion completion,
    ...
    if (request.target == StreamLaunchTicketTarget::kApplicationInstance) {
        if (request.instance_id.empty()) {
            auto start_call = co_await RunBlockingCall<px_console::ConsoleUserAppInstance>(
                ...
                [hooks, app_id = request.app_id, nonce = request.client_nonce]() { return hooks.start_app(app_id, nonce); });
            auto start_result = TakeConsoleValue(std::move(start_call), "stream-launch.start-app");
            if (!start_result) {
            ...
            if (instance->instance_id.empty()) {
                completion(generation, StreamLaunchAuthResult::Failure(MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "stream-launch.start-app",
                                                                                        "Console returned an application instance without an ID",
                                                                                        false, "INVALID_APP_INSTANCE")));
                co_return;
            }
        ...
        while (instance && instance->state == "starting") {
            if (!co_await WaitForPoll(executor, request.deadline, cancelled)) {
                completion(generation,
                           StreamLaunchAuthResult::Failure(MakePxAsyncError(
                               cancelled->load(std::memory_order_acquire) ? PxAsyncErrorCode::kCancelled : PxAsyncErrorCode::kTimeout,
                               "stream-launch.wait-running",
                               cancelled->load(std::memory_order_acquire) ? "stream launch was replaced"
                                                                          : "Console application did not reach running before the deadline",
                               !cancelled->load(std::memory_order_acquire))));
                co_return;
            }
            auto query_call = co_await RunBlockingCall<std::vector<px_console::ConsoleUserApplication>>(
                hooks, executor, request.deadline, cancelled, "stream-launch.query-apps", [hooks]() { return hooks.query_apps(); });
            auto query_result = TakeConsoleValue(std::move(query_call), "stream-launch.query-apps");
            if (!query_result) {
                if (query_result.Error().code == PxAsyncErrorCode::kCancelled || query_result.Error().code == PxAsyncErrorCode::kTimeout) {
            ...
            const auto app = std::find_if(applications.begin(), applications.end(), [&request](const px_console::ConsoleUserApplication& candidate) {
                return candidate.app_id == request.app_id;
            });
            if (app != applications.end() && app->running_instance && app->running_instance->instance_id == request.instance_id) {
        ...
        if (instance && instance->state != "running") {
            completion(generation,
                       StreamLaunchAuthResult::Failure(MakePxAsyncError(PxAsyncErrorCode::kServiceRejected, "stream-launch.wait-running",
                                                                        "Console application entered state: " + instance->state, false,
                                                                        instance->error_code.empty() ? "APP_NOT_RUNNING" : instance->error_code)));
            co_return;
        }
    }

    PxResult<px_console::ConsoleConnectionTicket> ticket_result = PxResult<px_console::ConsoleConnectionTicket>::Failure(
        MakePxAsyncError(PxAsyncErrorCode::kProtocolError, "stream-launch.issue-ticket", "ticket request was not executed"));
    if (request.target == StreamLaunchTicketTarget::kApplicationInstance) {
        auto ticket_call = co_await RunBlockingCall<px_console::ConsoleConnectionTicket>(
            ...
            [hooks, instance_id = request.instance_id, nonce = request.client_nonce, permissions = request.permissions]() {
                return hooks.issue_instance_ticket(instance_id, nonce, permissions);
            });
        ticket_result = TakeConsoleValue(std::move(ticket_call), "stream-launch.issue-instance-ticket");
    } else {
        auto ticket_call = co_await RunBlockingCall<px_console::ConsoleConnectionTicket>(
            hooks, executor, request.deadline, cancelled, "stream-launch.issue-device-ticket",
            [hooks, device_id = request.device_id, nonce = request.client_nonce, permissions = request.permissions]() {
                return hooks.issue_device_ticket(device_id, nonce, permissions);
            });
        ticket_result = TakeConsoleValue(std::move(ticket_call), "stream-launch.issue-device-ticket");
    }
    if (!ticket_result) {
    ...
    if (resolved_ticket.ticket.stream_id.empty() || resolved_ticket.ticket.renewal_token.empty()) {
        completion(generation, StreamLaunchAuthResult::Failure(MakePxAsyncError(
                                   PxAsyncErrorCode::kProtocolError, "stream-launch.resolve-ticket",
