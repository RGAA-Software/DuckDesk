...
public:
    ...
    void RestartRender() override {
        if (!runtime_->Service()->RestartRender()) runtime_->Notify(true, std::string{px::ui::ApplicationName()}, "Render service is not connected");
    }
    void InstallControllerDriver() override {
        runtime_->Notify(false, std::string{px::ui::ApplicationName()}, "Controller driver installation is managed by the installer");
    }
    void RefreshEnvironment() override {
        if (environment_) static_cast<void>(environment_->Refresh());
    }
