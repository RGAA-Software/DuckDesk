...
public:
    ...
    ui::AccountSnapshot Snapshot() const override { return runtime_->Console()->Account(); }
    void Login(std::string username, std::string password) override { Start(std::move(username), std::move(password), false); }
    void Register(std::string username, std::string password) override { Start(std::move(username), std::move(password), true); }
    void UpdateProfile(std::string username) override {
        RunAccountOperation([username = std::move(username)](
                                const std::shared_ptr<PanelProductRuntime>& runtime) { return runtime->Console()->UpdateProfile(username); },
    ...
    void UpdateAvatar(std::string imagePath) override {
        RunAccountOperation([imagePath = std::move(imagePath)](
                                const std::shared_ptr<PanelProductRuntime>& runtime) { return runtime->Console()->UpdateAvatar(imagePath); },
                            "Avatar updated", "Avatar update failed");
    }
    void Logout() override {
        if (runtime_->Console()->Account().operation == ui::AccountOperationState::Working) return;
        ...
        static_cast<void>(runtime_->Worker()->Post([runtime, weakSelf] {
            ...
            runtime->Notify(!success, success ? std::string{px::ui::ApplicationName()} : "Error", success ? "Signed out" : "Sign out failed");
        }));
    }
...
private:
    ...
    void RunAccountOperation(Operation operation, std::string successMessage, std::string failureMessage) {
        ...
        const auto runtime = runtime_;
        const std::weak_ptr<ProductAccountPort> weakSelf{shared_from_this()};
        static_cast<void>(runtime_->Worker()->Post([runtime, weakSelf, operation = std::move(operation), successMessage = std::move(successMessage),
                                                    failureMessage = std::move(failureMessage)] {
            const bool success = operation(runtime);
            if (!weakSelf.lock()) return;
            runtime->Console()->SetAccountOperation(success ? ui::AccountOperationState::Succeeded : ui::AccountOperationState::Failed);
            runtime->Notify(!success, success ? std::string{px::ui::ApplicationName()} : "Error", success ? successMessage : failureMessage);
        }));
    }
    ...
    void Start(std::string username, std::string password, const bool registration) {
        ...
        static_cast<void>(runtime_->Worker()->Post([runtime, weakSelf, username = std::move(username), password = std::move(password), registration] {
            ...
            if (!weakSelf.lock()) return;
            runtime->Console()->SetAccountOperation(success ? ui::AccountOperationState::Succeeded : ui::AccountOperationState::Failed);
            runtime->Notify(!success, success ? std::string{px::ui::ApplicationName()} : "Error",
