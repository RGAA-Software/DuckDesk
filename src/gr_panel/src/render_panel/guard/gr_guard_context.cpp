...
namespace tc
{
    ...
    void GrGuardContext::StartTimers() {
        ...
        timer_->start_timer(1, 100, [weak_self]() {
            if (auto self = weak_self.lock(); self && !self->exiting_) {
                self->SendAppMessage(MsgGrGuardTimer100{});
            }
        });

        timer_->start_timer(2, 1000, [weak_self]() {
            if (auto self = weak_self.lock(); self && !self->exiting_) {
                self->SendAppMessage(MsgGrGuardTimer1S{});
            }
        });

        timer_->start_timer(5, 5000, [weak_self]() {
            if (auto self = weak_self.lock(); self && !self->exiting_) {
                self->SendAppMessage(MsgGrGuardTimer5S{});
            }
        });
    }
}
