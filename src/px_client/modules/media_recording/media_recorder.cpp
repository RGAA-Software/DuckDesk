...
void MediaRecorder::RecvVideoFrame(const VideoFrame& frame) {
    ...
    const auto codec = frame.type() == px::VideoType::kNetHevc
...
void MediaRecorder::EnsureWriter() {
    ...
    config.dir = ResolveRecordDir(configured_record_path_);
    config.monitor_name = "mon" + std::to_string(index_);
    // The server emits an IDR every second, so segment rollover can wait for
    // the next key frame without a separate control message.
    writer_ = RecordWriter::Make(config);
