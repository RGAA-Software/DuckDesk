...
void ClientContext::SetModuleManager(const std::shared_ptr<ClientModuleManager>& manager) {
...
uint64_t ClientContext::SetRecording(bool recording) {
    auto previous = recording_intent_.load();
    for (;;) {
        const auto next = ((previous & ~uint64_t{1}) + 2U) | static_cast<uint64_t>(recording);
        if (recording_intent_.compare_exchange_weak(previous, next))
            return next;
    }
}

bool ClientContext::GetRecording() {
    return (recording_intent_.load() & 1U) != 0;
}

uint64_t ClientContext::GetRecordingIntent() const {
    return recording_intent_.load();
}

bool ClientContext::FailRecordingIntent(uint64_t intent) {
    if ((intent & 1U) == 0)
        return false;
    return recording_intent_.compare_exchange_strong(intent, intent & ~uint64_t{1});
}

void ClientContext::InitNotifyManager(QWidget* parent) {
    notify_manager_ = std::make_shared<NotifyManager>(parent);
    const auto weak_self = weak_from_this();
    connect(notify_manager_.get(), &NotifyManager::notifyDetail, this, [weak_self](const NotifyItem& data) {
