...
namespace px
{
    ...
    class CtCmsClientImpl : public CtCmsClient,
                            public std::enable_shared_from_this<CtCmsClientImpl<ClientT>> {
    public:
        explicit CtCmsClientImpl(const std::shared_ptr<ThunderSdk>& sdk,
        ...
        void Start() override {
            ...
            LOGI("will connect => {}:{}/cms/client, ssl: {}", host_, port_, std::is_same_v<ClientT, asio2::wss_client>);
    ...
    std::shared_ptr<CtCmsClient> CtCmsClient::Make(const std::shared_ptr<ThunderSdk>& sdk,
                                                   const std::shared_ptr<ClientContext>& ctx,
                                                   const std::string& host,
                                                   int port,
                                                   const std::string& device_id,
                                                   ...
                                                   bool ssl) {
        if (ssl) {
            return std::make_shared<CtCmsClientImpl<asio2::wss_client>>(sdk, ctx, host, port, device_id, remote_device_id, remote_device_ip, appkey);
        }
        return std::make_shared<CtCmsClientImpl<asio2::ws_client>>(sdk, ctx, host, port, device_id, remote_device_id, remote_device_ip, appkey);
    }
