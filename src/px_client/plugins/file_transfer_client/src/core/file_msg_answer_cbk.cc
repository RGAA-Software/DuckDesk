...
namespace px {
	void FileMsgAnswerCbkStructure::Add(const std::shared_ptr<px::Message>& msg, OnMsgParseRespCallbackFuncType parse_msg_callbck) {
		...
		send_msg_resp_callback_map_[seq].callback = [=](const std::shared_ptr<px::Message>& message) {
			...
			do {
				if (px::kFileOperateRespGetFileList == message->type()) {
				...
				else if (px::kFileOperateRespBatchCreateFolders == message->type()) {
				...
				else if (px::kFileOperateRespExists == message->type()) {
				...
				else if (px::kFileOperateRespCreateNewFolder == message->type()) {
				...
				else if (px::kFileOperateRespRename == message->type()) {
				...
				else if (px::kFileOperateRespDel == message->type()) {
