...
class RtcServer : public std::enable_shared_from_this<RtcServer> {
  public:
    static std::shared_ptr<RtcServer> Make(const std::shared_ptr<WebRtcRemoteRuntime>& runtime);
    explicit RtcServer(const std::shared_ptr<WebRtcRemoteRuntime>& runtime);
    [[nodiscard]] std::shared_ptr<WebRtcExecutionContext> GetExecutionContext() const;
    void DispatchEvent(WebRtcEvent event) const;

    bool Start(const std::string& stream_id, const std::string& offer_sdp, const std::string& ice_config_json,
               const std::vector<std::string>& permissions);
    bool RestartWithOffer(const std::string& offer_sdp, const std::string& ice_config_json, const std::vector<std::string>& permissions);
    void Exit();
    ...
    // (与 px::Message.stream_id 一致),不用 datachannel 内部 the_connection_id_(MD5)。
    ...
    [[nodiscard]] const std::string& GetStreamId() const {
        return stream_id_;
    }
