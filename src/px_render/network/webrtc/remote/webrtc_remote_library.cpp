...
#include "webrtc_remote_library.h"
...
namespace px
{
    WebRtcRemoteRuntime::WebRtcRemoteRuntime(
        WebRtcRemoteLibrary& owner, std::weak_ptr<PxPluginContext> context,
    ...
    std::shared_ptr<PxPluginContext> WebRtcRemoteRuntime::GetContext() const {
        return context_.lock();
    }

    void WebRtcRemoteRuntime::QueueEvent(
        const std::shared_ptr<PxPluginBaseEvent>& event) const {
        const auto context = context_.lock();
        if (!context || !event) {
            return;
        }
        event->plugin_name_ = kNetWebRtcRemoteLibraryId;
    ...
    void WebRtcRemoteRuntime::DispatchClientEvent(
    ...
    std::string WebRtcRemoteLibrary::GetPluginId() {
        return kNetWebRtcRemoteLibraryId;
    }

    std::string WebRtcRemoteLibrary::GetPluginName() {
        return "Net RTC";
    }

    std::string WebRtcRemoteLibrary::GetVersionName() {
        return "1.0.2";
    }

    uint32_t WebRtcRemoteLibrary::GetVersionCode() {
        return 102;
    }

    std::string WebRtcRemoteLibrary::GetPluginDescription() {
        return "Network via RTC";
    }

    bool WebRtcRemoteLibrary::OnCreate(const px::PxPluginParam &param) {
        PxNetPlugin::OnCreate(param);
        runtime_ = std::make_shared<WebRtcRemoteRuntime>(
    ...
    bool WebRtcRemoteLibrary::OnDestroy() {
        PxNetPlugin::OnStop();
    ...
    void WebRtcRemoteLibrary::ApplyRtcRemoteSdp(const MsgRtcRemoteSdp& message) {
        OnRemoteSdp(message);
    }

    void WebRtcRemoteLibrary::ApplyRtcRemoteIce(const MsgRtcRemoteIce& message) {
        OnRemoteIce(message);
    }

    void WebRtcRemoteLibrary::ApplyLogicalSessionCapabilities(
    ...
    void WebRtcRemoteLibrary::PostProtoMessage(std::shared_ptr<Data> msg, bool run_through) {
    ...
    bool WebRtcRemoteLibrary::PostTargetStreamProtoMessage(const std::string &stream_id, std::shared_ptr<Data> msg, bool run_through) {
        WaitForMediaChannelActive();
    ...
    FileTransferSendResult WebRtcRemoteLibrary::PostTargetFileTransferProtoMessage(
    ...
    void WebRtcRemoteLibrary::WaitForMediaChannelActive() {
    ...
    void WebRtcRemoteLibrary::OnRemoteSdp(const MsgRtcRemoteSdp& m) {
        const auto weak_runtime = std::weak_ptr<WebRtcRemoteRuntime>(runtime_);
    ...
    void WebRtcRemoteLibrary::OnRemoteIce(const MsgRtcRemoteIce& m) {
        const auto weak_runtime = std::weak_ptr<WebRtcRemoteRuntime>(runtime_);
        PostWorkTask([weak_runtime, m]() {
            if (const auto runtime = weak_runtime.lock()) {
                auto conn_id = m.device_id_ + ":" + m.stream_id_;
    ...
    int WebRtcRemoteLibrary::GetConnectedClientsCount() {
    ...
    int64_t WebRtcRemoteLibrary::GetQueuingMediaMsgCount() {
    ...
    int64_t WebRtcRemoteLibrary::GetQueuingFtMsgCount() {
    ...
    bool WebRtcRemoteLibrary::HasEnoughBufferForQueuingMediaMessages() {
        bool flag = true;
        runtime_->servers.ApplyAll([&](const std::string&, const std::shared_ptr<RtcServer>& srv) {
    ...
    bool WebRtcRemoteLibrary::HasEnoughBufferForQueuingFtMessages() {
        bool flag = true;
        runtime_->servers.ApplyAll([&](const std::string&, const std::shared_ptr<RtcServer>& srv) {
...
#endif
    static px::WebRtcRemoteLibrary library;
