...
#define PX_NET_RTC_LOCAL_API __declspec(dllexport)
#else
#define PX_NET_RTC_LOCAL_API __declspec(dllimport)
#endif
#else
#define PX_NET_RTC_LOCAL_API
#endif
...
class WebRtcLocalRuntime final {
  public:
    ...
    void OnRemoteVoiceCallPcm(const std::string& stream_id, const std::string& call_id, std::span<const std::int16_t> samples, int sample_rate,
                              int channels);
    ...
    [[nodiscard]] std::shared_ptr<RtcLocalEncodedVideoFrame> ReadNextEncodedVideoFrame(const std::string& mon_name, uint64_t after_seq,
                                                                                       bool& out_gap);
    [[nodiscard]] bool WaitForEncodedFrame(const std::string& mon_name, uint64_t after_seq, int timeout_ms);
...
class PX_NET_RTC_LOCAL_API WebRtcLocalTransport final : public std::enable_shared_from_this<WebRtcLocalTransport> {
  public:
    ...
    void OnRawVideoFrameSharedTexture(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, uint64_t handle,
                                      int64_t adapter_id, uint64_t frame_format);

    // raw video frame in rgba format
    // image: Raw image
    void OnRawVideoFrameRgba(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, const std::shared_ptr<Image>& image);

    // raw video frame in yuv(I420) format
    // image: Raw image
    void OnRawVideoFrameYuv(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, const std::shared_ptr<Image>& image);
    ...
    bool SetVoiceCallAuthorization(const std::string& stream_id, const std::string& call_id, bool authorized);
    void OnVoiceCallPcm(const std::string& stream_id, const std::string& call_id, std::span<const std::int16_t> samples, int sample_rate,
                        int channels);
    void OnRemoteVoiceCallPcm(const std::string& stream_id, const std::string& call_id, std::span<const std::int16_t> samples, int sample_rate,
                              int channels);
    ...
    void NotifyRtcServerTerminal(const std::string& conn_id, const std::shared_ptr<RtcServer>& target);
