...
namespace px
{
    WebRtcLocalRuntime::WebRtcLocalRuntime(
        WebRtcLocalLibrary& owner, std::weak_ptr<PxPluginContext> context,
        PxPluginEventCallback dispatcher)
        : owner_(owner), context_(std::move(context)),
          dispatcher_(std::move(dispatcher)) {}

    void WebRtcLocalRuntime::WithOwner(
        const std::function<void(WebRtcLocalLibrary&)>& operation) {
    ...
    std::shared_ptr<PxPluginContext> WebRtcLocalRuntime::GetContext() const {
        return context_.lock();
    }

    void WebRtcLocalRuntime::QueueEvent(
        const std::shared_ptr<PxPluginBaseEvent>& event) const {
        const auto context = context_.lock();
        if (!context || !event) {
            return;
        }
        event->plugin_name_ = kNetWebRtcLocalLibraryId;
    ...
    void WebRtcLocalRuntime::DispatchClientEvent(
    ...
    void WebRtcLocalRuntime::NotifyTerminal(
    ...
    WebRtcLocalRuntime::GetRtcTrackMonitors() {
        std::vector<CaptureMonitorInfo> monitors;
        WithOwner([&](WebRtcLocalLibrary& owner) {
    ...
    void WebRtcLocalRuntime::OnRemoteVoiceCallPcm(
    ...
    uint64_t WebRtcLocalRuntime::GetLatestEncodedSeq(
    ...
    size_t WebRtcLocalRuntime::GetCachedFrameCount(
    ...
    WebRtcLocalRuntime::ReadNextEncodedVideoFrame(
    ...
    bool WebRtcLocalRuntime::WaitForEncodedFrame(
    ...
    std::string WebRtcLocalLibrary::GetPluginId() {
        return kNetWebRtcLocalLibraryId;
    }

    std::string WebRtcLocalLibrary::GetPluginName() {
        return "RTC Local";
    }

    std::string WebRtcLocalLibrary::GetVersionName() {
        return "1.1.0";
    }

    uint32_t WebRtcLocalLibrary::GetVersionCode() {
        return 110;
    }

    std::string WebRtcLocalLibrary::GetPluginDescription() {
        return "RTC in local mode";
    }

    void WebRtcLocalLibrary::On1Second() {
        PxPluginInterface::On1Second();
        SweepDeadRtcServers();
    }

    void WebRtcLocalLibrary::NotifyRtcServerTerminal(
        const std::string& conn_id,
        const std::shared_ptr<RtcServer>& target) {
        runtime_->NotifyTerminal(conn_id, target);
    }

    void WebRtcLocalLibrary::SweepDeadRtcServers() {
    ...
    bool WebRtcLocalLibrary::OnCreate(const px::PxPluginParam &param) {
        PxPluginInterface::OnCreate(param);
        plugin_type_ = PxPluginType::kNet;
        runtime_ = std::make_shared<WebRtcLocalRuntime>(
    ...
    bool WebRtcLocalLibrary::OnDestroy() {
    ...
    void WebRtcLocalLibrary::ApplyRtcRemoteSdp(const MsgRtcRemoteSdp& message) {
        OnRemoteSdp(message);
    }

    void WebRtcLocalLibrary::ApplyRtcRemoteIce(const MsgRtcRemoteIce& message) {
        OnRemoteIce(message);
    }

    void WebRtcLocalLibrary::ApplyLogicalSessionCapabilities(
    ...
    void WebRtcLocalLibrary::OnRemoteSdp(const MsgRtcRemoteSdp& message) {
        const auto weak_runtime = std::weak_ptr<WebRtcLocalRuntime>(runtime_);
    ...
    void WebRtcLocalLibrary::OnRemoteIce(const MsgRtcRemoteIce& message) {
        const auto weak_runtime = std::weak_ptr<WebRtcLocalRuntime>(runtime_);
        PostWorkTask([weak_runtime, message]() {
            const auto runtime = weak_runtime.lock();
            if (!runtime) {
    ...
    void WebRtcLocalLibrary::PostProtoMessage(std::shared_ptr<Data> msg, bool run_through) {
    ...
    bool WebRtcLocalLibrary::PostTargetStreamProtoMessage(const std::string &stream_id, std::shared_ptr<Data> msg, bool run_through) {
        if (IsMediaFrameMessage(msg)) {
            return true;
        }
        WaitForMediaChannelActive();
    ...
    FileTransferSendResult WebRtcLocalLibrary::PostTargetFileTransferProtoMessage(
        ...
        if (!connection_instance_id.empty()) {
            ...
            if (!(*target)->IsFtDataChannelConnected()) {
                return FileTransferSendResult::Disconnected(
                    "direct RTC file data channel is not connected");
            }
            if ((*target)->GetFtPendingMessages() >= kMaxFileTransferQueuedMessages ||
                !(*target)->HasEnoughBufferForQueuingFtMessages()) {
                return FileTransferSendResult::Busy(
                    "direct RTC file data channel is congested",
                    (*target)->AcquireFtWritableSignal());
            }
            return (*target)->PostTargetFileTransferProtoMessage(
                ...
                : FileTransferSendResult::Disconnected(
                      "direct RTC file data channel closed before enqueue");
        }
        bool matched = false;
    ...
    void WebRtcLocalLibrary::WaitForMediaChannelActive() {
    ...
    int WebRtcLocalLibrary::GetConnectedClientsCount() {
        int count = 0;
    ...
    int WebRtcLocalLibrary::GetMediaConsumersCount() {
        int count = 0;
        runtime_->servers.ApplyAll([&](const auto&, const std::shared_ptr<RtcServer>& srv) {
    ...
    int64_t WebRtcLocalLibrary::GetQueuingMediaMsgCount() {
    ...
    int64_t WebRtcLocalLibrary::GetQueuingFtMsgCount() {
        uint32_t total_pending_messages = 0;
        runtime_->servers.ApplyAll([&](const std::string&, const std::shared_ptr<RtcServer>& srv) {
            // 跳过死连接:其 pending 计数不会再变化,统计进来只会造成误判
            if (!srv->IsFtDataChannelConnected()) {
    ...
    bool WebRtcLocalLibrary::HasEnoughBufferForQueuingMediaMessages() {
        bool flag = true;
        runtime_->servers.ApplyAll([&](const std::string&, const std::shared_ptr<RtcServer>& srv) {
    ...
    bool WebRtcLocalLibrary::HasEnoughBufferForQueuingFtMessages() {
        bool flag = true;
        runtime_->servers.ApplyAll([&](const std::string&, const std::shared_ptr<RtcServer>& srv) {
            // 跳过死连接:死连接的 channel 缓冲永远是满的,会全票否决所有投递
            if (!srv->IsFtDataChannelConnected()) {
    ...
    void WebRtcLocalLibrary::OnEncodedVideoFrame(const std::string& mon_name,
    ...
    void WebRtcLocalLibrary::OnRawVideoFrameSharedTexture(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, uint64_t handle, int64_t adapter_id, uint64_t frame_format) {
    ...
    // image: Raw image
    void WebRtcLocalLibrary::OnRawVideoFrameRgba(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, const std::shared_ptr<Image>& image) {

    }

    // raw video frame in yuv(I420) format
    // image: Raw image
    void WebRtcLocalLibrary::OnRawVideoFrameYuv(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, const std::shared_ptr<Image>& image) {
    ...
    void WebRtcLocalLibrary::OnRawAudioData(const std::shared_ptr<Data>& data, int samples, int channels, int bits) {
        if (!data || data->Size() == 0) {
            return;
        }
    ...
    bool WebRtcLocalLibrary::SetVoiceCallAuthorization(
        const std::string& stream_id, const std::string& call_id,
        bool authorized) {
        bool applied = false;
        runtime_->servers.ApplyAll([&](const std::string&, const std::shared_ptr<RtcServer>& srv) {
    ...
    void WebRtcLocalLibrary::OnVoiceCallPcm(
        const std::string& stream_id, const std::string& call_id,
    ...
    void WebRtcLocalLibrary::OnRemoteVoiceCallPcm(
        const std::string& stream_id, const std::string& call_id,
        const int16_t* samples, size_t sample_count,
        int sample_rate, int channels) {
    ...
    std::shared_ptr<RtcLocalEncodedVideoFrame> WebRtcLocalLibrary::ReadNextEncodedVideoFrame(const std::string& mon_name, uint64_t after_seq, bool& out_gap) {
        std::lock_guard<std::mutex> lk(encoded_video_frames_mtx_);
        out_gap = false;
    ...
    bool WebRtcLocalLibrary::WaitForEncodedFrame(const std::string& mon_name, uint64_t after_seq, int timeout_ms) {
    ...
    void WebRtcLocalLibrary::InsertIdr(const std::string& mon_name) {
        if (mon_name.empty()) {
    ...
    uint64_t WebRtcLocalLibrary::GetLatestEncodedSeq(const std::string& mon_name) {
        std::lock_guard<std::mutex> lk(encoded_video_frames_mtx_);
        auto it = encoded_seq_by_mon_.find(mon_name);
        return it != encoded_seq_by_mon_.end() ? it->second : 0;
    }

    size_t WebRtcLocalLibrary::GetCachedFrameCount(const std::string& mon_name, uint64_t after_seq) {
        std::lock_guard<std::mutex> lk(encoded_video_frames_mtx_);
        size_t count = 0;
    ...
    void WebRtcLocalLibrary::PrintCachedVideoFrames() {
        std::lock_guard<std::mutex> lk(encoded_video_frames_mtx_);
    ...
    std::vector<CaptureMonitorInfo> WebRtcLocalLibrary::GetRtcTrackMonitors() {
    ...
    void WebRtcLocalLibrary::EnableAllMonitorCapture() {
    ...
    void WebRtcLocalLibrary::UpdateCaptureMonitorInfo(
        const CaptureMonitorInfoMessage& message) {
        std::scoped_lock lock(capture_topology_mutex_);
        capture_topology_ = message.monitors_;
    }

    PxLocalRtcAllocResult WebRtcLocalLibrary::AllocNewLocalRtcInstance(const std::shared_ptr<PxLocalRtcRequestInfo>& req,
        ...
        rtc_server->SetOnAnswerCallback(
            ...
            if (const auto locked = weak_runtime.lock()) {
                locked->WithOwner([&](WebRtcLocalLibrary& owner) {
    ...
    std::string WebRtcLocalLibrary::AddCandidateIpToAnswer(const std::string& ip,const std::string& answer) {
...
// Established DLL singleton ABI. The Render process immediately wraps this
// borrowed value in the typed WebRtcLibrary RAII facade.
#if defined(_WIN32)
extern "C" __declspec(dllexport) void* GetInstance() { // NOLINT(gammaray-raw-pointer-boundary): retained DLL compatibility ABI
#else
extern "C" __attribute__((visibility("default"))) void* GetInstance() { // NOLINT(gammaray-raw-pointer-boundary): retained DLL compatibility ABI
#endif
    static px::WebRtcLocalLibrary library;
    return std::addressof(library); // NOLINT(gammaray-raw-pointer-boundary): retained DLL compatibility ABI
}
