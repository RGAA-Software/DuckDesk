...
class RtcServer : public std::enable_shared_from_this<RtcServer> {
  public:
    ...
    // 不用 datachannel 内部 the_connection_id_(MD5)——否则按消息 stream_id 键控的
    ...
    void OnNewFrameCaptured(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, uint64_t handle, int64_t adapter_id,
                            uint64_t frame_format);
  ...
  private:
    ...
    void DispatchCapturedFrameNotify(const std::string& mon_name, uint64_t frame_idx, int frame_width, int frame_height, uint64_t handle,
