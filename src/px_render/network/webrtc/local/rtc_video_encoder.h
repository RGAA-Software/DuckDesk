...
class RtcSharedVideoEncoder : public webrtc::VideoEncoder {
  ...
  private:
    ...
    std::optional<std::reference_wrapper<webrtc::EncodedImageCallback>> encoded_image_callback_{};
    ...
    // 编码产出序号消费游标:严格按编码器产出顺序取帧(见 WebRtcLocalTransport 注释),
