...
RemoteAudioSink::RemoteAudioSink(PcmCallback pcm_callback) : pcm_callback_(std::move(pcm_callback)) {}
...
void RemoteAudioSink::OnData(const void* audio_data, // NOLINT(gammaray-raw-pointer-boundary): libwebrtc AudioTrackSinkInterface ABI
    ...
    if (first_frame) {
        LOGI("RemoteAudioSink first PCM (decode ok): {} Hz, {} ch, {} bits, {} frames", sample_rate, (int)number_of_channels, bits_per_sample,
             (int)number_of_frames);
    }
    if (log_stats) {
        LOGI("RemoteAudioSink stats: authorized rx frames total={}", total_frames);
    }
}
