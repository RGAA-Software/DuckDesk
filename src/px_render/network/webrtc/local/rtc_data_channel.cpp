...
RtcDataChannel::RtcDataChannel(const std::string& name, const std::shared_ptr<RtcServer>& rtc_server,
...
void RtcDataChannel::OnStateChange() {
    if (data_channel_->state() == webrtc::DataChannelInterface::kOpen) {
        ...
        if (this->name_ == "media_data_channel") {
            ...
            rtc_server->OnMediaDataChannelOpened();
            ...
            rtc_server->DispatchEvent(WebRtcClientConnectedEvent{
                .connection_id = rtc_server->GetConnId(),
                .stream_id = rtc_server->GetStreamId(),
                .connection_type = "RTC",
                .visitor_device_id = rtc_server->GetStreamId(),
                .begin_timestamp = created_timestamp_,
            });
        }
...
void RtcDataChannel::OnMessage(const webrtc::DataBuffer& buffer) {
    ...
    if (header->this_buffer_length_ > payload_len) {
        LOGE("RtcDataChannel TLV length invalid: declared={}, available={}", header->this_buffer_length_, payload_len);
        return;
    }
...
void RtcDataChannel::On100msTimeout() {
    ...
    static_cast<void>(execution_context_->PostWork([weak_channel]() {
...
std::shared_ptr<FileTransferWritableSignal> RtcDataChannel::AcquireFileTransferWritableSignal() {
