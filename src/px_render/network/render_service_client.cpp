...
RenderServiceClient::~RenderServiceClient() { Exit(); }
...
void RenderServiceClient::ParseMessage(const std::string& msg) {
    ...
    } else if (sm.type() == ServiceMessageType::kSrvFrontendAdmissionResult) {
        const auto& admission = sm.frontend_admission_result();
        MsgFrontendAdmissionServiceResult result;
        result.request_id_ = admission.request_id();
        result.accepted_ = admission.accepted();
        result.error_code_ = admission.error_code();
        result.session_id_ = admission.session_id();
        result.revision_ = admission.revision();
        result.target_kind_ = admission.target_kind();
        result.device_id_ = admission.device_id();
        result.application_id_ = admission.application_id();
        result.instance_id_ = admission.instance_id();
        result.client_type_ = admission.client_type();
...
void RenderServiceClient::Exit() {
    ...
    if (scope && scope->IsScopeThread()) {
        LOGI(
            ...
            "operation=stop_client outcome=deferred "
    ...
    if (!scope_drained || !adapter_stopped) {
        LOGE(
            ...
            "operation=stop_client outcome=timeout recoverable=false "
            "outstanding={}",
...
void RenderServiceClient::ScheduleDeferredExit() {
    ...
    if (!runtime || !runtime->DeferBlocking([weak_self]() {
        ...
        LOGE(
            "event=async.scope_drain component=render_service "
            "code=ASYNC_DEFER_FAILED operation=stop_client outcome=failed "
            "recoverable=false");
    }
}
...
PxResult<void> RenderServiceClient::TryPostSensitiveNetMessage(
    ...
    const auto weak_self = weak_from_this();
    client->async_send(*payload, [weak_self, payload]() {
...
RenderServiceClient::RequestFrontendAdmissionAsync(
    ...
    px::ServiceMessage message;
    message.set_type(ServiceMessageType::kSrvFrontendAdmissionRequest);
    auto& request = *message.mutable_frontend_admission_request();
    request.set_request_id(request_id);
    request.set_session_id(std::move(session_id));
    request.set_revision(revision);
    request.set_frontend_token(frontend_token);
    std::fill(frontend_token.begin(), frontend_token.end(), '\0');
    auto encoded = message.SerializeAsString();
