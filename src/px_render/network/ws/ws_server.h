...
class WsServer : public std::enable_shared_from_this<WsServer> {
   public:
    explicit WsServer(std::weak_ptr<WsTransport> transport,
                      ...
                      uint16_t listen_port, std::uint16_t rdp_proxy_port = 0);
    ...
    void PostNetMessage(std::shared_ptr<Data> message);
    void PostIpcBinaryMessage(std::shared_ptr<Data> message);
    bool PostIpcBinaryMessageForPid(std::uint32_t pid,
                                    std::shared_ptr<Data> message,
                                    std::function<bool()> authorize);
    bool PostTargetStreamMessage(const std::string& stream_id,
                                 std::shared_ptr<Data> message);
    ...
    void PostUserProxyMessage(std::shared_ptr<Data> message);
    bool IsUserProxyConnected();

    // Game-hook /ipc 鉴权：只允许注册过的游戏 pid 连接(PrepareGameHookBoot
    // 时注册)。 游戏重启后 render 会为新的 pid 重写 boot 配置并注册,旧 render
    // 的残留游戏 不在集合里,连接会被拒绝(防多实例串帧)。
   ...
   private:
    ...
    static PxAwaitable<void> OpenWebSocketAsync(
        ...
        std::shared_ptr<asio2::http_session> session, std::string path,
   ...
   private:
    ...
    WsDataPtr ws_data_{};
    px::ConcurrentHashMap<uint64_t, std::shared_ptr<WsStreamRouter>>
        stream_routers_;
    px::ConcurrentHashMap<uint64_t, std::shared_ptr<WsFileTransferRouter>>
        ft_routers_;
    // Injected px_gh.dll sessions on /ipc (host → game input downlink).
    px::ConcurrentHashMap<uint64_t, std::shared_ptr<asio2::http_session>>
        ipc_sessions_;
    // Pids allowed on /ipc: this render instance wrote hook boot config for
    // them.
    ...
    // The runtime is injected by the Render composition root. This server owns
