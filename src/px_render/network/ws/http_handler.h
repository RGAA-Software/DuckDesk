...
class HttpHandler : public BaseHandler, public std::enable_shared_from_this<HttpHandler> {
  ...
  private:
    ...
    void CloseAdmittedLogicalSessionBinding(const std::string& logical_session_id, const std::string& binding_id);
    static PxAwaitable<void> AllocateLocalRtcAsync(std::weak_ptr<HttpHandler> owner, std::shared_ptr<asio2::http_session> session,
                                                   std::unordered_map<std::string, std::string> params, std::string body, std::string remote_address,
