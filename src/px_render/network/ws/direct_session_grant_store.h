...
class DirectSessionGrantStore {
  public:
    ...
    // therefore need to present the receipt independently.
    [[nodiscard]] bool Validate(const std::string& token, const DirectSessionGrantBinding& expected, int64_t now_ms);
