...
static inline bool attempt_hook(void) {
    ...
    if (!vulkan_hooked) {
        ...
        LOGI("hook Vulkan result: {}", vulkan_hooked);
        if (vulkan_hooked) {
            return true;
        }
    }
    ...
    if (!d3d12_hooked) {
        ...
        LOGI("Hook d3d12 result: {}", d3d12_hooked);
    }
#endif
    ...
    if (!dxgi_hooked) {
        if (!dxgi_hookable()) {
            ...
            LOGI("Hook dxgi result: {}", dxgi_hooked);
            if (dxgi_hooked) {
                return true;
            }
        }
...
static DWORD WINAPI HookDeferredInitThread(LPVOID param) {
    ...
    if (!g_hook_manager->app_shared_msg_ || g_hook_manager->app_shared_msg_->ipc_port_ == 0) {
        auto fallback_log = std::format(L"{}/px_gh_boot_fail.log", dll_path);
        px::Logger::InitLog(fallback_log, true);
        LOGE("Hook boot config missing — refuse to hook (avoid crashing the game)");
        return 0;
    }

    // dll_path is a full file path; put logs next to the DLL, not under ".../px_gh.dll/".
    auto log_path = (std::filesystem::path(dll_path).parent_path() /
                     std::format(L"px_gh_{}.log", g_hook_manager->app_shared_msg_->ipc_port_))
                        .wstring();
