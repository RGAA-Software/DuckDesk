...
namespace px
{
    ...
    PluginEventRouter::PluginEventRouter(const std::shared_ptr<RdApplication>& app) {
        ...
        stream_event_router_ = PluginStreamEventRouter::Make(app);
        net_event_router_ = PluginNetEventRouter::Make(app);
    ...
    void PluginEventRouter::ProcessPluginEvent(const std::shared_ptr<PxPluginBaseEvent>& event) {
        ...
        if (event->event_type_ == PxPluginEventType::kPluginEncodedVideoFrameEvent) {
            ...
            app_->OnCapturedCursorBitmap(target_event->cursor_info_);
        }
            ...
            plugin_manager_->VisitEncoderPlugins([mon_name](PxVideoEncoderPlugin* plugin) { // NOLINT(gammaray-raw-pointer-boundary): plug-in visitor ABI
                // TODO:
    ...
    void PluginEventRouter::SendAnswerSdpToRemote(const std::shared_ptr<PxPluginBaseEvent>& event) {
        ...
        plugin_manager_->VisitNetPlugins([stream_id, msg](PxNetPlugin* plugin) { // NOLINT(gammaray-raw-pointer-boundary): plug-in visitor ABI
    ...
    void PluginEventRouter::SendIceToRemote(const std::shared_ptr<PxPluginBaseEvent>& event) {
        ...
        plugin_manager_->VisitNetPlugins([stream_id, msg, ice](PxNetPlugin* plugin) { // NOLINT(gammaray-raw-pointer-boundary): plug-in visitor ABI
            if (plugin->GetPluginId() == kRelayPluginId) {
                if (stream_id.empty()) {
                    plugin->PostProtoMessage(msg, true);
                }
