...
void NetworkEventIngress::ProcessClientConnectedEvent(
    ...
    if (settings_.IsRdpMode()) {
        ...
        // In particular, never send Ctrl+Alt+Delete to the Service/host
        // desktop.
        return;
    }
...
void NetworkEventIngress::ProcessNetEvent(
    ...
    if (event->is_proto_ && event->message_) {
        ...
        // Validate the logical-session input lease before dispatching to
        // built-in services.
        ...
        if (IsControllerOnlyMessage(msg->type())) {
            ...
            if (!input_lease.has_value()) {
                LOGW(
                    "Drop controller-only message without an active controller "
                    "lease");
                return;
            }
        }
        ...
        if (msg->type() == MessageType::kClipboardInfo) {
            ...
            context_->PostTask([weak_self, msg, event]() {
                ...
                if (!self->module_registry_->IsWsUserProxyConnected()) {
                    LOGW(
                        "user-proxy not connected, drop clipboard buffer msg, "
                        "type={}, len={}",
                        (int)msg->type(), event->message_->Size());
                    return;
                }
                ...
                sub.set_device_id(msg->device_id());
                LOGI(
                    "PostUserProxyMessage clipboard buffer msg, type={}, "
                    "len={}",
                    (int)msg->type(), event->message_->Size());
                self->app_->PostUserProxyMessage(RpProtoAsData(&rp_msg));
            });
            ...
            case kStopRender: {
                ...
                if (settings_.IsGameHookMode() || settings_.IsWebViewMode()) {
                    LOGW(
                        ...
                        "instance; use Console stop action instead.");
                    break;
                }
...
void NetworkEventIngress::ProcessHelloEvent(std::shared_ptr<Message>&& msg) {
    ...
    auto client_hello_event = std::make_shared<MsgClientHello>(event);
...
void NetworkEventIngress::ProcessMouseEvent(
    ...
    if (settings_.app_.IsGlobalReplayMode()) {
        // Desktop replay is handled by the built-in input replay service; the
        // router still owns lease-scoped state above.
        return;
    }
    if (settings_.IsWebViewMode()) {
        ...
        mouse_event.pressed(), mouse_event.released());
    {
        ...
        if (mouse_event_count <= 5 || (mouse_event_count % 200) == 0) {
            LOGI(
...
void NetworkEventIngress::ProcessSwitchMonitor(std::shared_ptr<Message>&& msg) {
    ...
    app_->PostGlobalTask([weak_self, msg]() {
        ...
        const auto switch_monitor_request = msg->switch_monitor();
        auto capture_source = self->app_->GetWorkingMonitorCaptureSource();
        if (!capture_source) {
            return;
        }
        capture_source->SelectMonitor(switch_monitor_request.name());
        auto encoders = self->app_->GetWorkingVideoEncoders();
        for (const auto& [monitor_name, encoder] : encoders) {
...
void NetworkEventIngress::ProcessSwitchWorkMode(
    ...
    app_->PostGlobalTask([weak_self, msg]() {
        ...
        const auto work_mode_request = msg->work_mode();
        if (work_mode_request.mode() == SwitchWorkMode::kWork) {
            self->app_->SetFrameRate(30);
        } else if (work_mode_request.mode() == SwitchWorkMode::kGame) {
