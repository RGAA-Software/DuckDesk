...
LogicalSessionRegistry::LogicalSessionRegistry(const bool allow_observer, const bool allow_takeover, const int64_t controller_reconnect_grace_ms)
    : allow_observer_(allow_observer), allow_takeover_(allow_takeover),
...
LogicalSessionAdmission LogicalSessionRegistry::AdoptControllerLocked(const LogicalSessionGrant& grant, const LogicalSessionTransport transport,
    ...
    const bool reserves_controller_seat{transport != LogicalSessionTransport::kFileTransfer};
    const auto existing_controller = controller_session_id_;
    if (reserves_controller_seat && !existing_controller.empty() && existing_controller != grant.logical_session_id) {
        if (!takeover) {
            result.code = LogicalSessionAdmissionCode::kOccupied;
            return result;
        }
        const auto previous = sessions_.find(existing_controller);
        const bool controller_allows_takeover = previous == sessions_.end() || previous->second.allow_takeover;
    ...
    const bool renew_controller_lease = controller_session_id_ != grant.logical_session_id || !HasControllerBinding(session);
...
LogicalSessionAdmission LogicalSessionRegistry::Bind(const LogicalSessionGrant& grant, const LogicalSessionTransport transport,
                                                     const std::string& binding_id, const bool takeover, const int64_t now_ms) {
    std::scoped_lock lock(mutex_);
    ...
    if (transport == LogicalSessionTransport::kFileTransfer) {
        ...
        if (session_it->second.stream_id != grant.stream_id || session_it->second.subject_id != grant.subject_id ||
            session_it->second.role != LogicalSessionRole::kController) {
            return {.code = LogicalSessionAdmissionCode::kInvalidGrant};
        }
        auto& session = session_it->second;
        session.bindings.insert_or_assign(binding_id, Binding{transport, binding_id, next_binding_generation_++, grant.input_allowed});
        return {.code = LogicalSessionAdmissionCode::kAccepted, .role = session.role, .lease_generation = session.lease_generation};
    }
    if (grant.join_mode == "control") {
        return AdoptControllerLocked(grant, transport, binding_id, takeover, now_ms);
    }
    ...
    session.bindings.insert_or_assign(binding_id, Binding{transport, binding_id, next_binding_generation_++, grant.input_allowed});
    return {.code = LogicalSessionAdmissionCode::kAccepted, .role = LogicalSessionRole::kObserver, .lease_generation = session.lease_generation};
}

LogicalSessionBindingClosed LogicalSessionRegistry::CloseBinding(const std::string& logical_session_id, const std::string& binding_id,
...
LogicalSessionBindingClosed LogicalSessionRegistry::CloseBindingById(const std::string& binding_id, const int64_t now_ms) {
    std::scoped_lock lock(mutex_);
...
LogicalSessionBindingClosed LogicalSessionRegistry::CloseFailedBindingById(const std::string& binding_id, const int64_t now_ms) {
    std::scoped_lock lock(mutex_);
    for (auto session_it = sessions_.begin(); session_it != sessions_.end(); ++session_it) {
        if (session_it->second.bindings.contains(binding_id)) {
...
LogicalSessionBindingClosed LogicalSessionRegistry::CloseBindingLocked(const std::unordered_map<std::string, Session>::iterator session_it,
                                                                       const std::string& binding_id, const int64_t now_ms,
                                                                       const bool preserve_reconnect_grace) {
    auto& session = session_it->second;
    ...
    if (session.role == LogicalSessionRole::kController && !HasControllerBinding(session)) {
        const bool owned_controller_seat{controller_session_id_ == session_it->first};
        result.release_controller_input = owned_controller_seat;
        result.lease_generation = session.lease_generation;
        if (owned_controller_seat && preserve_reconnect_grace) {
            session.controller_disconnected_at_ms = now_ms;
            return result;
        }
        session.controller_disconnected_at_ms = 0;
        if (owned_controller_seat) {
            controller_session_id_.clear();
        }
...
bool LogicalSessionRegistry::AuthorizeControllerInputBinding(const std::string& binding_id, const int64_t now_ms) const {
    return FindControllerInputLeaseByBinding(binding_id, now_ms).has_value();
}

bool LogicalSessionRegistry::AuthorizeControllerInputStream(const std::string& stream_id, const int64_t now_ms) const {
...
std::optional<LogicalSessionInputLease> LogicalSessionRegistry::FindControllerInputLeaseByBinding(const std::string& binding_id,
    ...
    for (const auto& [logical_session_id, session] : sessions_) {
        ...
        if (controller_session_id_ == logical_session_id && session.role == LogicalSessionRole::kController && HasInputBinding(session)) {
...
std::optional<LogicalSessionInputLease> LogicalSessionRegistry::AuthorizeAuxiliaryInputGrant(const LogicalSessionGrant& grant,
                                                                                             const std::string& parent_binding_id,
                                                                                             const int64_t now_ms) const {
    std::scoped_lock lock{mutex_};
...
std::optional<LogicalSessionInputLease> LogicalSessionRegistry::FindControllerLeaseByBinding(const std::string& binding_id,
    ...
    for (const auto& [logical_session_id, session] : sessions_) {
        ...
        const auto& binding{session.bindings.at(binding_id)};
        const bool standalone_file_transfer{binding.transport == LogicalSessionTransport::kFileTransfer};
        if ((controller_session_id_ == logical_session_id || standalone_file_transfer) && session.role == LogicalSessionRole::kController) {
            return LogicalSessionInputLease{
                .logical_session_id = logical_session_id,
                .binding_id = binding_id,
                .generation = session.lease_generation,
...
std::optional<LogicalSessionInputLease> LogicalSessionRegistry::FindControllerInputLeaseByStream(const std::string& stream_id,
    ...
    for (const auto& [logical_session_id, session] : sessions_) {
        ...
        const auto binding = std::find_if(session.bindings.begin(), session.bindings.end(), [](const auto& item) {
            return item.second.transport != LogicalSessionTransport::kFileTransfer && item.second.input_allowed;
        });
        if (binding == session.bindings.end()) {
            return std::nullopt;
...
std::optional<LogicalSessionRole> LogicalSessionRegistry::FindRole(const std::string& logical_session_id) const {
    std::scoped_lock lock(mutex_);
    const auto found = sessions_.find(logical_session_id);
    if (found == sessions_.end()) {
        return std::nullopt;
...
std::optional<std::string> LogicalSessionRegistry::FindStreamId(const std::string& logical_session_id) const {
    std::scoped_lock lock(mutex_);
    const auto found = sessions_.find(logical_session_id);
    if (found == sessions_.end()) {
        return std::nullopt;
...
std::vector<LogicalSessionSnapshot> LogicalSessionRegistry::SnapshotActive(const int64_t now_ms) const {
    ...
    for (const auto& [logical_session_id, session] : sessions_) {
        ...
        snapshot.transports.erase(std::unique(snapshot.transports.begin(), snapshot.transports.end()), snapshot.transports.end());
...
size_t LogicalSessionRegistry::ObserverCount() const {
    ...
    return static_cast<size_t>(
        std::count_if(sessions_.begin(), sessions_.end(), [](const auto& item) { return item.second.role == LogicalSessionRole::kObserver; }));
}
