...
bool RdSettings::LoadSettings(const std::string& path) {
    ...
    for (const auto key : {std::string_view{"listen_port"}, std::string_view{"service_port"}}) {
        ...
        if (field && !field.value<int>().has_value()) {
            return false;
        }
    }
    ...
    app_.inject_method_ = [&]() -> TargetApplication::InjectMethod {
        return std::string(inject_method) == "prepare" ? TargetApplication::InjectMethod::kEasyHook : TargetApplication::InjectMethod::kOBS;
    }();
    if (app_.IsSteamUrl()) {
    ...
    app_.event_replay_mode_ = std::string("global") == result["application"]["event-replay-mode"].value_or("global")
                                  ? TargetApplication::EventReplayMode::kGlobal
                                  : TargetApplication::EventReplayMode::kHookInner;
...
void RdSettings::ApplyApplicationMode() {
    if (IsRdpMode()) {
        ...
        LOGI(
            "application.mode=game-hook → inner capture + start/inject game "
...
std::string RdSettings::Dump() {
    ...
    ss << "RdApplication: \n";
    const std::string application_mode = IsRdpMode() ? "rdp"
                                         : application_mode_ == ApplicationMode::kGameHook
                                             ? "game-hook"
...
bool RdSettings::EnableFullColorMode() { return enable_full_color_mode_; }
