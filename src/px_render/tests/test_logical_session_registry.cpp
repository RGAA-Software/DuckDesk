...
LogicalSessionGrant ControlGrant(const std::string& id, const std::string& stream, const std::string& subject) {
    return {.logical_session_id = id, .stream_id = stream, .subject_id = subject, .join_mode = "control", .expires_at_ms = 60'000};
}

TEST(LogicalSessionRegistry, ControllerAndObserverAreSeparatedByLease) {
    LogicalSessionRegistry registry;
    const auto controller =
        registry.Bind(ControlGrant("controller", "stream-controller", "alice"), LogicalSessionTransport::kRtcLocal, "rtc-controller", false, 1);
    ...
    const auto admitted = registry.Bind(observer, LogicalSessionTransport::kRtcLocal, "rtc-observer", false, 2);
...
TEST(LogicalSessionRegistry, TakeoverInvalidatesOldControllerLease) {
    LogicalSessionRegistry registry;
    const auto original = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kRtcLocal, "rtc-one", false, 1);
    ASSERT_EQ(original.code, LogicalSessionAdmissionCode::kAccepted);
    const auto occupied = registry.Bind(ControlGrant("two", "stream-two", "bob"), LogicalSessionTransport::kRtcLocal, "rtc-two", false, 2);
    EXPECT_EQ(occupied.code, LogicalSessionAdmissionCode::kOccupied);

    const auto replacement = registry.Bind(ControlGrant("two", "stream-two", "bob"), LogicalSessionTransport::kRtcLocal, "rtc-two", true, 3);
    ...
    const auto replacement_snapshot =
        std::find_if(sessions.begin(), sessions.end(), [](const LogicalSessionSnapshot& snapshot) { return snapshot.logical_session_id == "two"; });
...
TEST(LogicalSessionRegistry, OldControllerDisconnectCannotReleaseReplacementLease) {
    ...
    const auto original = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kRtcLocal, "rtc-one", false, 1);
    ASSERT_EQ(original.code, LogicalSessionAdmissionCode::kAccepted);
    const auto replacement = registry.Bind(ControlGrant("two", "stream-two", "bob"), LogicalSessionTransport::kRtcLocal, "rtc-two", true, 2);
    ASSERT_EQ(replacement.code, LogicalSessionAdmissionCode::kAccepted);
...
TEST(LogicalSessionRegistry, InputLeaseIdentifiesTheExactControllerGeneration) {
    LogicalSessionRegistry registry;
    const auto original = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kWs, "ws-one", false, 1);
    ASSERT_EQ(original.code, LogicalSessionAdmissionCode::kAccepted);
    ...
    const auto replacement = registry.Bind(ControlGrant("two", "stream-two", "bob"), LogicalSessionTransport::kWs, "ws-two", true, 3);
    ASSERT_EQ(replacement.code, LogicalSessionAdmissionCode::kAccepted);
...
TEST(LogicalSessionRegistry, ActiveSnapshotDeduplicatesTransportBindings) {
    ...
    ASSERT_EQ(registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kWs, "ws-two", false, 2).code,
              LogicalSessionAdmissionCode::kAccepted);
    ASSERT_EQ(registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kFileTransfer, "file-one", false, 3).code,
              LogicalSessionAdmissionCode::kAccepted);
    const auto sessions = registry.SnapshotActive(4);
    ...
    EXPECT_EQ(std::count(sessions.front().transports.begin(), sessions.front().transports.end(), LogicalSessionTransport::kWs), 1);
}

TEST(LogicalSessionRegistry, ControllerDisconnectReleasesInputAndCanReconnect) {
    LogicalSessionRegistry registry;
    const auto admitted = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kWs, "ws-one", false, 1);
    ASSERT_EQ(admitted.code, LogicalSessionAdmissionCode::kAccepted);
    ...
    const auto reconnected = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kWs, "ws-two", false, 12);
...
TEST(LogicalSessionRegistry, FailedRtcReservationReleasesControllerSeatWithoutReconnectGrace) {
    LogicalSessionRegistry registry;
    const auto admitted =
        registry.Bind(ControlGrant("failed", "stream-failed", "alice"), LogicalSessionTransport::kRtcLocal, "rtc-local:stream-failed", false, 1);
    ASSERT_EQ(admitted.code, LogicalSessionAdmissionCode::kAccepted);
    ...
    const auto native = registry.Bind(ControlGrant("native", "stream-native", "bob"), LogicalSessionTransport::kWs, "ws:stream-native", false, 2);
...
TEST(LogicalSessionRegistry, FileTransferBindingNeverExtendsControllerInputLease) {
    LogicalSessionRegistry registry;
    const auto controller = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kWs, "ws-one", false, 1);
    ASSERT_EQ(controller.code, LogicalSessionAdmissionCode::kAccepted);
...
TEST(LogicalSessionRegistry, FileTransferCanCreateControllerSessionWithoutInputBinding) {
    LogicalSessionRegistry registry;
    const auto admitted = registry.Bind(ControlGrant("one", "stream-one", "alice"), LogicalSessionTransport::kFileTransfer, "file-one", false, 1);
    ASSERT_EQ(admitted.code, LogicalSessionAdmissionCode::kAccepted);
...
TEST(LogicalSessionRegistry, StandaloneFileTransferDoesNotOccupyDesktopControllerSeat) {
    LogicalSessionRegistry registry;
    ASSERT_EQ(registry.Bind(ControlGrant("files", "stream-files", "alice"), LogicalSessionTransport::kFileTransfer, "file-one", false, 1).code,
              LogicalSessionAdmissionCode::kAccepted);

    const auto desktop = registry.Bind(ControlGrant("desktop", "stream-desktop", "alice"), LogicalSessionTransport::kWs, "ws-one", false, 2);
    EXPECT_EQ(desktop.code, LogicalSessionAdmissionCode::kAccepted);
...
TEST(LogicalSessionRegistry, FindsAnyActiveRoleByTransportBindingWithoutMutatingIt) {
    ...
    ASSERT_EQ(registry.Bind(observer_grant, LogicalSessionTransport::kRtcLocal, "rtc-local:stream-observer", false, 1).code,
              LogicalSessionAdmissionCode::kAccepted);

    const auto logical_session_id = registry.FindLogicalSessionIdByBinding("rtc-local:stream-observer", 2);
...
TEST(LogicalSessionRegistry, TicketPolicyRejectsObserverAndTakeover) {
    ...
    const auto controller = registry.Bind(controller_grant, LogicalSessionTransport::kRtcLocal, "rtc-one", false, 1);
    ASSERT_EQ(controller.code, LogicalSessionAdmissionCode::kAccepted);

    auto observer_grant = ControlGrant("observer", "stream-observer", "bob");
    observer_grant.join_mode = "observe";
    observer_grant.allow_observer = false;
    EXPECT_EQ(registry.Bind(observer_grant, LogicalSessionTransport::kRtcLocal, "rtc-observer", false, 2).code,
              LogicalSessionAdmissionCode::kObserversDisabled);

    const auto replacement = registry.Bind(ControlGrant("two", "stream-two", "carol"), LogicalSessionTransport::kRtcLocal, "rtc-two", true, 3);
...
TEST(LogicalSessionRegistry, AuxiliaryInputCannotReviveWhenParentIdentifierIsReused) {
    ...
    const auto grant{ControlGrant("one", "stream-one", "alice")};
    ASSERT_EQ(registry.Bind(grant, LogicalSessionTransport::kWs, "parent", false, 1).code, LogicalSessionAdmissionCode::kAccepted);
    ASSERT_EQ(registry.Bind(grant, LogicalSessionTransport::kRtcLocal, "other-media", false, 1).code, LogicalSessionAdmissionCode::kAccepted);
