...
void DdaCaptureSource::ConfigureMediaBacklogProbe(MediaBacklogProbe probe) {
...
bool DdaCaptureSource::InitializeVideoCaptures() {
    ...
    do {
        ...
        res = D3D11CreateDevice(adapter1, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT, nullptr, 0, D3D11_SDK_VERSION,
                                &d3d11_device, &feature_level, &d3d11_device_context);
        ...
        do {
            ...
            if (res == S_OK) {
                ...
                LOGE("Failed to get output description of device: {} in adapter index: {}, adaper id: {}", monitor_index, adapter_index, adapter_uid);
                break;
            }
        } while (true);
...
void DdaCaptureSource::InitializeCursorCapture() {
    const auto weak_self = std::weak_ptr<DdaCaptureSource>(std::dynamic_pointer_cast<DdaCaptureSource>(shared_from_this()));
    cursor_capture_thread_ = Thread::MakeOnceTask(
        [weak_self]() {
...
bool DdaCaptureSource::InitializeCapture() {
    ...
    const auto owner = std::dynamic_pointer_cast<DdaCaptureSource>(shared_from_this());
    for (const auto& [dev_name, monitor_info] : monitors_) {
...
bool DdaCaptureSource::StartCapturing() {
    ...
    for (const auto& [dev_name, monitor_info] : monitors_) {
        ...
        capture->SetDDAErrorCallback([weak_self](const MonitorCaptureError& err) {
            if (const auto self = weak_self.lock(); self && self->capture_error_callback_) {
...
void DdaCaptureSource::RestartCapturing() {
    ...
    for (int attempt = 1; attempt <= kMaxAttempts; ++attempt) {
        if (StartCapturing()) {
            LOGI("DDA topology rebuild succeeded on attempt {}, monitors: {} -> {}", attempt, old_count, monitors_.size());
            return;
        }
...
void DdaCaptureSource::RequestStaticFrame() {
    auto captures = captures_.Clone();
    for (const auto& capture : captures | std::views::values) {
        if (!capture || capture->IsPausing()) {
            continue;
...
void DdaCaptureSource::CalculateVirtualDeskInfo() {
    ...
    std::sort(sorted_monitors_.begin(), sorted_monitors_.end(),
              [](const CaptureMonitorInfo& lh, const CaptureMonitorInfo& rh) -> bool { return lh.left_ < rh.left_; });
