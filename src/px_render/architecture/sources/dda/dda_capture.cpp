...
DDACapture::DDACapture(const std::shared_ptr<DdaCaptureSource>& owner, const CaptureMonitorInfo& my_monitor_info)
...
bool DDACapture::InitInternal() {
    ...
    do {
        ...
        res = D3D11CreateDevice(adapter1.Get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_BGRA_SUPPORT, nullptr, 0, D3D11_SDK_VERSION,
                                d3d11_device.GetAddressOf(), &feature_level, d3d11_device_context.GetAddressOf());
        ...
        do {
            ...
            if (res == S_OK) {
                ...
                auto func_valid_rect = [](const RECT& rect) -> bool { return rect.right > rect.left && rect.bottom > rect.top; };
                ...
                if (output_desc.AttachedToDesktop && is_valid_rect) {
                    ...
                    for (int j = 0; j < max_retry_count; ++j) {
                        ...
                        if (error != S_OK || !dxgi_output_duplication_.duplication_) {
                            ...
                            LOGE("Failed to duplicate output from IDXGIOutput1, error {} with code {:x}", StringUtil::GetErrorStr(error).c_str(),
                                 (uint32_t)error);
                            continue;
                        } else {
                    ...
                    std::stringstream ss;
                    ss << (output_desc.AttachedToDesktop ? "Attached" : "Detached") << " output " << monitor_index << " ("
                       << output_desc.DesktopCoordinates.top << ", " << output_desc.DesktopCoordinates.left << ") - ("
                       << output_desc.DesktopCoordinates.bottom << ", " << output_desc.DesktopCoordinates.right << ") is ignored.";
...
void DDACapture::Capture() {
    ...
    while (!stop_flag_) {
        ...
        const auto queuing_msg_count = owner ? owner->GetNetworkMediaBacklog() : std::int64_t{0};
        ...
        if (res == S_OK) {
            ...
            if (refresh_screen_) {
                if (!last_list_texture_ || !last_list_texture_->texture2d_ || !last_list_texture_->shared_handle_) {
                    continue;
                }
                if (used_cache_times_++ > 5) {
                    refresh_screen_ = false;
                }
                texture = last_list_texture_->texture2d_;
                is_cached = true;
                LOGI("Use retained desktop texture");
            } else {
                continue;
            }
        }
        ...
        if (texture) {
            ...
            auto copy_us = (uint64_t)std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - copy_beg).count();
            ++lat_copy_cnt;
            lat_copy_us_sum += copy_us;
            if (copy_us > lat_copy_us_max) {
                lat_copy_us_max = copy_us;
            }
...
void DDACapture::OnCaptureFrame(const ComPtr<ID3D11Texture2D>& texture, bool is_cached) {
    ...
    bool texture_changed = (input_width != shared_width) || (input_height != shared_height) || (input_format != shared_format);
    ...
    if (texture_changed) {
        ...
        LOGI("Create retained desktop texture success: {}", my_monitor_info_.name_);
    }
    ...
    if (!is_cached) {
        // The shared texture is also the retained static image. One GPU
        // copy updates both purposes; cached sends reuse it in place.
...
void DDACapture::SendCachedTexture() {
    ...
    auto task = [weak_self]() {
        ...
        if (self->last_list_texture_ && self->last_list_texture_->texture2d_ && self->last_list_texture_->shared_handle_) {
            self->OnCaptureFrame(self->last_list_texture_->texture2d_, true);
            return;
        }
        // A newly created duplication may not have retained its first
        // image yet. Wake it only in that bootstrap case; established
        // static desktops reuse the retained image without synthetic
        // input or continuous capture.
        self->RefreshScreen();
        self->TryWakeOs();
    };
...
void DDACapture::SendTextureHandle(const HANDLE& shared_handle, uint32_t width, uint32_t height, DXGI_FORMAT format, bool request_idr) {
    ...
    if (mon_win_info.Valid()) {
        if (StringUtil::CopyCStringToArray(cap_video_frame.display_name_, mon_win_info.name_)) {
            LOGW("display_name truncated for monitor: {}, src_len: {}, dst_len: {}", mon_win_info.name_, mon_win_info.name_.size(),
                 sizeof(cap_video_frame.display_name_));
        }
