...
class RenderCompositionRoot final : public std::enable_shared_from_this<RenderCompositionRoot> {
  public:
    static std::shared_ptr<RenderCompositionRoot> Create(const std::shared_ptr<PxAsyncRuntime>& runtime,
                                                         const std::shared_ptr<BuiltinModuleCatalog>& catalog,
                                                         std::shared_ptr<FlowNodePluginRegistry> flow_node_plugins = {});

    RenderCompositionRoot(std::shared_ptr<PxAsyncScope> lifecycle_scope, std::shared_ptr<BuiltinModuleCatalog> catalog,
                          std::shared_ptr<FlowNodePluginRegistry> flow_node_plugins = {});
    ...
    [[nodiscard]] ModuleLifecycleResult Register(BuiltinModuleRegistration registration);
    [[nodiscard]] ModuleLifecycleResult SetEnabled(const std::string& module_id, bool enabled);
  ...
  private:
    static PxAwaitable<void> RunStart(std::weak_ptr<RenderCompositionRoot> weak_owner, CompositionCompletion completion);
    static PxAwaitable<void> RunStop(std::weak_ptr<RenderCompositionRoot> weak_owner, CompositionCompletion completion);
    static PxAwaitable<std::optional<RenderError>> StopRegistrations(std::shared_ptr<RenderCompositionRoot> owner,
