...
RenderError MakeFrameDebuggerError(const RenderErrorCode code, std::string operation, std::string reason, const bool recoverable) {
    return RenderError{
        .code = code,
...
std::string SafeFilePart(std::string value) {
    std::ranges::transform(value, value.begin(),
                           [](const unsigned char ch) { return static_cast<char>(std::isalnum(ch) || ch == '-' || ch == '_' ? ch : '_'); });
...
std::shared_ptr<FrameDebuggerObserver> FrameDebuggerObserver::Create(const std::shared_ptr<PxAsyncRuntime>& runtime, FrameDebuggerOptions options) {
...
FrameDebuggerObserver::FrameDebuggerObserver(std::shared_ptr<PxAsyncScope> worker_scope, FrameDebuggerOptions options)
    : worker_scope_(std::move(worker_scope)), options_(std::move(options)), raw_log_gate_(options_.raw_log_interval, 128), drop_log_gate_(1s, 8) {}
...
BuiltinModuleRegistration FrameDebuggerObserver::MakeRegistration() {
    ...
    return BuiltinModuleRegistration{
        ...
        .start = [weak_owner]() -> PxAwaitable<ModuleLifecycleResult> {
            ...
            if (!owner) {
                co_return std::unexpected(MakeFrameDebuggerError(RenderErrorCode::kModuleStartFailed, "start", "observer owner expired", false));
            }
        ...
        .stop = [weak_owner]() -> PxAwaitable<ModuleLifecycleResult> {
            ...
            co_return co_await StopAsync(owner, std::chrono::steady_clock::now() + 2s);
        },
        .set_enabled =
            [weak_owner](const bool enabled) {
                ...
                if (!owner) {
                    return ModuleLifecycleResult(std::unexpected(
                        MakeFrameDebuggerError(RenderErrorCode::kModuleLifecycleRejected, "set_enabled", "observer owner expired", false)));
                }
...
ModuleLifecycleResult FrameDebuggerObserver::Start() {
    const auto queue = std::make_shared<BoundedMediaQueue<FrameDebuggerEvent>>(options_.queue_capacity, QueueOverflowPolicy::kDropOldest);
    const auto completion = PxAsyncOneShot<void>::Create(worker_scope_->Executor());
    const std::weak_ptr<FrameDebuggerObserver> weak_owner = weak_from_this();
    ...
    bool accepted = false;
    {
        ...
        accepted = worker_scope_->Spawn("frame_debugger_consumer", [weak_owner, queue, completion, executor]() mutable {
            return ConsumeLoop(weak_owner, std::move(queue), std::move(completion), std::move(executor));
        });
        if (accepted) {
    ...
    if (!accepted) {
        queue->Close(QueueCloseMode::kCancel);
        return std::unexpected(MakeFrameDebuggerError(RenderErrorCode::kModuleStartFailed, "start", "worker scope rejected the consumer", true));
    }
    LOGI("event=observer.start component=frame_debugger queue_capacity={} "
         "save_encoded_video={} outcome=success",
         options_.queue_capacity, options_.save_encoded_video);
    return {};
}

PxAwaitable<ModuleLifecycleResult> FrameDebuggerObserver::StopAsync(std::shared_ptr<FrameDebuggerObserver> owner,
    ...
    if (!queue || !completion) {
        co_return std::unexpected(MakeFrameDebuggerError(RenderErrorCode::kModuleStopFailed, "stop", "consumer state is incomplete", false));
    }
    ...
    if (!drained) {
        ...
        co_return std::unexpected(
            MakeFrameDebuggerError(RenderErrorCode::kAsyncScopeDrainTimeout, "stop", "encoded frame queue did not drain before deadline", true));
    }
    owner->CloseFiles();
    ...
    LOGI("event=observer.stop component=frame_debugger outcome=success "
         ...
         "write_failures={}",
         snapshot.accepted, snapshot.dropped, snapshot.high_watermark, owner->encoded_bytes_written_.load(std::memory_order_relaxed),
...
FrameDebuggerSubmitResult FrameDebuggerObserver::SubmitEncoderReady(VideoEncoderReady event) {
    return SubmitEvent(std::make_shared<const FrameDebuggerEvent>(std::move(event)));
}

FrameDebuggerSubmitResult FrameDebuggerObserver::SubmitEncodedFrame(std::shared_ptr<const EncodedVideoFrame> frame) {
    ...
    return SubmitEvent(std::make_shared<const FrameDebuggerEvent>(std::move(frame)));
}

FrameDebuggerSubmitResult FrameDebuggerObserver::SubmitClientConnected() {
    return SubmitEvent(std::make_shared<const FrameDebuggerEvent>(FrameDebuggerClientConnected{}));
}

void FrameDebuggerObserver::ObserveRawFrame(RawVideoFrameObservation frame) {
    if (!running_.load(std::memory_order_acquire) || !enabled_.load(std::memory_order_acquire) || frame.monitor_id.empty()) {
        return;
    }
    raw_frames_observed_.fetch_add(1, std::memory_order_relaxed);
    const auto decision = raw_log_gate_.Evaluate("raw_frame:" + frame.monitor_id, std::chrono::steady_clock::now());
    if (decision.emit) {
        LOGI("event=observer.raw_frame component=frame_debugger monitor={} "
             "frame_index={} width={} height={} suppressed={} outcome=observed",
             PrivacyLogId(frame.monitor_id), frame.frame_index, frame.width, frame.height, decision.suppressed_since_last_emit);
    }
}

bool FrameDebuggerObserver::WantsEncodedFrames() const noexcept {
    return options_.save_encoded_video && running_.load(std::memory_order_acquire) && enabled_.load(std::memory_order_acquire);
}
...
FrameDebuggerSnapshot FrameDebuggerObserver::Snapshot() const {
    ...
    return FrameDebuggerSnapshot{
        ...
        .raw_frames_observed = raw_frames_observed_.load(std::memory_order_relaxed),
        .encoded_frames_submitted = encoded_frames_submitted_.load(std::memory_order_relaxed),
        .encoded_bytes_written = encoded_bytes_written_.load(std::memory_order_relaxed),
        .file_write_failures = file_write_failures_.load(std::memory_order_relaxed),
        .queue = queue_snapshot,
    };
}

PxAwaitable<void> FrameDebuggerObserver::ConsumeLoop(std::weak_ptr<FrameDebuggerObserver> weak_owner,
                                                     std::shared_ptr<BoundedMediaQueue<FrameDebuggerEvent>> queue,
...
void FrameDebuggerObserver::ProcessEvent(const FrameDebuggerEvent& event) {
    ...
    case 0:
        ...
        return;
    case 1:
        ProcessEncodedFrame(std::get<std::shared_ptr<const EncodedVideoFrame>>(event));
        return;
    case 2:
...
void FrameDebuggerObserver::ProcessEncoderReady(const VideoEncoderReady& event) {
    ...
    if (directory_error) {
        ...
        LOGE("event=observer.file_open component=frame_debugger "
             ...
             StableErrorCode(RenderErrorCode::kFrameDebuggerDirectoryFailed), PrivacyLogId(event.monitor_id), directory_error.value(),
    ...
    if (!file || !file->IsOpen()) {
        ...
        LOGE("event=observer.file_open component=frame_debugger "
             ...
             StableErrorCode(RenderErrorCode::kFrameDebuggerFileOpenFailed), PrivacyLogId(event.monitor_id), event.codec);
        return;
    }
    if (const auto existing = encoded_video_files_.find(event.monitor_id); existing != encoded_video_files_.end()) {
        existing->second->Close();
    }
    encoded_video_files_[event.monitor_id] = file;
}

void FrameDebuggerObserver::ProcessEncodedFrame(const std::shared_ptr<const EncodedVideoFrame>& frame) {
    ...
    if (written != static_cast<std::int64_t>(bytes.size())) {
        ...
        LOGE("event=observer.file_write component=frame_debugger "
             ...
             "actual_bytes={}",
             StableErrorCode(RenderErrorCode::kFrameDebuggerFileWriteFailed), PrivacyLogId(frame->identity.monitor_id), frame->identity.frame_index,
             bytes.size(), written);
        return;
    }
    encoded_bytes_written_.fetch_add(static_cast<std::uint64_t>(written), std::memory_order_relaxed);
}
...
FrameDebuggerSubmitResult FrameDebuggerObserver::SubmitEvent(std::shared_ptr<const FrameDebuggerEvent> event) {
    ...
    case QueueSubmitResult::kAcceptedAfterDroppingOldest:
        if (const auto decision = drop_log_gate_.Evaluate("encoded_frame", std::chrono::steady_clock::now()); decision.emit) {
            const auto snapshot = queue->Snapshot();
            LOGW("event=observer.queue_overflow component=frame_debugger "
                 ...
                 StableErrorCode(RenderErrorCode::kObserverQueueOverflow), snapshot.depth, snapshot.dropped, snapshot.high_watermark,
