...
class RdContext : public std::enable_shared_from_this<RdContext> {
  public:
    ...
    std::shared_ptr<MessageListener> CreateMessageListener(MessageExecutionLane lane = MessageExecutionLane::kControl);
    std::shared_ptr<RenderModuleRegistry> GetRenderModuleRegistry();
    void SetRenderCompositionRoot(const std::shared_ptr<render::RenderCompositionRoot>& root);
    std::shared_ptr<render::RenderCompositionRoot> GetRenderCompositionRoot();
    void SetFrameDebuggerObserver(const std::shared_ptr<render::FrameDebuggerObserver>& observer);
    std::shared_ptr<render::FrameDebuggerObserver> GetFrameDebuggerObserver();
    void SetEncodedMediaBus(const std::shared_ptr<render::EncodedMediaBus>& media_bus);
    std::shared_ptr<render::EncodedMediaBus> GetEncodedMediaBus();
    void SetMediaRecorderSink(const std::shared_ptr<render::MediaRecorderSink>& recorder);
    std::shared_ptr<render::MediaRecorderSink> GetMediaRecorderSink();
    void SetFrameCarrierProcessor(const std::shared_ptr<render::FrameCarrierProcessor>& processor);
    std::shared_ptr<render::FrameCarrierProcessor> GetFrameCarrierProcessor();
    void SetFrameResizerProcessor(const std::shared_ptr<render::FrameResizerProcessor>& processor);
    std::shared_ptr<render::FrameResizerProcessor> GetFrameResizerProcessor();
    void SetInputReplayService(const std::shared_ptr<render::InputReplayService>& service);
    std::shared_ptr<render::InputReplayService> GetInputReplayService();
    void SetJoystickService(const std::shared_ptr<render::JoystickService>& service);
    std::shared_ptr<render::JoystickService> GetJoystickService();
    void SetFileTransferService(const std::shared_ptr<render::FileTransferService>& service);
    std::shared_ptr<render::FileTransferService> GetFileTransferService();
    void SetNetworkTransportHub(const std::shared_ptr<render::NetworkTransportHub>& hub);
    std::shared_ptr<render::NetworkTransportHub> GetNetworkTransportHub();
    void SetVoiceCallService(const std::shared_ptr<render::VoiceCallService>& service);
  ...
  private:
    ...
    std::atomic<std::shared_ptr<PxBlockingExecutor>> task_executor_{};
