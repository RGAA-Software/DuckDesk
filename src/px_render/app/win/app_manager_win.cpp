...
namespace px
{
    ...
    // 暂无 32 位版 px_graphics.dll，32 位游戏明确拒绝注入（见 InjectDll）
    constexpr auto kX86DllName = "";
    constexpr auto kX64DllName = "px_graphics.dll";
    ...
    void AppManagerWinImpl::Init() {
        ...
        if (settings_->capture_.IsVideoInnerCapture()) {
            ...
            // 避免残留的旧游戏带着 px_graphics.dll 重连新 render 的 /ipc 串帧
    ...
    bool AppManagerWinImpl::StartProcessWithHook() {
        ...
        std::wstring x86_dll;
        std::wstring x64_dll = L"px_graphics.dll";
    ...
    void AppManagerWinImpl::VerifyInjectedStillAlive() {
        ...
        LOGW("Target pid: {} gone or px_graphics.dll unmapped (alive: {}, mapped: {}), will re-inject.",
        ...
        if (settings_->capture_.IsVideoInnerCapture()) {
            ...
            if (!injected) {
                ...
                if (again.ok_ && again.value_) {
                    LOGW("Injector timed out/failed but px_graphics.dll is mapped — treat as success");
                    injected = true;
                }
            }
            if (injected) {
    ...
    bool AppManagerWinImpl::InjectDll(uint32_t pid, uint32_t tid, bool is_x86, const std::string& x86_dll, const std::string& x64_dll) {
        ...
        // 32 位目标明确拒绝：暂无 32 位 px_graphics.dll，且 inject-library 会把
        // 64 位 LoadLibraryW 地址写进 WoW64 进程（行为未定义，可能崩游戏）。
        // 记 permanent failure，停止重试
        if (is_x86) {
            inject_gave_up_ = true;
            LOGE("暂不支持 32 位游戏, pid: {}, 注入已放弃（需要 32 位版 px_graphics.dll）", pid);
            return false;
        }
        {
            HANDLE process = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, pid);
