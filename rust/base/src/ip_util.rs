...
fn is_virtual_interface(interface_name: &str) -> bool {
    ...
    // WSL/WSL2
    if interface_lower.contains("wsl") ||
        interface_lower.contains("wsl2") {
    ...
    if interface_lower.contains("lo") ||  // 回环接口
        ...
        interface_lower.contains("plip") {  // PLIP
        ...
        match result {
            Ok(ips) => {
                ...
                for ip in &ips {
                    println!("  {}", ip);
                    let details = get_ipv4_details(*ip);
                    println!("    类型: {:?}, 可能用途: {}", details.ip_type, details.possible_use);
                }
            }
            Err(e) => {
                // 在某些环境下可能无法获取网络接口信息
...
pub fn test_ip_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (iface, ip) in &all_ips {
        let virtual_marker = if is_virtual_interface(iface) { " [虚拟]" } else { "" };
    ...
    if clean_ips.is_empty() {
        ...
        for (i, ip) in clean_ips.iter().enumerate() {
            let details = get_ipv4_details(*ip);
            println!("  {}. {} - {:?} ({})", i + 1, ip, details.ip_type, details.possible_use);
        }
    }
    ...
    if let Some(preferred_ip) = get_preferred_real_ipv4()? {
        ...
        println!("  {} - {:?} ({})", preferred_ip, details.ip_type, details.possible_use);
    ...
    println!("虚拟接口数: {}", all_ips.iter().filter(|(iface, _)| is_virtual_interface(iface)).count());
    println!("真实接口数: {}", all_ips.iter().filter(|(iface, _)| !is_virtual_interface(iface)).count());
