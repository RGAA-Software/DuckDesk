...
python "%~dp0..\set_app_version.py" --bump
if errorlevel 1 exit /b %errorlevel%
...
if exist "%~dp0..\build_official\dist\%WEB_DIST_NAME%" (
    echo Removing stale %~dp0..\build_official\dist\%WEB_DIST_NAME% ...
    rmdir /s /q "%~dp0..\build_official\dist\%WEB_DIST_NAME%"
)
