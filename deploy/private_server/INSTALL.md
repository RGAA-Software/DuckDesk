...
2. 为本次部署生成一个固定的小写 UUID。准备 PostgreSQL 18 的 Console/Desk 空库及 owner、runtime、只读 backup 角色，以及客户自己的数据库 CA 和服务 HTTPS 证书；CA 和证书可自行生成或自签，不要求公有 CA。客户负责终端的信任配置和实际域名或 IP。从 Auth 管理面下载绑定该 UUID 的 `PXLIC2` 许可证，由 Auth 运维人员交付 CN 主机上通过 `px_auth_admin create-trust-store` 生成、当前启用的公钥信任文件；经可信运维渠道分别核对信任文件 SHA-256 和 key ID，不信任许可证或普通下载响应自带的公钥。首次安装前先创建 `pixels-console` 系统组和用户（如不存在；安装器也会确保它们存在），把两份文件放在 `/etc/pixels/<部署 UUID>/`，由该用户持有、权限 `0400`，以该用户执行 `px_console_admin validate-license`。私有 Console 本地验签，不依赖 Auth 在线服务；本阶段不逐张验证客户证书的签发来源。
...
- 许可证续期不需重装：经上述渠道核对新公钥信任文件并下载新 `.pxlic`，保留旧文件备份，然后以 root 执行 `sh "$package_dir/tools/renew_console_license.sh" "$deployment_id" /absolute/new-trust.json /absolute/new-license.pxlic`。工具停服前以 `pixels-console` 身份验签并核对 deployment，替换后重启；失败时恢复原文件并尝试启动旧服务。随后仍须检查 readiness、管理员许可证摘要和业务准入。此工具随下一次正常发版进入包；已发布的 1.0.2 包不含它。
