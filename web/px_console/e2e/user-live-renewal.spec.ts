...
const consoleBase = process.env.PX_LIVE_CONSOLE_URL || process.env.PX_LIVE_CMS_URL || 'https://127.0.0.1:30500'

test('live Web Client rotates its ticket and reconnects after peer failure', async ({ browser }) => {
  test.skip(!LIVE, 'Set PX_LIVE_USER_RENEWAL=1 to run against the local Console/Service/Render stack')
  ...
  let csrf = ''
  try {
    await page.goto(`${consoleBase}/user/public-apps`)
    ...
    if (instanceId && csrf) {
      await context.request.post(`${consoleBase}/api/v1/public/instances/${encodeURIComponent(instanceId)}/stop`, {
        headers: { Origin: consoleBase, 'X-CSRF-Token': csrf },
