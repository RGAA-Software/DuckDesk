...
async function refresh() {
    try {
        [apps.value, instances.value] = await Promise.all([getPublicApps(), getGuestInstances()]);
    } catch {
        message.error("公共应用加载失败");
    }
}
async function enter(app: ApplicationCard, viewOnly = false) {
    ...
    try {
        ...
        if (current) {
            const instance = current.reconnectable
                ? current
                : await waitForGuestInstance(current.instance_id);
            await openGuestInstance(
                instance,
                sessionStorage.getItem(`px_guest_nonce_app_${app.app_id}`) || crypto.randomUUID(),
                viewOnly,
            );
        } else {
            ...
            await openGuestInstance(instance, "", viewOnly);
        }
    } catch (error: any) {
...
function stop(app: ApplicationCard) {
    ...
    Modal.confirm({
        ...
        okType: "danger",
        async onOk() {
            await stopGuestInstance(instance.instance_id);
            await refresh();
        },
    });
}
    ...
    <main class="min-h-screen bg-slate-950 px-6 py-10">
        <div class="mx-auto max-w-6xl">
            <header class="mb-8 flex items-center justify-between text-white">
                <div>
                    <h1 class="text-2xl font-semibold">公共云端应用</h1>
                    <p class="mt-2 text-slate-400">
                        无需账号；启动和连接仍受一次性票据与访客配额保护。
                    </p>
                </div>
                ...
                <a-col v-for="app in apps" :key="app.app_id" :xs="24" :md="12" :xl="8">
                    <a-card hoverable
                        ><a-card-meta
                            :title="app.name"
                            :description="
                                activeByApp.get(app.app_id)
                                    ? `我的实例：${activeByApp.get(app.app_id)?.state}`
                                    : '公开应用'
                            "
                        />
                        <div class="mt-5 flex justify-end">
                            <a-space
                                ><a-button
                                    ...
                                    :disabled="activeByApp.get(app.app_id)?.state === 'stopping'"
                                    @click="enter(app, true)"
                                    >仅观看</a-button
                                ><a-button
                                    v-if="activeByApp.get(app.app_id)"
                                    danger
                                    :disabled="activeByApp.get(app.app_id)?.state === 'stopping'"
                                    @click="stop(app)"
                                    >停止</a-button
                                ><a-button
                                    ...
                                    @click="enter(app)"
                                    >{{
                                        activeByApp.get(app.app_id) ? "进入" : "启动并进入"
                                    }}</a-button
                                ></a-space
                            >
                        </div>
