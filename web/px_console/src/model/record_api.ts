...
function toRecordApiError(error: unknown): RecordApiError {
    if (error instanceof AxiosError && error.response?.data) {
        const responseBody = error.response.data;
        if (typeof responseBody.code === "number") {
            return new RecordApiError(
                responseBody.code,
                responseBody.message || `request failed: ${error.response.status}`,
            );
        }
        return new RecordApiError(
    ...
    return new RecordApiError(-1, error instanceof Error ? error.message : String(error));
}
...
export async function getRecordAccess(deviceId: string): Promise<RecordAccessInfo> {
    try {
        ...
        throw toRecordApiError(requestError);
    }
}
...
export async function downloadRecordToConsole(
    ...
    try {
        const resp = await axiosHttp.post("/api/v1/record/download", {
            device_id: deviceId,
            filename: filename,
        });
...
export async function deleteRecord(id: string): Promise<void> {
    try {
        const resp = await axiosHttp.delete("/api/v1/record/" + encodeURIComponent(id));
        unwrap<string>(resp);
    } catch (requestError) {
        throw toRecordApiError(requestError);
    }
