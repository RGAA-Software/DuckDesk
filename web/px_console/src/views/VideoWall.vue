...
const sortedDevices = computed(() =>
    [...devices.value].sort((leftDevice, rightDevice) => {
        if (leftDevice.online !== rightDevice.online) return leftDevice.online ? -1 : 1;
        return `${leftDevice.device_name}\u0000${leftDevice.device_id}`.localeCompare(
            `${rightDevice.device_name}\u0000${rightDevice.device_id}`,
            "zh-CN",
        );
    }),
);
...
function videoInfo(cell: WallCell) {
    const streamStats = cell.stats;
    if (!streamStats.width) return "等待视频信息";
    return `${streamStats.codec || "H264"} · ${streamStats.width}×${streamStats.height} · ${streamStats.fps} fps · ${streamStats.bitrateKbps} Kbps`;
}
            ...
            <article v-for="cell in cells" :key="cell.device.device_id" class="wall-cell">
                <header class="cell-header">
                    <div class="device-title">
                        <strong :title="cell.device.device_name">{{
                            cell.device.device_name || "未命名设备"
                        }}</strong>
                ...
                <div class="video-stage">
                    ...
                    <div v-if="cell.state !== 'playing'" class="video-placeholder">
                        ...
                        <a-button
                            v-if="cell.device.online && cell.state === 'error'"
                            size="small"
                            @click="connectCell(cell)"
                        >
                            重试
                        </a-button>
                    </div>
...
.wall-cell {
    ...
    border-radius: 7px;
    box-shadow: 0 2px 8px rgba(18, 32, 56, 0.05);
}
.cell-header {
    display: flex;
    align-items: center;
...
.video-placeholder {
    ...
    color: #aeb7c5;
    font-size: 13px;
    background: radial-gradient(circle at center, #202938 0, #0d1118 72%);
}
.cell-footer {
    display: flex;
    align-items: center;
