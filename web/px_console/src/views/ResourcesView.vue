...
const updateStatistics = () => {
    ...
    for (let sampleIndex = 0; sampleIndex < sys_info_array.value.length; sampleIndex++) {
        const systemSample = sys_info_array.value[sampleIndex];
        if (!systemSample || !systemSample.cpu) {
            continue;
        }
        tmpCpuUsageXAxis.push("");
        tmpCpuUsageYAxis.push(systemSample.cpu.usage);

        tmpCpuFreqXAxis.push("");
        tmpCpuFreqYAxis.push(systemSample.cpu.current_frequency);

        tmpMemUsageXAxis.push("");
        tmpMemUsageYAxis.push(systemSample.mem.used / 1024 / 1024 / 1024);

        if (last_send_data === -1) {
            tmpNetworkSendXAxis.push("");
            tmpNetworkSendYAxis.push(0);
            if (systemSample.networks.length > 0 && systemSample.networks[0]) {
                last_send_data = systemSample.networks[0].sent_data;
            }
        } else {
            if (systemSample.networks.length > 0 && systemSample.networks[0]) {
                const current_sent_data = systemSample.networks[0].sent_data;
        ...
        if (last_recv_data === -1) {
            ...
            if (systemSample.networks.length > 0 && systemSample.networks[0]) {
                last_recv_data = systemSample.networks[0].received_data;
            }
        } else {
            if (systemSample.networks.length > 0 && systemSample.networks[0]) {
                const current_recv_data = systemSample.networks[0].received_data;
            ...
            if (systemSample.gpus && systemSample.gpus.length > 0) {
                for (const gpuInfo of systemSample.gpus) {
...
<template>
    <div>
        ...
        <div class="bg-gray-100 rounded-lg pl-5 pt-5 pr-5">
            <div
                class="text-xl font-semibold pl-1 text-slate-700 flex items-center justify-between"
            >
                基础信息({{
                    onlineDevice
                        ? "当前设备: " +
                          onlineDevice.device_name +
                          ", ID: " +
                          onlineDevice.device_id
                        : "UnKnown"
                }})
                <div class="w-2" />
                    ...
                    <div class="flex justify-center items-center w-full h-full bg-white rounded-xl">
                        <div class="">
                            ...
                            <div class="flex justify-start">
                                <a-typography-text class="!w-20 font-semibold"
                                    >OS版本</a-typography-text
                                >
                                <div class="w-5"></div>
                                <a-typography-text class="w-full">
                                    {{
                                        sysInfo?.os.sys_os_long_version +
                                        " - " +
                                        sysInfo?.os.sys_host_name
                                    }}</a-typography-text
                                >
                            </div>
                                ...
                                <div class="w-full">
                                    ...
                                    >
                                        <a-progress
                                            ...
                                            "
                                            :stroke-color="
                                                ((disk.total_gb - disk.available_gb) * 100) /
                                                    disk.total_gb >
                                                50
                                                    ? diskWarnColor
                                                    : diskNormalColor
                                        ...
                                        />
                                        <div class="text-sm">
                                            {{ disk.mount_on.substring(0, 1) }}({{
                                                disk.disk_type
                                            }}) {{ disk.total_gb - disk.available_gb }}GB/{{
                                                disk.total_gb
                                            }}
                                            GB
                                        </div>
