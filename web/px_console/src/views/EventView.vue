...
const selectedEvent = ref<ConsoleEvent | null>(null)
...
async function queryEvents(type: EventType) {
  ...
  try {
    ...
    const items = Array.isArray(response.data.data) ? (response.data.data as ConsoleEvent[]) : []
...
function usageOf(event: ConsoleEvent) {
...
function resourceOf(event: ConsoleEvent) {
...
function occurrenceCount(event: ConsoleEvent) {
  return Math.max(1, Number(event.occurrence_count || 0))
}

function firstReportedAt(event: ConsoleEvent) {
  return event.first_readable_timestamp || event.readable_timestamp || '-'
}

function openDetail(event: ConsoleEvent) {
  selectedEvent.value = event
  detailOpen.value = true
}

async function handleCopyEvent(event: ConsoleEvent) {
