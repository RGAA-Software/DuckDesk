...
import {
    refreshSharedAuthorization,
    sharedAuthorization as authorization,
} from "@/model/auth_state.ts";
import { changeAdminPassword, logoutAdmin } from "@/model/admin_session_api.ts";
...
async function handleChangePassword() {
    ...
    try {
        await changeAdminPassword(oldPassword.value, newPassword.value);
        notification.success({ message: "密码修改成功，请重新登录" });
        await logoutAdmin();
        wsStore.close();
        await router.replace("/");
    } catch {
        notification.error({ message: "密码修改失败，请检查当前密码和新密码格式" });
    }
}
...
async function queryAccessInfo() {
    const resp = await axiosHttp.get("/api/v1/console/control/gen/access/info");
    if (resp.status !== 200) {
        ...
        <a-tabs class="custom-tabs pl-3">
            ...
            <a-tab-pane key="client-access">
                ...
                <div class="flex items-center">
                    ...
                    <div class="w-[500px]">
                        <div
                            class="p-3 rounded-lg bg-gray-100 font-mono text-sm break-all whitespace-pre-wrap"
                        >
                            {{ accessInfo }}
                        </div>
                    </div>
                    ...
                    <a-button type="primary" class="w-20" @click="handleCopyAccessInfo"
                        >复制</a-button
                    >
                </div>
            </a-tab-pane>
            ...
            <a-tab-pane key="auth-info">
                ...
                <div>
                    ...
                    <div class="flex items-center h-8">
                        ...
                        <div class="text-medium text-slate-600 font-semibold">
                            {{
                                authorization?.created_timestamp_ms
                    ...
                    <div class="flex items-center h-8">
                        ...
                        <div v-if="authorization" class="text-medium text-amber-600 font-semibold">
                            {{ authorization?.days }}天, 已使用{{
                                authorization?.used_time_ms
                                    ? formatDurationHMS(authorization?.used_time_ms)
                                    : 0
                            }}
                        </div>
                    </div>
            ...
            <a-tab-pane key="change-password">
                ...
                <div>
                    ...
                    <div class="h-2" />
                    <div class="flex items-center h-8">
                        <div class="w-25 text-slate-500">重复新密码</div>
                        <div class="w-50">
                            <a-input-password
                    ...
                    <div class="h-5" />
                    <div class="flex items-center h-8">
                        <a-button class="w-25" type="primary" @click="handleChangePassword"
                            >确定</a-button
                        >
                        <div class="w-50"></div>
                    </div>
            ...
            <a-tab-pane key="general-settings">
                ...
                <div>
                    ...
                    <div class="flex items-center h-8">
                        ...
                        <div class="w-50">
                            <a-button type="danger" class="w-20" @click="handleLogout"
                                >退出</a-button
                            >
                        </div>
                    </div>
