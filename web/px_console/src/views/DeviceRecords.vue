...
// topology: '' = detecting, 'direct' = topology 1 (panel LAN direct), 'console' = topology 2 (via console)
const topology = ref<"" | "direct" | "console">("");
...
let directBase = "";
let directIp = "";
// names present in the panel-side list; console-only copies (rotated out on the
// device) are not in it and must be played/downloaded from the console url
...
// codec == '' means the console-side copy without probe info; default encoder is h264
...
const stateText = (file: RecordFileItem): string => {
    switch (file.state) {
        ...
        case "ready":
            return file.keep ? "已存 Console" : "已回传";
        case "error":
            return "回传失败";
        default:
            return "设备本地";
...
const showApiError = (prefix: string, error: unknown) => {
    if (error instanceof RecordApiError) {
        if (error.code === RECORD_ERR_DEVICE_OFFLINE) {
            notification.error({ message: prefix + "：设备离线，无法查看录像" });
            return;
        }
        if (error.code === RECORD_ERR_TIMEOUT) {
            notification.error({ message: prefix + "：设备响应超时" });
            return;
        }
        notification.error({ message: prefix + "：" + error.message });
        return;
    }
...
const loadErrorText = (error: unknown): string => {
    if (error instanceof RecordApiError) {
        if (error.code === RECORD_ERR_DEVICE_OFFLINE) return "设备离线，无法查看录像";
        if (error.code === RECORD_ERR_TIMEOUT) return "设备响应超时";
...
const loadDirectList = async () => {
    ...
    const list: RecordFileItem[] = (data.files || []).map(
        (panelFile: {
            ...
            name: panelFile.name,
            size: panelFile.size,
            mtime: panelFile.mtime,
            monitor: panelFile.monitor || "",
            codec: panelFile.codec || "",
            state: "none",
            keep: false,
            progress: 0,
            total: 0,
    ...
    files.value = list;
    panelNames = new Set(list.map(file => file.name));
    // best effort: merge the console-side download state (keep/fetching progress)
    await mergeConsoleState();
};

// merge console-side c_records state into the current list (by filename)
const mergeConsoleState = async () => {
    try {
        const resp = await getRecordList(deviceId);
        const byName = new Map(files.value.map(file => [file.name, file]));
        for (const item of resp.files) {
            ...
            if (existing) {
                ...
                // console-only copy (the device file may have been rotated out)
                files.value.push(item);
            }
        }
    } catch (error) {
        // tunnel down etc: keep the panel list as-is
        console.warn("mergeConsoleState failed", error);
    }
};

// topology 2: console merged list (design 6.3)
...
const refreshList = async () => {
    ...
    } else if (topology.value === "console") {
        await loadConsoleList();
    }
};

const hasInFlight = (): boolean => files.value.some(file => file.state === "fetching");
...
// poll the console list while any file is fetching, to show "回传中 x%" (design 6.1)
const ensurePolling = () => {
    ...
    pollTimer = setInterval(async () => {
        ...
        try {
            ...
            console.warn("poll record list failed", error);
        }
    }, 1500);
};
...
const init = async () => {
    ...
    try {
        ...
        // topology selection (design 5.3): try panel lan ips first, fall back to console
        if (access.online && access.panel_lan_ips.length > 0) {
            try {
                ...
                console.warn("record ticket failed, fall back to console topology", error);
            }
        }
        if (topology.value === "direct") {
            await loadDirectList();
        } else {
            topology.value = "console";
            await loadConsoleList();
        }
        ensurePolling();
    } catch (error) {
        console.error("init records page failed", error);
        loadError.value = loadErrorText(error);
    } finally {
        loading.value = false;
    }
};
...
const handlePlay = async (file: RecordFileItem) => {
    // console-only copy (rotated out on the device): play from the console url directly
    ...
    if (topology.value === "direct") {
        try {
            ...
            showApiError("获取播放地址失败", error);
        }
        return;
    }
    await ensureConsoleReady(file, url => openPlayer(url, file.name));
};
...
const handleDownloadLocal = async (file: RecordFileItem) => {
    // console-only copy (rotated out on the device): download from the console url
    if (topology.value === "direct" && !panelNames.has(file.name)) {
        if (file.state === "ready" && file.url) {
    ...
    if (topology.value === "direct") {
        try {
            ...
            showApiError("获取下载地址失败", error);
        }
        return;
    }
    await ensureConsoleReady(file, url => downloadLocal(url, file.name));
};
...
const updateFetchProgress = () => {
    ...
    const current = files.value.find(file => file.name === fetchDialogTarget!.name);
    if (!current) return;
...
// console topology: trigger /fetch and wait until the file is ready on console,
// showing the "回传中 x%" progress so the first-play delay is explicit (design 6.1)
const ensureConsoleReady = async (file: RecordFileItem, onReady: (url: string) => void) => {
    if (file.state === "ready" && file.url) {
...
const handleFetchDialogCancel = () => {
    // cancel only closes the dialog; the console-side fetch continues
    fetchDialogVisible.value = false;
    fetchDialogTarget = null;
    fetchDialogOnReady = null;
};

// ---------------- download to console / delete ----------------

const downloadingNames = ref<Set<string>>(new Set());

const handleDownloadConsole = async (file: RecordFileItem) => {
    downloadingNames.value.add(file.name);
    try {
        const resp = await downloadRecordToConsole(deviceId, file.name);
        file.id = resp.id;
        file.keep = true;
        if (resp.state === "ready") {
            file.state = "ready";
            file.url = resp.url;
            notification.success({ message: "已保存到 Console" });
        } else {
            file.state = "fetching";
            notification.info({ message: "正在下载到 Console，可在列表查看进度" });
            ensurePolling();
        }
    } catch (error) {
        showApiError("下载到 Console 失败", error);
    } finally {
...
const handleDelete = async (file: RecordFileItem) => {
    ...
    try {
        ...
        notification.success({ message: "已删除 Console 上的副本" });
        await handleRefresh();
    } catch (error) {
        showApiError("删除失败", error);
    }
};
        ...
        <a-card class="w-full" :bordered="false">
            <template #title>
                <div class="flex items-center">
                    ...
                    <div class="w-3" />
                    <a-tag v-if="topology === 'direct'" color="success"
                        >直连设备 {{ directIp }}</a-tag
                    >
                    <a-tag v-else-if="topology === 'console'" color="warning"
                        >经 Console 回传</a-tag
                    >
                </div>
            </template>
            ...
            <a-table :data-source="files" :loading="loading" style="width: 100%" row-key="name">
                ...
                <a-table-column title="编码" :min-width="90">
                    <template #default="{ record }">
                        ...
                        <a-tag v-if="!isPlayable(record)" color="warning" class="!ml-1"
                            >仅可下载</a-tag
                        >
                    </template>
                </a-table-column>
                ...
                <a-table-column title="操作" :min-width="260">
                    <template #default="{ record }">
                        ...
                        <a-button size="small" @click="handleDownloadLocal(record)"
                            >下载到本机</a-button
                        >

                        <template v-if="record.keep && record.state === 'ready'">
                            <a-tag color="success">已存 Console</a-tag>
                            <a-popconfirm
                                title="确定删除 Console 上的副本吗？设备本地录像不受影响。"
                                ok-text="删除"
                                cancel-text="取消"
                        ...
                        <a-button
                            ...
                            @click="handleDownloadConsole(record)"
                        >
                            下载到 Console
                        </a-button>
                    </template>
        ...
        >
            <div class="flex flex-col items-start">
                ...
                <span class="text-xs text-slate-500">
                    首次播放需要先把整段录像回传到 Console，大文件可能需要几分钟，请耐心等待。
                </span>
            </div>
        </a-modal>
    </div>
