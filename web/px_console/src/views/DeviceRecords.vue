...
const mergeConsoleState = async () => {
  try {
    ...
    console.warn('mergeConsoleState failed', e)
  }
}
  ...
  try {
    ...
    if (access.online && access.panel_lan_ips.length > 0) {
      try {
        ...
        console.warn('record ticket failed, fall back to console topology', e)
      }
    }
    if (topology.value === 'direct') {
      await loadDirectList()
    ...
    <a-card class="w-full" :bordered="false">
      <template #title>
        <div class="flex items-center">
          ...
          <a-tag v-else-if="topology === 'console'" color="warning">经 Console 回传</a-tag>
        </div>
      </template>
      <template #extra>
