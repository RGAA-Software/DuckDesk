...
function parseAuthInfo(json: string): ServiceAuthInfo | null {
    ...
    try {
        ...
        console.error("parse auth_info_json failed", parseError);
        return null;
    }
}
...
<template>
    <div>
                    ...
                    <a-table-column title="操作" min-width="70">
                        <template #default="{ record }">
                            <a-button
                                type="link"
                                size="small"
                                @click="showRemoteSessionDetails(record.device_id)"
                                >详情</a-button
                            >
                        </template>
                    </a-table-column>
        ...
        <a-drawer
            v-model:open="remoteSessionDrawerOpen"
            :title="`远控会话：${selectedDeviceId}`"
            width="780"
        >
            <a-spin :spinning="remoteSessionLoading">
                <h4>当前与历史会话</h4>
                <a-table
                    :data-source="remoteSessions"
                    row-key="logical_session_id"
                    :pagination="false"
                    size="small"
                >
                    ...
                    <a-table-column title="入口">
                        <template #default="{ record }">
                            <a-tag
                                :color="
                                    sessionEntry(record.subject_id) === '直接连接'
                                        ? 'orange'
                                        : 'blue'
                                "
                            >
                    ...
                    <a-table-column title="状态">
                        <template #default="{ record }"
                            ><a-tag :color="record.active ? 'success' : 'default'">{{
                                record.active ? "进行中" : "已结束"
                            }}</a-tag></template
                        >
                    </a-table-column>
                    <a-table-column title="传输">
                        <template #default="{ record }">{{
                            displayList(record.transports)
                        }}</template>
                    </a-table-column>
                    <a-table-column title="接管自" ellipsis>
                        <template #default="{ record }">{{
                            record.takeover_previous_session_id || "-"
                        }}</template>
                    </a-table-column>
                    <a-table-column title="更新时间">
                        <template #default="{ record }">{{
                            formatTs(record.updated_timestamp)
                        }}</template>
                    </a-table-column>
                </a-table>
                ...
                <a-table
                    :data-source="remoteSessionEvents"
                    row-key="event_id"
                    :pagination="false"
                    size="small"
                >
                    ...
                    <a-table-column title="角色变化">
                        <template #default="{ record }"
                            >{{ record.previous_role || "-" }} → {{ record.role || "-" }}</template
                        >
                    </a-table-column>
