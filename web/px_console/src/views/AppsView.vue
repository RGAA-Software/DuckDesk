...
const accessForm = ref({
    app_id: "",
    app_name: "",
    access_mode: "public" as "public" | "acl",
    group_ids: [] as string[],
});
...
const nodeListApp = computed<ViewRow | undefined>(() =>
    rows.value.find(row => row.app_id === nodeListAppId.value),
);
...
const onlineIds = computed(() => new Set(services.value.map(service => service.device_id)));
const rdpNodeForm = computed(
    () => rowsRaw.value.find(row => row.app_id === nodeForm.value.app_id)?.app_type === "rdp",
);
const rdpReadyIds = computed(
    () =>
        new Set(
            services.value
                .filter(service => service.rdp_available === true)
                .map(service => service.device_id),
        ),
);
...
const deviceOptions = computed<DeviceOption[]>(() => {
    ...
    return [...options.values()].sort(
        (leftOption, rightOption) =>
            Number(rightOption.online) - Number(leftOption.online) ||
            leftOption.label.localeCompare(rightOption.label),
    );
});

function seqOf(id: string): number {
    const sequenceMatch = id.match(/^(?:inst|req)-(\d+)-/);
    return sequenceMatch ? Number(sequenceMatch[1]) : 0;
}
    ...
    return instances.value
        ...
        .sort(
            (leftInstance, rightInstance) =>
                seqOf(rightInstance.instance_id) - seqOf(leftInstance.instance_id),
        )[0];
}
...
const rows = computed<ViewRow[]>(() =>
    rowsRaw.value.map(row => ({
        ...
        nodes: (row.nodes || []).map(node => ({
            ...node,
            instance: activeInstanceOf(row.app_id, node),
            online: onlineIds.value.has(node.device_id),
        })),
    })),
);
...
function deviceIp(deviceId: string): string {
    return (
        devices.value.find(device => device.device_id === deviceId)?.device_ip_addr || "127.0.0.1"
    );
}
...
function runningCount(row: ViewRow): number {
    return row.nodes.filter(node => node.instance?.state === "running").length;
}

function onlineNodeCount(row: ViewRow): number {
    return row.nodes.filter(node => node.online).length;
}

function accessOptionOf(row: ViewRow): AppOption {
    return (
        appOptions.value.find(app => app.app_id === row.app_id) || {
            app_id: row.app_id,
            name: row.name,
...
async function saveAccess() {
    ...
    try {
        ...
        appOptions.value = appOptions.value.map(app =>
            app.app_id === updated.app_id ? updated : app,
        );
        rowsRaw.value = rowsRaw.value.map(row =>
            row.app_id === updated.app_id
...
async function openNodeCreate(row: ViewRow) {
    ...
    const deviceId =
        (row.app_type === "rdp"
            ? services.value.find(service => service.rdp_available === true)?.device_id
            : "") ||
        services.value[0]?.device_id ||
        deviceOptions.value[0]?.device_id ||
        "";
...
async function submitSave() {
    const appForm = form.value;
    if (!appForm.name.trim()) {
        message.warning("请填写应用名称");
        return;
    }
    if (appForm.app_type === "game-hook" && !appForm.game_path.trim()) {
        message.warning("请填写程序路径");
        return;
    }
    if (appForm.app_type === "webview" && !appForm.entry_url.trim()) {
    ...
    try {
        const result = await saveApp({
            app_id: editing.value ? appForm.app_id : undefined,
            version: editing.value ? appForm.version : undefined,
            name: appForm.name.trim(),
            app_type: appForm.app_type,
            entry_url: appForm.app_type === "webview" ? appForm.entry_url.trim() : undefined,
            game_path: appForm.app_type === "game-hook" ? appForm.game_path.trim() : "",
            default_game_args:
                appForm.app_type === "game-hook"
                    ? appForm.default_game_args || undefined
                    : undefined,
            encoder_bitrate: appForm.encoder_bitrate,
            encoder_format: appForm.encoder_format,
            allow_observer: appForm.allow_observer,
            allow_takeover: appForm.allow_takeover,
        });
        if (!result.ok) {
...
async function submitNodeSave() {
    const submittedNode = nodeForm.value;
    if (!submittedNode.device_id) {
        message.warning("请选择机器");
        return;
    }
    if (
        !Number.isInteger(submittedNode.listen_port) ||
        submittedNode.listen_port < 0 ||
        submittedNode.listen_port > 65535
    ) {
    ...
    try {
        const result = await saveNode({
            node_id: nodeEditing.value ? submittedNode.node_id : undefined,
            app_id: submittedNode.app_id,
            name: submittedNode.name.trim() || undefined,
            device_id: submittedNode.device_id,
            listen_port: submittedNode.listen_port,
        });
        if (!result.ok) {
...
async function handleDelete(row: ViewRow) {
    const busy = row.nodes.some(node => {
        const instanceState = stateOf(node);
        return (
...
async function handleStart(row: ViewRow) {
    ...
    const nodeName = row.nodes.find(node => node.node_id === result.data.node_id)?.name;
...
<template>
    <div class="p-4">
        ...
        <a-table :data-source="rows" size="small" row-key="app_id" :loading="loading">
            ...
            <a-table-column title="授权用户组" min-width="190">
                <template #default="{ record }">
                    <span v-if="accessModeOf(record) === 'public'" class="text-gray-500"
                        >所有访客和用户</span
                    >
                    <a-space v-else-if="authorizedGroupsOf(record).length" wrap :size="[4, 4]">
                        <a-tag v-for="group in authorizedGroupsOf(record)" :key="group.gid">{{
                            group.name
                        }}</a-tag>
                    </a-space>
            ...
            <a-table-column title="机器状态" width="120">
                <template #default="{ record }">
                    <a-tag
                        v-if="record.nodes.length > 0"
                        :color="onlineNodeCount(record) > 0 ? 'success' : 'default'"
                    >
        ...
        <a-modal
            ...
            :ok-button-props="{
                disabled: accessForm.access_mode === 'acl' && accessForm.group_ids.length === 0,
            }"
            ok-text="保存"
            cancel-text="取消"
            width="620px"
        ...
        >
            <div class="flex justify-between items-center mb-2">
                <span class="text-sm text-gray-500">
                    共
                    {{
                        nodeListApp?.nodes.length || 0
                    }}
                    个节点；节点启动只影响本行，应用启动会自动挑选空闲节点
                </span>
                <a-button
                    v-if="nodeListApp"
                    type="primary"
                    size="small"
                    @click="openNodeCreate(nodeListApp)"
                >
                    新建节点
                </a-button>
            </div>
            <a-table :data-source="pagedNodes" size="small" row-key="node_id">
                ...
                <a-table-column title="错误" min-width="200" ellipsis>
                    <template #default="{ record: node }">
                        <span v-if="node.instance?.error" class="err-text">{{
                            node.instance.error
                        }}</span>
                    </template>
                </a-table-column>
                <a-table-column title="操作" width="360" fixed="right">
                    <template #default="{ record: node }">
                        <a-button
                            ...
                            :disabled="
                                !node.online ||
                                stateOf(node) === 'running' ||
                                stateOf(node) === 'starting'
                            "
                        ...
                        <a-button
                            ...
                            :disabled="
                                !node.instance ||
                                stateOf(node) === 'stopped' ||
                                stateOf(node) === 'stopping'
                            "
        ...
        >
            <a-form :model="form" :label-col="{ style: { width: '100px' } }">
                ...
                <a-form-item label="应用类型" required>
                    <a-radio-group
                        v-model:value="form.app_type"
                        :disabled="
                            editing &&
                            rows
                                .find(row => row.app_id === form.app_id)
                                ?.nodes.some(node => !!node.instance)
                        "
                    >
                ...
                <a-form-item v-else-if="form.app_type === 'webview'" label="入口 URL" required>
                    ...
                    />
                    <div class="text-xs text-gray-400 mt-1">
                        允许 HTTPS；HTTP 仅允许 localhost、私有 IP 或 .local 内网页面。URL
                        不会写入进程日志。
                    </div>
                </a-form-item>
                ...
                <a-form-item v-if="form.app_type !== 'rdp'" label="远控会话">
                    <a-space direction="vertical">
                        <a-switch
                            v-model:checked="form.allow_observer"
                            checked-children="允许观看"
                            un-checked-children="禁止观看"
                        />
                        <a-switch
                            v-model:checked="form.allow_takeover"
                            checked-children="允许接管"
                            un-checked-children="禁止接管"
                        />
                    </a-space>
                    <div class="text-xs text-gray-400 mt-1">
                        观看者只接收音视频；接管会立即替换当前操作员。
                    </div>
                </a-form-item>
            </a-form>
        ...
        >
            <a-form :model="nodeForm" :label-col="{ style: { width: '100px' } }">
                ...
                <a-form-item label="机器" required>
                    ...
                    >
                        ...
                        >
                            ...
                            {{
                                rdpNodeForm && device.online
                                    ? rdpReadyIds.has(device.device_id)
                                        ? " · RDP 已就绪"
                                        : " · RDP 未部署或未建立信任"
                                    : ""
                            }}
                ...
                <a-form-item label="端口" required>
                    <a-input-number
                        v-model:value="nodeForm.listen_port"
                        :min="0"
                        :max="65535"
                        :precision="0"
                    />
                    <span class="ml-2 text-xs text-gray-400"
                        >0 表示由目标机器按本机端口池自动分配；指定端口需属于该机器的端口池</span
                    >
                </a-form-item>
                <a-alert
                    v-if="rdpNodeForm"
            ...
            <div class="mt-4 text-right">
                ...
                <a-button type="primary" :loading="nodeSaving" @click="submitNodeSave"
                    >保存</a-button
                >
            </div>
        </a-modal>
