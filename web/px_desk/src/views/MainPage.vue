...
const issueRequestId = submissionIdentity();
...
const issueRules = computed<FormRules<Issue>>(() => ({
    ...
    version: [{ required: true, message: t("issue.required"), trigger: "blur" }],
    os: [{ required: true, message: t("issue.required"), trigger: "blur" }],
}));
...
async function confirmIssue() {
    if (!issueFormRef.value || issueSubmitting.value) return;
    ...
    try {
        ...
        await axiosHttp.post("/api/desk/issues", {
            ...body,
            request_id: issueRequestId.next(body),
        });
        issueRequestId.reset();
        ElNotification({
...
<template>
    ...
    >
        <div class="section-container flex h-14 items-center justify-between">
            ...
            <button class="flex items-center gap-2.5 cursor-pointer" @click="goPage('/main')">
                ...
                <span class="font-tech text-lg font-bold tracking-[0.22em] text-cyber-text"
                    >GODESK</span
                >
            </button>
            ...
            <nav class="hidden md:flex items-center gap-2">
                ...
                <el-dropdown trigger="hover" popper-class="product-dropdown" @command="goProduct">
                    <button
                        class="nav-item font-tech"
                        :class="isProductActive ? 'nav-item-active' : ''"
                    >
                        {{ t("nav.products") }} ▾
                    </button>
                    <template #dropdown>
            ...
            <div class="hidden md:flex items-center gap-5">
                <el-dropdown trigger="click" @command="handleTranslateClick">
                    <img
                        :src="transIcon"
                        alt="language"
                        class="h-5 w-5 cursor-pointer invert opacity-70 hover:opacity-100 transition-opacity"
                    />
                    <template #dropdown>
                        <el-dropdown-menu>
            ...
            <div class="flex items-center gap-5 px-4">
                <el-dropdown trigger="click" @command="handleTranslateClick">
                    <img
                        :src="transIcon"
                        alt="language"
                        class="h-5 w-5 cursor-pointer invert opacity-70"
                    />
                    <template #dropdown>
                        <el-dropdown-menu>
                            <el-dropdown-item command="zh">简体中文</el-dropdown-item>
        ...
        <div class="section-container grid gap-10 py-12 md:grid-cols-3">
            ...
            <div class="flex flex-col items-center md:items-start gap-3">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <div
                            class="font-tech text-lg font-bold tracking-[0.22em] text-cyber-text leading-5"
                        >
                            GODESK
                        </div>
                        <div class="font-tech text-[10px] tracking-[0.18em] text-cyber-brand">
                            {{ t("footer.slogan") }}
                        </div>
                    </div>
                </div>
                <div class="flex gap-4 text-sm">
                    <el-link
                        class="!text-cyber-muted hover:!text-cyber-text"
                        :underline="false"
                        @click="goTerms"
                    >
                        {{ t("footer.terms") }}
                ...
                <!-- 状态行 -->
                <div
                    class="mt-2 flex items-center gap-2 border border-cyber-line bg-cyber-bg2 px-3 py-1.5"
                >
                    <span class="cyber-dot"></span>
                    <span class="font-tech text-[10px] tracking-[0.14em] text-cyber-muted"
                        >SYS.STATUS: ONLINE</span
                    >
                </div>
            </div>
            ...
            <div class="flex flex-col items-center md:items-start gap-2">
                ...
                <el-link
                    class="h-6 !text-cyber-muted hover:!text-cyber-text"
                    :underline="false"
                    @click="goHelp"
                >
                    {{ t("footer.helpCenter") }}
            ...
            <div class="flex flex-col items-center md:items-start gap-2">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-tech text-sm text-cyber-muted"
                        >{{ t("consult.email") }}: godesk-sales@outlook.com</span
                    >
                </div>
                <div class="flex items-center gap-2">
                    <img :src="wechatIcon" alt="" class="h-4.5 w-4.5 footer-icon" />
                    <span class="font-tech text-sm text-cyber-muted"
                        >{{ t("consult.wechat") }}: AlmostDawn</span
                    >
                </div>
            </div>
        </div>
    ...
    <el-dialog v-model="issueVisible" align-center class="!max-w-[92vw] !w-140">
        <template #header>
            <span class="font-tech text-base tracking-wider text-cyber-text">{{
                t("issue.dialogTitle")
            }}</span>
        </template>
