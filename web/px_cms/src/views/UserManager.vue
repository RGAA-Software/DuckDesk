...
async function save() {
  ...
  try {
    if (editing.value) {
      ...
      Modal.info({ title: '初始密码（仅显示一次）', content: r.initial_password, okText: '已安全保存' })
    }
...
async function reset(user: UserAdminView) { const r = await resetAdminUserPassword(user); await copyText(r.initial_password); Modal.info({ title: '新密码已复制（仅显示一次）', content: r.initial_password }); await refresh() }
...
async function generateBatch() {
  ...
  try {
    ...
    message.success('用户已创建；初始密码仅保存在本次下载的 CSV 中')
    await refresh()
...
<template>
  <a-card title="用户管理">
    ...
    <a-table :data-source="users" row-key="uid" :loading="loading" :pagination="{ current: page, total, pageSize: 20 }" @change="onTableChange">
      ...
      <a-table-column title="状态"><template #default="{ record }"><a-tag :color="record.disabled ? 'red' : record.must_change_password ? 'orange' : 'green'">{{ record.disabled ? '已禁用' : record.must_change_password ? '待改密' : '正常' }}</a-tag></template></a-table-column>
      <a-table-column title="操作" width="460"><template #default="{ record }"><a-space wrap><a-button size="small" @click="edit(record)">编辑</a-button><a-button size="small" @click="toggle(record)">{{ record.disabled ? '启用' : '禁用' }}</a-button><a-button size="small" @click="reset(record)">重置密码</a-button><a-button size="small" @click="showSessions(record)">会话</a-button><a-button size="small" @click="revoke(record)">撤销会话</a-button><a-button size="small" danger @click="remove(record)">删除</a-button></a-space></template></a-table-column>
    </a-table>
  </a-card>
