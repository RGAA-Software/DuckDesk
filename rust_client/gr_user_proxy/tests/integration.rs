...
use gr_user_proxy::clipboard::content::ClipboardFileEntry;
use gr_user_proxy::clipboard::virtual_file::VirtualFileCoordinator;
...
async fn integration_virtual_file_apply_installs_session() {
    ...
    gr_user_proxy::render_client::handle_inbound_rp(
        &bytes,
        &engine.clipboard,
...
async fn integration_virtual_file_full_stream_via_coordinator() {
    let coordinator = VirtualFileCoordinator::new();
    coordinator.install_session(gr_user_proxy::clipboard::virtual_file::VirtualFileSession {
        route: StreamRoute {
    ...
    for (index, chunk) in [b"abcd", b"efgh"].iter().enumerate() {
        ...
        assert!(coordinator.on_resp_buffer(gr_user_proxy::clipboard::virtual_file::RespBufferData {
...
async fn integration_data_channel_resp_buffer_dispatch() {
    ...
    let resp = gr_user_proxy::clipboard::virtual_file::RespBufferData {
...
fn integration_proto_resp_buffer_matches_panel_fields() {
    ...
    let bytes = gr_user_proxy::proto::build_tc_resp_buffer(&resp, &route);
