...
pub(crate) fn run_authorized_update() -> Result<(), String> {
    ...
    let result = apply_authorized_update(&store, &data_root, record);
...
fn apply_authorized_update(
    ...
    reject_reparse_point(&record.artifact_path)?;
    verify_sha256(&record.artifact_path, &record.prepared_sha256)?;
    verify_authenticode(&record.artifact_path)?;
    ...
    let rollback_available = if let Some(expected_rollback_sha256) = &record.rollback_sha256 {
        reject_reparse_point(&current_rollback)?;
        verify_sha256(&current_rollback, expected_rollback_sha256)?;
        verify_authenticode(&current_rollback)?;
        std::fs::copy(&current_rollback, &task_rollback)
            .map_err(|_| "cannot snapshot the previous installer".to_string())?;
        reject_reparse_point(&task_rollback)?;
        verify_sha256(&task_rollback, expected_rollback_sha256)?;
        verify_authenticode(&task_rollback)?;
        true
    } else {
        false
    };

    let install_result = std::process::Command::new(&record.artifact_path)
        .arg("/S")
        .status()
        .map_err(|_| "cannot start the verified update installer".to_string())?;
    let installed = install_result.success()
        && installed_product_matches(&record, record.to_build_number)
        && wait_for_service_health(record.service_port, SERVICE_HEALTH_TIMEOUT);
    if installed {
    ...
    let rollback_succeeded = rollback_available
        && std::process::Command::new(&task_rollback)
            .arg("/S")
            .status()
            .is_ok_and(|status| status.success())
        && installed_product_matches(&record, record.from_build_number)
        && wait_for_service_health(record.service_port, SERVICE_HEALTH_TIMEOUT);
