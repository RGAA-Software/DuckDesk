...
impl RdpDeployment {
    pub fn load(directory: &Path) -> Result<Self, String> {
        ...
        file.take(8193)
            ...
            .map_err(|_| "RDP deployment read failed".to_string())?;
        if bytes.len() > 8192 {
            return Err("RDP deployment exceeds limit".into());
        }
        let deployment: Self =
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        {
            return Err(
                "RDP deployment version, local domain or certificate identity invalid".into(),
            );
        }
        Ok(())
    }
...
pub fn prepare_runtime(
    ...
    node_id: &str,
    device_id: &str,
) -> Result<StagedRdpBootstrap, String> {
    ...
    let proxy_port = listener
        .local_addr()
        .map_err(|_| "RDP loopback port unavailable".to_string())?
        .port();
...
mod tests {
    ...
    fn newline_injection_and_unpinned_dependencies_are_rejected() {
        ...
        account.password.push_str("\nFixedTarget=false");
        assert!(deployment()
            .configuration(&std::env::temp_dir(), &account, 13389)
            .is_err());
        let mut deployment = deployment();
        deployment.freerdp_revision = "latest".into();
    ...
    fn dropping_staged_launch_removes_only_its_own_temporary_files() {
        ...
        let preserved = [
            root.join("other.bootstrap"),
            root.join("other.proxy.ini"),
            root.join("workspace.identity.json"),
        ];
