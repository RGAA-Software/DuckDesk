...
mod platform {
    ...
    use windows::Win32::Security::{
        CreateWellKnownSid, LookupAccountNameW, LookupAccountSidW, WinBuiltinAdministratorsSid,
        WinBuiltinGuestsSid, WinBuiltinRemoteDesktopUsersSid, WinBuiltinUsersSid, PSID,
    ...
    impl Drop for LocalString {
        fn drop(&mut self) {
            if !self.0.is_null() {
                unsafe {
                    LocalFree(Some(HLOCAL(self.0 .0.cast())));
                }
            }
        }
    }
    ...
    fn failure(operation: &str, code: u32) -> String {
        format!("RDP account {operation} failed (Windows code {code})")
    }
    ...
    fn account_sid(name: &[u16]) -> Result<(Vec<u8>, String), String> {
        ...
        unsafe {
            ...
            let value = text
                .0
                .to_string()
                .map_err(|_| "RDP SID text invalid".to_string())?;
            Ok((sid, value))
        }
    }
    ...
    fn reject_administrator(name: &[u16]) -> Result<(), String> {
        ...
        if result != 0 {
            return Err(failure("inspect group membership", result));
        }
        if count > 0 && buffer.0.is_null() {
            return Err("RDP group response invalid".into());
        }
        for index in 0..count as usize {
            let group = unsafe { &*buffer.0.cast::<LOCALGROUP_USERS_INFO_0>().add(index) };
            let group_name = unsafe { group.lgrui0_name.to_string() }
                .map_err(|_| "RDP group name invalid".to_string())?;
            if group_name.eq_ignore_ascii_case(administrators.trim_end_matches('\0')) {
                return Err(
                    "RDP workspace account has administrator membership; access refused".into(),
                );
            }
    ...
    pub fn ensure(spec: &RdpAccountSpec) -> Result<RdpAccountIdentity, String> {
        ...
        let status =
            unsafe { NetUserGetInfo(PCWSTR::null(), PCWSTR(name.as_ptr()), 1, &mut existing.0) };
        let mut applied_version = spec.credential_version;
        if status == 2221 {
            // NERR_UserNotFound
            if spec.expected_sid.is_some() {
                return Err("RDP account missing; refusing to replace its persisted SID".into());
            }
            let info = USER_INFO_1 {
                usri1_name: PWSTR(name.as_mut_ptr()),
            ...
            if status != 0 {
                return Err(failure("create standard user", status));
            }
            ...
            applied_version = marker.strip_prefix(&pixels_prefix)
                ...
                .ok_or_else(|| {
                    "RDP account ownership/version mismatch; refusing to modify an existing account"
                        .to_string()
                })?;
            ...
            return Err(failure("inspect user", status));
        }
        let (mut sid, sid_text) = account_sid(&name)?;
        if spec
            .expected_sid
            .as_ref()
            .is_some_and(|expected| *expected != sid_text)
        {
        ...
        for kind in [WinBuiltinUsersSid, WinBuiltinRemoteDesktopUsersSid] {
            ...
            let member = LOCALGROUP_MEMBERS_INFO_0 {
                lgrmi0_sid: PSID(sid.as_mut_ptr().cast()),
            };
            let status = unsafe {
            ...
            if status != 0 && status != 1378 {
                return Err(failure("grant standard workspace group membership", status));
            }
        }
        let mut verified = NetBuffer(std::ptr::null_mut());
        let status =
            unsafe { NetUserGetInfo(PCWSTR::null(), PCWSTR(name.as_ptr()), 1, &mut verified.0) };
        if status != 0 || verified.0.is_null() {
            return Err(failure("verify standard user", status));
        }
        ...
        if applied_version != spec.credential_version {
            let info = USER_INFO_1003 {
                usri1003_password: PWSTR(password.as_mut_ptr()),
            };
            let status = unsafe {
                NetUserSetInfo(
                    PCWSTR::null(),
            ...
            if status != 0 {
                return Err(failure("apply credential version", status));
            }
            let info = USER_INFO_1007 {
                usri1007_comment: PWSTR(comment.as_mut_ptr()),
            };
            let status = unsafe {
                NetUserSetInfo(
                    PCWSTR::null(),
                    PCWSTR(name.as_ptr()),
                    1007,
                    (&info as *const USER_INFO_1007).cast(),
                    None,
                )
            };
            if status != 0 {
                return Err(failure("record credential version", status));
            }
        }
...
mod tests {
    ...
    fn validates_identity_and_version_without_touching_windows() {
        ...
        spec.credential_version = 0;
        assert!(spec.validate().is_err());
        spec.credential_version = 1;
        spec.expected_sid = Some("S-1-5-32-544".into());
        assert!(spec.validate().is_err());
    }
}
