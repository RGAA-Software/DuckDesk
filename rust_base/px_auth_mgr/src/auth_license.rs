use crate::authorization::{default_product_console, PRODUCT_CONSOLE};
use base64::{engine::general_purpose, Engine as _};
use ring::rand::SystemRandom;
use ring::signature::{self, Ed25519KeyPair, KeyPair, UnparsedPublicKey, ED25519};
use serde::{Deserialize, Serialize};

fn is_default_product_console(product: &str) -> bool {
    product == PRODUCT_CONSOLE
}

/// License metadata that is cryptographically signed by the auth server.
/// This is the only information needed by the Console to enforce authorization,
/// including credentials (username/password/app_secret) generated by the
/// auth server.
...
pub struct AuthLicense {
    ...
    pub username: String,
    pub password: String,
    /// Product this license applies to: "console" | "gopico".
    /// Omitted from JSON when "console" so pre-existing signed deploy strings still verify.
    #[serde(
        default = "default_product_console",
        skip_serializing_if = "is_default_product_console"
    )]
    pub product: String,
}
...
mod tests {
    ...
    fn sample_license() -> AuthLicense {
        AuthLicense {
            ...
            product: PRODUCT_CONSOLE.to_string(),
        }
    }

    #[test]
    fn legacy_deploy_without_product_defaults_to_console() {
        ...
        // Sign a Console license (product omitted from canonical JSON).
        ...
        assert_eq!(parsed.license.product, PRODUCT_CONSOLE);
        ...
        assert_eq!(legacy.product, PRODUCT_CONSOLE);
