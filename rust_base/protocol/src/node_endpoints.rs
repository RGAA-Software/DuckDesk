...
impl NodeEndpoints {
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        ...
        if !self.access_host.is_empty() {
            ...
            if self.access_host.len() > 253
                || self.access_host.trim() != self.access_host
...
mod tests {
    ...
    fn sample() -> NodeEndpoints {
        NodeEndpoints {
            schema_version: 1,
            access_host: "render.example.com".into(),
            desktop_port: 4601,
    ...
    fn report_round_trip_and_destinations() {
        for host in ["render.example.com", "10.0.0.90", "[2001:db8::90]", ""] {
            ...
            assert_eq!(
                NodeEndpoints::decode(report.encode_to_vec().as_slice()).unwrap(),
                report
            );
