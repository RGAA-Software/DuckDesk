...
namespace ParsecDisplay.Vdd
{
    internal static unsafe class Core
    {
        ...
        // Keep the controller capacity aligned with the eight negotiated
        // Render/RTC display slots. Displays remain strictly on-demand.
        ...
        public static bool RemoveDisplay(IntPtr vdd, int index)
        {
            ...
            // Removing a monitor forces Windows to commit a new display
            // topology. Hosts with several physical/virtual adapters can
            // legitimately need more than the upstream one-second window.
            if (IoControl(vdd, IoCtlCode.IOCTL_REMOVE, input, 5000))
            {
                Update(vdd);
                return true;
            }
