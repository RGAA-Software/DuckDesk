...
rem ============================================================================
rem build_px_console_server.bat
rem
rem Build and deploy px_console_server in one step:
rem   [1/3] Build the Vue frontend   web\px_console          (npm run build)
rem   [2/3] Build the Rust server    px_console_server       (cargo build --release)
rem   [3/3] Copy artifacts into      output\px_console\
rem           exe      -> output\px_console\px_console.exe
rem           frontend -> output\px_console\web\
...
rem license public key (Console 校验授权签名用; 从打包过的 px_auth_server 种入, 只补缺不覆盖)
