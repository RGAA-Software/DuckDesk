...
try {
    ...
    if ($Action -eq 'PrepareQueries') {
        ...
        foreach ($item in @(
            @{Service='console';Crate='px_console_store';Path='rust_server/px_console_server/storage';Count=292},
            ...
            Invoke-Checked 'cargo' @('check','--release','--locked','--manifest-path',$manifest,'-p',$item.Crate,'--target-dir',$targetDir) | Out-Null
    ...
    if ($Action -eq 'TestSuite') {
        ...
        if ($Suite -eq 'console-browser') {
            ...
            foreach ($case in @('console-browser/login-dashboard','console-browser/identity-create-user-group',
                ...
                'console-browser/scheduling-preview-rejections',
                'console-browser/server-telemetry-trend',
        ...
        if ($runtimeTests.ContainsKey($Suite)) {
            ...
            if ($Suite -eq 'unit') {
                ...
                $testTarget = if ($Suite -eq 'relay-nodes') { 'relay_nodes' } else { $Suite }
                $suiteArgs += @('--test',$testTarget)
            }
        }
        $suiteArgs = @($suiteArgs[0], '--release') + $suiteArgs[1..($suiteArgs.Count - 1)]
    ...
    if ($expectedQueries.Count -ne 292 -or $actualQueries.Count -ne $expectedQueries.Count) { throw 'Missing or extra SQLx query metadata' }
    ...
    Add-Step 'QUERY: 292 Console SQLx queries compiled against fresh PG; offline metadata matches'
    ...
    Add-Step 'SESSIONS: explicit targets, original owner, license service/quota gates, descriptor leases, RDP occupancy, frontend retirement and atomic events'
    ...
    Add-TestCases $deskIntegration 'native/desk-api' 8
    ...
    $tablesByService = @{console=@('users','login_sessions','user_groups','group_members','authorization_outbox','authorization_audit','devices','user_devices','group_device_grants','device_audit','applications','group_app_grants','application_events','guest_sessions','guest_blocks','guest_source_blocks','guest_events','control_runtime','control_runs','nodes','node_audit','relay_nodes','relay_node_audit','application_deployments','deployment_audit','instances','instance_relays','instance_commands','instance_events','instance_admin_actions','rdp_workspaces','workspace_secrets','workspace_audit','resource_sessions','resource_session_relays','resource_session_events','resource_session_retirements'); auth=@('authors','author_sessions','customers','licenses','license_issuances','license_requests','license_audit'); desk=@('pg_fixture','feedback','versions','admin_sessions')}
