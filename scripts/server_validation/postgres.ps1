...
try {
    ...
    if ($Action -eq 'TestSuite') {
        ...
        $suiteCounts = @{unit=19;identity=12;control=8;devices=8;applications=8;guests=9;nodes=9;deployments=6;instances=14;commands=16;workspaces=6;database=2;sessions=10;transfers=8;recordings=6;preferences=7;files=8;backup=61;'backup-pg'=1;cache=17;activity=8;updates=7;desk=7;catalog=4;lease=6;postgres=14;accounts=9}
    ...
    if ($expectedQueries.Count -ne 268 -or $actualQueries.Count -ne $expectedQueries.Count) { throw 'Missing or extra SQLx query metadata' }
    ...
    Add-Step 'QUERY: 268 Console SQLx queries compiled against fresh PG; offline metadata matches'
    ...
    Add-TestCases $instanceIntegration 'native/instances' 14
    ...
    if ($consoleWebUnit -notmatch 'Tests\s+44 passed') { throw 'Console frontend contract tests missing' }
    Add-Step 'CONSOLE-WEB: 44 bearer identity, managed directory/activity/telemetry/GPU scheduling, authorized recording cache management/downloads, localization, descriptor secrecy and production bundle tests'
