// Browser-level smoke test for the Console HTTP-FLV live viewer.  It signs in
...
const pauseDurationMs = Number(process.env.CONSOLE_PLAYBACK_PAUSE_DURATION_MS || process.env.CMS_PLAYBACK_PAUSE_DURATION_MS || 0)
const summaryOnly = (process.env.CONSOLE_PLAYBACK_SUMMARY_ONLY || process.env.CMS_PLAYBACK_SUMMARY_ONLY) === '1'
process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0'

const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', [
  '--headless=new', '--ignore-certificate-errors', '--disable-gpu',
  `--remote-debugging-port=${cdpPort}`,
  `--user-data-dir=${path.join(os.tmpdir(), `console-live-viewer-${Date.now()}`)}`,
...
try {
  ...
  if (!username || !password) throw new Error('local Console credentials unavailable')
  ...
  if (login.code !== 200 || !appkey) throw new Error('Console login failed')
  ...
  await evaluate(`(() => {
    window.__consolePlaybackEvents = []
    const video = document.querySelector('video')
    for (const name of ['playing', 'waiting', 'stalled', 'error', 'pause', 'canplay']) {
      video?.addEventListener(name, () => window.__consolePlaybackEvents.push({
        name,
  ...
  await pauseTask
  const events = await evaluate('window.__consolePlaybackEvents || []')
