...
:: exe
copy /Y "%SERVER_WORKSPACE%\target\release\px_auth.exe" "%OUTPUT_DIR%\px_auth.exe" >nul
if errorlevel 1 (
    echo ERROR: Failed to copy px_auth.exe.
    exit /b 1
)

:: config
copy /Y "%SERVER_SRC%\src\px_auth.toml" "%OUTPUT_DIR%\px_auth.toml" >nul
if errorlevel 1 (
    echo ERROR: Failed to copy px_auth.toml.
    exit /b 1
)
