...
function sendRestart(socket, requestedRevision) {
  socket.send(encodeMessage({
    type: 40,
    streamId: config.stream_id,
    rtcIceRestart: {
      connectionTicket: config.connection_ticket,
      clientNonce: config.client_nonce,
      instanceId: config.instance_id ?? '',
      iceConfigJson: config.ice_config_json,
    ...
    if (type === 0 && !restartSent) {
      ...
      setTimeout(() => {
        try {
          ...
          sendRestart(socket, revision)
          console.log(`PANEL_PROBE_RESTART_SENT revision=${revision}`)
          setTimeout(() => {
            try {
              if (socket.readyState !== 1) return
              sendRestart(socket, revision)
              sendRestart(socket, revision - 1)
