...
:: 3. Copy the auth/auth.info license file.
:: 4. Build the Vue CMS frontend.
:: 5. Build the Rust CMS server.
:: 6. Copy exe + web assets + config + certs + license into output\gr_cms_server\.
::
...
echo [1/6] Ensuring TLS certificate...
...
if exist "%CERT_DIR%\auth_license_public.key" (
    echo [2/6] License public key already exists, skipping copy.
    goto :license_key_done
)

if exist "%AUTH_SERVER_OUTPUT%\certs\auth_license_public.key" (
    echo [2/6] Copying license public key from packaged gr_auth_server...
    ...
    echo [2/6] WARNING: auth_license_public.key not found.
    echo        Expected at: %AUTH_SERVER_OUTPUT%\certs\auth_license_public.key
    echo        Run scripts\package_gr_auth_server.bat first, or obtain the
...
:: --- 3. Ensure auth directory exists (license uploaded via web UI) ---
echo [3/6] Creating auth directory...
if not exist "%AUTH_DIR%" mkdir "%AUTH_DIR%"
echo       Auth dir: %AUTH_DIR%
echo       NOTE: No placeholder license is shipped. Upload a signed license
echo             via the web UI after starting the server.
echo.

:: --- 4. Build CMS frontend ---
echo [4/6] Building CMS frontend...
cd /d "%WEB_SRC%"
...
echo [6/6] Copying artifacts to %OUTPUT_DIR%...
...
:: config
copy /Y "%SERVER_SRC%\src\gr_cms_server_settings.toml" "%OUTPUT_DIR%\gr_cms_server_settings.toml" >nul
if errorlevel 1 (
    echo ERROR: Failed to copy gr_cms_server_settings.toml.
    exit /b 1
)
...
echo   3. Edit %OUTPUT_DIR%\gr_cms_server_settings.toml
...
echo   5. Replace %AUTH_DIR%\auth.info with the license deploy string
echo      issued by gr_auth_server for this machine.
echo.
