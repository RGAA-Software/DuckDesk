...
Packages `gr_auth_server` (the license/auth server).
...
Run `output\gr_auth_server\gr_auth_server.exe` and open `https://localhost:30400`.

### `package_gr_cms_server.bat`

Packages `gr_cms_server` (the CMS/management server). Depends on MongoDB and
Redis. Reuses the Ed25519 license public key generated by
`package_gr_auth_server.bat` (copy it from `output\gr_auth_server\certs\`
or set the `GR_AUTH_LICENSE_PUBLIC_KEY` env var).

```text
output\gr_cms_server\
├── gr_cms_server.exe
...
Packages `gr_desk_server` (the desk/official server). Depends on MongoDB.

```text
output\gr_desk_server\
├── gr_desk_server.exe
├── certs\          # cert.pem, key.pem
└── static\         # built frontend assets
```

Run `output\gr_desk_server\gr_desk_server.exe` and open
