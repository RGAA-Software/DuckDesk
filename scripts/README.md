...
Run `output\px_auth_server\px_auth.exe` and open `https://localhost:30400`.
...
`package_px_auth_server.bat` (copy it from `output\px_auth_server\certs\`
...
Run `output\px_cms_server\px_cms.exe --running-mode=server` and open
...
Run `output\px_desk_server\px_desk.exe` and open
