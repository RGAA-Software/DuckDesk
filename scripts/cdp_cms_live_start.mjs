// CMS live start smoke test: use a real browser context so self-signed TLS
// behaves exactly like the CMS Web. It starts a named app node and reports the
// resulting scheduler/live status without exposing CMS credentials.
import { spawn } from 'node:child_process'
import os from 'node:os'
import path from 'node:path'

const base = process.env.CMS_URL || 'https://127.0.0.1:30500'
const appName = process.env.CMS_APP_NAME || '1122'
const stopFirst = process.env.CMS_STOP_FIRST === '1'
...
const chrome = spawn('C:/Program Files/Google/Chrome/Application/chrome.exe', [
  ...
  `--user-data-dir=${path.join(os.tmpdir(), `cms-live-start-${Date.now()}`)}`,
