// Compare sustained byte delivery from ZLMediaKit and the Console HTTPS relay.
...
const observeMs = Number(process.env.CONSOLE_STREAM_PROBE_MS || process.env.CMS_STREAM_PROBE_MS || 20000)

const auth = await fetch(`${consoleBase}/api/v1/auth/control/get/auth/status`).then((response) => response.json())
...
const login = await fetch(`${consoleBase}/api/v1/auth/control/verify/auth/account`, {
  method: 'POST',
...
if (login.code !== 200 || !login.data) throw new Error('Console login failed')

const status = await fetch(
  `${consoleBase}/api/v1/live/control/status?device_id=${encodeURIComponent(deviceId)}`
...
const urls = {
  ...
  console: new URL(status.data.play_url, consoleBase).toString(),
}
