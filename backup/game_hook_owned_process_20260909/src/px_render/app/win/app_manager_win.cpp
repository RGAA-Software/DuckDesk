...
namespace px
{
    ...
    void AppManagerWinImpl::ProbeGaveUpTargetGone() {
        ...
        if (pid > 0) {
            ...
            if (process) {
                DWORD exit_code{};
                alive = GetExitCodeProcess(process.get(), &exit_code) && exit_code == STILL_ACTIVE;
            }
            if (!alive) {
        ...
        if (!recover && !settings_->app_.IsSteamUrl() && !settings_->app_.game_path_.empty()) {
            // 普通 app：相同完整路径出现了新 pid，也恢复注入；steam 场景靠
            // 上面的"旧 pid 消失"覆盖，恢复后由正常注入流程重新发现新 pid
            const auto target_path = PathFromUTF8(settings_->app_.game_path_);
            for (const auto& process : ProcessHelper::GetProcessList(false)) {
                if (process->pid_ != pid && SameGameExecutable(target_path, PathFromUTF8(process->exe_full_path_))) {
                    LOGW("Replacement pid: {} for exe: {} (gave-up pid: {}), resume injection.", process->pid_, settings_->app_.game_path_, pid);
                    recover = true;
                    break;
                }
            }
    ...
    void AppManagerWinImpl::EnsureGameRunning() {
        ...
        if (settings_->app_.IsSteamUrl()) {
            ...
            // 外部手动重启（相同完整路径的新 pid）→ 收养，不匹配其它目录的同名程序。
            if (!settings_->app_.game_path_.empty()) {
                const auto target_path = PathFromUTF8(settings_->app_.game_path_);
                for (const auto& process : ProcessHelper::GetProcessList(false)) {
                    if (process->pid_ != target_pid_.load() && SameGameExecutable(target_path, PathFromUTF8(process->exe_full_path_))) {
                        LOGW("Game restarted externally, adopt replacement pid: {} (old: {}), will re-inject.", process->pid_, target_pid_.load());
                        target_pid_ = process->pid_;
                        injected_ = false;
                        inject_alive_fail_count_ = 0;
                        ResetInjectRetryState();
                        waiting_first_frame_ = true;
                        NotifyGameStatus(px::GameStatusChanged::kGameRestarting, "game restarted externally, re-hooking");
                        return;
                    }
                }
            }
    ...
    bool AppManagerWinImpl::InjectCaptureDllForNormalApp() {
        ...
        if (!view_path.empty()) {
            ...
            const auto target_path = PathFromUTF8(settings_->app_.game_path_);
            for (const auto& process : processes) {
                if (process->pid_ == target_pid_ && SameGameExecutable(target_path, PathFromUTF8(process->exe_full_path_))) {
                    target_process_info = process;
                    break;
                }
            }
            ...
            if (!target_process_info->Valid()) {
                LOGE("Can't find app to inject, pid: {}, search by full path: {}", target_pid_.load(), settings_->app_.game_path_);
                uint32_t pid_by_exe{};
                for (const auto& process : processes) {
                    if (process->Valid() && SameGameExecutable(target_path, PathFromUTF8(process->exe_full_path_))) {
                        pid_by_exe = process->pid_;
                        target_process_info = process;
                        LOGI("find target process path: {}, pid: {}", process->exe_full_path_, pid_by_exe);
                        break;
                    }
                }
                if (pid_by_exe == 0) {
