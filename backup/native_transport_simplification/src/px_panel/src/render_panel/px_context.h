...
class PxContext : public QObject, public std::enable_shared_from_this<PxContext> {
  public:
    ...
    std::shared_ptr<MessageListener> ObtainMessageListener(MessageExecutionLane lane = MessageExecutionLane::kControl);
    ...
    const std::string& GetDatabaseError() const {
        return db_error_;
    }
    ...
    std::shared_ptr<px_relay::RelayDeviceInfo> GetRelayServerSideDeviceInfo(const std::string& relay_host, int relay_port,
  ...
  private:
    ...
    std::atomic<std::shared_ptr<PxBlockingExecutor>> network_executor_{};
    std::atomic<std::shared_ptr<PxBlockingExecutor>> database_executor_{};
