...
bool IsLoopbackPeer(const std::string& address) {
    ...
    if (parsed.is_v6()) {
        ...
        if (ipv6.is_v4_mapped()) {
            return asio::ip::make_address_v4(asio::ip::v4_mapped, ipv6).is_loopback();
        }
    }
    return false;
}
...
void WsPanelServer::Start() {
        ...
        if (auto self = weak_self.lock(); self && !self->exiting_) {
            self->context_->PostTask([weak_self]() {
                if (auto self = weak_self.lock(); self && !self->exiting_ && self->panel_sessions_.Size() > 0) {
                    static_cast<void>(self->app_->GetDeviceManager()->UpdateUsedTime(5000));
                }
            });
        }
    });
    ...
    AddHttpGetRouter(kPathPing, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
    ...
    AddHttpGetRouter(kPathSimpleInfo, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
    ...
    // start game
    AddHttpPostRouter(kPathGameStart, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                          if (!self->exiting_) {
    ...
    // stop game
    AddHttpPostRouter(kPathGameStop, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                          if (!self->exiting_) {
    ...
    // running games
    AddHttpGetRouter(kPathRunningGames, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
    ...
    AddHttpGetRouter(kPathAllRunningProcesses, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
    ...
    AddHttpPostRouter(kPathKillProcess, MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                          if (!self->exiting_) {
    ...
    // res
    AddHttpGetRouter("/res/*", MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
    ...
    AddHttpGetRouter("/records", MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
    ...
    AddHttpGetRouter("/records/info", MakeWeakVoidCallback(weak_self, [](const auto& self, const auto&, auto& req, auto& rep) {
                         if (!self->exiting_) {
        ...
        path, websocket::listener<asio2::http_session>{}
                  .on("message",
                      [weak_self, path, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr, std::string_view data) {
                          ...
                          if (IsInternalWebsocketPath(path) && !IsLoopbackPeer(sess_ptr->remote_address())) {
                              LOGW("Rejecting non-loopback message on internal websocket path: {}, peer: {}", path, sess_ptr->remote_address());
                              sess_ptr->stop();
                              return;
                          }
                  ...
                  .on("open",
                      [weak_self, path, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr) {
                          ...
                          if (IsInternalWebsocketPath(path) && !IsLoopbackPeer(sess_ptr->remote_address())) {
                              LOGW("Rejecting non-loopback connection to internal websocket path: {}, peer: {}", path, sess_ptr->remote_address());
                              sess_ptr->stop();
                              return;
                          }
                          ...
                          if (path == kUrlPanel) {
                              ...
                              ws_sess->socket_fd_ = socket_fd;
                              ws_sess->session_ = sess_ptr;
                              ws_sess->stream_id_ = params.contains("instance_id") ? params["instance_id"] : std::format("legacy-{}", socket_fd);
...
void WsPanelServer::AddHttpGetRouter(const std::string& path,
    ...
    server_->bind<http::verb::get>(
        path, [path, callback = std::move(cbk)](http::web_request& req, http::web_response& rep) { callback(path, req, rep); }, aop_log{});
}

void WsPanelServer::AddHttpPostRouter(const std::string& path,
                                      std::function<void(const std::string& path, http::web_request& req, http::web_response& rep)>&& cbk) {
    server_->bind<http::verb::post>(
        path, [path, callback = std::move(cbk)](http::web_request& req, http::web_response& rep) { callback(path, req, rep); }, aop_log{});
}
...
bool WsPanelServer::PostPanelMessageToStream(const std::string& stream_id, const std::string& msg) {
    ...
    panel_sessions_.VisitAll([&](uint64_t, std::shared_ptr<WSSession>& sess) {
        if (!sess || sess->stream_id_ != stream_id || !sess->session_ || sess->session_type_ != pxcp::CpSessionType::kWindowsClient) {
            return;
        }
        sess->session_->async_send(msg);
...
bool WsPanelServer::ParsePanelMessage(uint64_t socket_fd, std::string_view msg) {
    ...
    if (proto_msg->type() == pxcp::CpMessageType::kCpHello) {
    ...
    } else if (proto_msg->type() == pxcp::CpMessageType::kCpTransportConnected && !proto_msg->stream_id().empty()) {
        LOGI("Client remote transport connected: {}", proto_msg->stream_id());
        context_->SendAppMessage(MsgClientTransportConnectedPanel{
            .stream_id_ = proto_msg->stream_id(),
        });
    } else if (proto_msg->type() == pxcp::CpMessageType::kCpTransportRejected && !proto_msg->stream_id().empty() &&
               proto_msg->has_transport_rejected()) {
        LOGW("Client remote transport rejected: stream={}, reason={}", proto_msg->stream_id(),
    ...
    } else if (proto_msg->type() == pxcp::CpMessageType::kCpRtcIceRestartRequest && !proto_msg->stream_id().empty()) {
        ...
        context_->PostDBTask([weak_self, proto_msg, socket_fd]() {
            ...
            const auto status = sub.status().empty() ? (sub.success() ? "succeeded" : "failed") : sub.status();
            const auto reason = sub.end_reason().empty() ? (sub.success() ? "completed" : "transfer_failed") : sub.end_reason();
            self->ft_record_op_->UpdateFileTransferRecord(sub.the_file_id(), sub.end_timestamp(), sub.success(), status, reason);
...
void WsPanelServer::ParseRendererMessage(uint64_t socket_fd, std::string_view msg) {
    ...
    if (proto_msg->type() == pxrp::kRpCaptureStatistics) {
        ...
        LOGI("[VoiceCall] px_panel received consent request, call={}, stream={}, request={}", PrivacyLogId(sub.call_id()), sub.stream_id(),
             sub.request_id());
        ...
        LOGI("[VoiceCall] px_panel received consent cancel, call={}, stream={}, request={}, reason={}", PrivacyLogId(sub.call_id()), sub.stream_id(),
             sub.request_id(), sub.reason());
...
void WsPanelServer::NotifyInsertVisitRecordToConsole(const std::shared_ptr<VisitRecord> record) {
    ...
    auto db = context_->GetDatabase();
    if (!db->EnqueueAuditOutbox("visit:" + record->connection_id_ + ":begin", kUrlVisitRecord, record->AsJson2(), TimeUtil::GetCurrentTimestamp())) {
...
void WsPanelServer::NotifyUpdateVisitRecordToConsole(const std::shared_ptr<VisitRecord> record) {
    ...
    auto db = context_->GetDatabase();
    if (!db->EnqueueAuditOutbox("visit:" + record->connection_id_ + ":end", kUrlUpdateVisitRecord, record->AsUpdateJson(),
...
void WsPanelServer::NotifyInsertFileTransferRecordToConsole(const std::shared_ptr<FileTransferRecord> record) {
    ...
    auto db = context_->GetDatabase();
    if (!db->EnqueueAuditOutbox("file:" + record->the_file_id_ + ":begin", FileTransferRecord::kUrlInsertFileTransferRecord, record->AsJson2(),
                                TimeUtil::GetCurrentTimestamp())) {
...
void WsPanelServer::NotifyUpdateFileTransferRecordToConsole(const std::shared_ptr<FileTransferRecord> record) {
    ...
    auto db = context_->GetDatabase();
    if (!db->EnqueueAuditOutbox("file:" + record->the_file_id_ + ":end", FileTransferRecord::kUrlUpdateFileTransferRecord, record->AsUpdateJson(),
                                TimeUtil::GetCurrentTimestamp())) {
...
void WsPanelServer::FlushAuditOutbox() {
    ...
    context_->PostNetworkTask([weak_self, item = std::move(pending.value())]() {
        ...
        auto client = PxSettings::MakeConsoleHttpClient(serv_host, settings->GetConsoleServerPort(), item.endpoint_, 2000);
        ...
        self->context_->PostDBTask([weak_self, item, resp_status = resp.status, response_ok, completed_at]() {
            auto self = weak_self.lock();
            if (!self || self->exiting_) {
                return;
            }
...
void WsPanelServer::PanelSocketClosed(const std::shared_ptr<WSSession>& session) {
    ...
    context_->PostDelayTask(
        [weak_self, instance_id, disconnected_at]() {
            if (auto self = weak_self.lock(); self && !self->exiting_) {
...
void WsPanelServer::ClosePanelAuditRecordsIfOffline(const std::string& instance_id, int64_t disconnected_at) {
    ...
    context_->PostDBTask([weak_self, transfer_ids = std::move(transfer_ids), disconnected_at]() {
        ...
        for (const auto& file_id : transfer_ids) {
            ...
            self->ft_record_op_->UpdateFileTransferRecord(file_id, disconnected_at, false, "aborted", "client_disconnected", false);
...
void WsPanelServer::CloseRendererAuditRecordsIfOffline(const std::string& instance_id, int64_t disconnected_at) {
    ...
    auto weak_self = weak_from_this();
    context_->PostDBTask([weak_self, visit_ids = std::move(visit_ids), transfer_ids = std::move(transfer_ids), disconnected_at]() {
        auto self = weak_self.lock();
        if (!self || self->exiting_) {
            return;
        }
        for (const auto& conn_id : visit_ids) {
            ...
            self->visit_record_op_->UpdateVisitRecord(conn_id, disconnected_at, duration, "aborted", "renderer_disconnected", false);
        ...
        for (const auto& file_id : transfer_ids) {
            ...
            self->ft_record_op_->UpdateFileTransferRecord(file_id, disconnected_at, false, "aborted", "renderer_disconnected", false);
            if (auto updated = self->ft_record_op_->GetFileTransferRecordByFileId(file_id); updated.has_value()) {
                self->NotifyInsertFileTransferRecordToConsole(updated.value());
                self->NotifyUpdateFileTransferRecordToConsole(updated.value());
            }
