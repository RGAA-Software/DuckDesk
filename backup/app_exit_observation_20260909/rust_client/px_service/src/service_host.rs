        ...
        for record in candidates {
            ...
            if service_core::app_instance::is_rdp_launch(&record.launch) {
                ...
                match current {
                    ...
                    None if state != AppInstanceState::Failed => { let _ = self.app_registry.mark_stopped(&instance_id); }
                    None => { self.app_registry.remove(&instance_id); }
                    _ => {}
                }
                continue;
            }
            let current = processes.iter().find(|process| process.is_app_instance_render_process()
