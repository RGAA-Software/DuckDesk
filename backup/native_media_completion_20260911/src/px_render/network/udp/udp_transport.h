...
class UdpSession {
  public:
    std::string connection_id_;    // remote addr:port
...
class UdpTransport final : public RenderModule {
  public:
    ...
    RenderModuleKind Kind() const override {
        return RenderModuleKind::kNetwork;
    }

    bool Start(const RenderModuleConfiguration& configuration) override;
    bool Destroy() override;
    [[nodiscard]] static PxAwaitable<PxResult<void>> StopAsync(std::shared_ptr<UdpTransport> owner, std::chrono::steady_clock::time_point deadline);
    void UpdateUdpMediaAssociation(const UdpMediaAssociation& association);
  ...
  private:
    ...
    static constexpr uint64_t kRateControlBitsPerSec = 16000000ULL; // Bounded media pacing budget; includes the envelope and parity.
