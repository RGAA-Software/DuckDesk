...
namespace px
{
    ...
    int EncoderThread::FrameRate() const noexcept { return frame_rate_.load(); }
    ...
    void EncoderThread::EncodeOnWorker(
        CaptureVideoFrame cap_video_msg, const std::shared_ptr<void>& inflight_guard) {
            ...
            if (frame_rate_changed || full_color_mode_changed || frame_meta_info_changed || encoder_format_ != effective_format
