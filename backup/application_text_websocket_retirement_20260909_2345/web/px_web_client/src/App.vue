...
import { ApplicationTextTransport, applicationTextSocketUrl } from './rtc/application_text'
...
function renewConnectionTicket(updateProgress = true, auxiliary = false) {
  const operation = renewalQueue.then(() => exchangeConnectionTicket(updateProgress, auxiliary))
  renewalQueue = operation.catch(() => undefined)
  return operation
}

async function exchangeConnectionTicket(updateProgress: boolean, auxiliary: boolean) {
  ...
  if (!auxiliary) connectionTicket.value = renewed.ticket
  ...
  if (!auxiliary) ticketConsumed = false
...
async function discoverApplicationText(message: ApplicationTextMessage) {
  const caps = message.applicationTextCapabilities
  if (message.type !== 610 || !caps || caps.version !== 1 || !caps.finalTextSupported
    || !caps.parentBindingId || textBinding === caps.parentBindingId || !connectionInstanceId.value
    || !hasGrantedPermission('input') || viewOnly.value) return
  textBinding = caps.parentBindingId
  const epoch = ++textEpoch
  textTransport?.dispose()
  input?.setApplicationTextEnabled(true)
  textStatus.value = '正在连接可靠文字通道。'
  try {
    const renewed = await renewConnectionTicket(false, true)
    if (epoch !== textEpoch) return
    if (!renewed.permissions.includes('input')) throw new Error('input permission revoked')
    ...
    textTransport.connect(applicationTextSocketUrl(window.location.href, {
      ticket: renewed.ticket, client_nonce: clientNonce.value, instance_id: connectionInstanceId.value,
      stream_id: form.streamId, parent_binding_id: caps.parentBindingId, visitor_device_id: form.deviceId,
    }))
  } catch {
    if (epoch !== textEpoch) return
    textStatus.value = '文字通道授权失败，请从 Console 重新进入。'
...
function handleDcBinary(buf: ArrayBuffer) {
  for (const payload of dcReassembler.feed(buf)) {
    ...
    if (msg.type === 610) void discoverApplicationText(msg as ApplicationTextMessage)
...
async function initInput() {
  ...
  input.setApplicationTextEnabled(!!textBinding)
