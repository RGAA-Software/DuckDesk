...
- Existing authenticated RTC media control carries only the capability discovery
  request/reply (`610`). The reply identifies its physical parent binding.
- A separately renewed, one-time Console ticket opens the same-origin
  `/application-text` WebSocket child. It is not a second controller occupant.
- Ticket renewal exchanges are serialized; the auxiliary ticket is not written
  into the primary connection's pending ticket slot. Tickets remain in memory.
- This child uses raw `px.Message` protobuf, not RTC TLV, for `610`–`615`.
- HTTPS pages use WSS, HTTP intranet pages use WS. No certificate bypass or
  HTTPS-to-WS downgrade exists. Production reverse proxies must forward the
  `/application-text` upgrade to the same Render and preserve Host and Origin.
  Direct Render hosting already serves `/web_client/` on that origin. The Vite
  development proxy has an explicit WebSocket route.
...
and the socket send buffer is below 64 KiB. Target changes while editing disable
