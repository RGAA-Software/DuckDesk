...
import { ApplicationTextTransport, applicationTextSocketUrl } from '../src/rtc/application_text'
...
function fixture() {
  ...
  const changed = vi.fn()
  const socket = {
    readyState: 1, bufferedAmount: 0, binaryType: '', onopen: null as (() => void) | null,
    onmessage: null, onerror: null, onclose: null,
    send: (bytes: Uint8Array) => sent.push(decodeMessage(bytes)), close: vi.fn(),
  }
  const adapter = new ApplicationTextTransport({
    workflow, instanceId: 'app-a', deviceId: 'device', streamId: 'stream', suspend, changed,
    createSocket: () => socket as unknown as WebSocket,
  })
  adapter.connect('wss://host/application-text')
  socket.onopen?.()
  ...
  return { workflow, sent, suspend, changed, socket, adapter, acknowledge }
}

afterEach(() => vi.useRealTimers())

describe('reliable application text adapter', () => {
  it('uses same-origin WSS on HTTPS and never downgrades', () => {
    const url = new URL(applicationTextSocketUrl('https://trusted:8443/web_client/#secret', { ticket: 'a&b' }))
    expect(url.protocol).toBe('wss:')
    expect(url.host).toBe('trusted:8443')
    expect(url.pathname).toBe('/application-text')
    expect(url.searchParams.get('ticket')).toBe('a&b')
    expect(url.hash).toBe('')
    expect(applicationTextSocketUrl('http://local/web_client/', {})).toBe('ws://local/application-text')
    expect(() => applicationTextSocketUrl('file:///tmp/page', {})).toThrow()
  })
