...
void NetworkEventIngress::SendRtcSignalingError(const std::string& stream_id, const std::string& code, const std::string& message) const {
...
void ExpireVirtualDisplayRequests(const std::shared_ptr<RdApplication>& app, const std::shared_ptr<VirtualDisplayCoordinator>& coordinator) {
    ...
    const auto now = std::chrono::steady_clock::now();
    {
        ...
        for (auto it = coordinator->pending.begin(); it != coordinator->pending.end();) {
            ...
            const auto message = it->second.service_result ? "display topology changed but capture did not produce a frame in time"
...
void NetworkEventIngress::ProcessClientDisConnectedEvent(const std::shared_ptr<ClientDisconnectedEvent>& event, const std::string& source_id) {
    ...
    msg.connection_id_ = event->connection_id_;
...
void NetworkEventIngress::ProcessCapturingMonitorInfoEvent(const std::shared_ptr<CaptureMonitorInfoChangedEvent>& event) const {
    LOGI("Will update capture monitor information for the input replay service.");
...
void NetworkEventIngress::ProcessNetEvent(const std::shared_ptr<NetworkClientEvent>& event, const std::string& source_id) {
    if (event->is_proto_ && event->message_) {
        ...
        // Validate the logical-session input lease before dispatching to built-in services.
        ...
        if (application_text_ && input_lease &&
            ...
            application_text_->Handle(*msg, lease, event->transport_type_ == TransportKind::kWebSocket, alive,
                                       [weak_modules, stream = *stream, alive](Message response) {
                if (const auto modules{weak_modules.lock()}; modules && alive()) {
                    modules->BroadcastTargetStreamMessage(stream, Data::From(response.SerializeAsString()), false);
                }
            });
            return;
        }
        ...
        if (msg->type() == MessageType::kClipboardInfo) {
            ...
            context_->PostTask([weak_self, msg, event]() {
                ...
                if (!self->module_registry_->IsWsUserProxyConnected()) {
                    LOGW("user-proxy not connected, drop client clipboard, type={}, len={}", (int)msg->clipboard_info().type(),
                         event->message_->Size());
                    return;
                }
            ...
            LOGI("[LAT-clip] render recv clipboard buffer msg, type: {}, len: {}", (int)msg->type(), event->message_->Size());
            context_->PostTask([weak_self, msg, event]() {
                const auto self = weak_self.lock();
                if (!self) {
                    return;
                ...
                if (!self->module_registry_->IsWsUserProxyConnected()) {
                    LOGW("user-proxy not connected, drop clipboard buffer msg, type={}, len={}", (int)msg->type(), event->message_->Size());
                    return;
                }
                pxrp::RpMessage rp_msg;
                rp_msg.set_type(pxrp::kRpRawRenderMessage);
                ...
                sub.set_stream_id(msg->stream_id());
                sub.set_device_id(msg->device_id());
                LOGI("PostUserProxyMessage clipboard buffer msg, type={}, len={}", (int)msg->type(), event->message_->Size());
                self->app_->PostUserProxyMessage(RpProtoAsData(&rp_msg));
            });
        }
#else
...
void NetworkEventIngress::ProcessApplicationTextChannel(const std::shared_ptr<ApplicationTextChannelMessageEvent>& event) {
    if (!event || !event->channel || !event->message || event->message->Size() > 32768 || !application_text_) {
        return;
    }
    const auto channel{event->channel};
    if (!channel->open.load() || !channel->alive || !channel->alive()) {
        return;
    }
    auto message{Message{}};
    if (!message.ParseFromString(event->message->AsString()) ||
        (message.type() != kApplicationTextCapabilities && message.type() != kApplicationTextBarrier && message.type() != kApplicationTextSubmit)) {
        channel->open.store(false);
        return;
    }
    const auto registry{app_->GetLogicalSessionRegistry()};
    std::optional<LogicalSessionInputLease> lease{};
    {
        std::scoped_lock lock{channel->mutex};
        if (!channel->lease && registry) {
            channel->lease = registry->AuthorizeAuxiliaryInputGrant(channel->grant, channel->parent_binding_id, CurrentSystemMilliseconds());
        }
        lease = channel->lease;
    }
    if (!lease || !registry || !registry->IsCurrentInputBinding(*lease, CurrentSystemMilliseconds())) {
        channel->open.store(false);
        return;
    }
    const std::weak_ptr<ApplicationTextChannel> weak_channel{channel};
    const auto alive{[weak_channel] {
        const auto channel{weak_channel.lock()};
        return channel && channel->open.load() && channel->alive && channel->alive();
    }};
    application_text_->Handle(std::move(message), *lease, true, alive, [weak_channel, alive](Message response) {
        if (const auto channel{weak_channel.lock()}; channel && alive() && channel->reply) {
            channel->reply(Data::From(response.SerializeAsString()));
        }
    });
}
...
void NetworkEventIngress::ReleaseControllerInput(const LogicalSessionInputLease& lease) {
    ...
    if (settings_.IsWebViewMode()) {
        ...
        LOGI("controller WebView input released: session={}, generation={}, keys={}, mouse_buttons={}", lease.logical_session_id, lease.generation,
             input_state.pressed_keys_.size(), input_state.pressed_mouse_buttons_.size());
        return;
    }
    ...
    LOGI("controller lease released: session={}, generation={}, keys={}, mouse_buttons={}", lease.logical_session_id, lease.generation,
         input_state.pressed_keys_.size(), input_state.pressed_mouse_buttons_.size());
}
...
void NetworkEventIngress::ProcessMouseEvent(std::shared_ptr<Message>&& msg, const LogicalSessionInputLease& lease) {
    ...
    if (settings_.app_.IsGlobalReplayMode()) {
        // Desktop replay is handled by the built-in input replay service; the router still owns lease-scoped state above.
        return;
    }
    if (settings_.IsWebViewMode()) {
                                                                      ...
                                                                      mouse_event.pressed(), mouse_event.released());
    {
        ...
        const auto n = ++s_n;
        if (n <= 5 || (n % 200) == 0) {
            LOGI("hook-inner mouse: n={} hwnd={:x} screen=({},{}) ratio=({:.3f},{:.3f}) btn={}", n, hwnd_ptr, (int)x, (int)y, mouse_event.x_ratio(),
                 mouse_event.y_ratio(), mouse_event.button());
        }
    }
...
void NetworkEventIngress::ProcessKeyboardEvent(std::shared_ptr<Message>&& msg, const LogicalSessionInputLease& lease) {
                                                                      ...
                                                                      key_event.caps_lock_status());
    {
        ...
        const auto n = ++s_n;
        if (n <= 5 || (n % 200) == 0) {
            LOGI("hook-inner key: n={} hwnd={:x} key=0x{:x} down={}", n, hwnd_ptr, key_event.key_code(), key_event.down());
        }
    }
    ...
    if (data) {
        if (const auto media_bus = context_->GetEncodedMediaBus(); media_bus && media_bus->NeedsEncodedAudio()) {
            media_bus->PublishEncodedAudio(std::make_shared<const render::EncodedAudioFrame>(render::EncodedAudioFrame{
                ...
                .payload = render::MakeImmutableByteBuffer(data->AsString()),
            }));
        }
    }
}
...
void NetworkEventIngress::ReportClientConnected(const std::shared_ptr<ClientConnectedEvent>& event) {
    ...
    app_->PostGlobalTask([weak_self, event]() {
        ...
        sub.set_conn_id(event->connection_id_);
        sub.set_stream_id(event->stream_id_);
        sub.set_conn_type(event->connection_type_);
...
void NetworkEventIngress::ProcessVirtualDisplayRequest(std::shared_ptr<Message>&& msg) {
    ...
    LOGI("Virtual display request started: request={}, operation={}, owned={}, generation={}", request_id, static_cast<int>(request.operation()),
         initial_owned_count, initial_generation);
                                ...
                                [coordinator, app](const MsgVirtualDisplayServiceResult& result) {
                                    ...
                                    // arrive in either order.
                                    {
                                        ...
                                        if (it == coordinator->pending.end()) {
                                            LOGW("Late virtual display Service result ignored: request={}, accepted={}, code={}", result.request_id_,
                                                 result.accepted_, result.error_code_);
                                            return;
                                        }
...
void NetworkEventIngress::ProcessAck(const std::shared_ptr<NetworkClientEvent>& event, const std::shared_ptr<Message>& message) {
    ...
    ack->ch_type_ = event->channel_type_;
    ack->msg_type_ = message->type();
    if (event->ack_callback_) {
        event->ack_callback_(ack);
    }
}
} // namespace px
