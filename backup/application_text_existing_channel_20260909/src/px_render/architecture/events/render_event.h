...
struct ApplicationTextChannel final {
    LogicalSessionGrant grant{};
    std::string parent_binding_id{};
    std::atomic_bool open{true};
    std::mutex mutex{};
    std::optional<LogicalSessionInputLease> lease{};
    std::function<void(std::shared_ptr<Data>)> reply{};
    std::function<bool()> alive{};
};

struct ApplicationTextChannelMessageEvent final {
    std::shared_ptr<ApplicationTextChannel> channel{};
...
using RenderEvent =
    std::variant<std::shared_ptr<NetworkClientEvent>, std::shared_ptr<ApplicationTextChannelMessageEvent>,
                 std::shared_ptr<UdpVoiceFrameEvent>, std::shared_ptr<ClientConnectedEvent>,
