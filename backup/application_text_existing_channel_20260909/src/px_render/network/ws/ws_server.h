...
class WsServer : public std::enable_shared_from_this<WsServer> {
  ...
  private:
    ...
    void FinalizeApplicationTextOpen(const std::shared_ptr<asio2::http_session>& session, const WsTicketAdmission& ticket,
                                     const std::string& parent_binding_id, std::uint64_t socket_fd);
  ...
  private:
    ...
    px::ConcurrentHashMap<uint64_t, std::shared_ptr<ApplicationTextChannel>> text_channels_{};
