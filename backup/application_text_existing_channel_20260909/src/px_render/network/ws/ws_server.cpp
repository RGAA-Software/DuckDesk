...
static const std::string kUrlApplicationText{"/application-text"};
...
template <typename WireValue> static std::optional<WireValue> DecodeWireValue(const std::string_view bytes) {
...
bool WsServer::Start() {
    ...
    server_->bind_disconnect([weak_self](std::shared_ptr<asio2::http_session>& sess_ptr) {
        ...
        if (auto opt_val = self->stream_routers_.Remove(socket_fd); opt_val.has_value()) {
            ...
            LOGI("event=session.close component=net_ws outcome=removed "
                 "device={}",
                 PrivacyLogId(val->visitor_device_id_));
    ...
    if (rdp_proxy_port_ == 0) {
        ...
        AddWebsocketRouter(kUrlApplicationText);
...
void WsServer::FinishStop() {
    ...
    ft_routers_.Clear();
    text_channels_.ApplyAll([](const std::uint64_t&, const std::shared_ptr<ApplicationTextChannel>& channel) { channel->open.store(false); });
    text_channels_.Clear();
    ipc_sessions_.Clear();
...
FileTransferSendResult WsServer::PostTargetFileTransferMessage(const std::string& stream_id, const std::shared_ptr<Data>& msg,
...
void WsServer::AddUserProxyRouter() {
    ...
    auto fn_get_socket_fd = [](std::shared_ptr<asio2::http_session>& sess_ptr) -> uint64_t { return (uint64_t)sess_ptr->socket().native_handle(); };
    server_->bind(kUrlUserProxy,
                  websocket::listener<asio2::http_session>{}
                      .on("message",
                          [weak_self, weak_router, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr, std::string_view data) {
                              if (const auto self = weak_self.lock(); self && !self->exiting_) {
                      ...
                      .on("open",
                          [weak_self, weak_router, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr) {
                              auto self = weak_self.lock();
                              if (!self || self->exiting_) {
                                  return;
                              }
...
void WsServer::ReportPerformance() {
    ...
    LOGI("event=transport.window component=net_ws transport=ws "
         ...
         static_cast<double>(snapshot->inbound_bytes + snapshot->outbound_bytes) / seconds, snapshot->dropped_messages, snapshot->queue_depth,
...
void WsServer::AddIpcRouter() {
    ...
    auto weak_self = weak_from_this();
    server_->bind(kUrlIpc,
                  websocket::listener<asio2::http_session>{}
                      .on("message",
                          [weak_self](std::shared_ptr<asio2::http_session>& sess_ptr, std::string_view data) {
                              auto self = weak_self.lock();
                              ...
                              if (*first_u32 == kIpcCaptureVideoFrameMagic) {
                                  ...
                                  if (!ipc_msg || ipc_msg->version_ != kIpcCaptureVideoFrameVersion || ipc_msg->type_ != kCaptureVideoFrame) {
                                  ...
                                  if (ipc_msg->frame_width_ < 16 || ipc_msg->frame_width_ > 8192 || ipc_msg->frame_height_ < 16 ||
                                      ipc_msg->frame_height_ > 8192) {
                                  ...
                                  std::copy(std::begin(ipc_msg->display_name_), std::end(ipc_msg->display_name_), std::begin(frame.display_name_));
                              ...
                              if (*first_u32 == kCaptureVideoFrame) {
                                  ...
                                  if (n == 1 || (n % 100) == 0) {
                                      LOGW("event=transport.receive component=net_ws "
                                           ...
                                           "outcome=dropped recoverable=true count={}",
                                           n);
                                  }
                                  return;
                              }
                              if (*first_u32 == kCaptureAudioFrame) {
                                  ...
                                  if (data.size() != expect || hdr->data_length == 0) {
                                      LOGE("event=transport.receive component=net_ws "
                                           ...
                                           "outcome=dropped recoverable=true bytes={} expected_bytes={} "
                                           "pcm_bytes={}",
                                           data.size(), expect, hdr->data_length);
                                      return;
                                  }
                                  auto pcm = Data::From(std::string(data.substr(sizeof(IpcCaptureAudioFrame))));
                                  if (!pcm) {
                                      LOGE("event=transport.receive component=net_ws "
                                  ...
                                  if (n == 1 || (n % 200) == 0) {
                                      LOGI("event=ipc.audio.window component=net_ws count={} frame={} "
                                           "sample_rate_hz={} channels={} bits={} bytes={}",
                                           n, hdr->frame_index_, hdr->samples_, hdr->channels_, hdr->bits_, hdr->data_length);
                                  }
                                  if (const auto transport = self->transport_.lock()) {
                      ...
                      .on("open",
                          [weak_self](std::shared_ptr<asio2::http_session>& sess_ptr) {
                              auto self = weak_self.lock();
                              if (!self || self->exiting_) {
                                  return;
                              }
                              ...
                              if (client_pid == 0 || !self->IsIpcPidAllowed(client_pid)) {
                                  ...
                                  if (n == 1 || (n % 50) == 0) {
                                      LOGW("event=session.admit component=net_ws "
                                           ...
                                           client_pid, PrivacyLogId(remote_addr), sess_ptr->remote_port(), n);
                                  }
                                  sess_ptr->stop();
                                  return;
                              }
                              ...
                              LOGI("event=session.admit component=net_ws outcome=connected "
                                   ...
                                   PrivacyLogId(remote_addr), sess_ptr->remote_port(), socket_fd, client_pid, self->ipc_sessions_.Size());
                          })
...
PxAwaitable<void> WsServer::OpenWebSocketAsync(std::weak_ptr<WsServer> owner, std::shared_ptr<asio2::http_session> session, std::string path,
                                               std::unordered_map<std::string, std::string> params, const std::uint64_t socket_fd) {
    ...
    if (path == kUrlApplicationText) {
        const auto parent{params.find("parent_binding_id")};
        const auto stream{params.find("stream_id")};
        if (server->rdp_proxy_port_ != 0 || ticket.join_mode_ != "control" ||
            std::ranges::find(ticket.permissions_, "input") == ticket.permissions_.end() || parent == params.end() || parent->second.empty() ||
            parent->second.size() > 512 || stream == params.end() || stream->second != ticket.stream_id_) {
            RejectWebSocketSession(session, kWsAuthorizationRejectedSignal);
            co_return;
        }
        session->post_queued_event([owner, session, ticket = std::move(ticket), parent_binding_id = parent->second, socket_fd] {
            if (const auto server{owner.lock()}; server && !server->exiting_ && session->is_started()) {
                server->FinalizeApplicationTextOpen(session, ticket, parent_binding_id, socket_fd);
            }
        });
        co_return;
    }
...
void WsServer::FinalizeWebSocketOpen(const std::shared_ptr<asio2::http_session>& session, const std::string& path,
    ...
    if (path == kUrlMedia) {
        ...
        stream_routers_.Insert(socket_fd, router);
        NotifyMediaClientConnected(router->connection_id_, router->stream_id_, visitor_device_id);
        auto mutable_session = session;
        router->OnOpen(mutable_session);
...
void WsServer::AddWebsocketRouter(const std::string& path) {
    ...
    server_->bind(
        ...
        websocket::listener<asio2::http_session>{}
            .on("message",
                [weak_self, path, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr, std::string_view data) {
                    auto self = weak_self.lock();
                    if (!self || self->exiting_) {
                        return;
                    }
                    ...
                    if (path == kUrlApplicationText) {
                        ...
                        const auto channel{self->text_channels_.TryGet(socket_fd)};
                        if (channel && *channel) {
                            if (!(*channel)->open.load()) {
                                sess_ptr->stop();
                                return;
                            }
                            if (const auto transport{self->transport_.lock()}) {
                                const auto event{std::make_shared<ApplicationTextChannelMessageEvent>()};
                                event->channel = *channel;
                                event->message = Data::From(data);
                                transport->EmitEvent(event);
                            }
                        }
                        return;
            ...
            .on("open",
                [weak_self, path, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr) {
                    ...
                    const auto socket_fd = fn_get_socket_fd(sess_ptr);
                    if (path == kUrlApplicationText) {
                        const auto origin{std::string(session->get_request().base()[http::field::origin])};
                        const auto host{std::string(session->get_request().base()[http::field::host])};
                        // Native clients omit Origin. Browsers must use the same
                        // origin; HTTPS reverse proxies must preserve Host.
                        if (!origin.empty() && origin != "http://" + host && origin != "https://" + host) {
                            RejectWebSocketSession(session, kWsAuthorizationRejectedSignal);
                            return;
                        }
                    }
                    if (!self->async_scope_->Spawn("ws-session-open", [weak_self, session, path, socket_fd, params = std::move(params)]() mutable {
                            return WsServer::OpenWebSocketAsync(weak_self, session, path, std::move(params), socket_fd);
                        })) {
            ...
            .on("close",
                [weak_self, path, fn_get_socket_fd](std::shared_ptr<asio2::http_session>& sess_ptr) {
                    ...
                    auto socket_fd = fn_get_socket_fd(sess_ptr);
                    if (path == kUrlApplicationText) {
                        if (const auto removed{self->text_channels_.Remove(socket_fd)}; removed && *removed) {
                            (*removed)->open.store(false);
                        }
                        return;
                    }
                    ...
                    if (path == kUrlMedia) {
                        if (auto opt_val = self->stream_routers_.Remove(socket_fd); opt_val.has_value()) {
                            ...
                            self->NotifyMediaClientDisConnected(val->connection_id_, val->stream_id_, val->visitor_device_id_,
                                                                val->created_timestamp_, val->binding_id_, val->logical_session_id_);
                            LOGI("event=session.close component=net_ws outcome=removed "
                                 "device={}",
                                 PrivacyLogId(val->visitor_device_id_));
                        }
            ...
            .on("update", [](std::shared_ptr<asio2::http_session>& sess_ptr) { LOGI("update"); }));
}
...
void WsServer::FinalizeApplicationTextOpen(const std::shared_ptr<asio2::http_session>& session, const WsTicketAdmission& ticket,
                                          const std::string& parent_binding_id, const std::uint64_t socket_fd) {
    const auto channel{std::make_shared<ApplicationTextChannel>()};
    channel->grant = {.logical_session_id = ticket.logical_session_id_, .stream_id = ticket.stream_id_, .subject_id = ticket.subject_id_,
                      .join_mode = ticket.join_mode_, .expires_at_ms = ticket.expires_at_ms_};
    channel->parent_binding_id = parent_binding_id;
    const std::weak_ptr<asio2::http_session> weak_session{session};
    const auto weak{weak_from_this()};
    channel->alive = [weak, weak_session] {
        const auto server{weak.lock()};
        const auto session{weak_session.lock()};
        return server && !server->exiting_ && session && session->is_started();
    };
    channel->reply = [weak_session, weak_channel = std::weak_ptr<ApplicationTextChannel>{channel}](std::shared_ptr<Data> data) {
        const auto session{weak_session.lock()};
        const auto channel{weak_channel.lock()};
        if (!session || !channel || !channel->open.load()) {
            return;
        }
        PostReliableWebSocketWrite(session, std::move(data), [weak_session](bool success) {
            if (!success) {
            ...
            const auto channel{weak_channel.lock()};
            return channel && channel->open.load() && channel->alive();
        });
    };
    session->ws_stream().binary(true);
    session->set_no_delay(true);
    text_channels_.Insert(socket_fd, channel);
    // Client's first query can arrive while its one-time ticket is redeeming.
    // Send discovery after admission so it need not retry that early frame.
    if (const auto transport{transport_.lock()}) {
        auto query{Message{}};
        query.set_type(kApplicationTextCapabilities);
        const auto event{std::make_shared<ApplicationTextChannelMessageEvent>()};
        event->channel = channel;
        event->message = Data::From(query.SerializeAsString());
        transport->EmitEvent(event);
    }
}
...
int64_t WsServer::GetQueuingMediaMsgCount() {
    int64_t count = 0;
    stream_routers_.ApplyAll([&](const auto&, const auto& r) { count += r->GetQueuingMsgCount(); });
    return count;
}

int64_t WsServer::GetQueuingFtMsgCount() {
    int64_t count = 0;
    ft_routers_.ApplyAll([&](const auto&, const auto& r) { count += r->GetQueuingMsgCount(); });
    return count;
}
