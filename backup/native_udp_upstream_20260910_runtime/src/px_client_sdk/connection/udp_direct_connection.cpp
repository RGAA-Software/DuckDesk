...
namespace px
{
    ...
    void UdpDirectConnection::InstallCallbacks() {
        ...
        // Until RFI completion is tied to an encoded frame, only an IDR can
        // safely repair the reference chain. Throttle requests across all slots
        // because an empty monitor name requests a key frame for every monitor.
        reassembler_.on_frame_lost_ = [weak_self](uint8_t mon_slot, uint32_t lost_frame_index) {
            ...
            if (now - self->last_idr_request_ms_.load() < kIdrThrottleMs) {
                return;
            }
            self->last_idr_request_ms_ = now;
