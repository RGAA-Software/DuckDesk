...
void UdpTransport::SubmitEncodedVideo(const std::string& mon_name, const EncodedVideoType& video_type, const std::shared_ptr<Data>& data,
    ...
    // Receiving an RFI request is not proof that the encoder has applied it.
    // Never label an already queued dependent frame as independently recoverable.
    meta.rfi_recover_ = false;
