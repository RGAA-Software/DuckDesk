...
| CM-RECORDING | 录像目录、直读/拉取、下载到本机或 Console、删除 | 已迁移 | recording catalog + private cache/read-lease service + 管理/本人录像页 | Render 完成段、Service 登记/回传、Console 私有缓存、Range 下载、管理员与会话 owner 页面以及 Console 副本保留/释放/驱逐已接通；自动化覆盖 hash、并发、读取租约、撤销、CAS 和 pinned/在读拒绝。公网真实 H.264 MP4 已通过本人 Chromium 下载、管理员 Retain、Release、确认 Evict 及本人再次下载的完整浏览器闭环。旧 URL ticket 不恢复 |
...
| CM-LICENSE | 许可证状态与授权入口 | 已实现核心链 | `px_auth_server` + Console 本地许可证消费者 + 运维状态 | Auth 签发/续期/撤销管理记录；Console 本地验证 deployment、到期、服务和 stream 上限。Service 不解析许可证；离线副本不承诺即时撤销。仍需正式部署与跨端业务验收。旧浏览器 stub `AuthView` 不恢复 |
...
| CM-TRANSFER | 文件传输历史、终态与失败原因 | 已迁移 | managed transfer history + `SecurityInternal` + 本人活动页 | Render→Service→Console 的真实生产链、单调进度、实际文件/清单 SHA-256、哈希终态和 unknown 状态已接；开始传输仍由Console在线授权，已授权transfer的进度/终态先写Service LocalMachine DPAPI有界outbox再确认Render，并在通知、重连和周期任务中以同一sequence/正文补报，因此Render退出或Service重启不会丢失待报终态。Console本人活动页已消费owner-scoped历史，提供中英文方向、状态、进度、失败原因、筛选和分页，并明确不冒充可恢复任务队列；真实 PostgreSQL、node-control WebSocket、原生 Console 和实际 Chromium 已验证完成态记录在重启/断库恢复后的本人页面显示，且取消后使用新 transfer ID 重试时旧 Cancelled 与新 Completed 事实在重启/断库恢复后并存。Windows、Android、Web 均已有新 job 重试语义；Android 真机专项通过。Windows 公网 Relay 已完成随机文件双端 SHA-256、真实取消/重试，Direct 活动上传中 Service 重启则验证 `unknown` 收敛、精确 Render 回收与立即再调度。最终长测仍统一后置 |
