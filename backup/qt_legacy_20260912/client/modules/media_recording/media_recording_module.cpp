...
void ClientMediaRecordingModule::StartRecording(uint64_t intent) {
    std::string error{};
    {
        std::lock_guard lock(lifecycle_mutex_);
        if (stopped_)
        ...
        if (recording_ && recording_->WaitFor(std::chrono::milliseconds::zero()))
            recording_.reset();
        if (recording_) {
            if (active_intent_ == intent)
                return;
            recording_->Stop();
            finishing_.push_back(std::move(recording_));
        }
        ...
        if (finishing_.size() >= 4U) {
            ...
            recording_ = RecordingSession::Create({.writer = {.dir = ResolveRecordDir(screen_recording_path_)}, .monitor_count = kMaxRenderViewCount},
                                                  {.finished = [weak_services = services_, intent](const RecordingSessionResult& result) {
                                                      const auto services = weak_services.lock();
                                                      if (!services)
                                                          return;
                                                      if (!result.error.empty()) {
                                                          services->NotifyRecordingFailure(intent, result.error);
                                                          return;
                                                      }
    ...
    if (!error.empty()) {
        if (const auto services = services_.lock())
            services->NotifyRecordingFailure(intent, error);
    }
}
