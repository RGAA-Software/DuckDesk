//! Application / Placement / Instance orchestration for Console.
...
pub struct Application {
    ...
    /// Absolute game exe path shown/edited in Console Web.
    #[serde(default)]
...
/// Flattened row for Console Web list/edit. 节列表随行下发,实例由 Web 按
...
impl AppScheduleManager {
    ...
    pub async fn save_app(&self, req: SaveAppReq) -> Result<AppRowVo, String> {
        ...
        let app = {
            ...
            let (game_path, game_exe_rel, entry_url, default_game_args) =
                if app_type == ApplicationType::Rdp {
                    (String::new(), String::new(), String::new(), String::new())
                } else if app_type == ApplicationType::Webview {
                    (
                        String::new(),
                        String::new(),
                        validate_webview_entry_url(req.entry_url.as_deref().unwrap_or_default())?,
    ...
    pub async fn delete_app(&self, app_id: &str) -> Result<(), String> {
        ...
        // Drop this app's instances from DB too, otherwise a Console restart would
    ...
    async fn dispatch_start(
        &self,
        conn: crate::net_service::console_service_conn::ConsoleServiceConnPtr,
        app: &Application,
        inst: AppInstance,
        install_root: String,
        game_exe_rel: String,
        ...
        let start = ConsoleServiceStartAppInstance {
            request_id: request_id.clone(),
            instance_id: instance_id.clone(),
            app_id: app.app_id.clone(),
            install_root,
        ...
        match tokio::time::timeout(START_RESULT_TIMEOUT, wait_rx).await {
            ...
            Err(_) => {
                ...
                if let Some(current) = snapshot {
                    ...
                    if should_compensate {
                        let compensation = ConsoleServiceStopAppInstance {
    ...
    pub async fn stop_instance(&self, instance_id: &str) -> Result<AppInstance, String> {
        ...
        let stop = ConsoleServiceStopAppInstance {
            request_id: request_id.clone(),
    ...
    /// Align Console state with what Service reports in HB — in both directions.
    ...
    /// `stopped` entry with the same instance_id must not keep Console in Running.
    pub async fn reconcile_from_service_hb(&self, device_id: String, instances_json: &str) {
        ...
        let (snapshots, known_ids) = {
            ...
            for inst in instances.values_mut() {
                ...
                // A terminal report is evidence for one launch request only.
                ...
                if matches!(inst.state, InstanceState::Starting | InstanceState::Running) {
                    if let Some(rep) = reported.iter().find(|rep| rep.instance_id == inst.instance_id
                        ...
                        if let Some((state, error)) = classified {
                            inst.state = state;
                            inst.error = error;
                            inst.stop_reason = rep.stop_reason.clone();
                            inst.exit_code = rep.exit_code;
                            inst.pid = 0;
                            inst.version += 1;
                ...
                if let Some((rep, reported_state)) = active.get(&inst.instance_id) {
                    ...
                    match inst.state {
                        ...
                        InstanceState::Starting => {
                            // A Console restart loses the in-memory HTTP waiter, but
                ...
                match inst.state {
                    InstanceState::Starting | InstanceState::Running | InstanceState::Stopping => {
                        ...
                        if now.saturating_sub(*missing_since) >= PROCESS_LOST_GRACE_MS {
                            ...
                            inst.stop_reason = "process_lost".into();
                            inst.exit_code = None;
                            request_index.remove(&inst.request_id);
                            suspect_since.remove(&inst.instance_id);
                            out.push(inst.clone());
    ...
    /// Console restart cannot duplicate or incorrectly terminate a live process.
...
mod tests {
    ...
    async fn reconcile_ignores_stopped_hb_entries() {
        ...
        .await;
        // Same id still listed, but Service says stopped → Console must clear Running.
        mgr.inner
            .lock()
            .await
            .suspect_since
    ...
    #[tokio::test]
    async fn repeated_stop_of_stopped_instance_is_success_without_service_connection() {
        let mgr = AppScheduleManager::new();
        let (app, placement, mut instance) = fixture(InstanceState::Stopped);
        instance.pid = 0;
        instance.stopped_at_ms = 1234;
        let id = instance.instance_id.clone();
        let version = instance.version;
        mgr.inject_for_test(app, placement, instance).await;
        for _ in 0..3 {
            let result = mgr.stop_instance(&id).await.unwrap();
            assert_eq!(result.state, InstanceState::Stopped);
            assert_eq!(result.version, version);
            assert_eq!(result.stopped_at_ms, 1234);
        }
        assert!(mgr.inner.lock().await.stop_waiters.is_empty());
    }

    #[tokio::test]
    async fn repeated_stop_preserves_genuine_failed_state() {
        let mgr = AppScheduleManager::new();
        let (app, placement, instance) = fixture(InstanceState::Failed);
        let id = instance.instance_id.clone();
        mgr.inject_for_test(app, placement, instance).await;
        assert!(mgr.stop_instance(&id).await.is_err());
    ...
    async fn restored_starting_instance_advances_from_service_heartbeat() {
        ...
        // The original HTTP waiter disappeared with Console, so heartbeat must be
