...
impl RdpAccountSpec {
    pub fn validate(&self) -> Result<(), String> {
        if self.workspace_id.is_empty() || self.workspace_id.len() > 128
            ...
            || !(self.account_name.starts_with("prdp_") || self.account_name.starts_with("grdp_")) || self.account_name.len() > 20
...
mod platform {
    ...
    pub fn ensure(spec: &RdpAccountSpec) -> Result<RdpAccountIdentity, String> {
        ...
        if status == 2221 { // NERR_UserNotFound
            ...
            let legacy_prefix = format!("GammaRay RDP:{}:", spec.workspace_id);
            applied_version = marker.strip_prefix(&pixels_prefix).or_else(|| marker.strip_prefix(&legacy_prefix))
