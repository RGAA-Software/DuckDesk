...
impl RdpWorkspaceVault {
    ...
    pub fn load(path: &Path) -> Result<Self, String> {
        ...
        if !metadata.is_file() || metadata.len() != 32 {
            return Err("RDP master key must be a regular 32-byte file".into());
        }
        #[cfg(windows)]
        {
            use std::os::windows::fs::MetadataExt;
            if metadata.file_attributes() & 0x400 != 0 {
                return Err("RDP master key reparse point refused".into());
            }
        }
        #[cfg(unix)]
        {
            ...
            if metadata.permissions().mode() & 0o077 != 0 {
                return Err("RDP master key must not be accessible to group/other users".into());
            }
        }
        ...
        let file = options
            .open(path)
            .map_err(|_| "RDP master key open failed".to_string())?;
        if !file
            .metadata()
        ...
        file.take(33)
            ...
            .map_err(|_| "RDP master key read failed".to_string())?;
        Self::from_key(&bytes)
    }

    fn aad(record: &RdpWorkspaceRecord) -> Result<Vec<u8>, String> {
        if !matches!(record.secret_schema, 1 | SECRET_SCHEMA)
        ...
        let namespace = if record.secret_schema == 1 {
    ...
    pub fn create(
        ...
        node_id: &str,
        device_id: &str,
    ) -> Result<RdpWorkspaceRecord, String> {
        let random = SystemRandom::new();
        let mut entropy = Zeroizing::new([0u8; 32]);
        random
            .fill(entropy.as_mut())
            .map_err(|_| "RDP secure random unavailable".to_string())?;
    ...
    pub fn open(&self, record: RdpWorkspaceRecord) -> Result<RdpWorkspaceCredential, String> {
        ...
        let plaintext = self
            ...
            .map_err(|_| {
                "RDP credential authentication failed; key or workspace mismatch".to_string()
            })?;
        let password = Zeroizing::new(
            std::str::from_utf8(plaintext)
                .map_err(|_| "RDP credential encoding invalid".to_string())?
                .to_owned(),
        );
    ...
    /// Connection discovery must never create or rotate a live workspace identity.
    pub async fn open_existing(
        &self,
        collection: &Collection<RdpWorkspaceRecord>,
        app_id: &str,
        node_id: &str,
        device_id: &str,
        ...
        let record = collection
            .find_one(doc! { "app_id": app_id, "node_id": node_id, "device_id": device_id })
            .await
    ...
    pub async fn ensure(
        ...
        if record.device_id != device_id {
            return Err(
                "RDP workspace is pinned to another device; automatic migration is forbidden"
                    .to_string(),
            );
        }
        self.open(record)
...
mod tests {
    ...
    fn vault() -> RdpWorkspaceVault {
        RdpWorkspaceVault::from_key(&[7u8; 32]).unwrap()
    }
    ...
    #[test]
    fn schema_one_credentials_remain_readable_during_pixels_migration() {
        let vault = vault();
        let nonce = [3u8; 12];
        let password = "aA1!01234567890123456789012345678901";
        let mut record = RdpWorkspaceRecord {
        ...
        vault
            ...
            .seal_in_place_append_tag(
                aead::Nonce::assume_unique_for_key(nonce),
                aead::Aad::from(RdpWorkspaceVault::aad(&record).unwrap()),
                &mut *encrypted,
            )
            .unwrap();
        record.ciphertext_b64 = URL_SAFE_NO_PAD.encode(&*encrypted);
        assert_eq!(*vault.open(record).unwrap().password, password);
    }
    ...
    fn wrong_key_and_modified_bindings_are_rejected() {
        let vault = vault();
        let record = vault.create("app-1", "node-1", "device-1").unwrap();
        assert!(RdpWorkspaceVault::from_key(&[8u8; 32])
            .unwrap()
            .open(record.clone())
            .is_err());
        for index in 0..6 {
    ...
    fn generated_accounts_passwords_and_nonces_are_distinct() {
        ...
        assert_ne!(
            *vault.open(first).unwrap().password,
            *vault.open(second).unwrap().password
        );
    }
