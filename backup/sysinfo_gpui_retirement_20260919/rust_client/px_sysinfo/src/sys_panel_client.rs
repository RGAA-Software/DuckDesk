        ...
        tokio::spawn(async move {
            loop {
                ...
                *self_sender.lock().await = None;
                ...
                let web_socket_stream =
                    match timeout(Duration::from_secs(5), connect_async(address.clone())).await {
                        Ok(Ok((mut stream, _response))) => {
                            ...
                            if let Err(handshake_send_error) = timeout(
                            ...
                            {
                                tracing::error!(
                                    "handshake send timeout or failed: {:?}",
                                    handshake_send_error
                                );
                                None
                            } else {
                                Some(stream)
                ...
                if let Some(stream) = web_socket_stream {
                    ...
                    *self_sender.lock().await = Some(sender);
                    ...
                    tokio::spawn(async move {
                        loop {
                            if let Some(sender) = &mut *sender.lock().await {
                                ...
                                let encrypted_system_information =
                                    gSysInfoMgr.lock().await.load_system_info_as_encrypt_json();
                                let elapsed = start.elapsed();
                                tracing::info!("used: {}ms", elapsed.as_millis());

                                let payload_len = encrypted_system_information.len();
                                let send_start = Instant::now();
                                match timeout(
                                    Duration::from_secs(10),
                                    sender.send(TungsteniteMessage::Text(
                                        encrypted_system_information.into(),
                                    )),
                                )
                                .await
                                {
                                    ...
                                    Ok(Err(send_error)) => {
                                        tracing::error!("send info failed, break: {}", send_error);
                                        break;
                                    }
                                    Err(_) => {
                    ...
                    while let Some(received_message) = receiver.next().await {
                        match received_message {
                            Ok(TungsteniteMessage::Binary(_binary_payload)) => {}
                            ...
                            Err(receive_error) => {
                                tracing::error!("Error receiving message: {}", receive_error);
                                break;
                            }
                            _ => {
