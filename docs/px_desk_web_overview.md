# px_desk 站点（px_desk_server + web/px_desk）现状记录

> 记录时间：2026-07-18。本文档记录 GoDesk 官网站点（前端 `web/px_desk` + 后端 `rust_server/px_desk_server`）的架构现状，作为后续网页优化调整的基线。
...
- 一键部署脚本：`rust_server/scripts/deploy_desk_server.sh`
  ...
  - 构建前端 `web/px_desk` → scp 二进制与 static.tar.gz → 备份旧二进制（`*.bak.时间戳`）→ pkill 旧进程 → nohup 启动 → 验证端口与 HTTPS 200
  - SSH 免交互：`SSH_ASKPASS` + `StrictHostKeyChecking=accept-new`，密码读自 `px_desk_server/tencent_server.txt`
- 2026-07-18 首次部署：旧进程 `px_off_site`（139 天）已替换为 `px_desk_server`，旧二进制保留备份
...
- 鉴权：exe 旁 `px_desk.toml`（`[admin] password`，首次启动自动生成随机密码落盘，不进 git）；请求头 `X-Admin-Token`；`POST /api/v1/admin/verify` 登录校验；未授权返回 606
...
- 管理密码仅存于服务器 `/root/off_site/px_desk.toml`
