...
1. 清理鉴权冗余。四种正式类型（远控、RDP、Game、WebView）的 Native 启动全部迁移出 Console 一次性连接票据，删除 Native
   签发、传递、兑换、过期和旧票据重连路径；保留 Web/RTC 与 Relay 使用的独立短期授权、Console 登录/节点管理、服务间鉴权、Render 设备密码鉴权和 Relay 必要
   安全边界。ID、IP、`link://` 三种入口在启动实际 Client 前完成地址解析、密码验证、在线/占用检查和能力检查。密码验证成功后
...
- 删除活动 ImGui Panel 的设备/应用票据启动路径、正式 Client 的票据启动参数及票据重连，以及 Native 用户可见的“票据已过期”错误路径。
  浏览器 Web/RTC 与 Relay 的短期授权属于不同安全边界，继续保留，不能因清理 Native 冗余而降级为设备密码明文直传。
...
- Web/RTC、Relay 的短期授权与兑换仍保留，因为它们是浏览器和转发边界的必要安全机制，不属于本次删除的 Native Console 票据。
