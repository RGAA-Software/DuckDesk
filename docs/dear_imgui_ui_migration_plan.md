...
Pixels 桌面端统一采用 [Lucide](https://github.com/lucide-icons/lucide) 的 24×24 SVG 视觉体系；首批几何核对 revision 固定为
`a79b2d131dab2bf20cb224bd0937b439a9c4fa99`。Lucide 使用
[ISC License](https://github.com/lucide-icons/lucide/blob/main/LICENSE)，引入或更新图标时必须保留来源和许可记录。Web 与支持 SVG
资源的原生平台优先直接使用上游 SVG。Dear ImGui 不原生解析 SVG，Panel/Client 通过 `px_ui/vector_icon.*` 将所需 Lucide SVG
几何集中映射为 ImDrawList 矢量绘制：它不生成位图、不增加各渲染后端的纹理上传分支，跨 DPI 仍保持清晰。

页面不得继续用 `_`、`[]`、`X`、Unicode 符号或临时文字模拟图标。标题栏、导航和高频动作使用“图标 + 本地化文字”；窗口按钮等
上下文明确的紧凑动作可以只显示图标，但必须提供 tooltip 和稳定的 ImGui ID。图标枚举、绘制和按钮布局属于 `px_ui`，业务页面只选择
语义图标，不复制 path 几何或颜色。首批覆盖窗口最小化/最大化/还原/关闭、导航、登录退出、复制、打开、刷新、连接、控制、观看、停止和更多操作。

Panel 对外展示的浏览器入口统一为 `/web/`，发布目录仍可保留内部名称 `web_client`；页面生成器、Render HTTP 路由、Console 调度和测试脚本
必须使用同一个对外路径，避免显示地址与实际服务路由不一致。

### Panel 身份区与设备列表改造（2026-09-12）

本轮按以下顺序完成，不能用静态样例或仅能展示的页面代替真实业务接入：

1. 本机身份区把设备 ID、设备名和临时密码做成明确的“值 + 动作”行。ID 支持复制；设备名通过编辑图标打开居中弹窗，通过设备 ID
   与当前临时密码摘要调用节点自改名接口，Console 更新成功后再同步本地身份；不得误用需要网页管理员会话与 CSRF 的后台改名接口。临时密码支持复制，并只用
   眼睛/闭眼图标切换显示状态。所有复制操作使用完整原值，不受界面省略显示影响。
2. 左侧删除品牌副标题和“账户”说明块，改为居中的默认头像与用户名纵向排列。未登录显示 Guest，点击直接打开登录框；已登录点击打开账户菜单，
   保留退出登录能力。导航项仍使用统一图标和选中态。
3. 管理服务在线状态不再占用本机身份区，合并到“远程设备”标题后并以括号显示。
4. 在“远程控制”下新增“设备列表”页。页面采用搜索/刷新、左侧设备列表、右侧选中设备详情与常用操作的主从布局；数据包含 Console 当前设备和
   曾成功连接的本地历史。列表以设备 ID 为持久化主键，Console 状态覆盖历史状态，禁止按显示名追加造成重复。
5. 设备历史由独立配置接口读写，成功连接后 upsert，删除时同时清理偏好与历史。自动注册的硬件指纹对网卡地址排序去重，避免网卡枚举顺序变化
   产生新设备。自动设备名为 `MC-{首选局域网 IPv4 最后一段}`；用户手工改名设置显式标记，后续启动和重新注册不得被自动命名覆盖。

验收包括：本地与 Console 改名一致、完整值复制、密码显示状态持久化、Guest 登录入口、设备搜索/选择/启动、重复刷新不增项、Console
设备状态变化与历史合并正确，以及清空设备登记后 Panel 可自动恢复唯一设备身份。
...
- Panel 启动后通过管理服务的 Panel 长连接注册本机身份并每秒发送在线心跳；断线后固定间隔重连，不使用指数退避。远程设备列表的
  在线状态只采用管理服务返回的实时状态，不再把“曾经连接过”或“当前有媒体会话”误当成在线依据。设备列表每 2 秒自动刷新，
  同时保留手动刷新入口。
- “远程控制”页采用两列设备卡片，只显示按最近成功连接时间倒序排列的前 6 台。卡片第一行优先显示格式化后的 9 位设备码，
  没有设备码时显示 IP；第二行显示设备名称。双击卡片直接开始控制，右键菜单提供开始控制、仅观看、编辑设备、文件传输和删除设备。
- “远程控制”和“设备列表”共用同一个设备状态端口与操作组件。重命名、连接时间、连接偏好和删除结果在两页同步；删除是当前用户本机的
  列表移除操作，会同时删除该设备的最近连接记录和显示偏好，并保存隐藏标记，避免下一次管理服务刷新时立即重新出现。再次通过设备码、
  IP 或 `link://` 成功连接后会自动恢复该设备。
- “设备列表”保留搜索和主从详情布局，设备行支持双击控制及与最近设备卡片相同的右键菜单，详情区也提供控制、仅观看、文件传输、复制、
  编辑和删除操作。
