...
| CM-TRANSFER | 文件传输历史、终态与失败原因 | 部分迁移 | managed transfer history + `SecurityInternal` | Render→Service→Console 的真实生产链、单调进度、实际文件/清单 SHA-256、哈希终态和 unknown 状态已接；Render 活跃期间的开始/进度/终态使用同一幂等键和同一 sequence 重试，收到权威确认后才推进。仍需公网双端独立字节校验、取消/重试、跨 Render 进程的持久补报和客户端展示验收 |
