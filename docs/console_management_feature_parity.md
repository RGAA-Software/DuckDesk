...
| CM-RECORDING | 录像目录、直读/拉取、下载到本机或 Console、删除 | 部分迁移 | recording catalog + private cache/read-lease service + 管理/本人录像页 | Render 完成段、Service 登记/回传、Console 私有缓存、Range 下载、管理员与会话 owner 页面以及 Console 副本保留/释放/驱逐已接通；自动化覆盖 hash、并发、读取租约、撤销、CAS 和 pinned/在读拒绝。公网真实录像的本人有数据浏览器下载已通过；隔离 PostgreSQL、真实 node-control、原生 Console 和 Chromium 已验证管理员 Retain、pinned 禁止驱逐、Release、确认 Evict。仍需公网真实录像上的管理员管理动作验收。旧 URL ticket 不恢复 |
