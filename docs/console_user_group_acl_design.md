# Console 用户、用户组与资源访问控制设计

> 状态：v6.6 已将个人设备改为登录用户共享资源：有效账号可连接全部 Console 设备并使用文件传输；用户组继续只控制私有应用（2026-08-22）。
...
- 本期不做多租户；一个 Console 部署只服务一个组织。所有集合默认属于同一租户。
...
- Panel 本地游戏库不是 Console 云端应用，不纳入本设计。
...
| 主体 | 身份来源 | 可进入的界面 | 权限边界 |
|---|---|---|---|
| 游客 `guest` | Console 签发的匿名访客会话 | `/user/**` 公共区域、公开应用 | 只能查看/启动 public 应用，受限流和配额约束 |
| 终端用户 `user` | `c_user` | Panel、`/user/**` 用户门户 | 使用全部 Console 设备、获授权应用和自己的实例；不能进入管理后台 |
| Console 管理者 `admin` | license 管理账号 | `/` 登录、`/home/**` 管理后台 | 管理用户、组、设备、应用和全部实例 |
| 内部服务 `service` | Console 与 px_service 的服务凭据 | 无人工界面 | 设备上报、调度、票据兑换；不能伪装成 user/admin |

“终端用户不能登录 Console Web”统一解释为：终端用户不能登录 **Console 管理后台**，但可以登录与后台同源部署的 `/user/**` **用户门户**。
...
| 应用策略 | 游客 | 已登录但无 ACL | 已登录且有 ACL | Console 管理者 |
|---|---|---|---|---|
...
3. `public` 的含义仍是“允许游客使用”，但不等于无身份：Console 会创建 guest session，并执行 IP、会话和应用三级限流。
4. 无权访问 `acl` 应用时，列表中不返回；直接猜测 app_id 启动统一返回 404，避免泄露资源存在性。
5. 应用 ACL 与设备访问相互独立。所有有效登录用户均可访问 Console 设备，应用仍按 public/ACL 策略控制。
...
- `ConsoleUserProfile`、`ConsoleUserAdminView` 均不含密码、密码哈希和内部 Mongo 字段。
...
- 自助注册用户初始不属于任何用户组，但登录后立即可见并可连接全部 Console 设备；管理员分组只影响私有应用。
...
2. “远程控制”显示手工添加的远程设备和当前 Console 的全部设备；“云端应用”只显示 Console 下发的 public/ACL 应用；已有“安装的游戏”仍表示 Panel 本地游戏库，三者不得混用。
...
用户门户继续放在 `web/px_console`，与管理后台同源构建，但路由、布局、Cookie 和 HTTP client 分离。
...
3. 单 public 应用的全局并发上限；容量不足立即返回 429，客户端退避重试，不在 Console 内维护悬挂队列。
...
本次是破坏性升级，不编写旧数据迁移器，也不保留新旧双轨接口。部署前先停止 Console、Panel 测试客户端和所有相关应用实例，确认连接的 MongoDB 是测试库，然后执行一次性重置。
...
| P4：前端 | Panel 云端应用、Console 用户组管理、`/user/**` 门户、双会话隔离 | 三端只展示各自主体资源 |
...
| 7 | 自动化与部署 | Rust、Web 和客户端关键逻辑测试通过；本机 Console + Service + Render/应用完成游客与用户两套真实链路验收 |

### 12.2 v6.2 验收矩阵

| 场景 | 操作 | 预期结果 |
|---|---|---|
| Panel 页面分区 | 分别进入远程控制、云端应用、安装的游戏 | 远程控制只含设备；云端应用只含 Console 应用；安装的游戏只含本地游戏库，三类资源不串页 |
...
| 生命周期恢复 | Console/Service 重启并重新对账 | running/starting/stopping 实例恢复或进入稳定失败状态，不重复启动进程 |
...
- Console 全量 Rust 测试 118/118 通过；px_service 测试 37/37 通过，其中包含 Windows WMI 终止后延迟消失的回归场景。
- Console Web 类型检查与生产构建通过；Vitest 10/10、Chromium Playwright 4/4 通过。
- Panel `RelWithDebInfo/Official` 目标编译并链接通过，已部署到本机运行目录；Console、Web 与 px_service Release 也已部署。
...
- 远程列表只查询/对账设备并从本地数据库加载设备卡片；云端应用列表只查询 Console 应用且仅保存在内存，不写入远程设备 SQLite。
- 登录、退出或切换账号时先清空上一主体的 Console 设备/应用投影，再按新主体刷新，网络失败不会让上一用户的私有资源残留在新会话界面。
...
- Console Rust 120/120、Console Web Vitest 13/13、Chromium Playwright 常规用例 6/6 及现场 RTC 续票用例 1/1 通过；Console Web 与 Web Client 生产构建通过。部署后真实 guest 实例完成 `view` 签票、renewal 轮换、旧 renewal 重放拒绝，并由浏览器主动关闭 PeerConnection 验证自动续票、RTC 重连、视频继续播放和实例清理。
...
- Console 重启恢复覆盖 Starting/Running/Stopping：Service 心跳可把已实际启动的 Starting 推进到 Running；连续缺失超过 15 秒时 Starting 稳定进入 Failed，Running/Stopping 稳定进入 Stopped，错误统一为 `PROCESS_LOST`。
...
- 新增可重复执行的现场脚本 `scripts/test_console_restart_recovery.ps1 -AllowRestart`：创建临时 public 实例、重启工作区 Console、等待 Service 对账、验证实例仍为 Running 且可重新签发仅观看票据，最后停止实例并检查 game-hook Render 无残留。
- 最终回归：Console Rust 126/126、px_service 37/37、Console Web Vitest 13/13、Chromium 常规 E2E 8/8、真实 RTC 续票 1/1、真实 Console 重启恢复连续 2/2 通过；Release Console 与 Web 已部署，Console/px_media/px_service/桌面 Render 健康。

## 13. 测试与验收

完整的分层测试、故障注入、远端双机部署、证据留存和发布门槛见 [Console 用户、ACL 与实例恢复完整测试方案](console_user_acl_recovery_test_plan.md)。
...
1. 终端用户可登录 Panel 和 `/user/**` 门户，但不能登录 Console 管理后台。
2. 管理者与终端用户是独立主体；不恢复 `ConsoleUser.administrator`。
...
8. 设备和应用实例统一使用 Console 一次性连接票据，Render 通过 px_service 向 Console 原子兑换。
9. appkey 永不承担 user/admin 身份；私有连接不可在 Console 故障时降级到静态密码。
10. P0/P1 先于用户组 UI，P3 是 ACL 安全上线阻塞项。
11. 测试阶段采用破坏性数据重置，不迁移旧身份/ACL 数据，也不保留旧认证与授权接口。
12. Panel 的远程设备、Console 云端应用和本地游戏是三个独立资源域；底层连接能力可以复用，导航、列表模型和持久化边界不得合并。
...
- Console 重启从 Mongo 恢复 starting/running/stopping，等待 Service 心跳对账；对账窗口内不重复启动。
...
- 自助注册必须通过 guest session 和限流；Web 请求同时校验 CSRF，Panel 使用 guest bearer token；注册成功后创建无用户组的普通用户，并立即拥有全部 Console 设备的连接和文件传输权限。
- 管理员可生成随机初始密码或提供满足规则的密码。随机密码使用至少 96 bit CSPRNG，不包含易混淆字符。
- 管理员创建或重置的密码可直接登录，访问全部 Console 设备及用户组授权的私有应用，不设置首次登录强制改密状态。
