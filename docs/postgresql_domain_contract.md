...
节点硬件/容量的第一份实现使用 `node_telemetry_latest` 与 `node_gpu_latest` 保存最新快照。每份机器快照绑定 node generation、
...
利用率、显存及编码器压力，故这些列保持 NULL，P3 调度不得将其解释成 0 或空闲。时间序列、阈值告警、断线补报和逐 GPU 预约仍是
后续领域，不因 latest 表和管理页面存在而视为完成。
