...
- 登录接口已补 router 级测试：
  - 空 body 返回 400，且不访问数据库。
  - 非法 JSON 返回 400，且不访问数据库。
  - 缺字段返回 400，且不访问数据库。
- 授权管理：
  - 创建授权成功。
...
- 已覆盖 HTTP client：
  ...
  - HTTP 500 不清理 token、不跳转。
  - 网络无 response 不清理 token、不跳转。
- 已覆盖 router guard：
...
- 已覆盖授权输入校验纯函数：
  ...
  - 修改授权的非法边界。
- 已覆盖登录页组件：
  - 登录成功保存 token 并跳转 `/main`。
  - 业务失败不保存 token，显示后端错误。
  - 请求异常不保存 token，显示兜底错误。
- 已覆盖主页面组件：
  - admin 用户显示创建授权按钮。
  - visitor 用户隐藏创建授权按钮。
  - logout 请求失败时仍清理本地 token 并返回登录页。
- 已覆盖授权列表组件：
  - 首次加载使用后端 `total`，并计算剩余天数。
  - 列表加载失败时清空表格和 total，并显示错误。
  - 搜索按名称走 `/query/authorization/like/name`。
  - 搜索为空时恢复当前分页的全量列表。
  - page/page_size 变化后按最新分页重新查询。
  - 修改授权成功后按当前分页刷新并关闭弹窗。
  - 修改授权非法输入时不调用更新 API。
  - 修改授权请求未结束时重复保存只发送一次更新请求。
- 已覆盖创建授权组件：
  - 创建成功后关闭创建弹窗，展示 deploy 信息，并触发列表刷新。
  - 非法输入不调用创建 API。
  - 创建失败显示后端错误。
  - 创建请求未结束时重复点击只发送一次创建请求。
  - deploy 信息复制成功写入剪贴板并提示。
  - deploy 信息复制失败显示错误。
  - deploy 信息下载生成 `auth-{name}.info` 文件。
- 已增加重复点击保护：
  - 登录请求未结束时重复点击只发送一次登录请求。
  - 创建授权请求未结束时重复点击只发送一次创建请求。
  - 保存授权请求未结束时重复点击只发送一次更新请求。
  - logout 请求未结束时重复点击只发送一次退出请求，本地 token 仍会清理。
