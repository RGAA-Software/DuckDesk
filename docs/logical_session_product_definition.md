...
4. **双客户端验收**：使用 Console 当前配置的公网节点，以一主控一观察者验证禁止接管、允许接管、5 秒重连窗口、UDP 回退、文件传输隔离和 Console 展示。游戏与 WebView 还应分别验证一操作员一观看者、接管后的输入终点，以及 WebView 观看者不获得桌面/本地文件能力。验收不得写死节点地址、设备 ID 或端口。
...
- 旧固定节点、旧票据流程和旧安装包的验收记录已删除。当前公网/NAT、接管、Observer、文件传输隔离和产品安装矩阵必须按第 11 节重新执行。
