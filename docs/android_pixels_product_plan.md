...
- 一级导航使用设备、传输和设置三个平级根目的地；跨 Tab 以一次原子栈替换进入目标根页，不恢复已经离开的详情、弹层或临时状态，重复点击根 Tab 不重复入栈，
  点击当前栏 Tab 会从子页回到该栏根页。应用列表是设备栏的子页并保留设备 Tab 选中态，顶部和系统返回均回设备根页；一级根页的系统返回遵循 Android
  约定退出应用，不把 Tab 访问顺序伪造成页面历史。远控与远控文件任务是无底栏的独立会话栈，结束会话固定回设备根页。
