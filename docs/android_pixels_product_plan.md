...
Android 已具备设备接入、远控音视频、完整输入、已有显示器切换、远程应用、文件/剪贴板、录制、语音和诊断等主功能。达到最终产品标准前还剩：

- 补齐按设备保存的画质、分辨率、码率和 codec，以及连接前能力范围校验。首选帧率、远端音频、默认输入模式和自动/软件解码策略已按设备保存并真实作用于连接；触控灵敏度与手柄布局仍是全局偏好。
- 完成 WebRTC/原生路径的 codec 能力协商和连接前范围校验。原生串流的 MediaCodec→FFmpeg 软件解码回退、实际解码器展示及双解码器失败提示已完成。
- 使用已登录 Console 账号完成远程应用启动/停止和实例票据实测，并完成 UDP Direct、Relay、WebRTC Direct、ICE/TURN 在 Wi-Fi、蜂窝、弱网与切网场景的真机矩阵。
- 在真实 WebRTC/Relay 会话中完成文件、文件型剪贴板、双向语音以及带远端音频录制的端到端验收；验证来电、锁屏、蓝牙/有线耳机切换和系统资源压力。
- 执行功耗、温度、内存、包体和启动性能收口，并在 API 31 至当前系统、手机/平板/折叠屏和 200% 字体矩阵中运行无障碍及仪器化测试。
- 注入正式签名证书和批准摘要，提供最终 FFmpeg 源码及 LGPL 可重链接归档，完成法律复核、正式 APK/AAB、符号/清单归档与候选包安装验收。
...
- 完成隐私、许可、签名、混淆、符号和发布元数据。**工程链路和许可证随包门禁已完成；最终 LGPL 归档、法律复核与正式证书待发布前验收**
