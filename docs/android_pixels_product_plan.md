...
> 状态：M0、M1、M2 已完成；M3 的桌面输入已实现并通过 USB 真机验收，手柄与多显示器继续实施
> 更新日期：2026-09-06  
...
状态：**进行中（2026-09-06 已完成直接触控、触控板、物理鼠标、物理键盘、软键盘快捷键与安全注意序列）**。
...
- Android 当前如实声明视频、远程音频与桌面输入能力；手柄、多显示器、文件、剪贴板、录制、语音和 WebRTC 仍按 M3–M5 实施，
  未完成的能力不在握手后伪装为可用。
...
这次验收关闭了 M2 的旋转/Surface 重建、前后台和切网恢复门禁，并验证了 M3 桌面输入主路径；它不代表 M3–M6 的手柄、多显示器、
文件/剪贴板/录制/语音、完整网络矩阵和发布工作已经完成。
