...
- 一级导航使用独立的保存/恢复状态栈；远控文件任务是会话详情页而非一级 Tab。结束会话固定返回设备页，一级页面系统返回也回到设备页，底栏只在
  设备、应用和设置三个真正的一级目的地显示，避免切 Tab 或从详情返回时落入错误页面。
...
它不代表 M3–M6 的虚拟显示成功创建、远程应用 Console 实测、目录浏览、带音真机录制、UDP 媒体交付、Relay/WebRTC 网络矩阵、正式签名、
发布合规材料和设备矩阵已经完成。
