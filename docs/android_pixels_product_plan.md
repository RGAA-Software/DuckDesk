...
- 一级导航使用设备、传输和设置三个平级根目的地；跨 Tab 按独立返回栈保存并恢复页面状态，重复点击根 Tab 不重复入栈，点击当前栏 Tab 会从子页回到该栏根页。
  应用列表是设备栏的子页并保留设备 Tab 选中态，顶部和系统返回均回设备根页。远控与远控文件任务是无底栏的独立会话栈，结束会话固定回设备根页；
  传输/设置根页使用真实导航栈返回设备页，不再以页面级 BackHandler 伪造跳转。
