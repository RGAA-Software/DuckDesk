...
> 状态：Native 传输与入口精简、SDK 抽离已实施；解码启动花屏已修复并覆盖安装，USB 真机账号直连、实际画面及前后台短测通过；工具能力仍待原生路径实测；M6 本机长期签名已完成，发布归档待验收；完整视频能力协商暂缓
> 更新日期：2026-09-07
...
1. **P0，主路径验收补全**：Android RTC、原生 Relay、旧 UDP/KCP 与 WS 视频回退已退出活动 SDK；USB 真机已短测局域网直连、
...
5. **P2，发布前清理**：首轮旧源码、无引用资源、无用依赖和过期兼容分支审计已完成；正式候选前仍需基于最终代码再跑一次相同门禁。
6. **P2，正式发布输入与候选包**：50 年 Pixels 本机签名和批准摘要已生成并接入被 Git 忽略的 Gradle 本机配置；仍需提供最终 FFmpeg 源码及
...
强制要求对应源码包和可重链接目标包；缺失时 release 任务立即失败。2026-09-07 已生成独立的 Pixels RSA-4096、SHA256withRSA、50 年本机产品签名，
证书 SHA-256 为 `C8C97549200D26FC44E17DA5694C8B1FBE80321417F924F1BDD42A863A60A0D0`，密钥库和密码仅写入被 Git 忽略的本机 Gradle 配置。最终 FFmpeg 源码/重链接归档、法律复核、
设备矩阵和正式候选安装尚未完成，不能将当前结果标记为正式发行。**

- 完成隐私、许可、签名、混淆、符号和发布元数据。**工程链路、许可证随包门禁和本机长期签名已完成；最终 LGPL 归档、法律复核与签名密钥离线备份待发布前验收**
...
- M4 录制直接订阅 SDK 收到的 H.264/H.265 与 Opus 编码包，在独立串行线程复用 `px_media_record` 封装 MP4；开始和分段时主动请求关键帧，停止或
  ...
  暂存目录限制在应用缓存并清理超时残留，不申请旧外部存储权限。标准 WebRTC 路径复用同一状态、发布和通知层，把已解码 VideoFrame 与远端 PCM 送入有界
  串行队列，经 Android MediaCodec AVC/AAC 编码为双轨 MP4；录制不依赖 MediaProjection，系统声静音只调整播放音量，不截断录制 PCM。
...
- 一级导航由单一 `AppDestination` 状态机管理设备、传输和设置三个平级根页，不再同时维护 Compose 页面状态与 Navigation 回退栈。跨 Tab 直接替换为目标根页，
  不恢复已经离开的详情、弹层或临时状态；点击当前栏 Tab 会从子页回到该栏根页。应用列表是设备栏的子页并保留设备 Tab 选中态，顶部和系统返回均回设备根页；
  传输和设置根页的系统返回固定回设备首页，设备首页再次返回才退出应用，不把 Tab 访问顺序伪造成页面历史。异步设备票据和远程应用启动结果仅在发起它的页面仍处于前台时
  才允许进入远控，用户切换 Tab 或返回后到达的旧结果会被丢弃。远控与远控文件任务是无底栏的显式会话状态，结束会话固定回设备根页。
...
- release 构建只接受 CI 环境变量或被 Git 忽略的本机签名配置；缺少完整配置时打包任务立即失败。静态 FFmpeg 发布还必须提供对应源码 ZIP 和包含
  可重链接对象与说明的 relink ZIP，流水线会校验其内容，并将二者、第三方 notices、mapping、arm64 native symbols 和 SHA-256 清单一起归档；
  APK/AAB 内每一项要求的许可证资源也会被逐项校验。流水线同时执行 release lint、单元测试、R8/resource shrink 和签名验证。一次性测试证书验证中，release APK 从约 298 MiB 降至
...
- 2026-09-07 导航层级复核修正了设备图子页被误判为非一级界面的问题。底栏和选中态统一由 `NavDestination.hierarchy` 决定，设备图内的应用页保留
  Tab 导航，返回拦截只作用于传输/设置根页，子页继续使用系统逐级返回。后续实机复查发现包含式移除当前导航图会丢失嵌套目的地，现统一在根起点
  `saveState`、切入目标图时 `restoreState`；重复点击当前 Tab 仍只回到本图根页。debug APK 使用 `adb install -r -d` 覆盖安装；设备→应用→传输→设备
  恢复应用、重复点击设备 Tab 回设备根页、应用页顶部/系统返回、设置页系统返回以及快速连续切换均通过，无页面层叠、错选 Tab 或 `AndroidRuntime`
  崩溃。单元测试通过，最终 APK SHA-256 为 `A7868FEE291B5E9D99AD0789DFAB8FC7391B6A5226F97B213574006F33EE30D6`。
- 2026-09-07 最终导航交互取消嵌套图的子栈保存/恢复，设备、传输和设置改为平级根目的地；切换 Tab 总是清理旧根页和详情，不再恢复已过期的应用页。
  远程应用、远控工作区和会话文件页统一为无底栏的全屏详情，顶部返回与系统返回都落到明确父页。USB 真机覆盖安装后验证设备→传输→设置→设备、
  设置系统返回→设备、快速连续切换、应用页顶部/系统双返回以及远控系统返回→设备；没有页面叠加、错选 Tab、旧页恢复或导航崩溃。Android 全套单元
  测试、lint、arm64 C++ 增量构建和 debug APK 构建通过，最终 APK SHA-256 为
  `595B5F7E821E3F5E19F2CFEFA8EAF277BFB556C9C5F6EDFB86A6098D50FF7007`，安装使用 `adb install -r -d`，未卸载应用。
...
- 2026-09-07 M6 许可材料将 Android 使用的 Apache 2.0、Boost 1.0、BSD、LGPL 2.1、GLM、zlib 等完整文本和组件清单作为 raw resource 随包，设置页直接读取
  这些同源材料，不再展示容易失真的许可证摘要。release 构建新增静态 FFmpeg 合规硬门禁，必须提供实际对应源码和可重链接对象归档；发布脚本还会校验归档
  内容及 APK/AAB 中每一项 notices，缺少材料的 release dry-run 已按预期失败。Android 全套单元测试、lint、arm64 native 和 debug APK 构建通过；最终归档、
  正式证书与法律复核仍是发布前置条件。
- 2026-09-07 导航再次按产品交互收口：设备、传输和设置恢复为三个真正独立的导航图，Tab 切换保存各栏页面和滚动状态，重复点击当前 Tab 回本栏根页；
...
- 2026-09-07 导航按最新实机反馈从“跨栏清空”调整为每个一级 Tab 保存和恢复自己的返回栈；设置与传输页移除脱离栈状态的返回拦截，应用子页仍可由顶部、系统返回或
  重复点击设备 Tab 回到设备根页。Xiaomi 22021211RC 覆盖安装后验证设备→设置→传输→设置→系统返回，页面内容和唯一选中 Tab 始终同步，最终准确返回设备首页，
  全程无 Java/JNI/native fatal。Compose 导航回归、app 单元测试、lint 和测试 APK 编译通过。
- 2026-09-07 M5 标准 WebRTC 录制已接入无屏幕捕获权限的本地媒体链路：接收 VideoTrack/AudioTrack sink，串行完成 EGL→H.264 与 PCM→AAC 编码，等待双轨格式后
  统一写入 MP4，并限制待处理帧、音频包和编码样本内存。开始、停止、轨道更换、编码失败和会话销毁均按类型化事件收尾，录制能力只有在 `view` 权限、所需媒体轨道和
  平台编码器同时就绪时才显示。设备内 Debug MediaCodec 门禁通过，生成 `audio/mp4a-latm` 与 `video/avc` 双轨、510 ms、19906 字节的可读取 MP4；最终 APK 与手机已安装
  `base.apk` SHA-256 均为 `C35576CF98D5D2D9330E26D00DBD7E1C311B75312B0C1CD7029EEFBF409D5251`。手机未登录 Console，因此真实远端画面与声音录制仍归入网络矩阵待验收。
- 2026-09-07 导航子栈复验发现平级叶子路由只能保存一级页面状态，设备栏的应用子页在跨 Tab 后会丢失。设备、传输、设置现改为三个嵌套导航图，Tab 选中态按目的地
  层级判断；跨 Tab 恢复各栏完整子栈，重复点击当前 Tab 回该栏根页，应用页顶部/系统返回逐级回设备首页。新增应用子页跨 Tab 恢复及返回回归用例；Xiaomi
  22021211RC 使用 `adb install -r -d` 覆盖安装后验证应用→设置→设备恢复应用、重复点击设备回首页，以及应用/设置/传输系统返回，均无串栈或重复页面。Android
  单元测试、debug APK 与仪器化测试源码编译通过；构建 APK 和手机已安装 `base.apk` SHA-256 均为 `A472E97E37C15EE490B9AA5FD7FC3AA02F9A2221728B2C02DCF0A6EBFAE4CDF6`。
- 2026-09-07 最新真机反馈确认恢复旧 Tab 子栈仍会把用户带回已经离开的页面，最终交互改为切换一级 Tab 必定打开该栏干净根页；应用子页顶部/系统返回回设备页，
  一级根页系统返回直接离开应用，Tab 不再充当返回历史。远控工具栏同时从覆盖视频的半透明层改为不透明的占位顶栏，收起和展开均先参与布局，视频 Surface 从工具栏
...
- 2026-09-07 页面跳转与返回再次按真机反馈收敛：删除 AndroidX Navigation 依赖及嵌套图/页面状态的双重来源，由单一可保存的 `AppDestination` 明确表示三个一级根页、
  设备应用子页、远控工作区和会话文件页。切换或重复点击 Tab 都直接进入该栏根页；应用返回设备、会话文件返回远控，顶层返回退出应用。会话状态持续观察只在真实活动会话
  首次恢复远控页，用户确认结束后以退出门禁抑制 `Stopping` 状态造成的瞬时反跳，并在 `Idle` 时回设备页。Xiaomi 22021211RC 使用 `adb install -r -d` 覆盖安装后，
...
- 2026-09-07 页面导航按最新真机反馈修正两类行为：设置和传输根页的系统返回先回设备首页，设备首页再次返回才退出；设备票据和远程应用启动等异步结果增加
  来源页面门禁，用户已经返回或切换 Tab 后到达的旧结果不再强制打开远控。切换或重复点击 Tab 仍只打开目标根页，不恢复旧子页。Compose 回归同步覆盖二级根页返回，
...
- 2026-09-07 按设备连接偏好第一批完成。设备卡“连接偏好”可保存 30/60 FPS、远端音频默认状态和直接触控/触控板/游戏手柄默认输入模式；配置使用独立 DataStore，
  Direct 与 Account 设备使用稳定设备标识隔离。连接请求携带不可变偏好快照，原生 UDP/Relay 通过新增 typed JNI 命令、标准 WebRTC 通过可靠媒体 DataChannel 下发
  `kModifyFps`，连接和重连都会重新应用；会话内音频和输入模式变更同步写回当前设备。无协议支持的码率、codec 和缩放选项没有以假 UI 暴露。领域、存储降级和 RTC
  协议单元测试、Android 全套单元测试、lint、仪器测试源码编译、arm64 native 增量构建及 C++ 所有权门禁均通过。Xiaomi 22021211RC 使用 `adb install -r -d`
  覆盖安装后，设置 30 FPS/默认静音/触控板，退出并重开对话框仍保持；真实 Direct 会话以触控板启动并稳定到 26 FPS（21 ms），证明偏好不是仅保存的占位状态。
  默认静音仍会协商远端音频能力，工具栏显示“播放远程声音”且可在同一会话恢复播放，避免把默认播放状态误作永久禁用音频轨道。
  随后已恢复 60 FPS/开启音频/直接触控。构建 APK 与手机已安装 `base.apk` SHA-256 均为
  `82D687011C48B53FF0B3EB226623F78614317039BF2C095999B0255EA49F5D9E`，全程未卸载应用且无 Java/JNI/native fatal。
- 2026-09-07 原生视频解码回退闭环完成。Android 新增 RAII 管理的 FFmpeg 软件解码器和 ANativeWindow RGBA 渲染器；MediaCodec 初始化失败或连续解码失败时
  自动切换软件路径，软件路径也无法解码时才上报类型化 `DecoderUnavailable`，界面明确说明硬件与软件解码均失败。工作区实时统计展示实际解码器，统计移到设备标题
  下方的独立横向区域，避免长解码器名称挤压标题。arm64 native 增量构建、Android 单元测试、lint、debug APK、仪器化测试源码编译及 C++ 所有权门禁通过。
  Xiaomi 22021211RC 的真实 Direct 会话默认使用 `MediaCodec hardware`；临时强制软件路径后以 `FFmpeg software H.264` 正常渲染 1920×1080 桌面并达到 37 FPS，
  随后恢复硬解优先并再次覆盖安装。最终构建 APK 与手机已安装 `base.apk` SHA-256 均为
  `92D37335FD26D3C66C921A66622C73ABE845FE80021A4DE03856F85488ADF43A`，全程未卸载应用且无 Java/JNI/native fatal。
- 2026-09-07 按设备解码策略完成。连接偏好增加“硬件解码优先”开关：开启时原生链路使用 MediaCodec 并保留自动软件回退，关闭时直接使用 FFmpeg；标准
  WebRTC 同步选择默认解码工厂或纯软件解码工厂，统计明确显示当前策略。策略随 Direct/Account 设备键持久化并在每次新会话创建前固化到请求快照。领域默认值、
  DataStore 有效值/损坏值降级和 RTC 统计策略均有单元测试。Xiaomi 22021211RC 对话框布局、关闭开关后的持久化重开、真实 Direct 软件会话及恢复默认均通过；
  保存设置后的会话显示 `FFmpeg software H.264`，1920×1080 实际画面达到 48 FPS。构建 APK 与手机已安装 `base.apk` SHA-256 均为
  `B948A96FAF876DF9E07023D1335477D53617066E145DA869FA0B5EBD084D7422`；全程仅使用 `adb install -r -d` 覆盖安装，无 Java/JNI/native fatal。
...
它不代表 M3–M6 的远程应用 Console 实测、完整 Relay/WebRTC 网络矩阵、最终 FFmpeg 源码/重链接归档、正式法律复核和设备矩阵已经完成。

## 14. 跨平台视频能力探测与协商（已规划，暂缓实施）

### 14.1 决策与边界

- 该能力是后续独立的跨平台媒体控制面项目，当前阶段只保留设计，不修改协议、Windows Render、Windows Client 或 Android 运行代码。
- “Android 不做旧版兼容”不表示忽略终端硬解上限。后续有效配置必须取 Windows Render 编码能力、观看端解码能力、采集源、用户偏好和实时网络预算的交集。
- 统一的是数据模型、协议和选择规则，不强行共用一份平台探测实现。Android 使用 MediaCodec，iOS 使用 VideoToolbox，Windows Client 使用
  D3D11/DXVA 或实际解码后端，Windows Render 使用当前实际选择的 NVENC/QSV/AMF/软件编码后端。
- 本节中的分辨率始终指**编码输出分辨率**。不得复用 `ChangeMonitorResolution` 修改远端 Windows 的物理显示器分辨率。
- 暂缓期间不展示自动/流畅/均衡/清晰四档和无法真实生效的 codec、码率、缩放控件；现有 30/60 FPS 和解码策略保持不变。

### 14.2 公共模型

公共协议后续增加以下版本化、类型化模型：

- `VideoCodecCapability`：codec、profile、level、最大宽高、受支持的尺寸/FPS 组合、码率范围、硬件加速属性和必要的像素/色彩约束。
- `VideoEndpointCapabilities`：端点角色、协议版本及编码或解码能力集合。
- `VideoQualityPreference`：自动/流畅/均衡/清晰以及经高级设置产生的用户目标；偏好不是未经校验即可执行的参数。
- `NegotiatedVideoConfiguration`：最终 codec、profile、编码输出尺寸、FPS、码率和单调递增的 `configuration_id`。
- `VideoConfigurationResult`：接受或拒绝、类型化原因、失败的候选项及可继续尝试的能力边界。
- 网络带宽、丢包、RTT、持续解码掉帧和温控属于运行时反馈，不伪装成静态硬件能力。

所有平台共享相同字段语义、候选排序和协议测试向量。平台探测结果必须来自实际后端查询；对于容易误报的硬件实现，还需要轻量创建/配置验证，不能只按设备名称猜测。
...
服务端是最终配置的权威执行者，但不能选择客户端未声明的能力。`configuration_id` 用于隔离重配前后的确认和帧状态，防止晚到结果覆盖新配置。
协商应有次数和时间上限，禁止黑屏式无限重试；恢复更高档位需要稳定窗口和迟滞，避免弱网中来回震荡。

### 14.5 选择规则与产品预设

选择顺序固定为：用户目标 → 采集源尺寸 → Render 编码能力 → Client 解码能力 → 当前网络预算 → 实际启动验证。四档预设只定义目标：

| 预设 | 选择倾向 |
|---|---|
| 自动 | 综合网络、终端性能和温控动态选择，并采用有迟滞的升降档 |
| 流畅 | 优先交互延迟和稳定帧率，主动降低输出尺寸与码率 |
| 均衡 | 默认办公场景，在清晰度、帧率和带宽之间平衡 |
| 清晰 | 在双方能力和网络预算内优先输出尺寸与画质 |

任何预设都不得越过真实能力边界。例如 Render 支持 HEVC 4K60、手机仅能硬解 H.264 1080p60 时，结果不得选择 HEVC 4K60；软件解码也无法稳定承担时，
应要求 Render 继续降低输出尺寸或帧率，而不是让客户端持续失败。

### 14.6 后续实施切片与验收

恢复该项目时严格按以下切片实施，每一片均可独立构建和回滚：

1. 公共协议、纯函数候选选择器和跨平台测试向量。
2. Android MediaCodec 解码能力探测、缓存、无效化及创建验证。
3. Windows Render 编码能力探测、配置应用和安全重建。
4. Direct 链路强制握手、失败反馈和真机验收。
5. WebRTC/Relay 接入同一控制模型及运行时有界重协商。
6. Android 四档预设、按设备持久化、高级范围校验和诊断展示。
