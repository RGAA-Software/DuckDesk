...
render 端录屏已落地：录像文件落在本机 `C:\Users\Public\Pixels\px_render_records\`（`rec_{monitor}_{YYYYMMDD}_{HH.MM.SS}.mp4`，1GB/段滚动，24 个文件上限）。目标是在 **Console Web** 上按设备查看这些录像（列表 + 播放 + 下载）。

两种部署拓扑：

1. **同网段**：Console 与所有 render（及其所在机器）在同一个局域网，可直接访问。
2. **上层网段**：Console 在上层/中心网段，管理大量分散的 render，设备在 NAT/防火墙后，Console 无法主动直连。

### 1.1 决策记录（评审后锁定）

| 决策点 | 结论 | 理由 |
|---|---|---|
| Console web 部署协议 | **局域网场景 Console 整体走 HTTP** | HTTPS Console 页面内嵌 `http://` 内容（panel 录像、render 托管的 web client iframe）会被浏览器混合内容拦截；现有 VideoWall 嵌 `http://设备IP` web client 已踩此坑。给每台设备 IP 签 SAN 证书运维成本过高 |
...
| console_server 改造 | 加 HTTP 监听配置开关 | 当前 `console_server.rs` 只绑 rustls（证书缺失直接拒启动），HTTP 明文口仅 `/ping`。需加配置项使主服务可绑纯 HTTP |
| Console 反代路由 | **v1 不做** | HTTP 下同网段直连无混合内容问题；反代唯一不可替代的场景是"web 与 panel 不同网段、但 Console 同时够得到两边"，出现时再加 `/api/v1/record/proxy`（流式透传 Range） |
...
| panel `/records` 鉴权 | **强制开启**，console 签发短时效 ticket（HMAC,§5.3） | 20369 在局域网裸露，录像内容敏感；`<video>` 无法带 Authorization 头，裸口令放 URL 会进浏览器历史/日志 |
...
| panel→console 长连接 | `/console/panel`（panel ws 客户端；console 侧 `console_server.rs:153`，HMAC token 鉴权 `console_ws_token_filter.rs`） | 双向消息通道；另有 `/console/service`（service 长连） |
| panel→console HTTP 客户端 | `HttpClient::MakeSSL`（panel 多处使用）+ `px_console_client` 库 | 鉴权上报模式现成；Console 改 HTTP 后需支持明文连接 |
| console 文件上传接口模式 | `rust_server/px_console_server/src/update/update_handle.rs:37`（multipart → 存 `uploads/` → mongo 记录 → 返回下载路径） | **按需拉取可完全仿照** |
| console 静态伺服 | `console_server.rs:94` `"/uploads"` ServeDir | `uploads/` 下文件已可直接 HTTP 播放（mp4 + Range 拖拽） |
| console 路由 | `/api/v1/record`（visit/ft 上报）、`/api/v1/update` | 新录像接口同构挂载 |
...
```
  ...
  拓扑2(上层网段): web ──► console ──经 /console/panel 隧道发指令──► panel
                        ◄── panel 按需调 console 上传接口回传文件(临时缓存) ──► web 播放
```

- 设备本地录像**保留**，沿用本地 1GB/24 文件滚动清理；console 侧**不做长期归档**。
- 拓扑 1 与拓扑 2 共用同一套 panel 端实现（HTTP 路由 + 回传能力），差异只在触发方。
- Console web 按 HTTP 部署（§1.1），拓扑 1 直连无混合内容问题；**v1 不做 Console 反代**（出现"web 与 panel 不同网段但 Console 同时够得到两边"的部署时，再加 `/api/v1/record/proxy` 流式透传）。
...
- 鉴权：**强制开启**,console 签发短时效 ticket（HMAC,§5.3），panel 本地校验；后续可接安全密码体系。
- 录制中文件（moov 未写）不可播：以 **sidecar 标记**（`xxx.mp4.recording`）为准过滤，仅暴露已完成的段/已停止的文件。**不用 mtime 启发式**（滚动写盘间隙会误判）。

### 5.2 web 端

- Console Web 录像页：设备列表（console 已知设备）→ 展开录像 → 播放地址 = `http://{panel_ip}:20369/records/{file}`。
- **panel IP 来源**：panel 在 `/console/panel` 长连接握手时上报本机网卡 IP 列表，console 存设备表字段 `panel_lan_ips`（数组）透传前端；前端逐个尝试（通常第一个即通）。**不用长连接来源 IP**（跨路由/NAT 场景拿到的是网关地址）。
- 播放：`<video>` + Range（浏览器原生拖拽）；下载按钮分两种：**下载到本机**（浏览器直下，同 URL）与**下载到 Console**（`POST /api/v1/record/download`,console 服务端从 panel 拉取落盘为保留副本，见 §6.4）。

### 5.3 拓扑选择与拓扑 1 的鉴权 ticket

**web 侧自动选择拓扑**：console 不知道浏览器的网络位置，由前端探测——拿到 `panel_lan_ips` 后对第一个 IP 发 `GET /records/info`（约 2s 超时）；通则走拓扑 1 直连，不通（或全部超时）则自动回退拓扑 2 回传流程，用户无感知。

**拓扑 1 鉴权与 `<video>` 的矛盾**：`<video>`/`<a download>` 无法携带自定义 Authorization 头，口令只能走 query string 或 cookie。v1 采用 **console 签发的短时效 ticket**：

```
web 向 console 请求播放地址 → console 用与 panel 的共享口令计算
...
- panel 侧校验是纯本地 HMAC 计算，无需回问 console（设备离线查看不受影响）。
...
```
web 请求设备 X 的录像列表/播放
  → console 经与设备 X 的 panel 的 /console/panel 隧道发送指令"列出录像 / 回传文件 F"
  → panel 读本地 px_render_records 响应列表 / 调 console 上传接口回传文件 F(带 device_id + 会话令牌)
  → console 存 uploads/records/{device_id}/ 临时目录, 写 mongo c_records(临时标记)
  → console 静态伺服该文件给 web 播放(/uploads, Range 天然支持)
  → 临时文件 TTL 到期/磁盘阈值触发清理
```

- 隧道只传**指令和小 JSON**（列表、回传请求），大文件走 panel→console 的普通 HTTP 上传接口（复用现有 multipart 模式），console 内部零流式转发复杂度。
- **回传进度机制**：console 在上传端点按 `Content-Length` 累计已收字节，写 `c_records` 进度字段；web 轮询 `list`/状态接口展示百分比，无需额外通道。
- **并发去重**：同一 `device_id + 文件名` 的 in-flight 回传在 console 侧去重（多用户同时点播同一文件只触发一次），panel 侧按设备串行上传（§7.2）。
- **已知限制：首播延迟 = 整段回传时间**（1GB 分段在百兆网约 1 分钟级）。web UI 必须显式展示"回传中 x%"进度，避免用户误判为故障；后续可演进为流式上行边下边播（不在 v1）。

### 6.2 隧道消息（/console/panel 通道内，建议新 protobuf 或 JSON 消息族）

| 方向 | 消息 | 说明 |
|---|---|---|
| console→panel | `RecordListReq{device_id}` | 请求录像列表 |
| panel→console | `RecordListResp{files[]}` | 文件名/大小/mtime/monitor |
| console→panel | `RecordFetchReq{device_id, filename, token}` | 请求回传单个文件（token 防重放） |
| panel→console | 回传动作：调用 `POST /api/v1/record/upload`（multipart，带 token） | 复用文件上传接口 |
| console→panel | `RecordFetchDone{filename, ok}` | 可选：回传完成通知/错误 |

- 消息收发挂在现有 `/console/panel` 连接处理里（panel 侧现有 console ws 客户端；console 侧现有 panel ws 会话管理），新增消息类型即可。
- **实现落地（已锁定）**：该通道本就是 protobuf 二进制帧（`ConsolePanelMessage`,`src/px_deps/px_server_protocol/console_panel.proto`，两端自动生成），新消息族直接扩 proto（`kRecordListReq/kRecordListResp/kRecordFetchReq/kRecordFetchDone` + `RecordFileInfo` 等，字段号 50-53）;`ConsolePanelHello` 增加 `panel_lan_ips`/`panel_http_port`。未采用 JSON 包层。
...
| `POST /api/v1/record/download` | **下载到 Console 机器**（§6.4）；两种拓扑统一入口，落盘并标记 `keep` |
...
### 6.4 下载到 Console（用户触发的服务端落盘与保留）

区别于"浏览器下载到操作者电脑"，此功能把录像**保存到 Console 服务器本机**，供集中留存/分发：

| 拓扑 | 数据路径 |
|---|---|
| 拓扑 1（同网段） | console 服务端作为 HTTP 客户端直连 `http://{panel_ip}:20369/records/{file}`（带口令），流式落盘，不经浏览器 |
...
- **设备侧文件不动**：下载是"拷一份到 Console"，设备本地录像保留并沿用滚动清理（§7.3）。

## 7. 关键设计点（通用）

1. **只有已完成的文件可看/可传**：以 sidecar 标记（`xxx.mp4.recording` 存在即录制中）为准，不用 mtime 启发式。
2. **panel 上传失败重试**：panel 侧按设备串行上传 + 限速（避免挤爆 console 带宽）；失败指数退避重试，不丢任务（本地待传清单）。
3. **不删除本地文件**：console 只是"临时查看缓存"（+ 用户触发的 `keep` 保留副本，§6.4），本地保留并沿用滚动清理；若未来要"归档中心"，加全量上传开关即可（同接口）。
...
| 1 | console_server HTTP 开关 | 主服务可配置绑纯 HTTP（现状仅 rustls + HTTP `/ping`）；配置项 `ssl_enable = false` 时走明文监听 | 小 | 无 |
...
| 4 | panel 上报 `panel_lan_ips` | `/console/panel` 握手携带本机网卡 IP 列表；console 存设备表 | 小 | 无 |
| 5 | panel 隧道消息 | /console/panel 新增消息族 + 回传触发（串行 + 限速 + 退避重试） | 小 | 3 |
| 6 | console rust | record/upload（仿 update_router）+ list/fetch/download 路由 + ticket 签发 + 回传进度透出 + 并发去重 + 临时清理（豁免 keep）+ mongo `c_records`（含 `keep`/进度字段） | 中 | 5 |
| 7 | web/px_console | 录像页（设备→拓扑自动探测→列表→播放→下载到本机/**下载到 Console**→回传进度与下载状态展示；H265 文件标记"仅可下载"） | 中 | 4（拓扑1）、6（拓扑2） |
...
| 鉴权 | 无 ticket/错 ticket/过期 ticket 一律 403；正确 ticket 200；HMAC 校验纯本地（不回问 console） |
...
- **下载到 Console**:`POST /api/v1/record/download` 后文件落盘 `uploads/records/{device_id}/`(md5 与设备一致)、`c_records` 标记 `keep=true`；重复下载幂等（不重传仅翻标记）；list 接口可见下载状态流转（下载中→已保留）
- 验证 `panel_lan_ips` 透传：设备多网卡时下发的列表正确
- **浏览器控制台无混合内容告警**（HTTP 部署的回归点）

### 9.4 拓扑 2 端到端（Console 与设备隔离直连）

- 设备防火墙阻断 Console→panel 方向（模拟上层网段），仅允许 panel→Console 出方向
- web 请求列表 → 隧道指令 → 列表回显
- web 点播放 → 回传触发 → **UI 显示"回传中 x%"** → 完成后可播可拖拽（走 console `/uploads`）
- 幂等：同一文件重复点播不重复回传（`文件名+大小+mtime` 命中缓存）
- **下载到 Console（拓扑 2)**：走同一回传链路，落盘后标记 `keep=true`;TTL 到期/磁盘阈值触发清理时**保留副本不被删**，手动 DELETE 后才消失
...
- Console 改 HTTP 后：登录、设备列表、WebSocket（`ws://` 自适应）、VideoWall 嵌 web client（混合内容消除，顺手修掉现有问题）回归通过
...
- **对象存储**：规模化后演进方向，v1 不上。
- **Console 反代路由**（`/api/v1/record/proxy` 流式透传）：仅在出现"web 与 panel 不同网段、Console 同时够得到两边"的部署时追加。
- **拓扑 2 流式上行边下边播**：消除首播延迟的演进方向，v1 以进度展示兜底。

## 11. 已确认项（原待确认，评审后关闭）

1. 临时缓存 TTL / 磁盘阈值：**TTL 24h + 10GB 阈值双触发**（§6.3）。
2. panel `/records` 鉴权：**强制开启，console 签发短时效 ticket（HMAC）**，口令不放 URL（§5.3）。
...
1. **ticket 双重 MD5(panel 侧 bug)**:`records_http_handler.cpp` 用 `MakeRecordsTicketKey(GetDeviceSecurityPwd())` 再 hash 一次，而 panel 存的 `device_safety_pwd` 本身就是 md5,Console 直接用 `safety_pwd_md5` 当 HMAC key → 所有 ticket 校验必然 403。修复：handler 直接用存储值；两侧各加同一组 pinned HMAC 向量测试（`cross_side_pinned_vector` / `CrossSidePinnedVector`）锁定字节级兼容。
2. **panel /records 缺 CORS 头**:Console web(https 源）跨源 `fetch` 探测/列表被浏览器拦截，前端误判直连不通而回退拓扑 2。修复：三个 handler 统一加 `Access-Control-Allow-Origin: *`（无自定义头，无 preflight;`<video>` 本就不受 CORS 限）。
...
- 浏览器直下 md5 与设备一致 ✓
- 下载到 Console：fresh→ready、`keep=true`、md5 一致、重复调用幂等（仅翻标记）✓；DELETE 生效 ✓

§9.4 拓扑 2（断 20369 模拟隔离；90 防火墙原为全关，临时开启+6 分钟自动恢复保险）:
- 自动回退"经 Console 回传"徽标，无报错弹窗 ✓
...
- ~~**HTTP 部署的前置**:panel 连 Console 长连硬编码 `wss`~~ → 已完成，见 §13(2026-08-18 全 HTTP 化）。
- ticket TTL 600s，超长视频跨过期点拖拽会 403（前端需续期或重取）。
- 直连拉取落盘 mtime 记 0（幂等键精度略低）；Console 重启后 `c_records` 可能残留 fetching 态。

## 13. 全 HTTP 化改造（2026-08-18,ssl_enable 开关贯通四端）

### 13.1 目标与设计

Console 以 `ssl_enable=false` 纯 HTTP 运行时，panel / px_service / px_client 三端原先硬编码的 `wss://`/`https://` 必须能跟随切换，浏览器不再需要 `--allow-running-insecure-content`。

开关源头是 Console 配置 `px_console.toml` 的 `ssl_enable`（缺省 true，向后兼容旧 HTTPS 部署），经 access 广播下发，逐端持久化：

- **Console(rust)**:`ConsoleServerConfig` 新增 `srv_ssl_enable`(serde 缺省 true，旧 access 串无此字段时视为 HTTPS);`get_server_config` 从 `ssl_enable` 填充。`console_server.rs` 本就有 `ssl_enable=false` 的纯 HTTP 监听分支。
- **panel(C++)**:PxSettings 新键 `console_ssl_enable`（缺省 true)+ `IsConsoleSslEnabled()`;access 解析链（`console_access_info_parser.cpp` → `ConsoleSrvConfig::srv_ssl_enable_` → `console_scanner` → `st_network.cpp::Save`）落盘；`PxConsoleClient` 重构为抽象接口 + `PxConsoleClientImpl<ClientType>` 模板（`asio2::wss_client`/`asio2::ws_client`)，工厂按开关实例化，开关变化触发长连重建；约 30 处 `HttpClient::MakeSSL` 收口为 `PxSettings::MakeConsoleHttpClient` / `px_console::MakeConsoleHttpClient` / `ProfileApi`(px_deps 两库用包级 atomic 布尔 + setter 注入，panel `Load()` 时同步）。
- **proto**:`MsgAuthInfo` 新增 `bool console_ssl = 12`(proto3 缺省 false，注入工具需显式传 true,`inject_service_auth.mjs` 已加 `--console-ssl` 参数）。
- **px_client(C++)**:`--console_ssl` 命令行（panel `running_stream_manager` 启动时透传）;`CtConsoleClient` 同样模板化按开关选 ws/wss。
- **px_service(rust)**:`console_client.rs` 按 `console_ssl` 选 `connect_async`(ws）或 `connect_async_tls_with_config`(wss，保留 NoCertVerifier);`auth_info_to_json` 带 `console_ssl` 字段。
...
原 `console_port - 1`(30499）的 ping-only 明文监听已删除，`/ping` 由主端口统一提供（主路由本就有该路由）。panel 侧 `ConsoleDeviceApi::Ping` / `CanPingServer` 本来就打主端口，无需改动；Console 控制台（egui)"打开"按钮由写死 `http://ip:30499` 改为按 `ssl_enable`/`console_port` 拼主端口 URL。验证：`30500/ping` 200、30499 连接拒绝、90 明文 ws 重连正常。
