...
| 更新目录 | UpdateStore + px_release_catalog | 管理登记/审批/撤回；平台身份；制品、仓库 publication SHA-256 与 root version 不可变；供 Android 等明确消费者发现及人工运维查询；不保存 Windows 节点信任水位或激活任务，目录存在不代表已验签/可安装 |
...
本人/管理员授权请求和有界 Range 下载。Render 已在完成 MP4 trailer/flush/close 后发送 bearer 鉴权的类型化完成事件，事件把唯一
resource-session UUID 传给 Windows Service；同段多 owner 或 owner 不明时显式写成 null，不得以设备/账号或当前连接猜测。Service 只
登记完成事件，不发现裸 MP4，并已实现当前 generation 元数据上报、一次性能力轮询和独立 HTTP 字节上传，不通过控制 WebSocket 传大文件。
