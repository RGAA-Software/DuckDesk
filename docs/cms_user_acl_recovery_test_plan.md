# CMS 用户、ACL 与实例恢复完整测试方案

版本：1.0

适用范围：当前测试阶段的 CMS、CMS Web、px_service、px_render 与 Web Client

关联设计：[cms_user_group_acl_design.md](cms_user_group_acl_design.md)
...
| 对象 | 本方案关注点 |
| --- | --- |
| CMS | 身份、ACL、应用目录、实例生命周期、票据、状态恢复、审计 |
| CMS Web | 用户门户、公共应用、管理端授权、会话失效、错误恢复 |
| px_service | 节点心跳、启动/停止命令、启动回执、CMS 重连 |
...
| L2 | 本机完整栈 | CMS、Service、Render、应用、RTC、进程恢复 | 是 |
| L3 | 两台机器/NAT | 真实网络、证书、端口映射、重连与稳定性 | 发布前是 |
| L4 | 5 分钟稳定性与故障注入 | 延迟漂移、资源泄漏、组件重启 | 发布前是 |

本机基线使用 `https://127.0.0.1:30500` 或当前局域网 CMS 地址。远端地址、账号和密钥只放在部署机的私有配置或密码管理器中，不写入仓库、测试报告、命令历史和截图。
...
- Node.js 满足 `web/px_cms/package.json` 的版本要求，已安装当前 Chrome。
- MongoDB、Redis、px_media、CMS、px_service 能按部署配置启动。
...
- 重启脚本只能操作工作区 `output/px_cms/px_cms.exe` 的精确绝对路径。
- 远端脚本禁止按进程名批量结束其他目录中的 CMS 或 Render。
...
| 标识 | 数据 | 权限关系 |
| --- | --- | --- |
| Admin1 | CMS 管理员 | 可管理用户、组、设备和应用 ACL |
...
$env:PX_LIVE_CMS_URL = 'https://127.0.0.1:30500'
...
Remove-Item Env:PX_LIVE_CMS_URL -ErrorAction SilentlyContinue
```
...
该脚本会强制重启 CMS，只能在专用测试环境执行：

```powershell
powershell -ExecutionPolicy Bypass -File scripts/test_cms_restart_recovery.ps1 `
  -CmsUrl https://127.0.0.1:30500 `
  -AllowRestart
```

验收点：实例在 CMS 重启前已经 Running；CMS 恢复后仍由 Service 心跳确认 Running；能够为原实例重新签发仅观看票据；清理后没有本轮 game-hook Render 或 Client 遗留。
...
| 编号 | 故障注入 | 预期 |
| --- | --- | --- |
| ST-01 | 实例 Starting 时重启 CMS，Service 实际仍在启动 | 恢复 Starting，后续心跳 Running 后进入 Running |
| ST-02 | 实例 Running 时重启 CMS | 恢复 Running，可重新签票，不重复启动 |
| ST-03 | 实例 Stopping 时重启 CMS | 保留 Stopping；收到停止心跳后进入 Stopped |
...
- CMS 用户入口默认签发 view-only；观察者不参与编码器码率、分辨率、帧率等控制反馈。
...
- H.265：验证客户端切换编码后推流链路、ZLMediaKit 流信息和解码端均识别 H.265；若当前 Chrome/系统解码能力不支持该组合，使用项目支持的专用 H.265 客户端验收，不把“浏览器无解码能力”误判为 CMS 推流失败。
...
- 记录 CMS、Service、Render 的私有工作集、句柄、线程和连接数；结束后回落到基线允许范围。
...
| CMS HTTPS | TCP | 5200-5210 中已分配端口 | CMS HTTPS 端口 | `/health` 200 |
...
如果 RTC 需要的 UDP 端口数量超过现有公网范围，必须先调整路由器映射或部署 TURN；不能只开放 CMS HTTPS 就宣称双机链路通过。

### 13.2 执行步骤

1. 本机 L0-L2 全绿后生成 CMS、CMS Web、px_service、px_media 及依赖包。
...
7. 分别重启 CMS、Service 和 px_media，记录恢复时间和实例/媒体影响。
...
- CMS、Service、Render、px_media 的相关时间窗日志。
...
- CMS Rust 全量测试：126 个通过。
- px_service 测试：37 个通过。
- CMS Web Vitest：13 个通过。
- Chromium 常规 E2E：8 个通过；真实链路用例在普通运行中按设计跳过。
- 真实 Web Client 续票与 PeerConnection 重连：通过。
- CMS 重启后运行实例恢复、重新签发 view-only 票据和最终进程清理：通过。
- CMS Web 类型检查、构建，以及 CMS release 构建部署：通过。
...
测试人员：
Git commit：
CMS / Service / Render / px_media SHA-256：
环境：L0 / L1 / L2 / L3 / L4
用例范围：
