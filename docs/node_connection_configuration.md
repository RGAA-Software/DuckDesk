...
本机配置位于 Service 数据根下独立的 `node-control` 目录；目录只允许 SYSTEM 和 Administrators，内容使用 machine-scope
Windows DPAPI 加密。加载时拒绝 reparse point、宽权限目录、未知字段、未知 schema、非法 token 和歧义地址。旧 schema 直接退役，
不会导入或兼容；`--clear-node-control` 是管理员显式清除配置后重新接入另一个 Console 的入口。

## 2. 地址约束

- `endpoint` 必须是精确的 `/api/console/node-control` WebSocket URL。生产只允许 `wss://`；`ws://` 仅允许 loopback 开发测试。
- endpoint 不允许 userinfo、query、fragment。Service 通过标准 TLS 验证 `wss://` Console，并在首个严格 JSON 消息中发送节点 token。
...
Service 不读取或执行 `PXLIC2` 许可证，也不再持有部署证书、信任仓库、nonce 挑战或身份水位。节点属于哪个部署由 Console 中创建并摘要保存的
节点 token 决定；服务能力和并发 stream 额度只由 Console 的许可证门禁裁决。
