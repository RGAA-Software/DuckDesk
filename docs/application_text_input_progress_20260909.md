...
更新：2026-09-10。90 上线后，Qt Windows 和桌面 Web → WebView 的真实系统拼音输入、远端文字和选区替换已通过。
本轮仅验收 WebView；用户确认 90 无法运行 game，不再在该节点尝试游戏。详见
[WebView 中文输入实机验收](webview_chinese_input_acceptance_20260910.md)。
...
此前离线阶段完成本地开发；本轮已按用户授权更新 90 并执行短批次 WebView 测试。
未注销用户、重启机器或停止无关应用。真实 IME 验收范围限于下面列明的客户端与网页，不代表所有应用兼容。

以下为实施基线；本轮实机结果及修复另见最新验收记录：
...
1. Qt/桌面 Web → WebView 的本机拼音、选区替换、emoji 已通过；多行/富文本网页、真实断线和失权仍需扩展验收。
   game 不在本轮验收范围，待有可运行游戏的机器再验证。
...
|---|---|---|---|
| Qt → WebView | Windows 拼音 nihao → 你好通过 | 中文/emoji、选区替换、光标处追加通过 | 本轮范围通过 |
| Web → WebView | 桌面 Chromium + Windows 拼音通过 | 中文/emoji、选区替换通过 | 本轮范围通过 |
| Qt → 自有 game | 待测 | 待测 | 不在本轮范围，90 无法运行游戏 |
| Web → 自有 game | 待测 | 待测 | 不在本轮范围，90 无法运行游戏 |
...
上述哈希和离线暂停状态属于先前本机收尾阶段；最新修复、部署哈希及实机结果见
[WebView 中文输入实机验收](webview_chinese_input_acceptance_20260910.md)。
