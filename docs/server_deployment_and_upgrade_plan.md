...
定义续期、迁机、密钥恢复、灾备和机器时间回退处理；不能只绑定单机硬件而使迁机或灾备失效。
...
| Windows Panel/Client | 用户或运维运行完整未签名安装包覆盖升级 | 不承诺活动播放器或文件进程替换无中断；持久任务只在身份重新验证后恢复 |
...
仅接受相同 root 或经旧/新门限授权的 `N+1`，保留全部历史版本化 root，并从最早保留根重新加载候选。

本地/挂载式静态源站已有 `promote-filesystem`：它要求候选审计清单的带外审批 SHA-256，首次整目录原子提交，后续强制精确下一代版本和历史对象
不可变；持久 journal 支持在 targets、root 或非 timestamp 元数据切换中断后用同一候选续跑，最终始终以 `timestamp.json` 为客户端可见提交点并在
...
从已保存根完成精确 `N+1` 验证后才能提交。在线路径现按版本连续刷新 root，再验证 timestamp/snapshot/targets 并原子推进元数据水位；目录或元数据通过
仍不能作为已升级证据，后续消费者也不得自行选择或回退维度。
...
升级执行不建立跨节点状态机。每次只处理一个明确组件：取得完整包、只读预检产品/发行/版本/清单摘要及平台签名策略、进入该组件的维护窗口、覆盖安装、
...
2026-09-24 P5 可用环境短验收：先前已覆盖的公网 90 Console 仍返回 readiness 204，节点与 90/SG 两台 Relay 均重新上报 ready/fresh；现有 Windows Client 在该 Server 上完成 WebView 云应用登录、启动、Native Direct 首帧与输入，Web Client 完成 Direct Host WebRTC/SCTP、1920×1080 画面和解码首帧。由于本地 Cloud Node development dist 的 Web Client 目录尚未生成，Web 验收脚本显式读取同次 Vite 构建的 `web/px_web_client/dist`，不将该开发目录冒充正式产品制品。受控 Windows 会话经 SG Relay 保持 100 秒并在排空期间仍有一个房间，最终完整 PASS；SG 排空时新 WebSocket 准入由正常状态的 401（无效认证）变为 503，90 主 Relay 仍接入，恢复后 SG ready/fresh、零房间，Console 无活动资源会话。此结果关闭当前设备可做的 Server 先行与双 Relay 维护短链路，不代替相邻正式版本 API 窗口、第二台物理 Render 或长期运行验收。该次测试时 `Public Web Application` 暂以 Console 根路径为入口，显示管理登录页；它不是 Windows Client 的第二次登录。后续已改为独立免登录测试页，见下条。

2026-09-25 测试 WebView 应用已改为 Console 同域静态 `/webview-test.html`，不触发管理员登录流程。Console 登录页和侧栏也改为 Pixels 标识，移除旧 GoDesk 图片；网页 57 项单测及构建通过，公网六个静态文件逐件 SHA-256 相符，测试页公网 GET 返回 200。应用与部署均更新至 revision 2 后，Windows Native Direct 首帧与输入、Web Direct Host WebRTC 首帧短测通过；两次实例均进入 `stopped`。这些是开发期短测，不重复运行已有的相邻版本安装矩阵。

同日 90 的 Console 曾因 PostgreSQL 服务租约本地五秒截止时间到期而安全退出；当时 PostgreSQL Windows 服务已运行，但现有计划任务未把 Console 拉起。System 日志显示 PostgreSQL 服务在 00:08:08 进入 Running，不能单凭该事件判定导致后来租约到期的具体原因。公网开发部署入口 `scripts/publish_console_public.ps1` 现同步仓库内的监督脚本和日志保留脚本、校验哈希并更新启动包装器；部署返回六个网页文件校验通过。零活动会话时结束 Console 进程，验证监督器自动换新 PID，readiness 恢复 204，节点与应用部署重新 ready/fresh。首次新进程出现时 HTTPS 尚未就绪，必须等待 readiness，不能把出现 PID 当作恢复完成。这只是进程恢复，不改变租约失效时 fail-closed 的安全行为，也不宣称数据库故障根因已复现。
