...
> 日期：2026-09-16
>
> 状态：实施规划，未开始本计划的功能开发；用户已确定产品边界，具体技术方案和升级指标为建议基线。
>
...
这替代原“首次迁移保留现有数据库”的安排。模型、事务、Mongo 一次性转换、备份保留及主备方案见
...
- P0 清点现有 Windows 安装标记与 Android applicationId/签名，冻结首次发行归属；不擅自改现有包名导致覆盖安装失效。
- 首次从开发基线切换为新发行如无法安全原位升级，应明确维护迁移步骤，不建立永久旧协议兼容层。
...
本次开发基线迁移不提供旧协议 fallback。建立新基线后，未来滚动升级建议支持当前与上一条已发布协议代际在明确窗口内共同运行。
...
| DB0–DB5 数据库优先 | PostgreSQL 模型/事务、Console/Auth/Desk 持久化、一次性转换、自动备份与恢复、现有功能回归 | 无产品 Mongo 依赖/双写/fallback；并发和故障不假成功；备份实际恢复成功，源数据受保护 |
