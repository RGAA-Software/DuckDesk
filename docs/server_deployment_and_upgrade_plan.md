...
2026-09-24 P5 可用环境短验收：先前已覆盖的公网 90 Console 仍返回 readiness 204，节点与 90/SG 两台 Relay 均重新上报 ready/fresh；现有 Windows Client 在该 Server 上完成 WebView 云应用登录、启动、Native Direct 首帧与输入，Web Client 完成 Direct Host WebRTC/SCTP、1920×1080 画面和解码首帧。由于本地 Cloud Node development dist 的 Web Client 目录尚未生成，Web 验收脚本显式读取同次 Vite 构建的 `web/px_web_client/dist`，不将该开发目录冒充正式产品制品。受控 Windows 会话经 SG Relay 保持 100 秒并在排空期间仍有一个房间，最终完整 PASS；SG 排空时新 WebSocket 准入由正常状态的 401（无效认证）变为 503，90 主 Relay 仍接入，恢复后 SG ready/fresh、零房间，Console 无活动资源会话。此结果关闭当前设备可做的 Server 先行与双 Relay 维护短链路，不代替相邻正式版本 API 窗口、第二台物理 Render 或长期运行验收。测试 WebView 应用 `Public Web Application` 的入口是 Console 根路径，因此测试画面显示 Console 管理登录页；那不是 Windows Client 的第二次登录。
