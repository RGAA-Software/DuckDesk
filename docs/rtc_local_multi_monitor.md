...
## Console 设备监控观察者反馈边界（2026-08-21）

- 普通交互会话仍保持单控制端；Console 设备监控使用 `wall_observer` 特殊会话，允许与控制端并存。
- 观察者保留自身 WebRTC 的 BWE、pacer、NACK、RTT 等传输反馈，但其 `SetRates` 只更新本 peer
  的发送目标，不再下发 `PxPluginConfigEncoder`，因此不能修改共享物理编码器的码率或帧率。
- 物理编码器仅接受普通交互会话的速率配置。多个观察者读取共享编码帧缓存时各有独立消费游标，
  不会互相抢帧，也不会与控制端争夺编码器控制权。
- PLI/断链恢复仍可请求关键帧，但请求在插件层按显示器全局合并，800ms 内同屏只下发一次，
  防止控制端与多个观察者同时触发 IDR 风暴；新连接的首次全屏关键帧请求保留即时广播。
- 观察者一旦 ICE 成功连接，后续瞬时 `Disconnected` 只走通用断线宽限；首次连接 15 秒超时
  不再误清理已经播放的监控会话。
...
### 客户端（px_webrtc_client / px_client_sdk / px_client)
