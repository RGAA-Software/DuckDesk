...
| 5 | 日志路径与其他进程一致 | `C:\Users\Public\GoDesk\px_logs\` |
...
内层 `tc::Message` 使用现有 `px_message.pb`（C++ 已有；Rust 侧可用 `prost` 生成或 Phase 1 仅处理文本路径手工解析）。
...
与 `px_service` 一致，使用 `px_base::log_util::init_log`（原 `px_guard` 已删除，其保活日志也并入本文件）：

| 项 | 值 |
|----|-----|
| 目录 | `%PUBLIC%\GoDesk\px_logs` → `C:\Users\Public\GoDesk\px_logs` |
...
| `user_proxy_log_root` | `user_proxy_log_root()` = `%PUBLIC%\GoDesk\px_logs` |
...
| `px_clipboard_info_roundtrip` | 内层 `tc::Message` `kClipboardInfo` + `kClipboardText` |
| `px_clipboard_info_resp_roundtrip` | `kClipboardInfoResp` |
...
| `log_root_under_godesk` | 目录在 `GoDesk/px_logs` 下 |
...
| `px_user_proxy::keepalive` | 进程名大小写不敏感匹配、tick 决策（缺谁拉谁）、initial check | 单元（原 `px_guard::process_monitor` 用例迁入） |
...
等价于 `cargo test -p px_user_proxy -p px_service -p service_core`（原 `px_guard` 已并入 `px_user_proxy::keepalive`，随 `-p px_user_proxy` 一起测试）。
...
失败时打包以下文件（均在 `C:\Users\Public\GoDesk\px_logs\`）：
...
| Rust/C++ proto 不一致 | 共用 `px_render_panel_message.proto`；集成测试覆盖 |
