...
| DB2-A | 身份/管理 HTTP、密码计算/限流/Origin、访客 HMAC/会话/公开目录、Saved Connections HTTP、严格配置、稳定私钥加载、独立初始化 CLI、静态文件服务及进程生命周期已实现；本人资料/头像、其余新 Console Web API 改造、正式产品二进制/安装包切换及客户端全链路尚未接通 |
