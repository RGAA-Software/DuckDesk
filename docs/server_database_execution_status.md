...
“我的录像”导航和空 owner 目录；同时补齐测试 Console 的私有缓存初始化/启动配置。当前尚缺公网真实录像/有数据下载、保留/驱逐管理
入口，因此 CM-RECORDING 保持“部分迁移”，DB2/DB5 不关闭。
