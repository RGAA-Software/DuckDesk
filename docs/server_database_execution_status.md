...
| DB2-B/C/D | 设备/应用/节点/部署目录、user/guest 资源入口、更新与历史元数据入口及 Console 节点 WS 已接；Windows Service 已切到新节点协议并实现部署准备、调和、命令 fencing、精确 launch ACK、Render 前端准入转发、实际媒体/RDP connected/disconnected 通道生命周期上报，以及最新 CPU/内存/固定磁盘/GPU 库存采样；Console 已按 generation/sequence 原子持久化 latest 与 7 天原始历史，并在管理节点页展示 latest 和最近 100 条历史。非桌面 Render 已在建连前校验当前 CloudApplication session/instance/role/lease，并使用原 frontend token 周期续租；续租失败会按当前硬截止精确撤销 RTC allocation 和逻辑绑定，不引入短期 ticket。仍未完成真实公网首帧/输入/音频与持续续租/撤销、逐 GPU 负载/显存/编码器指标、趋势聚合/阈值/实时流、GPU/RDP 执行、周期通道指标、文件/录像生产与媒体投递、无人值守更新身份/执行器及其余 repository 产品入口 |
