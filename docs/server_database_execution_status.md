...
> 再逐节点或客户端运行完整包覆盖安装；Windows 包保持明确未签名，每个节点独立验收，不存在跨节点提交、统一回滚或旧协议兼容。
...
2026-09-22 增补对象仓库纵向短测。`pgbackrest.s3.conf.example` 固定生产必须启用 TLS 校验和 AES-256 客户端仓库加密，凭据与加密口令只允许由
部署密钥系统注入；`scripts/server_validation/postgres_pitr_object_store.ps1` 使用摘要固定的 MinIO/MinIO Client、一次性凭据和一次性自签证书建立
S3 兼容仓库。报告 `pitr-object-20260922-113446-05a43aa8` 在物理删除源 PostgreSQL 容器与数据卷后，先证明错误加密口令无法恢复，再只从
对象仓库按命名恢复点重建并精确得到误删前两条已提交记录，共核对 990 个对象；隔离容器、网络、卷和临时证书均已清理。该短测关闭
S3 协议、客户端加密、源损失恢复和 PITR 的开发门禁，但 MinIO 仍与数据库位于同一 Docker 宿主，且测试证书关闭校验，因此不冒充独立故障域、
生产 TLS、真实凭据托管、带宽/RPO/RTO 或正式灾难恢复验收。
...
上述早期 WSL 缺口已在 2026-09-22 被两个独立短测替代。`pg-20260922-111029-7b66e426` 使用 WSL2 原生 `px_console` 完成 SIGTERM 正常退出、
同状态重启和数据库 authority 丢失后的非零 fail-closed；新增 Linux 安装/注销入口则严格拒绝大写 UUID、宽权限环境、deployment 不匹配、符号链接和
另一活动 Console 实例，先确认旧实例已停止再原子覆盖。报告 `console-systemd-a71306b7-5bbe-4d5c-9629-23ad552e7cc9` 在 WSL2 systemd 完成
专用 `pixels-console` 身份、环境文件 `0400`、安装、覆盖升级、restart、干净 SIGTERM、重新启动及保留配置/状态的注销，且测试结束恢复此前可能存在的
WSL unit/二进制并精确删除测试 deployment。打包入口会把 unit 与两份生命周期脚本一并收入 Console release。该组合关闭 WSL2 的 Console 二进制和
安装层动态证据，不替代目标 Linux 发行版 VM 的 PostgreSQL/TLS/MAC/开机启动或正式包验收。
...
管理员录像缓存动作随后进入真实浏览器门禁。测试节点通过 node-control 上报带 session 归属和 SHA-256 的录像，管理员请求缓存后由同一节点
轮询一次性 upload token，并把真实字节 PUT 到 Console 私有缓存；页面随后依次执行 Retain、确认 pinned 时 Evict 禁用、Release 和带确认框的
Evict，最终显示 Not cached。报告 `pg-20260920-111710-98f82ed9` 在隔离 PostgreSQL、原生 Console 与 Chromium 上 17/17 通过，继续覆盖
重启、断库 fail-closed/恢复、本人录像/传输页和注销，源码 hash 稳定且隔离资源已清理。该证据关闭管理动作缺少真实产品浏览器覆盖的本地缺口；
公网真实录像文件上的管理员保留/释放/驱逐仍作为 DB5 环境验收保留。
...
不能用新 key 自签后跳断既有客户端信任链。轮换后的追加发布现验证并保留连续版本化 root 链，且从最早保留根重新加载整库，关闭“轮换文件合法但
下一代候选拒绝新根”及“新候选丢失旧根链”缺口。文件系统源站发布器继续补齐候选 SHA-256 带外审批、首次目录原子提交、严格下一代校验、
历史目标/根不可变、固定 metadata 切换顺序、持久 journal 中断续跑及提交后整库复验；它不持有 SSH/云厂商凭据，也不冒充 CDN 原子刷新。
6/6 单元测试覆盖多 root 门限/不覆盖、旧新门限轮换、轮换后追加与初始根验证、两代发布与历史保留、历史/当前制品篡改、错误角色 key、目标名复用，
以及首次/增量源站发布、错误审批拒绝和中断续跑，严格 Clippy 通过。对象存储/CDN 适配、轮换水位观察、审批私钥托管和正式安装包矩阵仍保持门禁。
统一验收入口报告 `pg-20260921-200226-859d647d` 为 6/6，源码 hash 冻结且隔离容器/卷已清理。
...
namespace、品牌、Windows 三产品安装身份与未签名策略、Android 签名证书固定值、独立 Android applicationId、Windows/Android/Web 品牌资源摘要、
...
产品水位 schema 2 与 dist/installer schema 4 携带精确 namespace/OEM/profile 摘要和 `windows_code_signing=unsigned`，OEM PE 使用 profile 公司名和图标；
...
随后关闭 OEM Host 产品描述代码阻塞：`px_service` 不再把所有 schema 4 manifest 的公司强制写死为 Pixels，而是对 Official/Customer 保持 Pixels，
对 OEM 同时要求非 Pixels 公司、规范 `oem_id/release_namespace`、非零 profile SHA-256 和未签名策略。产品事实直接使用 schema 4，绑定
product/distribution/namespace/OEM/profile/company；重启收敛、安装后 manifest 复核和失败回滚均逐项比较，不解析开发期 schema 2。NSIS 与 Service
回滚缓存名也加入发行 owner（Pixels 或精确 OEM ID），因此另一 OEM 的残留包即使字节或版本碰巧相同也不会进入回滚。Service 聚焦测试为
...
安装包的 TUF 发布、Console 登记/审批和真实节点部署仍需审批材料后执行。
...
因此关闭的是代码级独立 TUF 与跨 OEM 拒绝门禁，不代表已完成正式密钥审批、正式仓库发布、Console 登记/批准或真实 OEM 包部署。
...
Desk 和 Console 的发布入口随后也接入共享不可变 target 校验：Desk 在写版本目录前拒绝错位路径，Console 在登记审批事实前拒绝，并在数据库行读回时再次
fail-closed，避免损坏数据被当作有效 UpdateRelease。安全但错域的路径与 `../` 一样无副作用拒绝。最新 Docker 空库短测为 Console updates 11/11
`pg-20260922-051640-ee1c7a67`、Desk 8/8 `pg-20260922-051838-818eedaa`；受影响的目录 API 7/7
`pg-20260922-050608-3f98a8eb`、节点更新控制 3/3 `pg-20260922-051036-deb062b9` 也通过，全部隔离容器和卷已清理，严格 Clippy 通过。
首次目录 API 重编译因 D 盘耗尽未进入测试；只清理仓库 `.cache/pg-cargo` 30.2 GiB 可再生缓存后以关闭增量模式重跑通过，不把资源失败记作功能通过。
聚焦 development 输出已刷新并与构建树哈希一致：`px_console.exe`
...
Console Unix 运行时随后补齐可重复的聚焦跨平台门禁。现有真实进程用例在 Unix 先向 `px_console` 发送 SIGTERM，要求正常退出码 0，再使用同一私有状态与端点重启，
最后撤销数据库连接权威并要求进程非零 fail-closed；测试夹具同时把录像缓存和许可证水位目录显式限制为 0700，不降低生产私有文件校验。
`postgres.ps1 TestSuite -Suite console-process -Linux` 现只运行该 Windows/WSL2 对称短测并记录 Linux 二进制摘要，失败报告
`pg-20260922-110416-1b0c3a76` 保留了权限缺口；修复后的最终源码报告 `pg-20260922-111029-7b66e426` 为 2/2 PASS，Linux `px_console` SHA-256 为
`b10f642dbfc30cb59e5fcfd67a8cf413c73947340f38eef864c02e4470acf812`，源码冻结、一次性容器和卷清理均通过。这关闭 WSL2 Unix binary/SIGTERM
动态证据缺口。Windows/WSL2 的本目标 Clippy 均以 `-D warnings --no-deps` 通过，仅对 Console 中既存且非本切片引入的
`too_many_arguments` 债务作显式豁免；这不替代目标 Linux 发行版的 systemd、MAC 策略、开机启动、正式包或完整 747 项跨平台门禁。

Render 子进程私密启动参数随后从操作系统命令行移除。Service 只通过受限的 `PIXELS_RENDER_*` 子进程环境传递本机 IPC token、Relay
准入票据、设备随机口令/安全口令摘要和 WebView URL；Render 读取后立即清除环境，再启动 CEF 等后代进程。持久 launch spec、进程命令行和日志
均不再保存这些值，也不保留旧命令行参数兼容。Service/Render 聚焦测试、严格 Clippy 和 Cloud Node development 构建已通过，构建树与 dist
...
Windows 未签名正式安装包矩阵随后完成新的公网实物批次。六个当前安装包均通过 schema 4、发行身份、精确文件集合、SHA-256 和
`windows_code_signing=unsigned` 独立复核：Cloud Node Official/Customer 3.3.78 各 315 件，Client Official/Customer 3.3.74 各 42 件，
Remote Official/Customer 3.3.76 各 77 件。公网 Windows 节点完成 Cloud Node Official 旧版到 3.3.78 的真实升级、同版覆盖、卸载、
恢复和 20 秒 Service 稳定检查；Remote Official 完成安装、覆盖、卸载；Customer Cloud Node 3.3.77→3.3.78 与 Remote 3.3.75→3.3.76
完成升级、覆盖和卸载；两个 Client 发行完成安装、同版覆盖和卸载。跨产品及跨 Official/Customer 安装均以 1638 拒绝且原产品逐件哈希不变，
手工注册的 `px_service` 也会让 Client 安装以 1638 拒绝。安装器回滚缓存 ACL 已修正为根目录封闭、子项恢复继承并 reset，避免旧缓存文件
得到空 DACL；中断覆盖安装只允许全局 owner 元组完全相同的产品修复。Service 现直接验证当前 product manifest schema 4 和明确未签名策略，
不再读取退役的 schema 3 signer pin；完整 Service 单测 104/104 PASS，另 1 项物理 NVIDIA 按设计忽略。矩阵结束后节点已恢复 Cloud Node
Official 3.3.78，315/315 文件一致，Windows 云应用 Native Direct 再次取得 1920×1080 解码首帧。Client 因当前没有保留相邻旧版本安装包，
本批只完成同版覆盖而未重复构造旧版升级证据；升级实现与其他产品共用同一 NSIS 身份和替换路径，后续正式相邻 Client 版本再补实物升级。

2026-09-23 已从提交 `7f3ad551aa84` 隔离构建 Client 3.3.73，并与当前 3.3.74 组成真实相邻版本矩阵。Official 与 Customer 均在不卸载
3.3.73 的情况下直接覆盖到 3.3.74，安装器退出码为 0；升级前后注册版本、distribution、Publisher 均正确，两个阶段的 42 件运行时文件
分别与对应 release manifest 逐件 SHA-256 一致。3.3.74 Customer 升级后启动冒烟通过。测试 Client 与临时历史工作树随后清理，机器原有
`Pixels Yun 3.1.46` 未改动。至此 Windows Client 两个发行的相邻版本覆盖升级门禁关闭。

Android 本轮不重新编译；现存 Customer 1.0.28 fast-release APK 摘要仍为
`B6B73F9E612D88A9B3CC7707C7F1FC19E1D2EE184413D6DF8AA1CB89469591D3`。已连接 HyperOS 设备当前把 `com.android.shell` 的
`REQUEST_INSTALL_PACKAGES` 置为默认 deny，三次 `adb install -r` 均在写入前由系统以 `INSTALL_FAILED_USER_RESTRICTED` 取消；没有卸载、清数据或
绕过系统授权。已有 Customer Debug 冷启动及“设备/云应用/传输/设置”导航短测正常，但它不替代 fast-release 覆盖验收；重新允许“通过 USB 安装”后
只需重跑同一 APK 的覆盖与冷启动，不需要重新构建。
...
| DB4 | 恢复集、保留、异机复制、恢复准入/执行/封印、三库写屏障/安全水位、外部见证、Auth keyring、pgBackRest/WAL/PITR、S3兼容加密对象仓库短测、Windows SCM包以及Backup/Console两项WSL2 systemd生命周期已实现。公网 Windows 目标节点现已使用独立只读角色完成真实定时备份，且覆盖安装、重启、卸载保留数据、重装、跨主机复制和全新 PG18.6 恢复短测通过；CN Auth 的目标 Ubuntu systemd 备份服务也已运行并产生 verified 恢复集。正式独立对象存储灾难恢复、生产密钥托管、目标环境 keyring/见证轮换及自然周期稳定性保留在商业发布前专项清单；按 2026-09-23 范围决定不再阻塞本轮开发基线收口 |
| DB5 | 公网 Windows CloudApplication Native Direct/Relay 及 Relay 音频/文件hash/取消重试/在线撤销、活动文件上传中 Service 重启收敛与精确 Render 回收、Web Client Direct Host RTP/解码首帧、Android guest 与账号 CloudApplication Native Direct/Relay 首帧、启停清理、Relay 真实断线续签、设备目录 ACL 授权/撤销及公网录像本人下载和管理员副本生命周期已通过；RDP 的画面、输入、resize、系统音频、双向 Unicode/文件剪贴板、断线宽限、忙工作区、双工作区和五类终态也已通过公网短测。旧 PXLIC1/PXDC/PXDD 公网栈与自定义部署身份验收仅保留为历史证据，不是当前实现约束；活动源码只保留 `PXLIC2` 服务端许可证，Official 客户端固定官方 HTTPS 端点，Customer 客户端配置私有 HTTPS 端点并拒绝已知官方端点。Windows Panel 的账号、目录、实例与显式资源 descriptor 已切当前 API 并完成公网 API 短测，主机设置旧设备自注册/appkey/加密接入串路径也已删除，Relay 部署配置由已认证 Console→Service 节点控制下发。2026-09-23 当前全新公网基线已用最新 Windows Service/Render、Windows Client、Android Official 1.0.25 和 Customer 1.0.28 Debug instrumentation 重新通过 Direct/Relay 首帧与启停清理，活动实例和资源会话均收敛为 0；Relay 空闲连接回收和实例终态会话关闭已在公网实物验证。Windows 与 Android 双发行的一次预检/一次升版和独立沙箱编排已实现；六个 Windows 未签名正式安装包已通过清单复核，Cloud Node、Remote、Client 的 Official/Customer 相邻版本覆盖升级均已通过实物矩阵，Client 升级后运行时清单及启动冒烟通过。Android fast-release 当前设备覆盖、AMD/Intel 物理 GPU、目标 Linux、独立对象存储灾难恢复和统一长测按 2026-09-23 决定转入商业发布前延期清单，不阻塞本轮开发基线收口 |
| DB-HA / P1–P7 | 独立主机 HA、正式发行隔离、授权/连接服务、升级、运维与真实容量/稳定性验收 |

DB0–DB5 当前开发基线据上述范围正式收口，下一阶段进入
[P1/P2 发行隔离与最小授权执行计划](p1_p2_distribution_authorization_execution_plan.md)。延期清单仍是未来商业发布门槛，不能改写成已经完成，
但不再反复阻塞日常开发；恢复这些项目时按独立验收批次执行。
资源会话 repository 证据见[CloudApplication/桌面会话与描述符契约](postgresql_resource_session_contract.md)；
专项测试入口与独立 AES 固定向量已经通过，不再列为未开始。后续 P1/P2 先核对现状再补缺口，不重复实现已经通过的数据库、节点协议和
Windows/Web/Android 公网链路。
所有后续代码继续遵守全新开发、无旧数据导入、无旧接口/配置适配、无 fallback；不为了让旧测试通过重建退役行为。

2026-09-23 P1/P2 首批代码门禁已执行：Windows Panel 修复 Official origin 在固定/禁止比较前未规范化的问题，Customer 不能再以主机大小写、
尾斜杠或默认 HTTPS 端口等价值绕过，并以聚焦测试确认同 origin 保留账号绑定、切换私有 origin 清理旧账号绑定；Panel 产品与本地化聚焦测试通过，
Client development build/dist 的 `px_panel.exe` SHA-256 均为
`0FEA810A1ECD0CCCE78A8173241A67B2F415E865A00C3305C3FA140B0E9AACE7`。PXLIC2 契约 5/5、Console runtime 11/11、Android
`core-network` 单测通过；PostgreSQL `sessions` 新增独立实例争抢许可证最后一个 stream 槽位的原子并发测试并以 13/13 通过，报告为
`pg-20260923-184620-39ab2cb3`，directory API 7/7 报告为 `pg-20260923-185940-6f4796b3`。本轮未运行 release-only 完整构建，
实例预留 PostgreSQL 专项随后以 16/16 通过，确认未授权 Cloud/RDP 服务在产生节点命令前拒绝、失败事务不占容量、最后一槽竞争保持原子，报告为
`pg-20260923-193242-e6303921`。
新增的 `scripts_build/build_client_development.bat` 已从此前清理后的状态重建 RDP SDK，并增量构建 Panel、Client 和 `px_osinfo`，原子生成 Client 3.3.74
schema 4 development dist；后续执行会先完整验证并复用当前 RDP SDK，只有缓存缺失或来源/摘要不匹配才重建。完整收集还修正了制品组误将 Web 旧图标
重新映射为桌面顶层 `resources/icons/px_icon.png` 的回归，并加入该路径的制品拒绝门禁；当前桌面只保留 `resources/icons/brand/`。最终 40 件文件全部
通过 SHA-256 和依赖边界验证，
...
及目标 Ubuntu 24.04 上的相邻正式版本覆盖升级当时尚未验收，P4 出口仍未完成。

2026-09-24 后续正式包 1.0.2 已构建并核验：116 件清单、归档解包复核与 SHA-256 均通过。Ubuntu 20.04 WSL 隔离 full-systemd 短测包含新库、四服务、登录、覆盖失败回滚、备份恢复与回环离线重启，报告为 `test-results/server_validation/pg-20260924-202853-83f79526`；另以 1.0.1 → 1.0.2 完成 Relay 真正覆盖，确认新清单摘要生效、服务仍 active、旧版降级拒绝。目标 SG Ubuntu 24.04 主机也完成两版归档与清单校验、随机 UUID Relay 相邻覆盖、运行状态与降级拒绝；临时包及测试实例已清理，既有 Relay 保持 active。仍未以客户生产凭据、断公网拓扑或目标主机四服务同机部署做商业交付验收。
