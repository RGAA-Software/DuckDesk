...
当 `enable_hook_audio_ = 1` 时，`px_gh.dll` 内启用音频 hook。主要新增/改动：
...
2. 视频帧 wire 格式改为定长 152 字节纯 POD `IpcCaptureVideoFrame`（magic=`GRCV` 0x47524356 + version=1 + pack(1)），Host 侧逐字段转换构造 `CaptureVideoFrame`，不再对含 `std::shared_ptr` 的结构整体 memcpy（远程可触发崩溃的 UB）；旧格式 blob 显式拒绝 + 限流日志；宽高 clamp 16..8192。**协议变更：Host render 与 `px_gh.dll` 必须同批部署。** 音频帧 `IpcCaptureAudioFrame` 本就是 POD 未变。
