...
render 已有 H264/H265 视频编码、PCM 音频采集、Opus 音频编码以及“编码帧扇出给插件”的链路；录像插件已验证该模型可用。本方案新增一条独立的直播旁路：把**已有远控会话**的主屏媒体推到 Console 侧的 ZLMediaKit（下称 ZLM），由 Console Web 和标准播放器观看。
...
4. **Console Web 是正式观看端**：v1 使用 HLS.js / 原生 HLS；H264 可在 Chrome/Edge/Safari 播放，H265 不承诺通用浏览器解码。VLC/PotPlayer 可走 RTSP/RTMP/HLS。
...
已有远控客户端连接到 render
        ...
        ▼
主屏 H264 编码帧 ─────────┐
主屏 PCM 音频 ─ AAC 编码 ├─► live_pusher.dll ─ RTMP 主动外连 ─► ZLM（Console 侧）
                         │                                      │
                         │                                      ├─ HTTP-FLV ─ Console Web（低延迟）
                         │                                      ├─ HLS ─ Console Web / iOS / 电视（兼容）
                         │                                      ├─ RTSP ─ VLC / PotPlayer
                         │                                      └─ RTMP ─ 桌面播放器/二次分发
                         │
...
- **render** 只主动连接 ZLM，适合 render 在 NAT 后、Console 在上层网段的现有部署。
- **ZLM** 是独立进程，部署在 Console 主机或 Console 同网段的媒体主机；不嵌入 render，不随 render 生命周期崩溃。
- **Console** 是唯一业务入口：授权、流状态、播放地址和 Web 页面均由 Console 提供；浏览器不接触 ZLM 管理 API/secret。
- 单路输入由 ZLM 做零转码转封装。输入可以是 **H264 + AAC**，也可以是 **H265 + AAC**；H265 经 enhanced-RTMP 发布，ZLM 的 `[rtmp] enhanced=1` 必须开启。Console Web 对 H265 的兼容性见 §7.2。
...
远控客户端接入 render
  ...
  → Console 标记为 live，观看端可播放

远控客户端全部退出
  → 现有逻辑停止采集/编码
  → live_pusher 排空极短队列、写 trailer/关闭 RTMP
  → ZLM 注销流
  → Console 标记为 idle，观看端显示“当前没有远控会话，未直播”
```
...
- ZLM 恢复后，下一关键帧恢复直播；Console Web 轮询状态并提示“正在恢复”，播放器可自动重载一次。
...
ZLM 的 `app` 固定为 `live`。流 ID 由 Console/Panel 在启动 render 时确定并显式下发为 `live_stream_id`：
...
- 流 ID 只允许 `[A-Za-z0-9_-]`，最长 128 字符；Console 生成并校验，render 再做防御校验。不得拼接路径、IP、显示器名或用户输入。
- 同一 `live_stream_id` 第二次发布默认拒绝，而不是无条件踢掉旧流；Console 在明确“实例重启/接管”后才允许接管，避免错误实例覆盖正在观看的流。
...
- 主屏切换或分辨率变化：丢弃旧屏队列、重建视频流描述、请求 IDR，再恢复发布；Console 流名不变。
...
检测到 video_type 改变
  ...
  → ZLM 流重新注册；Console Web 自动重连并重新初始化解码器
```

- 不需要停止 render 的采集或远控会话，但观看端会有一次短暂中断；HTTP-FLV 目标为自动重连，HLS 等待下一个切片。具体时长以 P1/P5 实测为准。
- 切换期间 Console 状态为 `recovering`，不应显示为实例离线；ZLM 的注销/注册事件要按同一 `live_stream_id` 合并处理。
...
- Console 安装目录使用固定布局，媒体进程的工作目录也必须是该目录：

  ```text
  {console_install_dir}/
  ├─ px_console_server.exe
  ├─ px_media.exe
  ├─ config.ini
...
- `px_media.exe` 以独立 Windows service 或 systemd 服务运行；与 `px_console_server` 同机或同网段。服务名建议为 `px_media`，由安装/升级逻辑先停止、替换二进制和配置模板、再启动；用户改动的密钥和部署项必须保留。
- 本期直播只接入 RTMP ingest、HTTP-FLV、HLS、RTSP、RTMP playback；WebRTC/WHIP/WHEP 虽随完整 ZLM 一并编译和保留配置能力，但不出现在 Console 播放 URL、鉴权接口或本期验收范围。是否在生产环境开放其端口由部署配置单独决定。
- ZLM 管理 secret 仅供 Console 服务端使用，不能给浏览器。
...
| Console Web → 媒体 | HTTPS 443 → ZLM HTTP | 经 Console/Nginx 反代，承载 HLS/HTTP-FLV；生产环境不直接暴露 8080 |
...
- 开启 `on_stream_changed`：ZLM 注册/注销 `live/*` 时通知 Console，Console 写入内存状态或数据库缓存。
- Console 以 `getMediaList` 服务端轮询作为兜底校正，不能让浏览器直连该管理 API。
- `on_publish` 成功后才把流状态标为 `live`；ZLM 注销或持续健康检查失败则为 `offline`。

## 7. Console API 与 Web 观看端

### 7.1 Console 接口（v1 已实现）

实现位于 `rust_server/px_console_server/src/live/`，不把 ZLM 管理 API 或其 secret 下发给浏览器：
...
```json
{
  ...
  "video_codec": "h265",
  "flv_url": "https://console.example/live/live/...live.flv?token=...",
  "hls_url": "https://console.example/live/live/.../hls.m3u8?token=...",
...
### 7.2 Console Web 页面（v1 已实现）

`web/px_console/src/views/LiveViewer.vue` 已增加“直播观看”菜单入口。它选择在线设备和应用标识、每 10 秒刷新状态，使用 HLS.js（Safari 优先原生 HLS），并在离开页面时销毁播放器和定时器。该页不嵌入远控 Web 客户端、不建立 WebRTC 会话，也不会因为没有远控会话而唤醒 render。

入口：Console 设备详情增加“直播”；game-hook 应用实例列表也增加“观看直播”，两者都带目标 `device_id + instance_id` 调 `live/info`。

```text
打开直播页
  → Console 业务鉴权（设备 ACL / 应用实例权限）
  → GET live/info
  → state=live 且 H264：以 Console 票据 URL 播放 HLS
...
- 浏览器兼容性取决于 `video_codec`：H264 时 Chrome/Edge 优先 FLV、Safari/iOS 原生 HLS、Android 依能力先 FLV 后 HLS、智能电视承诺 HLS；H265 时 Console Web 不承诺 Chrome/Edge 的 FLV/MSE 播放，Safari/iOS 优先 HLS，桌面播放器走 RTSP/RTMP。`live/info` 必须返回 `video_codec`，页面在不支持时给出明确提示而非黑屏重试。

## 8. 鉴权与安全

| 环节 | 必须措施 |
|---|---|
| 推流 | RTMP URL 带 `stream_id`、过期时间、随机 nonce 和 HMAC 签名；ZLM `on_publish` 回调 Console 校验身份、签名、实例是否仍允许运行 |
| 观看 | Console `status` 先经既有 appkey 鉴权；短时播放 ticket 绑定单一 stream 并在 HLS 请求时滑动续期 |
| HTTP 媒体 | HLS m3u8 和每个切片均通过 Console 代理和 ticket 验证，浏览器不直连 ZLM HTTP 端口 |
| 运维 | ZLM secret、推流密钥和 token 不写入日志、不下发浏览器；管理 API 仅内网/服务端访问 |
| 公网 | Console Web 与媒体统一 HTTPS；限流、连接数上限、审计 publish/play/鉴权失败 |
...
| P2：ZLM 与 Console 状态 | ZLM 服务、协议转封装、publish/stream-changed hook、Console 状态缓存 | Console 能准确显示 live/idle/recovering，浏览器无管理 secret |
| P3：Console Web | 设备直播页、应用实例直播页、FLV 播放、HLS 回退、状态轮询和 token 刷新 | 受权用户可观看；无会话有清晰状态，不会试图唤醒 render |
...
6. 在远控会话仍在时切换 H264↔H265：render 不重启，ZLM 流受控重注册，Console 状态经过 `recovering` 后回到 `live`，支持该 codec 的播放器恢复播放。
...
| 无 ACL 或过期 token | Console `live/info` 或 ZLM 播放/HTTP 资源访问拒绝 |
| ZLM 重启 | 会话仍在时推流自动恢复；Console 从 recovering 回到 live |
...
1. Console/ZLM 部署系统（Windows 服务或 Linux systemd）及公网域名/HTTPS 终止位置。
2. `live_stream_id`、主屏标识和推流签名由 Panel 还是 Service 作为唯一签发方；建议由实际启动 render 的一方签发，并由 Console 校验。
3. Console 中桌面会话与 game-hook `instance_id` 的权限模型如何映射到现有用户组 ACL；默认应继承设备 ACL，并可叠加应用实例权限。
4. RTSP/RTMP 是否需要对公网开放；若只服务 Console Web，可仅开放 HTTPS 443 和 render→ZLM 的 1935。
