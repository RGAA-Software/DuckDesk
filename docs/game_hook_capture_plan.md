...
RdApplication::Run
  ...
  - AppManager 定时 InjectCaptureDll（px_gh_injector.exe）
...
| 组件 | 状态 |
|------|------|
| `px_gh.dll` / `px_gh_injector.exe`（OBS 移植） | 已有 |
...
| GammaRay | OBS |
|----------|-----|
| `hook_capture/win/hk_obs/` → `px_gh.dll` | `plugins/win-capture/graphics-hook/` |
| `hk_obs/injector` → `px_gh_injector.exe` | `plugins/win-capture/inject-helper/` |
...
7. `/ipc` 安全：仅接受 loopback 连接；帧为 POD wire 格式。Host render 与 `px_gh.dll` 必须同批部署（协议变更）
...
2. 视频帧 wire 改定长 152B 纯 POD `IpcCaptureVideoFrame`（magic=`GRCV` + version=1 + pack(1)），Host 逐字段转换，不再整体 memcpy 含 `std::shared_ptr` 的结构；旧格式显式拒绝 + 限流日志；宽高 clamp 16..8192。**协议变更：Host render 与 `px_gh.dll` 必须同批部署。**
...
2. **`D3D11DeviceWrapper::Release()` 双重释放**（render 崩溃根因，子模块 `px_common/win32/d3d11_wrapper.h`）：原来裸调 `->Release()` 而 ComPtr 仍持指针，wrapper 析构时二次释放，其他持有者的 ComPtr 悬空，device removed 后 `VideoFrameCarrier::Exit` 崩溃。改 `Reset()`。
...
1. **改代码 + 增量编译**:`cmd //c "build_official\_build_inc.bat <target>"`
   (hook 相关 target:`plugin_frame_carrier`、`px_gh`;改 hk_video 头文件
...
- **proto**（`px_message/px_message.proto`，web 两份副本同步）：
