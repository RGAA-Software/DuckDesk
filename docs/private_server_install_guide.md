...
- 从 Auth 正式签发流程取得绑定该 deployment UUID 的 `PXLIC2` 许可证，并由 Auth 运维人员交付 CN 主机上通过 `px_auth_admin create-trust-store` 生成、当前启用的公钥信任文件。通过可信运维渠道分别核对信任文件 SHA-256 和 key ID，再交给客户；不得信任许可证或普通下载响应自带的公钥。签发私钥、测试许可证和真实数据库口令均不在套件内；私有 Console 本地验签，正常会话不依赖 Auth 在线服务。
...
2. 首次安装前先创建 `pixels-console` 系统组和用户（如不存在；安装器也会确保它们存在），将已核对的信任文件和 `.pxlic` 放在 `/etc/pixels/<deployment UUID>/` 下，文件由该用户持有、权限 `0400`；安装前按 [Console 运行配置](px_console_server_runtime_config.md#全新部署)以该用户执行 `px_console_admin validate-license`。把 Console 环境文件中的 `PIXELS_CONSOLE_DISTRIBUTION=customer`、`PIXELS_CONSOLE_RELEASE_NAMESPACE=pixels.customer`、`PIXELS_DEPLOYMENT_ID`、`PIXELS_CONSOLE_LICENSE_TRUST_STORE`、`PIXELS_CONSOLE_LICENSE_FILE`、TLS/Origin、运行时数据库 URL 设为实际值。静态路径必须精确为 `/opt/pixels/private/<deployment UUID>/current-console/static/console`。Desk 如安装，其静态路径为 `/opt/pixels/private/<deployment UUID>/current-desk/static/desk`。Relay 的公开端点须使用当前配置，不得使用退役端口。
...
- 许可证续期不需要重装。由 Auth 管理面下载新 `.pxlic`，按上述可信渠道取得并核对当前公钥信任文件；在目标机保留旧文件备份后，以 root 执行 `sh "$package_dir/tools/renew_console_license.sh" "$deployment_id" /absolute/new-trust.json /absolute/new-license.pxlic`。脚本在停服前以 `pixels-console` 身份离线验签、核对 deployment，然后替换两份文件并重启；失败时恢复原文件并尝试重新启动旧服务。结束后仍须检查 Console readiness、管理员许可证摘要和业务准入；`systemctl active` 不是完整验收。此工具随下一次正常 Customer Server 发版进入包，已发布的 1.0.2 包不含它。
