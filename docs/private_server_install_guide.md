...
适用范围：当前正式套件 `1.0.1`，目标 Ubuntu 24.04 x86_64、systemd。本文是现有安装入口的操作顺序，不宣称真实客户生产凭据、断公网拓扑或四服务同机生产部署已经验收。后续正常发版时替换套件版本；不为跨版本测试专门发布新包。

## 1. 交付物与部署前准备

- 取得同一发行批次的 `1.0.1.tar.gz` 和 `1.0.1.tar.gz.sha256`，先在归档所在目录执行 `sha256sum -c 1.0.1.tar.gz.sha256`。解包后目录名为 `1.0.1`；执行 `python3 <解包绝对路径>/tools/verify_candidate.py <解包绝对路径>` 和 `sh <解包绝对路径>/tools/preflight_linux_host.sh`。不要使用 development 候选目录代替正式 Customer 包。
...
```sh
package_dir=/absolute/path/to/1.0.1
...
- 停用单个组件使用包内 `tools/uninstall_linux_component.sh <console|relay|desk|backup> <deployment UUID>`。该入口只停用注册，保留私有配置、release 和数据；**不要为了升级先卸载**。Linux Server 1.0.1 的同包覆盖和失败恢复已有短测，相邻正式版本覆盖留待下次正常发版顺带验证。
