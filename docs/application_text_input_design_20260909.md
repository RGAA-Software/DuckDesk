...
日期：2026-09-09。状态：设计与待实施计划，不是功能完成或验收报告。
...
本次仅编写文档，不授权修改外部参考仓库、不新增 Windows 用户、不改变 RDP 模式。
...
用户确定的新功能载体为可靠 WebSocket，但当前 Web InputController 使用 DataChannel。
必须新增/复用经过验证的 WS 应用控制适配器，不能只是更改文档注释。
先核对 Web 当前信令 WS 是否真的能路由应用消息；信令 WS 存在不等于可承载文本提交。
如需专用 WS 路由，它必须绑定当前已授权逻辑会话/控制租约，具有端点用途约束与断开清理，
不能把第二条通道计成第二个控制者，也不能因它断开终止仍在使用的媒体会话。
不得为此恢复被废弃的 Native RTC/KCP/Relay 或 WS 视频回退。

Web HTTPS 页面连接必须满足 WSS、证书信任、Origin 和部署代理路径要求，不能以忽略证书作为产品方案。
不可把 Windows 凭证放在 URL、localStorage、日志或新协议里；沿用既有票据/续期机制。

旧 kTextInput 保持原语义和版本兼容，新文本提交用明确的新语义或经协商的扩展，
不能改变旧按键消费者导致游戏重复输入。两种入口必须互斥，旧 Web 隐藏 textarea 的即时发送不能与面板同时生效。
无新能力的服务端显示“不支持该功能”，不尝试盲发或全局注入。
浏览器手写 protobuf/TypeScript 定义与 C++/Rust 协议生成结果必须一起更新并做双向序列化测试。

### 6.3 顺序、回执与去重

WS/TCP 保证一条连接内有序字节流，不保证跨通道顺序、应用执行成功或断线后的 exactly-once。
...
| Console | 复用 input 权限和既有会话授权；评估新 WS 子通道能力声明，不增加 clipboard 依赖 |
...
1. **基线与协议**：核对现有所有客户端输入通道、应用准入与生命周期；定 capability/结果/代次/WS 子通道契约。
...
- Web 的可靠 WS 子通道如何绑定既有控制租约、如何满足 WSS 部署，先用最小原型验证。
