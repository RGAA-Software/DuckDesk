...
- 只改 render 插件时，仅重编对应目标并覆盖远端 `gr_plugins\`，不必全量 `build_client.bat`。
- 覆盖前必须先杀 `GammaRayRender.exe`（dll 被占用会静默失败）。
- 日志：`\\10.0.0.70\C$\Users\Public\GoDesk\gr_logs\`（`plugin_dda_capture.dll.log`、
