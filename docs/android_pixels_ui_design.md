...
> 2026-09-14 决定：新增独立“云应用”一级 Tab；Console、游客、注册与账号实施细节见
> [`android_cloud_apps_implementation_plan_20260914.md`](android_cloud_apps_implementation_plan_20260914.md)。
...
1. **资源分区**：远程设备与 Console 云应用使用独立一级入口、列表模型和持久化边界。
...
- **设备**：快速连接、设备列表、扫码和设备详情。
- **云应用**：当前 PX Console 的 public 应用，以及登录账号获得 ACL 授权的应用。
- **传输**：全部设备的进行中、失败和完成任务。
- **设置**：PX Console、注册/登录、画质、输入、音频、网络诊断、权限和关于。设置页、设备偏好及连接弹窗均不提供强制通道、协议优先级、自动选择通道或媒体回退开关。
...
一级云应用入口 ── public/ACL 应用 ── 启动/连接 ── 远控工作区
一级传输入口 ── 传输中心
一级设置入口 ── PX Console/账号/权限/诊断/关于
```
...
│                                  │
│ 设备能力                         │
│ 2 台显示器 · H.265 · UDP+FEC     │
└──────────────────────────────────┘
```

- 详情页只展示服务器实际上报的能力，不用本地版本猜测。
- Console 云应用只在独立“云应用”一级页面展示，不并入设备详情或设备数据库。
...
```text
准备连接 → 认证 → 建立 UDP 媒体与 WebSocket 控制 → 准备音视频 → 已连接
               ↘ 可恢复失败 → 重试/网络诊断
               ↘ 最终失败   → 返回设备/复制诊断
```
...
当前设备卡的“连接偏好”已落地协议真实支持的 30/60 FPS、远端音频、默认输入模式和自动/软件解码策略，并按设备持久化；远控中切换音频或输入模式会更新该设备偏好。完整四档预设、分辨率、码率、codec 和色彩格式要等能力协商完成后再开放，不显示不可生效的控件。

跨平台视频能力协商已完成设计但于 2026-09-07 决定暂缓实施。暂缓期间 UI 不增加四档预设或高级编码参数占位项，也不把当前解码器策略包装成完整画质协商；
恢复实施时必须由 Windows Render 编码能力与 Android/iOS/macOS/Windows Client 解码能力的真实交集驱动可选项；iOS/macOS 属于后续适配平台，具体协议和阶段见产品规划第 14 节。

统计面板分两层：

- 简要：延迟、FPS、码率、丢包和实际解码器；长统计使用标题下方独立横向区域，不挤压设备名。
- 详细：编码/解码、渲染、音频 buffer、分辨率、颜色格式、重连次数和各阶段耗时。

远端已有显示器在手机上使用横向预览卡切换；平板支持主次或并排布局。Android 不提供创建或删除 Windows 虚拟显示器的入口。
