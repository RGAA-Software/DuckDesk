# Console 应用调度：状态同步与联调修复

> 状态：已落地（2026-08，commit `dc64453c`）  
> 范围：同机多实例端口、启停回执、状态恢复、路径校验、Web 列表展示  
> 相关计划：[`console_app_schedule_plan.md`](./console_app_schedule_plan.md)

---

## 1. 背景与问题

P3 调度链路打通后，本机 E2E（Console Web + console Service + game-hook Render）暴露出几类「状态与现实不一致」问题：
...
| 游戏/Render 已停，页面仍「停止中」+ `unknown instance_id` | Service 重启后本地注册表为空，Console 仍 Stopping；unknown 被当成失败 |
| 进程已死，页面仍「运行中」 | Service 心跳继续上报已死实例；Console 对账认为仍存活 |
...
目标：Console 列表状态反映 Service/进程真相；启动失败可 toast；同机多实例端口互不踩踏。
...
Console Web (/apps)
    ...
    ▼
px_console_server · AppScheduleManager
    │  WSS /console/service
    │  Start/Stop + HeartBeat(instances_json)
    ▼
px_service · AppInstanceRegistry
    │  起停 px_render(--app_mode=game-hook)
    ▼
...
- Console 默认池起点 `32000`，保存应用时按已占用端口 **+1** 建议下一端口。
- Service 本机池 `32000–32999`；Console 下发 `listen_port>0` 时按首选端口占用。
...
5. **kill 前 pid 身份校验（`pid_belongs_to_instance`）**  
   目标 pid 必须是该实例端口上的 game-hook Render 或游戏 exe，防 pid 复用误杀；身份全不符返回 Err 且不 `mark_stopped`。kill 后按端口 1s 复查，Render 仍在 → Err 保留端口，Console 标 Failed。
...
1. **`reconcile_from_service_hb(device_id, instances_json)`**  
   ...
   - Console 上该机器的 `running`/`stopping`
   - 若 **不在** HB 的「活跃」集合中 → 标 `stopped`，清 pid/error  
   - 活跃集合：HB 条目 `state` ∈ `{running, starting, stopping}`；`state` 为空（旧版）仍视为活跃  
   - HB 里同 id 但 `state=stopped` **不能** 保住 Console 的 Running
   - **双向对账**：HB 活跃条目对应 Console 实例为 `stopped`/`failed` 时 → 恢复 `running` 并回填 pid/listen_port（迟到 ok 回执不复活 Failed，靠此恢复）
...
| 模块 | 文件 | 职责 |
|------|------|------|
| Console 调度 | `rust_server/px_console_server/src/app_schedule/manager.rs` | 启停、路径拆分、对账、Start 等待 |
| Console HB | `.../net_service/console_service_conn.rs` | 心跳触发 `reconcile_from_service_hb` |
| Console 断线 | `.../net_service/console_service_ws_handler.rs` | 断开后延迟 15s 空列表对账；期间重连则取消 |
...
| Service 心跳 | `rust_client/px_service/src/console_client.rs` | HB 前 reap；Stop unknown→成功 |
| Web | `web/px_console/src/views/AppsView.vue` | 列表绑定、启停 toast |
...
- 保存时应填**游戏 exe 绝对路径**；Console 会拆成 `install_root` + 文件名。

### 7.3 验收清单

1. 保存两个应用，端口分别为 32000、32001（或 Console 建议值）。
2. 依次启动：两个 Render/游戏均起来；`netstat` 可见对应端口 LISTEN。  
3. 「打开」两个 Client，页面均非空。  
4. Console 停止 → 状态到已停止；进程退出。
5. 外部杀掉 Render/游戏 → 约一个心跳周期（~3s）内 Console 从「运行中」变为「已停止」。
...
| 症状 | 检查 |
|------|------|
| 机器下拉为空 | Service 是否在线；是否已 `inject_service_auth`；Console `/api/v1/service/control/query/all/service/conn` |
| 一直运行中但无进程 | Service 是否为含 `reap_dead_app_instances` 的版本；看 conn 里 `instances_json` |
| 一直停止中 | Stop 回执 / HB 对账；Service 是否连上 Console |
...
### 10.1 Console 侧（rust_server/px_console_server）
...
4. 双向对账：HB 活跃条目对应 Console 实例为 Stopped/Failed 时恢复 Running 并回填 pid/listen_port。
5. `load_from_db` 修复：`starting` → `failed("Console restarted")`、`stopping` → `stopped`。
...
2. kill 后按端口复查（1s 窗口），Render 仍在 → Err，保留端口占用，Console 标 Failed。
...
9. Console URL 日志脱敏；重连固定 2s（2026-08-08 起取消 2s→30s 指数退避），心跳 3s。
10. 测试：`service_core` 51/51、`px_service` 32/32 通过（含 pid 复用误杀防护、token 边界、端口校验等新测试）。

### 10.3 第二轮追加（2026-08-08）

- `suggest_next_port` 封顶：超 32999 回落池起点扫描第一个空闲端口，全满报「端口已用完（32000-32999）」；handler 适配 Result 返回。新增 3 单测（`px_console_server` 54/54）。
...
- `px_console.toml` 新增 `force_authorize`：`false` = WS token 过滤（client/panel/service/website）与 HTTP appkey 过滤一律放行（本机/测试）；缺省（不写）为 `true` 强制鉴权。
- 测试部署（`output/px_console/`）已置 `false`；生产部署应显式 `true`。
- 放行时 `/console/service` 不再需要 appkey/token 参数即可连接（`inject_service_auth` 不再是前置条件）。
