...
R90 已按修正后的稳定性规则取得干净的 10/10；随后虚拟显示器真实 RTC 全链路也
连续 10/10、50/50 阶段通过，最终官方构建产物再次完成创建、采集、切换、删除和
恢复冒烟。项目负责人确认 10 轮即为本次完整稳定性门禁，历史 100 轮与 2 小时、
断网耐久、跨夜项目仅保留为诊断或可选扩展压测。因此下文关于“R90 待复跑”和
“LAN RC 因历史 97/100 阻断”的文字是历史过程记录，已由本节结论取代。

当前局域网门禁 PASS；真实不同公网出口、对称 NAT/CGNAT、跨运营商 TURN 仍为
BLOCKED-ENV，不得据此宣称公网生产网络已完成验收。最终证据见
`docs/asio_notify_concurrency_acceptance_report_20260826.md`。

## 2026-08-25 增量复测

> 当前执行规范（2026-08-26 更新）：开发、自测、整体集成、RC 和最终验收
> 均执行 10 轮连续连接/退出并要求 10/10。下面保留的 100 轮数据是历史证据，
> 不再构成当前发布门禁。
...
- WebClient 未登录入口不申请 ticket：Direct RTC 和 Standard RTC 都由 Render 校验
  ...
  不接收设备密码；最终认证仍在 Render。R90 双机复测随后已完成并通过。
- Web Standard RTC 连续连接/退出增量复测 10/10 通过，最终临时用户、session、
  ...
  10/10，通过后临时用户、session、ticket、CDP Chrome 均为 0；R90 随后也按
  当前规则完成干净的 10/10。
...
> 历史稳定性记录：旧判据下的 100 轮结果为 97/100，其中静态桌面和 host-only
> 候选断言造成误判。判据修正后，本机和 R90 均取得当前门禁要求的干净 10/10，
> 因此历史记录不再阻断 LAN RC。2 小时、断网耐久和跨夜保留为可选扩展压测。
...
4. STAB-01 已在 R90 用修正脚本取得干净 10/10。跨夜和断网恢复耐久尚未执行，按 2026-08-26 决定属于可选扩展压测，不是当前门禁。
