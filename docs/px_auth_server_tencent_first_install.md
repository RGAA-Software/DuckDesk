# px_auth_server 腾讯云全新服务器首装指南

本文面向一台**全新的腾讯云 Ubuntu 服务器**，从零安装 px_auth_server 的运行环境并完成首次部署。
...
如果后期要开认证，在 `px_auth.toml` 的 `db_path` 连接串里带上账号密码，
参考 `docs/px_auth_server_package_and_run.md`。
...
新建 `/etc/supervisor/conf.d/px_auth_server.conf`：

```ini
[program:px_auth_server]
command=/opt/px_auth_server/px_auth_server
directory=/opt/px_auth_server
user=ubuntu
autostart=true
autorestart=true
...
```bash
sudo mkdir -p /opt/px_auth_server/logs
sudo chown -R ubuntu:ubuntu /opt/px_auth_server
...
```bash
sudo mkdir -p /opt/px_auth_server/certs
```

- **配置文件**：把 `rust_server/px_auth_server/src/px_auth.toml`
  拷到 `/opt/px_auth_server/px_auth.toml`，然后修改：
...
- **授权签名密钥对**：`certs/auth_license_private.key` / `auth_license_public.key`。
  ...
  公钥需同步给 Console（`PX_AUTH_LICENSE_PUBLIC_KEY`）。
...
**部署前必须先把前端编译好**。musl 构建不会现场编译 `web/px_auth`，
它用的是仓库里预编译的 `rust_server/px_auth_server/assets/`，
所以每次部署前（无论首装还是日常更新）都要先做这一步，否则会发布旧页面：

```bash
cd web/px_auth && npm run build && cd ../..
rm -rf rust_server/px_auth_server/assets/*
cp -r web/px_auth/dist/* rust_server/px_auth_server/assets/
```
...
它从 `rust_server/px_auth_server/tencent_server.txt` 读取 SSH 凭据（该文件不入库），
目标 IP 写在脚本里，换服务器时改脚本顶部的 `SERVER=`。

## 7. 部署后检查

```bash
sudo supervisorctl status px_auth_server
curl -sk -o /dev/null -w '%{http_code}\n' https://127.0.0.1:30400/
tail -f /opt/px_auth_server/logs/px_auth_server/*.log
```
...
- **前端不同步风险**：musl 部署用的前端来自仓库里预编译的 `rust_server/px_auth_server/assets/`，
  不是现场构建 `web/px_auth`。改了前端后，先把 `web/px_auth/dist` 内容同步进 `assets/` 再部署。
