...
> 状态：P3 已落地（2026-08）；状态同步与联调修复见 [`console_app_schedule_state.md`](./console_app_schedule_state.md)
> 目标：Console Web 配置应用与机器放置，向多台 Service 下发启停；Service 起停 game-hook Render 并回执；可打开 WebRTC Client。
...
| # | 决策 | 取值 |
|---|------|------|
| 1 | 应用分发 | 一期 **只登记路径**，不做 Console 推包安装 |
...
| 6 | 与 desktop 关系 | Console **只管 game-hook 应用实例**；Panel desktop 单槽保留、互不误杀 |
...
- **Machine**：现有 `device_id`；在线 = Service 已连 Console `/console/service`。
...
```
Console Web：选 Application + 在线 Machine → Start
        │
        ▼
Console：校验 Service 在线 + Placement → Instance=starting
...
Service：拼路径 → 分配/使用 listen_port → 起 Render(game-hook)
     上行 StartAppInstanceResult(ok/port/pid|error)
        │
        ▼
Console：Instance=running|failed
...
## 4. 协议（`console_service.proto`）

在 Hello / HeartBeat 之外增加：

| 方向 | 类型 | 用途 |
|------|------|------|
| Console→Service | `kConsoleServiceStartAppInstance` | 启动 |
| Service→Console | `kConsoleServiceStartAppInstanceResult` | 回执 |
| Console→Service | `kConsoleServiceStopAppInstance` | 停止 |
| Service→Console | `kConsoleServiceStopAppInstanceResult` | 回执 |
...
|-------|------|------|
| **P0** | 本文档 + 决策 | — |
| **P1** | Proto + Service/Console 编解码与命令分发（可 mock 进程） | 单测覆盖多机/多实例/失败 |
| **P2** | Service 真实起停 game-hook + 实例表 | 本机脚本级联调 |
| **P3** | Console 持久化 Application/Placement/Instance + HTTP + Web 页 | Console Web 启停 |
| **P4** | 多机联调 + Client URL | A/B 机同应用 |
| **P5** | 自动调度、ready、公网入口 | 后置 |

本期已落地（代码 + 单测）：

- Proto：`console_service.proto` Start/Stop + Result + HB `instances_json`
- Service：`service_core::app_instance` 注册表/端口/路径/启动参数；`ServiceRuntime::start/stop_app_instance`；`console_client` 收令回执
- Console：`app_schedule` Application/Placement/Instance；Mongo `c_app` / `c_app_placement` / `c_app_instance`；HTTP `/api/v1/app/control/...`；WSS 下发与结果回调
- Console Web：`/apps` 应用调度页（创建应用/放置、启停、打开 Client）
...
- Console 同应用两机 Placement；离线/无 Placement 失败；Start/Stop Result 状态机
- Proto Start/Stop 编解码 round-trip
- Console Web：`web_client_url` game-hook URL 拼装
- 状态恢复：`unknown instance_id`→Stopped、HB 对账、忽略 HB 中 stopped 条目、绝对路径治愈

后续：真实多机联调（P4）、自动调度 / ready / 公网入口（P5）。  
联调问题与状态同步细则：[`console_app_schedule_state.md`](./console_app_schedule_state.md)。
...
5. Proto 变更后需同步 rebuild `protocol` crate（Console + Service）。
6. Render/游戏被外部杀死后，依赖 Service 心跳 reap + Console HB 对账，否则列表会短暂假「运行中」。
