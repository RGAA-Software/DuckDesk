...
- 客户端日志：`C:\Users\Public\GoDesk\gr_logs\ct_plugin_client_clipboard.dll.log`（注意 `Get-Content -Tail` 对它可能返回空，用 `Select-String`）。
- 主机日志：`\\10.0.0.90\C$\Users\Public\GoDesk\gr_logs\godesk_user_proxy.log`、`godesk_render_20371.log`、`plugin_net_ws.dll.log`。
