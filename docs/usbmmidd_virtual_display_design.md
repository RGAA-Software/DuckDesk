...
拓扑变更专项验收：在已连接的标准 UDP 和 RTC local 会话中，分别执行“创建一块虚拟屏 → 切换到该屏 → 删除该屏”十轮。每轮必须确认：客户端在拓扑恢复后收到新的关键帧、无绿屏/白屏、服务端没有帧号无符号下溢或进程退出；同名显示器采集流重启时，Render 必须将其视为新的编码流并等待 IDR，而不是发送旧 delta 帧。
