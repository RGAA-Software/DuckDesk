...
fn copy_required(source: &str, destination: &Path) {
    if !Path::new(source).is_file() {
        panic!("required Console media artifact is missing: {source}");
    }
...
fn main() {
    ...
    // beside px_console.exe: FFmpeg/SRT/WebRTC/OpenSSL DLLs are dynamically loaded
    ...
    // CoDesk bundle and does not contain the current Vue Console (including the
