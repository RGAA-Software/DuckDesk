use crate::auth::console_auth_pull::{pull_once, PullOutcome};
use crate::console_api_error::ConsoleApiError;
use crate::console_context::ConsoleContext;
use crate::console_http_util::{get_body, get_body_str};
use crate::gAuthManager;
use crate::user::console_user_keys::KEY_PASSWORD;
use axum::body::Body;
...
) -> Result<Json<RespMessage<AuthStatus>>, ConsoleApiError> {
...
async fn build_auth_status(auth: Authorization, _include_credentials: bool) -> AuthStatus {
    ...
    let machine_code = crate::gConsoleContext.lock().await.machine_code.clone();
    AuthStatus {
        authorized,
...
pub async fn handle_get_used_time(
    State(_context): State<Arc<Mutex<ConsoleContext>>>,
) -> Result<Json<RespMessage<RespUsedTime>>, ConsoleApiError> {
    let used_time_ms = gAuthManager.lock().await.get_used_time().await;
    let auth = gAuthManager.lock().await.get_auth().await;
    let now_ms = get_current_timestamp();
    let (left_time_ms, expired) =
