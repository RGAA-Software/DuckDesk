...
use px_auth_mgr::authorization::{Authorization, PRODUCT_PIXELS_CONSOLE};
...
pub async fn pull_once() -> Result<PullOutcome, String> {
    let (base_url, appkey, app_secret) = {
        let st = gConsoleSettings.lock().await;
        (
    ...
    let machine_code = gConsoleContext.lock().await.machine_code.clone();
    if machine_code.is_empty() {
        return Err("machine code is empty".to_string());
    }

    let request = DevicePullRequest {
        product: PRODUCT_PIXELS_CONSOLE.to_string(),
