...
use crate::console_http_util::{get_int_param, get_int_param_or, get_str_param_allow_empty};
...
use crate::record::console_visit::{ConsoleUpdateVisit, ConsoleVisit};
use axum::body::Body;
...
async fn reporting_device_id(headers: &HeaderMap) -> Result<Option<String>, ConsoleApiError> {
...
fn validate_paging(page: i32, page_size: i32) -> Result<(), ConsoleApiError> {
...
pub async fn handle_upload_visit_info(
    ...
    _query: Query<HashMap<String, String>>,
    headers: HeaderMap,
    Json(mut up_visit): Json<ConsoleVisit>,
) -> Result<Json<RespMessage<String>>, ConsoleApiError> {
...
pub async fn handle_update_visit_info(
    ...
    _query: Query<HashMap<String, String>>,
    headers: HeaderMap,
    Json(update_visit): Json<ConsoleUpdateVisit>,
) -> Result<Json<RespMessage<String>>, ConsoleApiError> {
...
pub async fn handle_query_update_info(
    ...
    _body: Body,
) -> Result<Json<RespMessage<Vec<ConsoleVisit>>>, ConsoleApiError> {
...
pub async fn handle_upload_file_transfer_info(
    ...
    _query: Query<HashMap<String, String>>,
    headers: HeaderMap,
    Json(mut up_file_transfer): Json<ConsoleFileTransfer>,
) -> Result<Json<RespMessage<String>>, ConsoleApiError> {
...
pub async fn handle_update_file_transfer_info(
    ...
    _query: Query<HashMap<String, String>>,
    headers: HeaderMap,
    Json(up_file_transfer): Json<ConsoleUpdateFileTransfer>,
) -> Result<Json<RespMessage<String>>, ConsoleApiError> {
    let reporter_device = reporting_device_id(&headers).await?;
...
pub async fn handle_query_file_transfer_info(
    ...
    query: Query<HashMap<String, String>>,
    _body: Body,
) -> Result<Json<RespMessage<Vec<ConsoleFileTransfer>>>, ConsoleApiError> {
    let page = get_int_param(&query, "page")?;
    let page_size = get_int_param(&query, "page_size")?;
    validate_paging(page, page_size)?;
    let sort_time = get_int_param_or(&query, "sort_time", -1)?;
