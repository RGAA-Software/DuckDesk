...
use protocol::console_panel::RecordListResp;
...
impl RecordTunnelManager {
    ...
    ) -> Result<RecordListResp, ConsoleApiError> {
    ...
    pub async fn wait_list_with_timeout(
        ...
        match tokio::time::timeout(timeout, rx).await {
            Ok(Ok(resp)) => Ok(resp),
            Ok(Err(_)) => Err(ConsoleApiError::DeviceOffline),
            Err(_) => {
                self.pending_lists.lock().unwrap().remove(req_id);
                Err(ConsoleApiError::RequestTimeout)
            }
        }
    }
