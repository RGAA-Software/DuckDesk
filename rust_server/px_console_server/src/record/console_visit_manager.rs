...
pub struct ConsoleVisitManager {}

impl ConsoleVisitManager {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {})
    }
    ...
    pub async fn insert_visit_info(&self, mut info: ConsoleVisit) -> Result<ConsoleVisit, ConsoleApiError> {
        ...
        coll.find_one(filter)
            ...
            .ok_or(ConsoleApiError::VisitNotFound)
    }

    pub async fn update_visit_info(
        &self,
        update: ConsoleUpdateVisit,
    ...
    ) -> Result<Vec<ConsoleVisit>, ConsoleApiError>
    where
        T: Into<Bson>,
    {
        let c_visit_info = gConsoleDatabase.lock().await.visit();
...
fn validate_visit_start(info: &ConsoleVisit) -> Result<(), ConsoleApiError> {
...
fn normalize_visit_terminal_status(status: &str) -> Result<&'static str, ConsoleApiError> {
