...
pub struct ConsoleEventManager {}

impl ConsoleEventManager {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {})
    }
    ...
    pub async fn add_or_refresh_telemetry_event(
        &self,
        mut event: ConsoleEvent,
    ) -> Result<ConsoleEvent, ConsoleApiError> {
        let identity = telemetry_identity_filter(&event).ok_or(ConsoleApiError::InvalidParams)?;
        let collection = gConsoleDatabase.lock().await.event();
    ...
    ) -> Result<Vec<ConsoleEvent>, ConsoleApiError>
    where
        T: Into<Bson>,
    {
    ...
    pub async fn count_total_events(&self, event_type: String) -> Result<u64, ConsoleApiError> {
...
fn telemetry_identity_filter(event: &ConsoleEvent) -> Option<Document> {
    let base = doc! {
...
mod tests {
    ...
    fn non_telemetry_event_has_no_identity() {
        assert!(telemetry_identity_filter(&ConsoleEvent::default()).is_none());
    }
}
