...
pub type ConsolePanelConnPtr = Arc<Mutex<ConsolePanelConn>>;

#[derive(Clone)]
pub struct ConsolePanelConn {
    ...
    // local NIC IPv4 list reported in ConsolePanelHello (design doc 5.2)
    pub panel_lan_ips: Vec<String>,
    pub panel_http_port: i32,
}

#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct ConsolePanelConnVo {
    pub device_id: String,
    pub device_name: String,
    pub user_id: String,
    pub hello_timestamp: i64,
...
impl ConsolePanelConn {
    ...
    pub fn as_info(&self) -> ConsolePanelConnVo {
    ...
    async fn send_hello(&mut self, device_id: String, user_id: String) {
        let mut pl_msg = ConsolePanelMessage::default();
        pl_msg.set_msg_type(ConsolePanelMessageType::KConsolePanelHello);
        pl_msg.hello = Some(ConsolePanelHello {
            device_id,
            user_id,
    ...
    async fn send_heartbeat(&mut self, hb_index: i64, device_id: String, user_id: String) {
        let mut pl_msg = ConsolePanelMessage::default();
        pl_msg.set_msg_type(ConsolePanelMessageType::KConsolePanelHeartBeat);
        pl_msg.heartbeat = Some(ConsolePanelHeartBeat {
            hb_index,
            device_id,
