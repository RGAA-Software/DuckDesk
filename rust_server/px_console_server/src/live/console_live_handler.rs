...
use super::console_live_ticket_manager::ConsoleLiveTicketManager;

lazy_static::lazy_static! {
    static ref LIVE_TICKETS: Arc<ConsoleLiveTicketManager> = ConsoleLiveTicketManager::new();
}
...
) -> Result<Json<RespMessage<LiveStatus>>, ConsoleApiError> {
    let settings = gConsoleSettings.lock().await.live.clone();
...
/// Chrome's low-latency path.  Keep HTTP-FLV streaming through Console so the
...
/// Console keeps an explicitly configured secret authoritative.  For the bundled
/// local sidecar we can also read the sidecar's own config so a fresh Console
/// deployment works without duplicating that secret in px_console.toml.  Remote
/// media servers must always be configured explicitly.
fn effective_api_secret(settings: &ConsoleLiveSettings) -> Option<String> {
