...
use crate::console_defs::{
    KEY_ACTIVE, KEY_ALL, KEY_ALLOW_OBSERVER, KEY_ALLOW_TAKEOVER, KEY_DEVICE_DESKTOP_LINK,
    KEY_DEVICE_DESKTOP_LINK_RAW, KEY_DEVICE_ID, KEY_DEVICE_NAME, KEY_IP, KEY_OFFLINE, KEY_ONLINE,
    KEY_ONLINE_STATE, KEY_PWD_TYPE,
};
...
use axum::Json;
use px_base::{ok_resp, ok_resp_str_map, RespMessage, RespStringMap};
...
pub async fn handle_create_new_device(
    ...
    let device = loop {
        ...
        // new random pwd md5
        let new_random_pwd_md5 = px_base::md5_hex(&new_device_info.random_pwd);
        ...
        if device_found {
            ...
            let update_info = HashMap::<String, String>::from([
                (
                    ...
                    px_base::md5_hex(&new_random_pwd),
                ),
    ...
    if let Some(device) = device {
        ...
        Err(ConsoleApiError::CreateDeviceFailed)
    }
}
...
pub struct RespDeviceMessage {
    ...
    pub timestamp: i64,
    pub total: u64,
    pub data: Vec<ConsoleDeviceVo>,
}

pub async fn handle_query_devices(
    State(_ctx): State<Arc<Mutex<ConsoleContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespDeviceMessage>, ConsoleApiError> {
    ...
    for device in devices {
        ...
        let mut device_vo = ConsoleDeviceVo::from(&device);
...
pub async fn verify_device_info(
    State(_ctx): State<Arc<Mutex<ConsoleContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespStringMap>, ConsoleApiError> {
    let device_id = get_str_param(&query, "device_id")?;
...
pub async fn update_random_password(
    ...
    // update to database
    let random_pwd_md5 = px_base::md5_hex(&new_random_pwd.clone());
    let update_info = HashMap::<String, String>::from([
