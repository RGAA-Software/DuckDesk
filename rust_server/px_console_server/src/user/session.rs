...
pub struct IssuedUserSession {
    ...
    pub session: ConsoleUserSession,
}
...
pub struct ConsoleUserSessionManager;

impl ConsoleUserSessionManager {
    pub fn new() -> Arc<Self> {
        Arc::new(Self)
    }
    ...
    pub async fn list_subject_sessions(
        ...
        subject_type: &str,
        subject_id: &str,
    ) -> Result<Vec<ConsoleUserSession>, ConsoleApiError> {
        let collection = gConsoleDatabase.lock().await.user_session();
    ...
    pub async fn list_guest_sessions(&self) -> Result<Vec<ConsoleUserSession>, ConsoleApiError> {
        let collection = gConsoleDatabase.lock().await.user_session();
        let mut cursor = collection
            .lock()
            .await
    ...
    pub async fn block_guest_session(
        ...
        block_ip_hash: bool,
        reason: &str,
    ) -> Result<ConsoleUserSession, ConsoleApiError> {
        ...
        let blocks = gConsoleDatabase.lock().await.guest_block();
    ...
    ) -> Result<IssuedAdminSession, ConsoleApiError> {
        let now = px_base::get_current_timestamp();
    ...
    pub async fn issue_user_web(
        ...
        ip_hash: String,
        user_agent_hash: String,
    ) -> Result<IssuedWebUserSession, ConsoleApiError> {
        let now = px_base::get_current_timestamp();
        let session_token = Self::new_token()?;
        let csrf_token = Self::new_token()?;
        let policy = crate::gConsoleSettings.lock().await.user.clone();
    ...
    pub async fn issue_guest(
        ...
        let now = px_base::get_current_timestamp();
        let hours = crate::gConsoleSettings
            .lock()
            .await
            .user
    ...
    pub async fn authenticate(&self, token: &str) -> Result<AuthenticatedUser, ConsoleApiError> {
        ...
        let sliding_days = crate::gConsoleSettings.lock().await.user.panel_sliding_days;
    ...
    pub async fn authenticate_user_web(
        ...
        let sliding_hours = crate::gConsoleSettings.lock().await.user.web_sliding_hours;
        let extended_expires_at =
    ...
    pub async fn refresh_user_csrf(&self, sid: &str) -> Result<String, ConsoleApiError> {
        let csrf_token = Self::new_token()?;
    ...
    pub async fn revoke(&self, sid: &str, uid: &str) -> Result<(), ConsoleApiError> {
        gConsoleDatabase
            .lock()
            .await
            .user_session()
    ...
    pub async fn revoke_all(&self, uid: &str) -> Result<(), ConsoleApiError> {
        gConsoleDatabase
            .lock()
            .await
            .user_session()
    ...
    pub async fn revoke_token(&self, token: &str) -> Result<(), ConsoleApiError> {
        if token.is_empty() {
            return Ok(());
        }
        gConsoleDatabase
...
mod tests {
    ...
    fn generated_tokens_are_random_and_not_stored_verbatim() {
        let first = ConsoleUserSessionManager::new_token().unwrap();
        let second = ConsoleUserSessionManager::new_token().unwrap();
