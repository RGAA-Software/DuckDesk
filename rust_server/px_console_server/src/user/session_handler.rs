...
/// Console is opened through an IP address backed by a locally trusted/self-signed
...
async fn privacy_hash(value: &str) -> String {
    ...
    // still namespace the hash per Console instead of storing a raw unsalted hash.
...
) -> Result<Json<RespMessage<ConsoleUserView>>, ConsoleApiError> {
    let settings = crate::gConsoleSettings.lock().await.user.clone();
...
pub async fn guest_session(
    ...
    let limit = crate::gConsoleSettings
        .lock()
        .await
        .user
        .rate_limit
...
pub async fn login(
    ...
    let ip_hash = privacy_hash(&addr.ip().to_string()).await;
    let policy = crate::gConsoleSettings.lock().await.user.rate_limit.clone();
    crate::user::rate_limit::check(
...
pub async fn admin_logout(headers: HeaderMap) -> Result<Response, ConsoleApiError> {
...
) -> Result<Json<RespMessage<AdminProfile>>, ConsoleApiError> {
    let auth = gAuthManager.lock().await.get_auth().await;
...
pub async fn update_avatar(
    ...
    let (extension, bytes) = avatar.ok_or(ConsoleApiError::UploadFileFailed)?;
