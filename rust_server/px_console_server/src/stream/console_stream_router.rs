...
use crate::stream::console_stream_handler::{
...
pub fn make_stream_router(context: Arc<Mutex<ConsoleContext>>) -> Router<Arc<Mutex<ConsoleContext>>> {
    Router::new()
        .route(
            "/insert",
            post(handle_insert_stream).layer(middleware::from_fn(console_appkey_filter::filter)),
        )
        .route(
            "/delete",
            post(handle_delete_stream).layer(middleware::from_fn(console_appkey_filter::filter)),
        )
        .route(
            "/update",
            post(handle_update_stream).layer(middleware::from_fn(console_appkey_filter::filter)),
        )
        .route(
            "/query/by/id",
        ...
        .route(
            "/query/streams",
            get(handle_query_streams).layer(middleware::from_fn(console_appkey_filter::filter)),
        )
        .with_state(context)
}
