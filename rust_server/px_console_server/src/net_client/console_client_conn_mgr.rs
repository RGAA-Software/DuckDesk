...
use crate::net_client::console_client_conn::{ConsoleClientConnPtr, ConsoleClientConnVo};
...
pub struct StreamReservation {
    mgr: Arc<ConsoleClientConnManager>,
...
pub struct ConsoleClientConnManager {
    connections: Mutex<HashMap<String, ConsoleClientConnPtr>>,
...
impl ConsoleClientConnManager {
    pub fn new() -> ConsoleClientConnManager {
        Self {
    ...
    pub async fn add_conn(&self, conn_id: String, conn: ConsoleClientConnPtr) {
    ...
    pub async fn get_alive_connections_ptr(&self) -> Vec<ConsoleClientConnPtr> {
        tracing::info!(
            "alive connections count: {}",
            self.connections.lock().await.len()
        );
        let conns: Vec<ConsoleClientConnPtr> =
    ...
    pub async fn get_alive_connections(&self) -> Vec<ConsoleClientConnVo> {
        tracing::info!(
            "alive connections count: {}",
            self.connections.lock().await.len()
        );
    ...
    // -- DB --
    pub async fn insert_conn(&self, conn: ConsoleClientConnVo) {
        let c_client_conn = gConsoleDatabase.lock().await.client_conn();
    ...
    pub async fn update_conn(&self, conn_id: String, timestamp: i64) {
        ...
        let update = doc! {
            "$set" : doc! {
                ...
                "readable_update_ts": px_base::format_readable_timestamp(timestamp),
            }
        };
    ...
    pub async fn query_client_conns(
        ...
        page: i32,
        page_size: i32,
    ) -> Result<Vec<ConsoleClientConnVo>, ConsoleApiError> {
...
mod tests {
    ...
    async fn reservation_respects_max_streams() {
        let mgr = Arc::new(ConsoleClientConnManager::new());
