...
use crate::net_client::console_client_conn::ConsoleClientConn;
...
async fn handle_socket(
    ...
    let mut recv_task = tokio::spawn(async move {
        ...
        if device_id.is_empty() {
            tracing::error!("console client error, device id is empty!");
            return;
        }
        ...
        let conn = ConsoleClientConn::new(
            context.clone(),
            sender,
            device_id.clone(),
            remote_device_id.clone(),
        ...
        .await;
        tracing::info!("console client connection from {}", conn_id);
        ...
        // remove
        gConsoleClientConnMgr.remove_conn(conn_id).await;
    });
...
async fn process_message(
    ...
    console_conn: Arc<Mutex<ConsoleClientConn>>,
    msg: Message,
    who: SocketAddr,
) -> ControlFlow<(), ()> {
