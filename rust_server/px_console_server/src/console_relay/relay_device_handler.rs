...
use crate::console_relay::relay_api_error::RelayApiError;
...
use crate::{gConsoleSettings, gRelayConnMgr};
use axum::body::Bytes;
...
pub async fn hd_query_device(
    ...
    let server_w3c_ip = gConsoleSettings.lock().await.server_w3c_ip.clone();
    let relay_port = gConsoleSettings.lock().await.relay_port as i32;
...
pub async fn hd_notify_event(
    ...
    if conn
        ...
        .await
    {
        return Ok(Json(px_base::ok_resp(HashMap::default())));
    }
