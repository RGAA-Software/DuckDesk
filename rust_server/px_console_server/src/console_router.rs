...
pub fn make_console_router(
    ...
    Router::new()
        ...
        .route(
            "/get/cache/data/size",
            get(gen_cached_data_size).layer(middleware::from_fn(console_appkey_filter::filter)),
        )
        .route(
            "/clear/cache/data",
            post(clear_cached_data).layer(middleware::from_fn(console_appkey_filter::filter)),
        )
        .route(
