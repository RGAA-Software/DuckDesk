...
async fn authorized_apps(uid: &str) -> Result<Vec<ApplicationCard>, ConsoleApiError> {
...
) -> Result<Json<RespMessage<Vec<ApplicationCard>>>, ConsoleApiError> {
...
) -> Result<Json<RespMessage<ResourcePage<ApplicationCard>>>, ConsoleApiError> {
...
pub async fn start_user_app(
...
) -> Result<Json<RespMessage<InstanceView>>, ConsoleApiError> {
    ...
    if active_count >= settings.quota.user_concurrent_instances {
        return Err(ConsoleApiError::QuotaExceeded);
    }
...
pub async fn list_user_instances(
    Extension(subject): Extension<AuthenticatedUser>,
) -> Result<Json<RespMessage<Vec<InstanceView>>>, ConsoleApiError> {
    let instances = gAppScheduleManager
        .list_instances_for_owner("user", &subject.uid)
        .await;
...
pub async fn list_user_instances_page(
    Extension(subject): Extension<AuthenticatedUser>,
    Query(query): Query<ResourcePageQuery>,
) -> Result<Json<RespMessage<ResourcePage<InstanceView>>>, ConsoleApiError> {
    let keyword = query.keyword.trim().to_lowercase();
...
pub async fn user_resource_summary(
    Extension(subject): Extension<AuthenticatedUser>,
) -> Result<Json<RespMessage<ResourceSummary>>, ConsoleApiError> {
