#!/usr/bin/env bash
# Cross-build gr_auth_server for Linux (x86_64-unknown-linux-musl, static)
...
cd "$WS_ROOT"
echo ">> cargo zigbuild --release --target $TARGET -p gr_auth_server"
"$CZB" zigbuild --release --target $TARGET -p gr_auth_server "$@"
echo ""
echo "OK: $WS_ROOT/target/$TARGET/release/gr_auth_server"
