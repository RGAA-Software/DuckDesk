#!/usr/bin/env bash
# One-click deploy: cross-build px_auth_server (musl) and deploy to the Tencent server.
...
ssh "$SERVER" '
  ...
  sudo chown -R ubuntu:ubuntu /opt/px_auth_server/web_auth
  rm /tmp/web_auth.tar.gz
  sudo supervisorctl start px_auth_server
  sleep 3
