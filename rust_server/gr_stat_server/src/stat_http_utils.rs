...
pub async fn get_body_data(body: Body) -> Result<String, StatApiError> {
    ...
    tracing::info!("body: {:#?}", r);
    let rb: RequestBody =
        serde_json::from_str(r.as_str()).map_err(|_| StatApiError::InvalidParams)?;
    if rb.data.is_empty() {
...
pub fn get_str_param_allow_empty(
    ...
    query.get(key).cloned().ok_or(StatApiError::InvalidParams)
}
