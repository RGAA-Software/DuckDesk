...
pub fn serde_as_bool<'de, D>(deserializer: D) -> Result<bool, D::Error>  where D: Deserializer<'de> {
...
mod tests {
    ...
    fn test_calculate_dir_size() {
        let mut dir_path = std::env::current_dir().unwrap().to_str().unwrap().to_string();
        println!("--> {}", dir_path);
