...
CREATE TABLE pixels.licenses (
    ...
    expires_at TIMESTAMPTZ NOT NULL,
    max_devices BIGINT NOT NULL CHECK (max_devices>0 AND max_devices<=4294967295),
    max_sessions BIGINT NOT NULL CHECK (max_sessions>0 AND max_sessions<=4294967295),
    features TEXT[] NOT NULL CHECK (cardinality(features)>=1 AND cardinality(features)<=3 AND features <@ ARRAY['cloud_applications','desktop','rdp']::text[]),
    created_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
