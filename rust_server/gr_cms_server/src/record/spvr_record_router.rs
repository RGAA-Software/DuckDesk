...
use crate::record::spvr_record_handle::{
...
pub fn make_record_router(context: Arc<Mutex<SpvrContext>>) -> Router<Arc<Mutex<SpvrContext>>> {
    Router::new()
        .route(
            "/hello",
            get(handle_hello_world).layer(middleware::from_fn(spvr_appkey_filter::filter)),
        )
        .route(
            "/upload_visit_info",
            post(handle_upload_visit_info).layer(middleware::from_fn(spvr_appkey_filter::filter)),
        )
        .route(
            "/update_visit_info",
            post(handle_update_visit_info).layer(middleware::from_fn(spvr_appkey_filter::filter)),
        )
        .route(
            "/query_visit_info",
            get(handle_query_update_info).layer(middleware::from_fn(spvr_appkey_filter::filter)),
        )
        .route(
