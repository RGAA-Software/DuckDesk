...
use crate::spvr_defs::{KEY_ACTIVE, KEY_ALL, KEY_DEVICE_DESKTOP_LINK, KEY_DEVICE_DESKTOP_LINK_RAW, KEY_DEVICE_ID, KEY_DEVICE_NAME, KEY_IP, KEY_OFFLINE, KEY_ONLINE, KEY_ONLINE_STATE, KEY_PWD_TYPE};
use crate::spvr_http_util::{get_body, get_body_bool, get_body_str, get_bool_param, get_int_param, get_str_param, get_str_param_allow_empty, get_str_param_or};
...
pub async fn handle_create_new_device(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
    ...
    let device = loop {
        ...
        let exist_device = gDeviceManager
            .query_device_by_id_and_seed(new_device_info.device_id.clone(), new_device_info.seed.clone()).await;
        ...
        if device_found {
            ...
            let exist_device = gDeviceManager
                .query_device_by_id(new_device_info.device_id.clone()).await;
...
pub async fn handle_query_devices(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                  query: Query<HashMap<String, String>>)
    ...
    let devices = gDeviceManager
        .query_devices(device_name, device_id, ip, page, page_size).await?;
    let total = gDeviceManager
        .query_total_devices_count().await?;
    ...
    for device in devices {
        ...
        let online = gSpvrPanelConnMgr
            .is_panel_online(device.device_id.clone()).await?;

        if !online_state.is_empty() && online_state != KEY_ALL {
            if online_state == KEY_ONLINE {
                if !online {
                    tracing::info!("this device is NOT online: {}, ignore it.", device.device_id);
                    continue;
                }
            }
            else if online_state == KEY_OFFLINE {
                if online {
                    tracing::info!("this device is online: {}, but we need offline devices, ignore it.", device.device_id);
                    continue;
                }
            }
        }
    ...
    for device in &mut vo_devices {
        ...
        let conn = gSpvrPanelConnMgr
            .get_conn_info(device.device_id.clone()).await;
        if let Ok(conn) = conn {
...
pub async fn handle_count_devices(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                query: Query<HashMap<String, String>>)
...
pub async fn query_device_by_id(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                query: Query<HashMap<String, String>>)
                                -> Result<Json<RespMessage<SpvrDevice>>, SpvrApiError> {
...
pub async fn append_used_time(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                              query: Query<HashMap<String, String>>)
...
pub async fn handle_query_total_used_time(State(_ctx): State<Arc<Mutex<SpvrContext>>>,) -> Result<Json<RespMessage<u64>>, SpvrApiError> {
    let beg = gr_base::get_current_timestamp();
    let r = gDeviceManager
        .query_total_used_time().await?;
    tracing::info!("used: {}ms", (gr_base::get_current_timestamp() - beg));
    Ok(Json(ok_resp(r)))
}

pub async fn verify_device_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                query: Query<HashMap<String, String>>)
...
pub async fn update_random_password(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                    query: Query<HashMap<String, String>>)
                                    -> Result<Json<RespMessage<SpvrDevice>>, SpvrApiError> {
    let device_id = get_str_param(&query, "device_id")?;
    let mut device = gDeviceManager
        .query_device_by_id(device_id.clone()).await?;
    ...
    let r = gDeviceManager
        .update_device(device_id.clone(), update_info).await?;
    if !r {
...
pub async fn update_safety_password(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                    query: Query<HashMap<String, String>>)
                                    -> Result<Json<RespMessage<SpvrDevice>>, SpvrApiError> {
    let device_id = get_str_param(&query, "device_id")?;
...
pub async fn update_desktop_link(State(_context): State<Arc<Mutex<SpvrContext>>>,
                          b: Body)
                          -> Result<Json<RespMessage<SpvrDevice>>, SpvrApiError> {
    ...
    let _ = gDeviceManager
        .update_device_field(device_id.clone(), KEY_DEVICE_DESKTOP_LINK.to_string(), desktop_link).await?;

    let _ = gDeviceManager
        .update_device_field(device_id.clone(), KEY_DEVICE_DESKTOP_LINK_RAW.to_string(), desktop_link_raw).await?;
...
pub async fn update_device_name(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                 b: Body)
                                 -> Result<Json<RespMessage<SpvrDevice>>, SpvrApiError> {
    let body = get_body(b).await?;
    let r: Value = serde_json::from_str(body.as_str()).unwrap();
    ...
    let _ = gDeviceManager
        .update_device_field(device_id.clone(), KEY_DEVICE_NAME.to_string(), device_name).await?;
...
pub async fn update_device_active(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                b: Body)
                                -> Result<Json<RespMessage<SpvrDevice>>, SpvrApiError> {
    let body = get_body(b).await?;
    let r: Value = serde_json::from_str(body.as_str()).unwrap();
    ...
    let _ = gDeviceManager
        .update_device_field(device_id.clone(), KEY_ACTIVE.to_string(), active).await?;
