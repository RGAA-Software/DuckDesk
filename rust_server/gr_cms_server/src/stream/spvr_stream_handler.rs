...
use crate::stream::spvr_stream_keys::{
...
pub async fn handle_insert_stream(
    ...
    if let Err(_e) = r {
        ...
        let s = gSpvrStreamMgr.update_stream(stream).await?;
        Ok(Json(ok_resp(s)))
    }
}
...
pub async fn handle_delete_stream(
    ...
    let s = gSpvrStreamMgr.delete_stream(stream_id.to_string()).await?;
    Ok(Json(ok_resp(s)))
}
...
pub async fn handle_query_stream_by_id(
    ...
    let s = gSpvrStreamMgr.query_stream_by_id(stream_id).await?;
    Ok(Json(ok_resp(s)))
}
...
pub async fn handle_query_stream_by_name(
    ...
    let s = gSpvrStreamMgr.query_stream_by_name(stream_name).await?;
    Ok(Json(ok_resp(s)))
}
