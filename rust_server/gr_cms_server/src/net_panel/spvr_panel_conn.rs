...
impl SpvrPanelConn {
    ...
    pub async fn process_message(&mut self, _who: String, data: Bytes) -> bool {
        ...
        let m = m.unwrap();
        if m.msg_type == SpvrPanelMessageType::KSpvrPanelHello {
    ...
    async fn send_hello(&mut self, device_id: String, user_id: String) {
        let mut pl_msg = SpvrPanelMessage::default();
        pl_msg.set_msg_type(SpvrPanelMessageType::KSpvrPanelHello);
        pl_msg.hello = Some(SpvrPanelHello {
            device_id,
            user_id,
    ...
    async fn send_heartbeat(&mut self, hb_index: i64, device_id: String, user_id: String) {
        let mut pl_msg = SpvrPanelMessage::default();
        pl_msg.set_msg_type(SpvrPanelMessageType::KSpvrPanelHeartBeat);
        pl_msg.heartbeat = Some(SpvrPanelHeartBeat {
            hb_index,
            device_id,
