...
use crate::auth::spvr_auth_license_keys::{init_license_verifier, license_to_authorization};
use crate::auth::spvr_auth_manager::KEY_AUTHORIZATION;
use crate::spvr_settings::DEFAULT_AUTH_PULL_INTERVAL_SECS;
use crate::{gAuthManager, gKvStorage, gLicenseVerifier, gSpvrContext, gSpvrDatabase, gSpvrSettings};
use gr_auth_mgr::app_credential as cred;
use gr_auth_mgr::app_secret_util::calculate_app_secret;
use gr_auth_mgr::auth_license::{LicenseVerifier, SignedLicense};
use gr_auth_mgr::authorization::{Authorization, PRODUCT_GODESK_CMS};
...
pub async fn pull_once() -> Result<PullOutcome, String> {
    let (base_url, appkey, app_secret) = {
        let st = gSpvrSettings.lock().await;
        (
...
async fn query_device_count() -> i32 {
    let c_device = gSpvrDatabase.lock().await.c_device.clone();
    match c_device {
        Some(c) => c
            .lock()
            .await
