...
use crate::event::spvr_event_keys::{EVENT_CPU, EVENT_DISK, EVENT_GPU, EVENT_MEMORY, EVENT_TYPE, KEY_CPU_USAGE, KEY_DISK_PATH, KEY_DISK_USAGE, KEY_GPU_ID, KEY_GPU_NAME, KEY_GPU_USAGE, KEY_MEMORY_USAGE};
use crate::relay::relay_message::{KEY_DEVICE_NAME, KEY_PAGE_SIZE};
use crate::spvr_defs::{KEY_DEVICE_ID, KEY_DEVICE_IP, KEY_IP};
use crate::spvr_http_util::{get_body, get_body_bool, get_body_int, get_body_str, get_body_str_or_empty, get_int_param, get_str_param, get_str_param_or};
use crate::user::spvr_user::SpvrUser;
use crate::user::spvr_user_keys::{KEY_AVATAR_PATH, KEY_FILE, KEY_PAGE, KEY_USER_ID, KEY_USER_NAME};

pub async fn handle_add_event(State(_context): State<Arc<Mutex<SpvrContext>>>,
                              b: Body)
    ...
    if event_type == EVENT_CPU {
        ...
        let event = SpvrEvent::new_cpu(device_id, device_ip, device_name, uid, username, cpu_usage as u32);
        let _r = gSpvrEventMgr
            .add_event(event.clone()).await?;
        return Ok(Json(ok_resp(event)));
    }
    else if event_type == EVENT_MEMORY {
        let mem_usage = get_body_int(&r, KEY_MEMORY_USAGE)?;
        let event = SpvrEvent::new_memory(device_id, device_ip, device_name, uid, username, mem_usage as u32);
        let _r = gSpvrEventMgr
            .add_event(event.clone()).await?;
        return Ok(Json(ok_resp(event)));
    }
        ...
        let event = SpvrEvent::new_disk(device_id, device_ip, device_name, uid, username, disk_usage as u32, disk_path);
        let _r = gSpvrEventMgr
            .add_event(event.clone()).await?;
        return Ok(Json(ok_resp(event)));
    }
        ...
        let event = SpvrEvent::new_gpu(device_id, device_ip, device_name, uid, username, gpu_usage as u32, gpu_id, gpu_name);
        let _r = gSpvrEventMgr
            .add_event(event.clone()).await?;
        return Ok(Json(ok_resp(event)));
    }
...
pub async fn handle_remove_event(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                 b: Body)
                                 -> Result<Json<RespMessage<SpvrEvent>>, SpvrApiError>  {
    Ok(Json(ok_resp(SpvrEvent::default())))
}

pub async fn handle_query_events(State(_context): State<Arc<Mutex<SpvrContext>>>,
    ...
    let r = gSpvrEventMgr
        .query_events(page, page_size, filters, Some("timestamp".to_string()), Some(-1)).await?;

    Ok(Json(ok_resp(r)))
}

pub async fn handle_count_events(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                 query: Query<HashMap<String, String>>)
                                 -> Result<Json<RespMessage<u64>>, SpvrApiError> {
    let event_type = get_str_param_or(&query, EVENT_TYPE, "")?;
    let r = gSpvrEventMgr
        .count_total_events(event_type).await?;
    Ok(Json(ok_resp(r)))
}


pub async fn handle_add_log(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                  query: Query<HashMap<String, String>>,
