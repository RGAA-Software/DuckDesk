...
use gr_base::{get_current_readable_timestamp, ok_resp, RespMessage};
...
use crate::spvr_http_util::{get_body_str, get_body, get_int_param, get_str_param};
...
pub async fn handle_add_device_for_user(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                        b: Body)
    ...
    let r = gSpvrUserDeviceMgr
        .insert_user_device(user_device).await?;

    Ok(Json(ok_resp(r)))
}

pub async fn handle_remove_device_from_user(State(_context): State<Arc<Mutex<SpvrContext>>>,
                                            b: Body)
                                            -> Result<Json<RespMessage<SpvrUserDeviceAdapter>>, SpvrApiError>  {
    let body = get_body(b).await?;
    let r: Value = serde_json::from_str(body.as_str()).unwrap();
    ...
    let user_device = gSpvrUserDeviceMgr
        .remove_device_from_user(uid, device_id).await?;

    Ok(Json(ok_resp(user_device)))
}

pub async fn handle_query_user_devices(State(_context): State<Arc<Mutex<SpvrContext>>>,
    ...
    let devices = gSpvrUserDeviceMgr
        .query_user_devices(uid, page, page_size).await?;
