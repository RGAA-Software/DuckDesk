...
impl SpvrUserDeviceManager {
    ...
    pub async fn insert_user_device(
        &self,
        user_device: SpvrUserDevice,
    ) -> Result<SpvrUserDeviceAdapter, SpvrApiError> {
        ...
        let c_user_device = gSpvrDatabase.lock().await.user_device();
    ...
    pub async fn query_user_devices(
        ...
        page: i32,
        page_size: i32,
    ) -> Result<Vec<SpvrUserDeviceAdapter>, SpvrApiError> {
        let user = gUserManager.query_user_by_id(uid.clone()).await?;
