...
pub struct CmsClientConnManager {
    connections: Mutex<HashMap<String, CmsClientConnPtr>>,
...
impl CmsClientConnManager {
    pub fn new() -> CmsClientConnManager {
        Self {
    ...
    pub async fn add_conn(&self, conn_id: String, conn: CmsClientConnPtr) {
    ...
    pub async fn get_alive_connections_ptr(&self) -> Vec<CmsClientConnPtr> {
        tracing::info!(
            "alive connections count: {}",
            self.connections.lock().await.len()
        );
        let conns: Vec<CmsClientConnPtr> =
    ...
    pub async fn get_alive_connections(&self) -> Vec<CmsClientConnVo> {
        tracing::info!(
            "alive connections count: {}",
            self.connections.lock().await.len()
        );
    ...
    // -- DB --
    pub async fn insert_conn(&self, conn: CmsClientConnVo) {
        let c_client_conn = gCmsDatabase.lock().await.client_conn();
    ...
    pub async fn query_client_conns(
        ...
        page: i32,
        page_size: i32,
    ) -> Result<Vec<CmsClientConnVo>, CmsApiError> {
        ...
        let mut clients_conn: Vec<CmsClientConnVo> = Vec::new();
...
mod tests {
    ...
    async fn reservation_respects_max_streams() {
        let mgr = Arc::new(CmsClientConnManager::new());
