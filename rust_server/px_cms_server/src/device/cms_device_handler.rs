...
pub async fn handle_create_new_device(
...
) -> Result<Json<RespMessage<CmsDevice>>, CmsApiError> {
    ...
    let device = loop {
        ...
        let mut device: Option<CmsDevice> = None;
        if let Err(e) = exist_device {
            if e == CmsApiError::DatabaseError {
    ...
    if let Some(device) = device {
        ...
        Err(CmsApiError::CreateDeviceFailed)
    }
}
...
pub struct RespDeviceMessage {
    ...
    pub timestamp: i64,
    pub total: u64,
    pub data: Vec<CmsDeviceVo>,
}

pub async fn handle_query_devices(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespDeviceMessage>, CmsApiError> {
...
pub async fn verify_device_info(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespStringMap>, CmsApiError> {
    let device_id = get_str_param(&query, "device_id")?;
...
pub async fn update_random_password(
    ...
    if !r {
        ...
        return Err(CmsApiError::DeviceNotFound);
    }
