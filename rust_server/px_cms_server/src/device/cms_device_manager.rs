...
pub struct CmsDeviceManager {}

impl CmsDeviceManager {
    pub fn new() -> Arc<CmsDeviceManager> {
        Arc::new(CmsDeviceManager {})
    }
    ...
    pub async fn query_devices(
        ...
        page: i32,
        page_size: i32,
    ) -> Result<Vec<CmsDevice>, CmsApiError> {
        ...
        let mut devices: Vec<CmsDevice> = Vec::new();
    ...
    pub async fn insert_device(&self, device: CmsDevice) -> Result<bool, CmsApiError> {
        let c_device = gCmsDatabase.lock().await.device();
    ...
    pub async fn query_device_by_id(&self, device_id: String) -> Result<CmsDevice, CmsApiError> {
        let c_device = gCmsDatabase.lock().await.device();
        let filter = doc! {
            "device_id": device_id,
        };
    ...
    pub async fn query_total_devices_count(&self) -> Result<u64, CmsApiError> {
        let c_device = gCmsDatabase.lock().await.device();
    ...
    pub async fn query_total_used_time(&self) -> Result<u64, CmsApiError> {
        let c_device = gCmsDatabase.lock().await.device();
        ...
        let mut cursor = c_device
            ...
            .aggregate(pipeline)
            .await
            .map_err(|_e| CmsApiError::DatabaseError)?;
        // 6. 获取结果
        if let Some(result) = cursor.next().await {
            let doc = result.map_err(|_e| CmsApiError::DatabaseError)?;
