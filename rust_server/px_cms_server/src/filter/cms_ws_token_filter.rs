        ...
        let reservation = match crate::gCmsClientConnMgr.try_reserve_stream(auth.max_streams) {
            ...
            None => {
                tracing::error!(
                    ...
                    crate::gCmsClientConnMgr.count_alive_connections().await,
                    auth.max_streams
                );
                return CmsApiError::MaxStreamsReached.into_response();
            }
        };
...
/// `/cms/client` filter: verifies the HMAC token and enforces `max_streams`.
pub async fn client_filter(req: Request<Body>, next: Next) -> Response {
    verify_and_run(req, next, true).await
}

/// `/cms/panel` filter: verifies the HMAC token only.
...
mod tests {
    ...
    async fn client_rejects_when_auth_not_loaded() {
        ...
        let response = client_router()
            .oneshot(
                Request::builder()
                    .uri("/cms/client?appkey=k&token=t&ts=1&nonce=n")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn client_rejects_invalid_token() {
        ...
        let response = client_router()
            .oneshot(
                Request::builder()
                    .uri("/cms/client?appkey=appkey-1&token=bad-token&ts=1&nonce=n")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn client_accepts_valid_token() {
        ...
        let token = generate_connection_token("appkey-1", "secret-1");
        let uri = format!(
            "/cms/client?appkey=appkey-1&token={}&ts={}&nonce={}",
            token.token, token.ts, token.nonce
        );
    ...
    async fn client_rejects_when_max_streams_reached() {
        ...
        let reservation = crate::gCmsClientConnMgr.try_reserve_stream(1).unwrap();
    ...
    async fn panel_accepts_valid_token() {
        ...
        let token = generate_connection_token("appkey-1", "secret-1");
        let uri = format!(
            "/cms/panel?appkey=appkey-1&token={}&ts={}&nonce={}&device_id=d&user_id=u",
            token.token, token.ts, token.nonce
        );
    ...
    async fn panel_rejects_missing_token() {
        ...
        let response = panel_router()
            .oneshot(
                Request::builder()
                    .uri("/cms/panel?appkey=appkey-1&device_id=d&user_id=u")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
    ...
    async fn website_accepts_valid_token() {
        ...
        let token = generate_connection_token("appkey-1", "secret-1");
        let uri = format!(
            "/cms/website?appkey=appkey-1&token={}&ts={}&nonce={}",
            token.token, token.ts, token.nonce
        );
    ...
    async fn website_rejects_invalid_token() {
        ...
        let response = website_router()
            .oneshot(
                Request::builder()
                    .uri("/cms/website?appkey=appkey-1&token=bad&ts=1&nonce=n")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
