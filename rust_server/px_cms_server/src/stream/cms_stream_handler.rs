...
use crate::stream::cms_stream_keys::{
    ...
    KEY_STREAM_CONNECT_TYPE, KEY_STREAM_CREATED_TIMESTAMP, KEY_STREAM_DESKTOP_NAME,
    KEY_STREAM_DEVICE_ID, KEY_STREAM_DEVICE_RANDOM_PWD, KEY_STREAM_DEVICE_SAFETY_PWD,
    KEY_STREAM_HOST, KEY_STREAM_ID, KEY_STREAM_NAME, KEY_STREAM_NETWORK_TYPE,
    KEY_STREAM_OS_VERSION, KEY_STREAM_PORT, KEY_STREAM_REMOTE_DEVICE_ID,
    KEY_STREAM_REMOTE_DEVICE_RANDOM_PWD, KEY_STREAM_REMOTE_DEVICE_SAFETY_PWD,
    KEY_STREAM_SHOW_MAX_WINDOW, KEY_STREAM_SPLIT_WINDOWS, KEY_STREAM_UPDATED_TIMESTAMP,
};
use crate::user::cms_user_keys::{KEY_PAGE, KEY_PAGE_SIZE, KEY_SORT_DIRECTION, KEY_SORT_FIELD};
use axum::body::Body;
...
pub async fn handle_insert_stream(
    ...
    b: Body,
) -> Result<Json<RespMessage<CmsStream>>, CmsApiError> {
    let body = get_body(b).await?;
    let r: Value = serde_json::from_str(body.as_str()).unwrap();
    let mut stream = CmsStream::default();
    ...
    let r = gCmsStreamMgr
        ...
        .await;
    if let Err(_e) = r {
        let s = gCmsStreamMgr.insert_stream(stream).await?;
        Ok(Json(ok_resp(s)))
    } else {
        let s = gCmsStreamMgr.update_stream(stream).await?;
        Ok(Json(ok_resp(s)))
    }
}
...
pub async fn handle_delete_stream(
    ...
    let s = gCmsStreamMgr.delete_stream(stream_id.to_string()).await?;
    Ok(Json(ok_resp(s)))
}
...
pub async fn handle_update_stream(
    ...
    let _stream = gCmsStreamMgr.query_stream_by_id(stream_id.clone()).await?;
    ...
    if update_success {
        let stream = gCmsStreamMgr.query_stream_by_id(stream_id.clone()).await?;
...
pub async fn handle_query_stream_by_id(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespMessage<CmsStream>>, CmsApiError> {
    let stream_id = get_str_param(&query, KEY_STREAM_ID)?;
    let s = gCmsStreamMgr.query_stream_by_id(stream_id).await?;
    Ok(Json(ok_resp(s)))
}
...
pub async fn handle_query_stream_by_name(
    ...
    let s = gCmsStreamMgr.query_stream_by_name(stream_name).await?;
    Ok(Json(ok_resp(s)))
}
...
pub async fn handle_query_streams(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespMessage<Vec<CmsStream>>>, CmsApiError> {
    ...
    let streams = gCmsStreamMgr
        .query_streams(page, page_size, filters, key_sort_field, key_sort_direction)
        .await?;
