...
impl CmsContext {
    ...
    pub async fn gen_access_info(&self) -> CmsAccessInfo {
        // myself
        let self_config = gCmsSettings.lock().await.get_server_config().await;
        CmsAccessInfo {
            cms_srv_config: self_config,
        }
    }
    ...
    pub async fn get_encrypt_access_info(&self) -> Result<String, String> {
        ...
        if let Ok(v) = serde_json::to_string(&info) {
            ...
            return if let Ok(v) = v {
                Ok(format!("cms://access##{}", v))
            } else {
                Err("Failed to encrypt cms.".to_string())
            };
        }
    ...
    pub async fn broadcast_access_info(&self, port: u16) {
        tokio::spawn(async move {
            ...
            loop {
                ...
                let msg = gCmsContext.lock().await.get_encrypt_access_info().await;
                if let Err(_e) = msg {
