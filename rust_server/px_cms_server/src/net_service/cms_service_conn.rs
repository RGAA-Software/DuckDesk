...
pub type CmsServiceConnPtr = Arc<Mutex<CmsServiceConn>>;

#[derive(Clone)]
pub struct CmsServiceConn {
...
impl CmsServiceConn {
    ...
    pub async fn process_message(&mut self, _who: String, data: Bytes) -> bool {
        let m = CmsServiceMessage::decode(data);
        if let Err(e) = m {
            tracing::error!("parse error: {:?}", e);
            return false;
        }
        let m = m.unwrap();
        if m.msg_type == CmsServiceMessageType::KCmsServiceHello {
        ...
        } else if m.msg_type == CmsServiceMessageType::KCmsServiceHeartBeat {
        ...
        } else if m.msg_type == CmsServiceMessageType::KCmsServiceStartAppInstanceResult {
            if let Some(sub) = m.start_app_instance_result {
                crate::app_schedule::gAppScheduleManager
                    .on_start_result(self.device_id.clone(), sub)
                    .await;
            }
        } else if m.msg_type == CmsServiceMessageType::KCmsServiceStopAppInstanceResult {
    ...
    async fn send_hello(&mut self, device_id: String) {
        ...
        sv_msg.set_msg_type(CmsServiceMessageType::KCmsServiceHello);
        sv_msg.device_id = device_id.clone();
        sv_msg.hello = Some(CmsServiceHello {
            device_id,
    ...
    async fn send_heartbeat(&mut self, hb_index: i64, device_id: String) {
        let mut sv_msg = CmsServiceMessage::default();
        sv_msg.set_msg_type(CmsServiceMessageType::KCmsServiceHeartBeat);
        sv_msg.device_id = device_id.clone();
        sv_msg.heartbeat = Some(CmsServiceHeartBeat {
            hb_index,
            device_id,
            render_alive: false,
            auth_info_json: "".to_string(),
    ...
    pub async fn send_start_app_instance(
        &mut self,
        start: protocol::cms_service::CmsServiceStartAppInstance,
    ) -> bool {
        let mut sv_msg = CmsServiceMessage::default();
        sv_msg.set_msg_type(CmsServiceMessageType::KCmsServiceStartAppInstance);
    ...
    pub async fn send_stop_app_instance(
        &mut self,
        stop: protocol::cms_service::CmsServiceStopAppInstance,
    ) -> bool {
        let mut sv_msg = CmsServiceMessage::default();
