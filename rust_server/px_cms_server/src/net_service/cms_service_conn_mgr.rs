...
mod tests {
    ...
    fn make_conn(device_id: &str, appkey: &str) -> CmsServiceConnPtr {
        Arc::new(Mutex::new(CmsServiceConn {
            context: Arc::new(Mutex::new(CmsContext::new())),
            sender: None,
    ...
    async fn process_message_updates_state() {
        let conn = make_conn("d1", "appkey-1");
        {
            ...
            let mut hello = protocol::cms_service::CmsServiceMessage::default();
            hello.set_msg_type(protocol::cms_service::CmsServiceMessageType::KCmsServiceHello);
            hello.hello = Some(protocol::cms_service::CmsServiceHello {
