...
fn synchronize_http_port(config_path: &Path, expected_port: u16) -> Result<(), String> {
    ...
    if updated != config {
        ...
        tracing::info!(
            port = expected_port,
            "synchronized px_media HTTP port from CMS configuration"
        );
    }
    Ok(())
}
...
/// px_cms.exe, px_media.exe and config.ini reside in one directory.
pub async fn ensure_started(settings: &CmsLiveSettings) {
    ...
    let exe_path = match std::env::current_exe() {
        Ok(path) => path,
        Err(error) => {
            tracing::error!("cannot determine CMS executable path for px_media: {error}");
            return;
        }
    };
    let Some(directory) = exe_path.parent() else {
        tracing::error!("cannot determine CMS executable directory for px_media");
        return;
    };
    ...
    if !media_exe.is_file() || !media_config.is_file() {
        tracing::error!(
            ...
            "px_media sidecar is not deployed beside px_cms.exe"
        );
        return;
    }
