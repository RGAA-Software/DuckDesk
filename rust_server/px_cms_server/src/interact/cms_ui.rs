...
#[derive(Debug, Clone)]
pub struct CmsUIState {
    pub cms_alive: bool,
    pub cms_alive_pid: u32,
...
impl CmsUI {
    ...
    pub fn new(
        ...
        CmsUI::setup_custom_fonts(&cc.egui_ctx);
    ...
    pub fn check_server_state(&self) {
        ...
        std::thread::spawn(move || {
            loop {
                ...
                state.lock().unwrap().cms_alive = false;
                ...
                for (pid, process) in processes {
                    ...
                    if name.contains("px_cms") && pid.as_u32() != my_pid {
                        state.lock().unwrap().cms_alive = true;
                        state.lock().unwrap().cms_alive_pid = pid.as_u32();
                    }
                }

                if !state.lock().unwrap().cms_alive {
                    state.lock().unwrap().cms_alive_pid = 0;
                    // start it
    ...
    pub fn check_redis_mongodb_state(&self) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                // check mongodb
                let mongodb_uri = gCmsSettings.lock().await.mongodb_url.clone();
    ...
    pub fn kill_server(&self) {
        // kill the cms server
        let cms_pid = self.state.lock().unwrap().cms_alive_pid;
        if cms_pid > 0 {
            let mut sys = System::new_all();
            sys.refresh_processes(ProcessesToUpdate::All, true);
            // 查找进程
            if let Some(process) = sys.process(Pid::from_u32(cms_pid)) {
                // 发送终止信号
                let _ = process.kill_with(Signal::Kill).is_some();
                tracing::warn!("will close : {}", cms_pid);
            }
        }
    }
}

impl eframe::App for CmsUI {
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    egui::Grid::new("basic_info")
                        ...
                        .show(ui, |ui| {
                                ...
                                .clicked()
                            {
                                ...
                                tokio::task::spawn_blocking(move || {
                                    ...
                                    rt.block_on(async move {
                                        ...
                                        // 本进程内存中的授权用于展示。
                                        let r = crate::auth::cms_auth_pull::pull_once()
                                            .await
                                            .map(|_| ());
                                        if let Err(e) = &r {
                                            tracing::error!(
                            ...
                            // manager server
                            ui.label(self.language.st_cms_state.as_str());
                            if self.state.lock().unwrap().cms_alive {
                                ui.label(
                                    RichText::new("OK")
                                        .strong()
                                        .color(Color32::WHITE)
                            ...
                            //
                            ui.label(self.language.st_cms_website.as_str());
