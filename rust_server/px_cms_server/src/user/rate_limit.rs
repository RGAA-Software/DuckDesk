...
pub fn check(key: String, limit: usize, window_ms: i64) -> Result<(), CmsApiError> {
    if limit == 0 {
        return Err(CmsApiError::RateLimited);
    }
    let now = px_base::get_current_timestamp();
    let mut all = ATTEMPTS.lock().map_err(|_| CmsApiError::InternalError)?;
...
mod tests {
    ...
    fn limiter_rejects_after_configured_count() {
        ...
        assert!(check(key.clone(), 2, 60_000).is_ok());
        assert_eq!(check(key, 2, 60_000), Err(CmsApiError::RateLimited));
    }
}
