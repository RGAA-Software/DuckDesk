...
pub struct CmsSystemManager {}

impl CmsSystemManager {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {})
    }

    pub fn cal_logs_size(&self) -> i64 {
        px_base::calculate_dir_size("./logs".to_string())
    }

    pub fn cal_logs_size_readable(&self) -> String {
        let size = self.cal_logs_size();
        px_base::format_file_size(size)
    }

    pub fn cal_upload_size(&self) -> i64 {
        px_base::calculate_dir_size("./uploads".to_string())
    }
    ...
    pub async fn clear_data(&self) {
        ...
        let logs_path = gCmsSettings.lock().await.abs_upload_logs_path.clone();
