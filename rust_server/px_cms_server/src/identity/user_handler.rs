...
use crate::{gCmsUserDeviceMgr, gDeviceManager, gUserManager, gUserSessionManager};
...
impl UserAdminView {
    async fn from_user(user: CmsUser) -> Result<Self, CmsApiError> {
        Ok(Self {
...
pub async fn list_users(
    ...
    let (sort_field, sort_order) = match query.sort.as_deref().unwrap_or("-created_at") {
        ...
        "-username" => ("username_normalized", -1),
        _ => return Err(CmsApiError::InvalidParams),
    };
...
pub async fn create_user(Json(request): Json<CreateUserRequest>) -> Result<Response, CmsApiError> {
    ...
    if let Err(error) = gCmsUserDeviceMgr
...
async fn validate_group_ids(group_ids: &[String]) -> Result<(), CmsApiError> {
    let known: std::collections::HashSet<_> = IdentityManager::list_groups()
        .await?
        .into_iter()
        .map(|group| group.gid)
...
async fn validate_device_ids(device_ids: &[String]) -> Result<(), CmsApiError> {
...
pub async fn list_user_sessions(
    Path(uid): Path<String>,
) -> Result<Json<RespMessage<Vec<SessionAdminView>>>, CmsApiError> {
...
) -> Result<Json<RespMessage<Vec<GuestSessionAdminView>>>, CmsApiError> {
    let sessions = gUserSessionManager
        .list_guest_sessions()
        .await?
        .into_iter()
...
pub async fn list_personal_devices(
    Path(uid): Path<String>,
) -> Result<Json<RespMessage<Vec<String>>>, CmsApiError> {
    gUserManager.query_user_by_id(uid.clone()).await?;
    Ok(Json(ok_resp(
        gCmsUserDeviceMgr.personal_device_ids(&uid).await?,
    )))
}
