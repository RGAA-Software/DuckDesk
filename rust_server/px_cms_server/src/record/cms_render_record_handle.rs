//! Handlers for the cms render-records api family
//! (docs/cms_render_records_view_design.md section 6.3 / 6.4).

use crate::cms_api_error::CmsApiError;
use crate::cms_context::CmsContext;
use crate::cms_http_util::{get_str_param, get_str_param_allow_empty};
...
use crate::record::cms_render_record::{
    make_record_id, CmsRenderRecord, RECORD_STATE_FETCHING, RECORD_STATE_READY,
};
...
use protocol::cms_panel::{CmsPanelMessage, CmsPanelMessageType, RecordFetchReq, RecordListReq};
...
fn panel_port_of(device: &CmsDevice) -> i64 {
...
/// cms -> panel: RecordFetchReq; the caller owns in-flight dedupe
async fn send_fetch_req(device_id: &str, filename: &str) -> Result<(), CmsApiError> {
    let conn = gCmsPanelConnMgr
    ...
    let mut msg = CmsPanelMessage::default();
    msg.set_msg_type(CmsPanelMessageType::KRecordFetchReq);
...
async fn trigger_tunnel_fetch(device_id: &str, filename: &str) -> Result<(), CmsApiError> {
...
fn web_item_from_record(rec: &CmsRenderRecord) -> RecordWebItem {
    RecordWebItem {
        id: rec.id.clone(),
...
) -> Result<Json<RespMessage<RecordAccessInfo>>, CmsApiError> {
...
pub async fn handle_record_ticket(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespMessage<RecordTicketResp>>, CmsApiError> {
    let device_id = get_str_param(&query, "device_id")?;
...
/// the cms-side download state from c_records (design 6.3)
pub async fn handle_record_list(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespMessage<RecordListWebResp>>, CmsApiError> {
    let device_id = get_str_param(&query, "device_id")?;
    let conn = gCmsPanelConnMgr
    ...
    let mut msg = CmsPanelMessage::default();
    msg.set_msg_type(CmsPanelMessageType::KRecordListReq);
    ...
    // cms-only copies (device file may have been rotated out) stay visible
...
pub async fn handle_record_fetch(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    query: Query<HashMap<String, String>>,
) -> Result<Json<RespMessage<RecordFetchResp>>, CmsApiError> {
    let device_id = get_str_param(&query, "device_id")?;
    let file = get_str_param(&query, "file")?;
...
pub async fn handle_record_upload(
    ...
    if !skip_write {
        px_base::create_dir_all_if_not_exists(&dir).map_err(|_| CmsApiError::UploadFileFailed)?;
    }
...
/// POST /download  body {device_id, filename} — save a copy on the cms host
/// and pin it (keep=true, exempt from cleanup; design 6.4)
pub async fn handle_record_download(
    State(_ctx): State<Arc<Mutex<CmsContext>>>,
    Json(req): Json<RecordDownloadReq>,
) -> Result<Json<RespMessage<RecordDownloadResp>>, CmsApiError> {
...
/// cms server pulls http://{panel_ip}:{port}/records/{file}?tk&exp directly,
/// streaming to disk with progress updates (design 6.4, topology 1)
async fn direct_pull_from_panel(device: &CmsDevice, filename: &str) -> Result<i64, String> {
...
pub async fn handle_record_delete(
    ...
    match rec {
        ...
        None => Err(CmsApiError::RecordNotFound),
    }
}
