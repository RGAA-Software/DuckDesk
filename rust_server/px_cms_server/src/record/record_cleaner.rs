//! Temporary-cache cleanup for cms render records (design doc 6.3):
...
pub fn select_threshold_devices(
    records_oldest_first: &[CmsRenderRecord],
    total_bytes: i64,
    threshold: i64,
) -> Vec<String> {
...
pub async fn delete_record_file(rec: &CmsRenderRecord) {
...
pub fn start_cleanup_task(mgr: Arc<CmsRenderRecordManager>, tunnel: Arc<RecordTunnelManager>) {
...
pub async fn run_cleanup_once(
    mgr: &Arc<CmsRenderRecordManager>,
...
mod tests {
    use super::*;
    use crate::record::cms_render_record::RECORD_STATE_READY;

    fn rec(id: &str, device: &str, keep: bool, updated: i64, size: i64) -> CmsRenderRecord {
        CmsRenderRecord {
            id: id.to_string(),
