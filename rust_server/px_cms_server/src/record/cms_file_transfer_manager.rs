...
pub struct CmsFileTransferManager {}

impl CmsFileTransferManager {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {})
    }
    ...
    pub async fn update_file_transfer_info(
        ...
        {
            Ok(Some(doc)) => Ok(doc),
            Ok(None) => Err(CmsApiError::FileTransferNotFound),
            Err(e) => {
    ...
    ) -> Result<Vec<CmsFileTransfer>, CmsApiError>
    where
        T: Into<Bson>,
    {
        let c_file_transfer_info = gCmsDatabase.lock().await.file_transfer();
        ...
        let mut streams: Vec<CmsFileTransfer> = Vec::new();
