...
use crate::net_panel::cms_panel_conn::CmsPanelConn;
...
async fn handle_socket(
    ...
    let mut recv_task = tokio::spawn(async move {
        ...
        if device_id.is_empty() {
            tracing::error!("cms panel error, device id is empty!");
            return;
        }

        let sender = Arc::new(Mutex::new(sender));
        let panel_conn = CmsPanelConn::new(
            context.clone(),
            sender,
            device_id.clone(),
            appkey.clone(),
        ...
        // remove
        gCmsPanelConnMgr.remove_conn(device_id).await;
    });
...
async fn process_message(
    ...
    cms_conn: Arc<Mutex<CmsPanelConn>>,
    msg: Message,
    who: SocketAddr,
) -> ControlFlow<(), ()> {
