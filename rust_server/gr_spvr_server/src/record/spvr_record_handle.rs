...
use crate::spvr_http_util::{get_int_param, get_int_param_or, get_body, get_body_data, get_str_param_allow_empty};
...
pub async fn handle_upload_visit_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                       query: Query<HashMap<String, String>>,
    ...
    let _info = gRecordVisitManager
        .insert_visit_info(up_visit).await?;
    Ok(Json(ok_resp("upload ok".to_string())))
}

pub async fn handle_update_visit_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                      query: Query<HashMap<String, String>>,
    ...
    let _info = gRecordVisitManager
        .update_visit_info(update_visit).await?;
...
pub async fn handle_query_update_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
    ...
    let mut r = gRecordVisitManager
        .query_info::<String>(page,
                              page_size,
                              HashMap::default(),
                              Some(String::from("created_timestamp")),
                              Some(sort_time),
                              if visit_device_id.is_empty() { None } else { Some(visit_device_id) },
                              if target_device_id.is_empty() { None } else { Some(target_device_id) },
                              ).await?;
    for visit in &mut r {
...
pub async fn handle_upload_file_transfer_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                      query: Query<HashMap<String, String>>,
    ...
    let _info = gRecordFileTransferManager
        .insert_file_transfer_info(up_file_transfer).await?;
    Ok(Json(ok_resp("upload ok".to_string())))
}

pub async fn handle_update_file_transfer_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                              query: Query<HashMap<String, String>>,
    ...
    let _info = gRecordFileTransferManager
        .update_file_transfer_info(up_file_transfer).await?;
    Ok(Json(ok_resp("upload ok".to_string())))
}

pub async fn handle_query_file_transfer_info(State(_ctx): State<Arc<Mutex<SpvrContext>>>,
                                      query: Query<HashMap<String, String>>,
                                      body: Body)
