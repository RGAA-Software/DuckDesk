...
async fn run_as_panel(machine_code: String) {
    ...
    // log
    let _guard = log_util::init_log("logs/gr_spvr_server/".to_string(), "log_spvr_panel".to_string());
...
async fn run_as_server(machine_code: String) {
    // log
    let _guard = log_util::init_log("logs/gr_spvr_server/".to_string(), "log_spvr".to_string());
