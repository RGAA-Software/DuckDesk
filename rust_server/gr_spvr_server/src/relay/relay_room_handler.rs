...
use axum::Json;
use gr_base::{get_query_param, ok_resp, RespMessage, RespStringMap};
...
pub async fn hr_query_room(
    ...
    if let Some(room) = gRelayRoomMgr
        .find_room(room_id.unwrap(), false).await {
        Ok(Json(gr_base::ok_resp_str_map(room.as_str_map())))
    }
    else {
