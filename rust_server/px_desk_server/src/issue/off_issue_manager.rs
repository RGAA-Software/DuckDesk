...
impl OffIssueManager {
    ...
    ) -> Result<Vec<OffIssue>, OffApiError>
    where
        T: Into<Bson>,
    {
    ...
    pub async fn mark_processed(&self, id: String, p: bool) -> Result<(), OffApiError> {
        ...
        let r = c_issue
            ...
            .update_one(
                ...
                doc!{
                    KEY_PROCESSED: p,
                    KEY_UPDATED_TS: px_base::get_current_timestamp(),
                    KEY_UPDATED_TS_READABLE: px_base::get_current_readable_timestamp(),
                }},
            )
            .await;
        if let Err(e) = r {
